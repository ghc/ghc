...
--
-- Logarithm functions not part of the main GHC.Integer interface
-- Implementation is identical to integer-gmp.
...
integerLog2# (Bn#  _) = -1#
integerLog2# (Bp# bn) = go (s -# 1#)
  where
    s = wordsInBigNum# bn
    go i = case indexBigNum# bn i of
               0## -> go (i -# 1#)
