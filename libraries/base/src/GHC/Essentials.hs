...
{-# OPTIONS_GHC -fdefines-known-key-names #-} -- See Note [JS primitives known-keys for non-js target]
...
known-keys regardless of target, so it includes the JS primitives too.

The known-keys table cannot CPP the JS primitives out of the list, because
...
even if the target for which we are compiling GHC.Essentials is not JavaScript.

We define stubs which will be unused in the non-JS case (these known-keys won't
be used unless compiling to JS) and toggle -fdefines-known-key-names for these stubs.
-}
