...
-- @since 4.16.0.0
type x <= y = (Assert (x <=? y) (LeqErrMsg x y), (x <=? y) ~ 'True)
-- T10183 fails with a "Pattern match(es) are non-exhaustive" warning for head'
-- without the additional "(x <=? y) ~ True"
type family LeqErrMsg x y where
  LeqErrMsg x y = TypeError ('Text "Cannot satisfy: " ':<>: 'ShowType x ':<>: 'Text " <= " ':<>: 'ShowType y)

-- | Comparison (>=) of comparable types, as a constraint.
-- @since 4.16.0.0
type x >= y = Assert (x >=? y) (GeqErrMsg x y)
type family GeqErrMsg x y where
  GeqErrMsg x y = TypeError ('Text "Cannot satisfy: " ':<>: 'ShowType x ':<>: 'Text " >= " ':<>: 'ShowType y)
