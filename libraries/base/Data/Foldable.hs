...
-- this approach, by representing a list via its `foldMap` action.  Below we
-- take a look instead at a `foldr` representation, which is an example of
-- /Church encoding/ (named after Alonzo Church, inventor of the lambda
-- calculus).
--
-- > {-# LANGUAGE RankNTypes #-}
...
-- >      in print $ average l
-- >   where
-- >     sumlen :: Num a => PairS a Int -> a -> PairS a Int
