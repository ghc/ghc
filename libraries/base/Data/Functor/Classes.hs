...
-- decomposing it's main method into a canonical lifting on a canonical
...
--
-- @since 4.9.0.0
class (forall a b. (Eq a, Eq b) => Eq (f a b)) => Eq2 f where
...
--
-- @since 4.9.0.0
class (Eq2 f, forall a b. (Ord a, Ord b) => Ord (f a b)) => Ord2 f where
...
-- For more information, refer to the documentation for the 'Read' class.
-- @since 4.9.0.0
class (forall a b. (Read a, Read b) => Read (f a b)) => Read2 f where
...
--
-- @since 4.9.0.0
class (forall a b. (Show a, Show b) => Show (f a b)) => Show2 f where
    -- | 'showsPrec' function for an application of the type constructor
