...
newtype Predicate a = Predicate { getPredicate :: a -> Bool }
  deriving
    ( -- | @('<>') \@(Predicate)@ conjunctions @('&&')@ of the
      ...
      -- @
      Semigroup
    , -- | @mempty \@(Predicate a)@ always returns @True@, without
...
newtype Comparison a = Comparison { getComparison :: a -> a -> Ordering }
  deriving
  newtype
    ( -- | Uses @('<>') \@Ordering@ to combine the results. Without
      -- newtypes this equals @'liftA2' ('liftA2' ('<>'))@.
      --
      -- @
      ...
      -- @
      Semigroup
    , -- | Returns @EQ@ for any comparison. Without newtypes this
      -- equals @'pure' ('pure' EQ)@.
      --
      -- @
      -- mempty :: Comparison a
...
newtype Equivalence a = Equivalence { getEquivalence :: a -> a -> Bool }
  deriving
    ( -- | Conjunction @('&&')@ of the results, without newtypes this
      -- equals @'liftA2' ('liftA2' (&&))@
      --
      -- @
      ...
      -- @
      Semigroup
    , -- | Always returns @True@, without newtypes this equals @'pure'
      -- ('pure' True)@.
      --
      -- @
