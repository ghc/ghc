...
The Integer type is special because TidyPgm uses constructors in GHC.Num.Integer
to construct Integer literal values.  Currently it reads the interface file
whether or not the current module *has* any Integer literals, so it's important
that GHC.Num.Integer is compiled before any other module.
