...
a `shiftRAInt8#` b | isTrue# (b >=# 8#) = if isTrue# (a `ltInt8#` (narrowInt8# 0#))
                                             then narrowInt8# (-1#)
                                             else narrowInt8# 0#
                   | otherwise          = a `uncheckedShiftRAInt8#` b

shiftRAInt16# :: Int16# -> Int# -> Int16#
a `shiftRAInt16#` b | isTrue# (b >=# 16#) = if isTrue# (a `ltInt16#` (narrowInt16# 0#))
                                               then narrowInt16# (-1#)
                                               else narrowInt16# 0#
                    | otherwise           = a `uncheckedShiftRAInt16#` b

shiftRAInt32# :: Int32# -> Int# -> Int32#
a `shiftRAInt32#` b | isTrue# (b >=# 32#) = if isTrue# (a `ltInt32#` (narrowInt32# 0#))
                                               then narrowInt32# (-1#)
                                               else narrowInt32# 0#
