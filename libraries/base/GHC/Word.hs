...
instance Num Word8 where
    (W8# x#) + (W8# y#)    = W8# (wordToWord8# (x# `plusWord8#` y#))
    (W8# x#) - (W8# y#)    = W8# (wordToWord8# (x# `minusWord8#` y#))
    (W8# x#) * (W8# y#)    = W8# (wordToWord8# (x# `timesWord8#` y#))
...
instance Integral Word8 where
    quot    (W8# x#) y@(W8# y#)
        | y /= 0                  = W8# (wordToWord8# (x# `quotWord8#` y#))
        | otherwise               = divZeroError
    rem     (W8# x#) y@(W8# y#)
        | y /= 0                  = W8# (x# `remWord8#` y#)
        | otherwise               = divZeroError
    div     (W8# x#) y@(W8# y#)
        | y /= 0                  = W8# (x# `quotWord32#` y#)
        | otherwise               = divZeroError
    mod     (W8# x#) y@(W8# y#)
        | y /= 0                  = W8# (x# `remWord32#` y#)
        | otherwise               = divZeroError
    quotRem (W8# x#) y@(W8# y#)
...
eqWord16 (W16# x) (W16# y) = isTrue# (x `eqWord32#` y)
neWord16 (W16# x) (W16# y) = isTrue# (x `neWord32#` y)
{-# INLINE [1] eqWord16 #-}
{-# INLINE [1] neWord16 #-}
...
(W16# x) `gtWord16` (W16# y) = isTrue# (x `gtWord32#` y)
(W16# x) `geWord16` (W16# y) = isTrue# (x `geWord32#` y)
(W16# x) `ltWord16` (W16# y) = isTrue# (x `ltWord32#` y)
(W16# x) `leWord16` (W16# y) = isTrue# (x `leWord32#` y)
...
instance Num Word16 where
    (W16# x#) + (W16# y#)  = W16# (wordToWord16# (x# `plusWord16#` y#))
    (W16# x#) - (W16# y#)  = W16# (wordToWord16# (x# `minusWord16#` y#))
    (W16# x#) * (W16# y#)  = W16# (wordToWord16# (x# `timesWord16#` y#))
...
instance Integral Word16 where
    quot    (W16# x#) y@(W16# y#)
        ...
        | otherwise                 = divZeroError
    rem     (W16# x#) y@(W16# y#)
        | y /= 0                    = W16# (wordToWord16# (x# `remWord16#` y#))
        | otherwise                 = divZeroError
    div     (W16# x#) y@(W16# y#)
        | y /= 0                    = W16# (wordToWord16# (x# `quotWord16#` y#))
        | otherwise                 = divZeroError
    mod     (W16# x#) y@(W16# y#)
        | y /= 0                    = W16# (wordToWord16# (x# `remWord16#` y#))
        | otherwise                 = divZeroError
    ...
    divMod  (W16# x#) y@(W16# y#)
        | y /= 0                    = (W16# (wordToWord16# (x# `quotWord16#` y#))
                                      ,W16# (wordToWord16# (x# `remWord16#` y#)))
        | otherwise                 = divZeroError
...
instance Num Word32 where
    (W32# x#) + (W32# y#)  = W32# (wordToWord32# (x# `plusWord32#` y#))
    (W32# x#) - (W32# y#)  = W32# (wordToWord32# (x# `minusWord32#` y#))
    (W32# x#) * (W32# y#)  = W32# (wordToWord32# (x# `timesWord32#` y#))
...
instance Integral Word32 where
    quot    (W32# x#) y@(W32# y#)
        | y /= 0                    = W32# (wordToWord32# (x# `quotWord32#` y#))
        | otherwise                 = divZeroError
    rem     (W32# x#) y@(W32# y#)
        | y /= 0                    = W32# (wordToWord32# (x# `remWord32#` y#))
        | otherwise                 = divZeroError
    div     (W32# x#) y@(W32# y#)
        | y /= 0                    = W32# (wordToWord32# (x# `quotWord32#` y#))
        | otherwise                 = divZeroError
    ...
    divMod  (W32# x#) y@(W32# y#)
        | y /= 0                    = (W32# (wordToWord32# (x# `quotWord32#` y#))
                                      ,W32# (wordToWord32# (x# `remWord32#` y#)))
        | otherwise                 = divZeroError
    toInteger (W32# x#)
#if WORD_SIZE_IN_BITS == 32
        | isTrue# (i# >=# 0#)       = IS i#
