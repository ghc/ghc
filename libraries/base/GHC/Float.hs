...
Now if suppose we want to specialise (floor @Double d1 @Int d2).
It's best not to write one rule for this entire step. I found cases
(in nofib/readl/gamteb/Utils.hs, function fiRem) where we floated
out the (floor @Double d1) part, so the two-argument rule didn't fire.
Instead the class-op rule fired.
...
Best to define an instance
   ...
   floorDouble :: Integral b => Double -> b
    ...
    {-# RULE floorDouble @Int d = floorDoubleInt #-}
