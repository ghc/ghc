...
-- 'maximumSupportedRoots'. If called with too many the call is ignored and
-- a warning is printed to stderr, once.
--
  ...
  withArray descs $ \descs_arr ->
    withArray sps $ \sps_arr -> do
      ...
      when (len /= len') $ do
        ...
        when x $ do
          hPutStrLn stderr $
            "WARNING: setHeapProfilingRoots was called with "++show len++" roots, but only "++show len'++" are supported."
