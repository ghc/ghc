...
openFile' filepath iomode binary non_blocking = do
  ...
  -- we're out of the catch block. This whole situation seems a bit unlikely,
  -- but Yuras, at least, thinks it's possible, and it would be quite difficult
  -- to track down after the fact.
  touchHandle hndl pure hndl
