...
import GHC.Event.Windows (NativeHandle(), ConsoleHandle(), IoHandle(..), Io())
...
convertHandle io async = unsafePerformIO $ do
  let hwnd = fromHANDLE $ toHANDLE io
  return  $ hwnd { isAsynchronous = async }
...
hwndRead hwnd ptr offset bytes = do
  ...
  fmap fromIntegral $ Mgr.withException "hwndRead" $
     withOverlappedEx mngr "hwndRead" hwnd offset (startCB ptr) completionCB
  where
...
hwndReadNonBlocking hwnd ptr offset bytes
  = do mngr <- Mgr.getSystemManager
       val <- withOverlappedEx mngr "hwndReadNonBlocking" hwnd offset
...
hwndWrite hwnd ptr offset bytes
  = do mngr <- Mgr.getSystemManager
       _ <- Mgr.withException "hwndWrite" $
          withOverlappedEx mngr "hwndWrite" hwnd offset (startCB ptr)
                           completionCB
       return ()
  where
    startCB outBuf lpOverlapped = do
