...
--
-- @since 4.6.0.0
forkFinally :: IO a -> (Either SomeExceptionWithBacktrace a -> IO ()) -> IO ThreadId
forkFinally action and_then =
  mask $ \restore ->
    forkIO $ try (restore action) >>= and_then
  ...
  if bound
    then do
      ...
      mask $ \restore -> do
        ...
        let wait = takeMVar mv `catchException` \(e :: SomeExceptionWithBacktrace) ->
                     Exception.throwTo tid e >> wait
        wait >>= unsafeResult
    else action

unsafeResult :: Either SomeExceptionWithBacktrace a -> IO a
