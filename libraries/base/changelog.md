...
## 4.16.0.0 *TBA*
  ...
  * Add `Data.Type.Ord` as a module for type-level comparison operations.  The
    `<=?` type operator previously kind-specific to `Nat` is now kind-polymorphic
    and governed by the `Compare` type family in `Data.Type.Ord`.  Note that
    this means GHC will no longer deduce `0 <= n` for all `n` any more.
