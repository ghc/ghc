...
GHC sends a StartTH message to the server (see GHC.Tc.Splice.getTHState):
...
GHC sends a FinishTH message to the server (see GHC.Tc.Splice.finishTH).
...
Other Notes on TH / Remote GHCi
  ...
  * Note [TH recover with -fexternal-interpreter] in
    compiler/typecheck/GHC.Tc.Splice.hs
-}
...
instance TH.Quasi GHCiQ where
  ...
  -- See Note [TH recover with -fexternal-interpreter] in GHC.Tc.Splice
