...
createBCOs objs = do
  ...
  let (unl_objs, bcos) = partition (isUnliftedObj . snd) (zip [(0::Int)..] objs)

      n_bcos = length bcos
      n_objs = length unl_objs + n_bcos
  ...
  (unl_hvals, hvals) <- fixIO $ \ ~(_, hvs) -> do
    ...
    (unl_cons, unl_hvals) <- createUnliftedStaticCons (map snd unl_objs) arr
...
linkBCO' arr unl_arr resolved_obj =
  case resolved_obj of
    ResolvedBCO{..} -> do
      ...
      PtrsArr marr <- mkPtrsArray arr unl_arr resolvedBCOPtrs
      IO $ \s ->
        case unsafeFreezeArray# marr s of { (# s, arr #) ->
        case newBCO# insns_barr literals_barr arr arity# bitmap_barr s of { (# s, bco #) ->
          (# s, LinkedBCO resolvedBCOArity bco #)
        }}
    ResolvedStaticCon{..} -> do
      ...
      PtrsArr marr <- mkPtrsArray arr unl_arr resolvedStaticConPtrs
...
mkPtrsArray arr unl_arr ptrs = do
  ...
  let
    ...
    fill i (ResolvedStaticConRef n) = do
      writePtrsArrayHValue i (arr ! n) marr  -- must be lazy!
    fill i (ResolvedUnliftedStaticConRef n) = do
