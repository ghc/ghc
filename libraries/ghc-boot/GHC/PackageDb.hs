...
-- and in ghc itself.
--
data GenericUnitInfo compid srcpkgid srcpkgname instunitid modulename mod = GenericUnitInfo
   { unitId             :: instunitid
      -- ^ Unique unit identifier that is used during compilation (e.g. to
      -- generate symbols).

   , unitInstanceOfId   :: compid
...
mapGenericUnitInfo fuid fcid fsrcpkg fsrcpkgname fmodname fmod g@(GenericUnitInfo {..}) =
   g { unitId              = fuid unitId
     , unitInstanceOfId    = fcid unitInstanceOfId
...
instance Binary DbUnitInfo where
  put (GenericUnitInfo
         unitId unitInstanceOfId unitInstantiations
         unitPackageId
