...
--
-- Conventional primops are emitted by the compiler itself. They are special
-- bits of code with backend support. The primops in this module aren't
...
-- are eligible to live here, however. The compiler also sometimes emits custom
-- code for a primop under certain conditions, leaving the external version as a
-- fallback. Those sometimes-external sometimes internal-primops still have
-- compiler support, so they cannot live here.
--
-- That said, there are more eligible primops to move here than are here today.
-- But those primops require extensions to @foreign import prim@. Polymorphism,
-- strictness information, and @has_side_effects = False@ are currently
-- unsupported by @foreign import prim@. In the interest of removing spurious
-- interactions between the compiler and libraries, hopefully those restrictions
-- are lifted so the compiler-aware set of primops can shrink to a minimum.
--
