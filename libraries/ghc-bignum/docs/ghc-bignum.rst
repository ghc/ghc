...
GHC has been using two backends to provide arbitrary-precision integer support:
...
and that GHC is aware of them. Hence it is not easy to switch from on backend to
another: different binary distributions of GHC are provided which either support
GMP or Simple backend.
