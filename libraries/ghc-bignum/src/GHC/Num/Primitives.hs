...
-- | Reverse bytes in the Word32 subwords composing a Word#
wordReverse32# :: Word# -> Word#
#if WORD_SIZE_IN_BITS == 64
wordReverse32# x0 = r
   where
      x1 = ((x0 `and#` 0x5555555555555555##) `uncheckedShiftL#`  1#) `or#` ((x0 `and#` 0xAAAAAAAAAAAAAAAA##) `uncheckedShiftRL#`  1#)
...
#else
wordReverse32# x0 = wordReverse# x0
#endif
...
#if defined(WORDS_BIGENDIAN)
wordWriteAddrLE# w addr = writeWordOffAddr# addr 0# (wordReverse# w)
#else
wordWriteAddrLE# w addr = writeWordOffAddr# addr 0# w
#endif

-- | Write a full word with little-endian encoding
wordWriteAddrBE# :: Word# -> Addr# -> State# s -> State# s
#if defined(WORDS_BIGENDIAN)
wordWriteAddrBE# w addr = writeWordOffAddr# addr 0# w
#else
wordWriteAddrBE# w addr = writeWordOffAddr# addr 0# (wordReverse# w)
#endif
...
-- Exception trick taken from #14664
