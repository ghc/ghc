...
bignat_add mwa wa wb = addABc 0# 0##
   where
      ...
      addABc i carry s
         ...
         | isTrue# ((i ==# szA) &&# (i ==# szB))
         = mwaWriteMostSignificant mwa carry i s
...
bignat_mul_word mwa wa b = go 0# 0##
   where
      !szA = wordArraySize# wa
      go i carry s
         | isTrue# (i ==# szA) = mwaWriteMostSignificant mwa carry i s
         | True =
            let
   ...
   where
      ...
      mul bj j i carry s
         ...
         | True = let
                     ...
                     carry'       = plusWord# c' c''
                  in case mwaAddInplace mwa (i +# j) r s of
...
bignat_sub mwa wa wb s =
   ...
   case mwaArrayCopy# mwa 0# wa 0# (wordArraySize# wa) s of
      s' -> mwaSubArrayInplace mwa 0# wb s'
...
bignat_shiftl mwa wa n s1 =
   ...
   where
      ...
      fixW i s
         | isTrue# (i ==# szA) = s
         | True =
            case mwaWrite# mwa (i +# nw) (indexWordArray# wa i) s of
               s' -> fixW (i +# 1#) s'
      ...
      fix i c s
         -- write the carry
         | isTrue# (i ==# szA)
         = mwaWriteMostSignificant mwa c (i +# nw) s
...
bignat_shiftr mwa wa n s1
   ...
   where
      ...
      fixW i s
         | isTrue# (i <# 0#) = s
         | True =
            case mwaWrite# mwa i (indexWordArray# wa (i +# nw)) s of
               s' -> fixW (i -# 1#) s'
...
bignat_quotrem mwq mwr uwa uwb s0 =
   ...
   -- normalize A
   case mwaNormalize mwa uwa s1   of { s2 ->
   case mwaSize# mwq s2           of { (# s3, szQ #) ->
   -- normalized quotrem
   case bignat_quotrem_normalized mwq szQ mwa (normalize uwb) s3 of { s4 ->
   -- copy and denormalize remainder from mwa to mwr
   -- mwq is already not normalized
   mwaDenormalize mwr mwa s4
   }}}}
   ...
   where
      ...
      clzb  = clz# (indexWordArray# uwb (wordArraySize# uwb -# 1#))
      iclzb = word2Int# clzb

      szna = wordArraySize# uwa +# 1# -- normalizing A can add a newer higher limb

      normalize ws = bigNatShiftL# ws clzb

      mwaNormalize mwd ws s
         | isTrue# (iclzb ==# 0#)
         = case mwaArrayCopy# mwd 0# ws 0# (wordArraySize# ws) s of
            s -> mwaWriteMostSignificant' mwd 0## s

         | True
         = case bignat_shiftl mwd ws clzb s of
            s -> mwaTrimZeroes# mwd s

      mwaDenormalize mwd mws s
         = case mwaTrimZeroes# mws s of
            s -> case unsafeFreezeByteArray# mws s of
               (# s, ws #) -> case mwaSetSize# mwd (wordArraySize# ws) s of
                  s | isTrue# (iclzb ==# 0#) -> mwaArrayCopy# mwd 0# ws 0# (wordArraySize# ws) s
                    | True                   -> case bignat_shiftr mwd ws clzb s of
                                                   s -> mwaTrimZeroes# mwd s
...
bignat_quotrem_normalized mwq szQ mwa b s =
   ...
   where
      ...
      updateR k qb c s = case c of
         ...
         GT -> mwaSubArrayTrimInplace mwa k qb s
