...
#error You can't validate Native backed against itself. Choose another backend with `stack build --flag ghc-bignum:[gmp,ffi]` or `cabal configure -f [gmp,ffi]`
...
#else
#error Undefined BigNat backend. Use `stack build --flag bignum:[gmp,ffi,native]` or `cabal configure -f [gmp,ffi,native]`
#endif
