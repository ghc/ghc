...
-- For now we can't have rules matching on data constructors (cf
-- https://gitlab.haskell.org/ghc/ghc/issues/13290). This is sad because we need
-- those rules to convert datacons into Core literals.
...
integerMul (IS x#)  (IS y#)
  = case mulIntMayOflo# x# y# of -- TODO: replace with newer primitive
    0# -> IS (x# *# y#)
    _  -> integerFromIntMul# x# y#
...
integerFromIntMul# :: Int# -> Int# -> Integer
integerFromIntMul# x# y# = case (# isTrue# (x# >=# 0#), isTrue# (y# >=# 0#) #) of
...
integerShiftR# (IN bn) n   =
   case integerFromBigNatNeg (bigNatShiftR# bn n) of
      IS 0# -> IS -1#
      r     -> r
...
-- | Convert an Integer into an Int64# on 32-bit architectures
integerToInt64 :: Integer -> Int64
integerToInt64 i = I64# (integerToInt64# i)
...
-- | Convert an Integer into a Word64# on 32-bit architectures
integerToWord64 :: Integer -> Word64
integerToWord64 !i = W64# (integetToWord64# i)
...
-- powModInteger (IS b#) (IS e#) (IS m#)
--   | isTrue# (b# >=# 0#), isTrue# (e# >=# 0#)
--   = integerFromWord# (powModWord (int2Word# b#) (int2Word# e#)
--                               (int2Word# (absI# m#)))
-- powModInteger b e m = case m of
--     (IS m#) -> integerFromWord# (powModSBigNatWord b' e' (int2Word# (absI# m#)))
--     (IP m') -> integerFromBigNat (powModSBigNat b' e' m')
--     (IN m') -> integerFromBigNat (powModSBigNat b' e' m')
--   where
--     b' = integerToSBigNat b
...
-- powModSecInteger b e m = integerFromBigNat (powModSecSBigNat b' e' m')
--   where
--     b' = integerToSBigNat b
--     e' = integerToSBigNat e
...
-- recipModInteger (IS x#) (IS m#)
--   | isTrue# (x# >=# 0#)
--   = integerFromWord# (recipModWord (int2Word# x#) (int2Word# (absI# m#)))
-- recipModInteger x m = integerFromBigNat (recipModSBigNat x' m')
--   where
...
--     = integerFromBigNat (importBigNatFromByteArray ba ofs len msbf)
--
-- -- | Convert 'Integer' into 'SBigNat'
-- integerToSBigNat :: Integer -> SBigNat
-- integerToSBigNat (IS  i#) = intToSBigNat# i#
-- integerToSBigNat (IP bn) = PosBN bn
-- integerToSBigNat (IN bn) = NegBN bn
--
-- -- | Convert 'SBigNat' into 'Integer'
-- sBigNatToInteger :: SBigNat -> Integer
-- sBigNatToInteger (NegBN bn) = integerFromBigNatNeg bn
-- sBigNatToInteger (PosBN bn) = integerFromBigNat bn
--
