...
naturalFromIntThrow# i
   | isTrue# (i <# 0#) = case underflow of W# err -> NS err
...
naturalFromInt# !i
   ...
   | True               = case underflow of W# err -> NS err
...
naturalSubThrow (NS _) (NB _) = case underflow of W# err -> NS err
naturalSubThrow (NB x) (NS y) = naturalFromBigNat (bigNatSubWordUnsafe# x y)
naturalSubThrow (NS x) (NS y) =
   case subWordC# x y of
      (# l,0# #) -> NS l
      (# _,_  #) -> case underflow of W# err -> NS err
naturalSubThrow (NB x) (NB y) =
   case bigNatSub x y of
      (# () | #) -> case underflow of W# err -> NS err
      (# | z  #) -> naturalFromBigNat z
...
naturalNegate _        = case underflow of W# err -> NS err
...
naturalPowMod !_         !_       (NS 0##) = case divByZero of err -> err
