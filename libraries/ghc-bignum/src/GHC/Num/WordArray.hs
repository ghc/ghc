...
--
-- TODO: use unlifted newtypes (cf proposal:
...
wordsToBytes# i = i `uncheckedIShiftL#` WORD_SIZE_SHIFT#

-- | Convert byte count into limb count
bytesToWords# :: Int# -> Int#
bytesToWords# i = i `uncheckedIShiftRL#` WORD_SIZE_SHIFT#
...
mwaInitArrayPlusWord mwa wa = go 0#
   where
      ...
      go i carry s
         | isTrue# (i ># sz)  = s
         | isTrue# (i ==# sz) = mwaWriteMostSignificant mwa carry i s
...
--    * otherwise: write it
mwaWriteMostSignificant :: MutableWordArray# s -> Word# -> Int# -> State# s -> State# s
mwaWriteMostSignificant mwa 0## _i s = mwaShrink# mwa 1# s
mwaWriteMostSignificant mwa  w   i s = mwaWrite# mwa i w s
...
mwaWriteMostSignificant' mwa w s =
   case mwaSize# mwa s of
      (# s', sz #) -> mwaWriteMostSignificant mwa w (sz -# 1#) s'
...
mwaAddInplace :: MutableWordArray# d -> Int# -> Word# -> State# d -> State# d
mwaAddInplace   _ _ 0## s = s
mwaAddInplace mwa i y   s = case readWordArray# mwa i s of
   (# s1, x #) -> let !(# h,l #) = plusWord2# x y
                  in case mwaWrite# mwa i l s1 of
                        s2 -> mwaAddInplace mwa (i +# 1#) h s2
...
mwaSubWordInplace
   ...
   -> State# d
   -> (# State# d, Bool# #)
mwaSubWordInplace mwa ii iw s1 = case mwaSize# mwa s1 of
   (# is, sz #) -> 
      let
...
--
-- Return True# on overflow.
mwaSubArrayInplace :: MutableWordArray# d -> Int# -> WordArray# -> State# d -> (# State# d, Bool# #)
mwaSubArrayInplace mwa off wb = go (wordArraySize# wb -# 1#)
   where
      go i s
         | isTrue# (i <# 0#) = (# s, 0# #) -- no overflow
         | True
         = case mwaSubWordInplace mwa (off +# i) (indexWordArray# wb i) s of
            (# s2, 0# #) -> go (i -# 1#) s2 
            (# s2, _  #) -> (# s2, 1# #) -- overflow
...
--
-- Return True# on overflow.
mwaSubMutableArrayInplace :: MutableWordArray# d -> Int# -> MutableWordArray# d -> State# d -> (# State# d, Bool# #)
mwaSubMutableArrayInplace mwa off mwb s0 = 
   ...
   where
      go i s
         ...
         = case readWordArray# mwb i s of
            (# s1, bi #) -> case mwaSubWordInplace mwa (off +# i) bi s1 of
               (# s2, 0# #) -> go (i -# 1#) s2 
               (# s2, _  #) -> (# s2, 1# #) -- overflow
...
mwaSubArrayTrimInplace :: MutableWordArray# d -> Int# -> WordArray# -> State# d -> State# d
mwaSubArrayTrimInplace mwa off wb s =
   case mwaSubArrayInplace mwa off wb s of
