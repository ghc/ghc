   ...
   | True =
      ...
      in withNewWordArrayTrimed# szA \mwa s->
            case inline bignat_sub mwa a b s of
               (# s', 0# #) -> s'
               (# s', _  #) -> case raiseUnderflow of W# _ -> s'
...
bigNatQuotWord# a b
   | 1## <- b = a
   | 0## <- b = case raiseDivZero of W# err -> bigNatFromWord# err
   | True =
   let
      sz = wordArraySize# a
...
bigNatRemWord# a b
   | 0## <- b       = case raiseDivZero of W# err -> err
   | 1## <- b       = 0##
   | bigNatIsZero a = 0##
...
bigNatQuotRemWord# a b
   | 0## <- b = case raiseDivZero of W# err -> (# bigNatZero void#, err #)
   | 1## <- b = (# a, 0## #)
   | isTrue# (bigNatSize# a ==# 1#)
...
bigNatQuotRem# a b
   | bigNatIsZero b          = case raiseDivZero of W# err -> (# bigNatFromWord# err, bigNatZero void# #)
...
bigNatQuot a b
   | bigNatIsZero b          = case raiseDivZero of W# err -> bigNatFromWord# err
   | bigNatIsZero a          = bigNatZero void#
   | bigNatIsOne b           = a
...
bigNatLogBaseWord# base a
   | 0## <- base = case unexpectedValue of W# err -> err
   | 1## <- base = case unexpectedValue of W# err -> err
   | 2## <- base = bigNatLog2# a
   -- TODO: optimize log base power of 2 (256, etc.)
...
bigNatPowModWord# !_ !_ 0## = case raiseDivZero of W# err -> err
...
bigNatPowMod !b !e !m
   ...
   | bigNatIsZero m = case raiseDivZero of W# err -> bigNatFromWord# err
