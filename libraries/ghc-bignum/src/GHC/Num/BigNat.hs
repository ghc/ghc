...
#else
#error Undefined BigNat backend. Use `stack build --flag ghc-bignum:[gmp,ffi,native]` or `cabal configure -f [gmp,ffi,native]`
#endif
...
-- | A BigNat
--
-- Represented as an array of Word# stored in little-endian order (Word#
...
-- | BigNat Zero
bigNatZero :: () -> BigNat
{-# NOINLINE bigNatZero #-} -- share the CAF (static updatable thunk)
bigNatZero _ = withNewWordArray# 0# (\_ s -> s)
...
-- | Convert a number composed of `n` Word# stored in little endian order at the
-- given address. Null higher limbs are automatically trimed.
bigNatFromAddr# :: Word# -> Addr# -> BigNat
bigNatFromAddr# n addr = withNewWordArrayTrimed# n' \mwa s ->
      copyAddrToByteArray# addr mwa 0# (wordsToBytes# n') s
   where
      n' = word2Int# n

-- | Convert a number composed of `n` Word# stored in little endian order at the
-- given address. Null higher limbs are NOT automatically trimed.
bigNatFromAddrUnsafe# :: Word# -> Addr# -> BigNat
bigNatFromAddrUnsafe# n addr = withNewWordArray# n' \mwa s ->
      copyAddrToByteArray# addr mwa 0# (wordsToBytes# n') s
   where
      n' = word2Int# n
...
bigNatToWord64# b
  ...
  = if isTrue# (bigNatSize# b ># 1#)
      then
         let wh = wordToWord64# (indexBigNat# b 1#)
...
bigNatQuotWord# a b
   | 1## <- b = a
   | 0## <- b = case divByZero of _ -> bigNatZero ()
   | True =
   let
      sz = wordArraySize# a
...
bigNatQuotRemWord# a b
   | 0## <- b = case divByZero of _ -> (# bigNatZero (), 0## #)
   | 1## <- b = (# a, 0## #)
   | isTrue# (bigNatSize# a ==# 1#)
...
bigNatQuotRem# a b
   | bigNatIsZero b          = case divByZero of _ -> (# bigNatZero (), bigNatZero () #)
...
bigNatQuot a b
   | bigNatIsZero b          = case divByZero of _ -> bigNatZero ()
   | bigNatIsZero a          = bigNatZero ()
   | bigNatIsOne b           = a
   | LT <- cmp               = bigNatZero ()
   | EQ <- cmp               = bigNatOne ()
...
bigNatPowMod !b !e !m
   ...
   | bigNatIsZero m = case divByZero of _ -> bigNatZero ()
...
-- | Write a BigNat in base-256 representation and return the number of bytes
-- written.
--
...
-- zero bytes written.
bigNatToAddr# :: BigNat -> Addr# -> State# s -> (# State# s, Word# #)
bigNatToAddr# a addr s0
   | isTrue# (sz ==# 0#) = (# s0, 0## #)
   | True = go 0# 0## 0# WORD_SIZE_IN_BITS# s0
   where
     sz   = wordArraySize# a
     li   = sz -# 1#
     ...
     go ai w i sh s
      ...
      | WORD_SIZE_IN_BITS# <- sh
      = go ai (indexWordArray# a i) (i +# 1#) 0# s

      | True
      = case writeWord8OffAddr# addr ai (w' `and#` 0xFF##) s of
            s' -> go (ai +# 1#) w i (sh +# 8#) s'
      where
         w' = w `uncheckedShiftRL#` sh
