...
bigNatTests = testGroup "BigNat"
   ...
   , testProperty "Test zero" $
         bigNatIsZero (bigNatZero void#) === True
   ...
   , testProperty "1 `shiftL` n ==> isPowerOf2 n" $
         \n -> let
                  a = bigNatOne void# `bigNatShiftL` n
...
# error unsupported WORD_SIZE_IN_BITS config
   ...
   , testProperty "x `quotRem` x == (BigNat [1], 0)" $
         \xs ->
            let
               x     = bigNatFromWordList xs
            in not (bigNatIsZero x) ==> (x `bigNatQuotRem` x) === (BN (bigNatOne void#), BN (bigNatZero void#))
   ...
   , testProperty "ctz 0" $ bigNatCtz (bigNatFromWord 0) === 0

   , testProperty "ctz 0x1000..00" $ bigNatCtz (bigNatFromWord2# 0x1000## 0##) === WORD_SIZE_IN_BITS + 12

   , testProperty "x `shiftR` ctz x `shiftL` ctz x == x" $ \(BN x) ->
      let
         n = bigNatCtz x
   ...
   , testProperty "Roundtrip through base-256 storage (little-endian)" $
        \(BN n) -> monadicIO do
           let sz = bigNatSizeInBase 256 n
           p@(Ptr addr) <- run $ mallocBytes (fromIntegral sz)
           szw@(W# szw') <- run $ bigNatToAddr n addr 0#
           assert (szw == sz)
           bn' <- run $ IO \s -> case bigNatFromAddr# szw' addr 0# s of
                                   (# s', n' #) -> (# s', BN n' #)
           assert (BN n == bn')
           run (free p)

   , testProperty "Roundtrip through base-256 storage (big-endian)" $
        \(BN n) -> monadicIO do
           let sz = bigNatSizeInBase 256 n
           p@(Ptr addr) <- run $ mallocBytes (fromIntegral sz)
           szw@(W# szw') <- run $ bigNatToAddr n addr 1#
           assert (szw == sz)
           bn' <- run $ IO \s -> case bigNatFromAddr# szw' addr 1# s of
                                   (# s', n' #) -> (# s', BN n' #)
           assert (BN n == bn')
           run (free p)

   , testProperty "Roundtrip through base-256 ByteArray (little-endian)" $
        \(BN n) off -> monadicIO do
           let !(W# sz)   = bigNatSizeInBase 256 n
               !(W# off') = off
           MBA mba       <- run $ IO \s -> case newByteArray# (word2Int# sz +# word2Int# off') s of
                                       (# s', mba #) -> (# s', MBA mba #)
           szw@(W# szw') <- run $ IO \s -> case bigNatToMutableByteArray# n mba off' 0# s of
                                       (# s', w #) -> (# s', W# w #)
           assert (szw == W# sz)
           BA ba <- run $ IO \s -> case unsafeFreezeByteArray# mba s of
                                    (# s', ba #) -> (# s', BA ba #)
           bn'   <- run $ IO \s -> case bigNatFromByteArray# szw' ba off' 0# s of
                                    (# s', n' #) -> (# s', BN n' #)
           assert (BN n == bn')

   , testProperty "Roundtrip through base-256 ByteArray (big-endian)" $
        \(BN n) off -> monadicIO do
           let !(W# sz)   = bigNatSizeInBase 256 n
               !(W# off') = off
           MBA mba       <- run $ IO \s -> case newByteArray# (word2Int# sz +# word2Int# off') s of
                                       (# s', mba #) -> (# s', MBA mba #)
           szw@(W# szw') <- run $ IO \s -> case bigNatToMutableByteArray# n mba off' 1# s of
                                       (# s', w #) -> (# s', W# w #)
           assert (szw == W# sz)
           BA ba <- run $ IO \s -> case unsafeFreezeByteArray# mba s of
                                    (# s', ba #) -> (# s', BA ba #)
           bn'   <- run $ IO \s -> case bigNatFromByteArray# szw' ba off' 1# s of
                                    (# s', n' #) -> (# s', BN n' #)
           assert (BN n == bn')
   ]
