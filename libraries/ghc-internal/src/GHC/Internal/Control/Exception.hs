...
'GHC.Internal.Control.Concurrent.MVar.takeMVar'
(but not 'GHC.Internal.Control.Concurrent.MVar.tryTakeMVar'),
...
if the 'GHC.Internal.Control.Concurrent.MVar.takeMVar' was not interruptible,
then this particular
combination could lead to deadlock, because the thread itself would be
blocked in a state where it can\'t receive any asynchronous exceptions.
With 'GHC.Internal.Control.Concurrent.MVar.takeMVar' interruptible, however, we can be
safe in the knowledge that the thread can receive exceptions right up
until the point when the 'GHC.Internal.Control.Concurrent.MVar.takeMVar' succeeds.
Similar arguments apply for other interruptible operations like
'GHC.Internal.System.IO.openFile'.
 ...
 * STM transactions that do not use 'GHC.Internal.Conc.retry'

 * everything from the @Foreign@ modules

 * everything from "Control.Exception" except for 'throwTo'

 * 'GHC.Internal.Control.Concurrent.MVar.tryTakeMVar', 'GHC.Internal.Control.Concurrent.MVar.tryPutMVar',
   'GHC.Internal.Control.Concurrent.MVar.isEmptyMVar'

 * 'GHC.Internal.Control.Concurrent.MVar.takeMVar' if the 'Control.Concurrent.MVar.MVar' is
   definitely full, and conversely 'GHC.Internal.Control.Concurrent.MVar.putMVar' if the
   'Control.Concurrent.MVar.MVar' is definitely empty

 * 'GHC.Internal.Control.Concurrent.MVar.newEmptyMVar', 'GHC.Internal.Control.Concurrent.MVar.newMVar'
