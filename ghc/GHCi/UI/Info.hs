...
findNameUses infos span0 _string =do
    ...
    case resolveName (modinfoHieFile info) span0 of
        Nothing   -> return []
        Just name -> return $ map (\srcSpan -> RealSrcSpan srcSpan Nothing) $ concatMap (search name) (M.elems infos)
  where
...
resolveName hf span = resolveSmallestHieAST hf span >>= listToMaybe . rights . M.keys . nodeIdentifiers . nodeInfo
...
getHieFile m typechecked = do
    withSession $ \hscEnv ->
        case dependencyState (ms_hs_date m) (ms_hie_date m) of
            DSUpToDate -> do
                hieFileResult <- liftIO $ readHieFile (NCU $ updNameCache $ hsc_NC hscEnv) (ml_hie_file . ms_location $ m)
