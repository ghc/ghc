...
getNodeInfo = (M.! SourceInfo) . getSourcedNodeInfo .  sourcedNodeInfo
...
findNameLocs name ast = if contained then (nodeSpan ast) : rest else rest
  where
    contained = M.member (Right name) . nodeIdentifiers . getNodeInfo $ ast
...
findNameUses infos span0 _string = do
    ...
    case resolveName (modinfoHieFile info) span0 of
        Nothing   -> return []
        Just name -> return $ map (\srcSpan -> RealSrcSpan srcSpan Strict.Nothing) $ concatMap (search name) (M.elems infos)
  where
...
searchASTs :: HieFile -> (HiePath -> (HieAST TypeIndex) -> [a]) -> [a]
searchASTs hf = concat . flip M.mapWithKey (getAsts $ hie_asts hf)

-- | Try to resolve a source span to the smallest ast containing it
resolveSmallestHieAST :: HieFile -> RealSrcSpan -> Maybe (HieAST TypeIndex)
resolveSmallestHieAST hf span = listToMaybe $ searchASTs hf $ \_fs ast ->
    maybeToList $ selectSmallestContaining span ast

resolveHieASTAndTypes :: HieFile -> RealSrcSpan -> Maybe (HieAST HieTypeFix)
resolveHieASTAndTypes hf span = fmap (fmap toHieTypeFix) (resolveSmallestHieAST hf span)
  where
...
getAllTypesHieAST ast = directTypes ++ identifierTypes
  where
    directTypes = nodeType . getNodeInfo $ ast
    identifierTypes = catMaybes . fmap identType . fmap snd . M.toList . nodeIdentifiers . getNodeInfo $ ast

-- | Try to resolve the type display from the given span.
resolveName :: HieFile -> RealSrcSpan -> Maybe Name
resolveName hf span = resolveSmallestHieAST hf span >>= listToMaybe . rights . M.keys . nodeIdentifiers . getNodeInfo
...
getHieFile m typechecked = do
    withSession $ \hscEnv ->
        case dependencyState (ms_iface_date m) (ms_hie_date m) of
            DSUpToDate -> do
                hieFileResult <- liftIO $ readHieFile (hsc_NC hscEnv) (ml_hie_file . ms_location $ m)
