...
computeHieFile :: (GhcMonad m) => ModSummary -> TypecheckedModule -> m HieFile
computeHieFile summary typechecked = do
    ...
    withSession $ \hscEnv -> liftIO $ runHsc hscEnv (mkHieFile summary tc_gbl_env (fromJust rn_info))

wrapReadHieFile :: FilePath -> IO (Maybe HieFile)
wrapReadHieFile path = do
    u <- mkSplitUniqSupply 'a'
    (hieFileResult, _) <- readHieFile (initNameCache u []) path
...
getHieFile m typechecked =
    case dependencyState (ms_hs_date m) (ms_hie_date m) of
        DSUpToDate -> do
            hieFileOpt <- liftIO $ wrapReadHieFile (ml_hie_file . ms_location $ m)
            case hieFileOpt of
                Just hieFile -> do
                    liftIO . putStrLn $ "Using up to date hie file for " ++ srcFilePath m
                    return hieFile
                Nothing -> computeHieFile m typechecked
        _ -> computeHieFile m typechecked
