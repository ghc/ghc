...
checkReadHieFile :: FilePath -> NameCache -> IO (Maybe HieFile)
checkReadHieFile path nc = do
    (hieFileResult, _) <- readHieFile nc path
    if hie_file_result_version hieFileResult == hieVersion
     then
        return . Just . hie_file_result $ hieFileResult
     else
        return Nothing
    ...
    withSession $ \hscEnv ->
        case dependencyState (ms_hs_date m) (ms_hie_date m) of
            DSUpToDate -> do
                ...
                hieFileOpt <- liftIO $ checkReadHieFile (ml_hie_file . ms_location $ m) nc
                case hieFileOpt of
                    Just hieFile -> do
