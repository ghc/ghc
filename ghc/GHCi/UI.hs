...
installInteractiveHomeUnits = do
  ...
  ghciPromptUnit  <- setupHomeUnitFor logger dflagsPrompt  (ue_index $ hsc_unit_env hsc_env) all_unit_ids cached_unit_dbs
  ghciSessionUnit <- setupHomeUnitFor logger dflagsSession (ue_index $ hsc_unit_env hsc_env) all_unit_ids cached_unit_dbs
  let
  ...
  where
    ...
    setupHomeUnitFor logger dflags unit_index all_home_units cached_unit_dbs = do
      (dbs,unit_state,home_unit,_mconstants) <-
        liftIO $ initUnits logger dflags unit_index (Just cached_unit_dbs) all_home_units
...
addToProgramDynFlags lopts = do
  ...
  updates <- HUG.unitEnv_traverseWithKey (\uid homeUnitEnv -> do
    ...
    (newFlags, leftovers, warns) <- DynFlags.parseDynamicFlagsCmdLine oldFlags lopts
