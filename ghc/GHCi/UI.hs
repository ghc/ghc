...
-- compilation of this module to object-code, if the module uses (or is dependend
-- of one using) the UnboxedSums/Tuples extensions.
...
-- DynFlags.
-- In GHC versions >= 9.0.1 the `OPTIONS_GHC -fbyte-code` pragma
-- will be replaced with a `OPTIONS_GHC -fno-object-code-if-unboxed` pragma.
