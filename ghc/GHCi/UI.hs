...
-- Return a list of breakpoint idnetifiers with a given prefix
completeBreakpoint = wrapCompleter spaces $ \w -> do          -- #3000
    -- See Note [Tab-completion for :break]
    -- bid ~ breakpoint identifier = a name of a functio that is
  ...
  where
    ...
    bidsFromModBreaks mod_pref = do
        ...
        -- If the user doesn't supply any prefix, then for functions
        -- declared in a module in scope, we don't want a module prefix.
        -- (eg only `main` instead of `Main.main`)
    ...
    bidsByModule nonquals mod = do
      ...
      let bids = nub $ showDecl <$> elems decls
          rslt = case (moduleName mod) `elem` nonquals of
              True -> bids
...
breakById inp = do
    ...
    -- If the top-level name is not in scope, then `lookupModuleInscope` will
    -- throw an exception, and we lookup the module name in the module graph.
  ...
  where
    ...
    lookupModuleInscope mod_top_lvl = do
        ...
        pure $ Just $ head $ GHC.nameModule <$> names
          ...
          -- if it fails, we land in the exception
    ...
    validateBP :: GhciMonad m => String -> String -> Maybe Module
                       -> m (Maybe SDoc)
    validateBP mod_inp str_fun Nothing = pure $ Just $ quotes (text
        (combineModIdent mod_inp (Prelude.takeWhile (/= '.') str_fun)))
        <+> text "not in scope"
    validateBP _ "" (Just _) = pure $ Just $ text "Function name is missing"
    validateBP _ str_fun (Just modl) = do
        ...
        mb_err_msg <- case isInterpr of
          ...
          True -> case str_fun `elem` (showDecl <$> elems decls) of
                False -> pure $ Just $
                   text "No breakpoint found for" <+> quotes (text str_fun)
...
To lookup a breakpoint in the ModBreaks, the effective module name
is needed. In `:break M.foo` `M` may be a module name or a local alias
of an import statement. Even if a module called `M` exists, `M` may still be
...
-- Return a String with the name of the function of a breakpoint.
-- See Note [ModBreaks.decls] in GHC.ByteCode.Types
showDecl :: [String] -> String
showDecl = intercalate "."
