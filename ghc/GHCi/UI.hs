...
ignoreCmd argLine = withSandboxOnly ":ignore" $ do
    ...
    case result of
      ...
      Right (loc, mbCount)   -> do
        ...
        setIgnoreCount breakInfo count
...
setIgnoreCount :: GhciMonad m => GHC.BreakInfo -> Int -> m()
setIgnoreCount loc count = do
    hsc_env <- GHC.getSession
    GHC.setIgnoreCount hsc_env loc count
...
setBreakFlag  md ix enaDisa = do
  ...
  setIgnoreCount (GHC.BreakInfo md ix) $ enaDisaToCount enaDisa
