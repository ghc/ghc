...
doBuildPlan out units targets = do
  ...
  build_plan <- case hs_srcs of
    ...
    _  -> do
      ...
      GHC.setTargets targets'
      computeBuildPlan
  let on_usage ms (UsageFile {..}) acc = JSObject [("unit_id", JSString $ unitIdString $ ms_unitid ms), ("module_name", JSString $ moduleNameString $ moduleName $ ms_mod ms), ("usage_file", JSString $ unpackFS usg_file_path)] : acc
      on_usage _ _ acc = acc

      on_mod_summary ms acc = do
        maybe_iface <- speculateIface ms
        pure $ case maybe_iface of
          Just iface -> foldr (on_usage ms) acc $ mi_usages iface
          Nothing -> acc
