...
doInteractiveMulti dflags0 unitArgsFiles = do
  ...
  hsc_env <- getSession
  let (unitEnv, mainUnitId) = createUnitEnvFromFlags unitDflags

  let homeUnits = Map.keys $ unitEnv_graph unitEnv
  unitEnv <- forM unitEnv $ \env -> do
...
doMulti dflags0 unitArgsFiles  = do
  ...
  hsc_env <- GHC.getSession
  let (unitEnv, _mainUnitId) = createUnitEnvFromFlags unitDflags
  hsc_env <- pure $ set_hsc_internalUnitEnvGraph unitEnv hsc_env
  ...
  o_files <- liftIO $ mapM
                (\(src, uid, mphase) ->
                  compileFile (set_hsc_currentUnit uid hsc_env) StopLn (src, mphase)
                )
                non_hs_srcs
  dflags <- GHC.getSessionDynFlags
  let dflags' = dflags { ldInputs = map (FileOption "") o_files
