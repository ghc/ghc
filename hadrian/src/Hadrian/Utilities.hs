...
createFileLinkUntracked linkTarget link = do
    ...
    -- If link is a directory, we let createFileLink throw an exception.
    -- Note that `doesFileExist link == False` when link is a symlink.
    liftIO $ do
        linkExistsFile <- IO.doesFileExist link
        if linkExistsFile
        then IO.removeFile link
        -- link may still be a broken symlink.
