...
{- | Provides utilities for figuring out the right target to build when
     ...
     directory.
-}
module Rules.TargetDirs (getTestDir, getDirAction, performDirAction) where
...
getDirAction :: FilePath -> Action (Maybe DirAction)
getDirAction dir = do
  top <- topDirectory
  ...
  where verifyPrefix top d
          ...
          | otherwise = return $ targetForPath (makeRelative top d)

getTestDir :: Action (Maybe FilePath)
getTestDir = do
  mdir <- cmdTargetDir
  mact <- maybe (return Nothing) getDirAction mdir
  return $ case mact of
...
{-# NOINLINE allTestDirs #-}
allTestDirs :: [FilePath]
allTestDirs = unsafePerformIO $ do
  putStrLn "Looking for test directories..."
  map takeDirectory <$> getDirectoryFilesIO "." ["**/all.T"]

{-# NOINLINE targetsTrie #-}
targetsTrie :: Trie FilePath DirAction
targetsTrie = fromList $
  [ (pathComponents p,     BuildPackage p) | p <- knownPackages ] ++
  [ (splitDirectories dir, RunTestDir dir) | dir <- allTestDirs ]

  where pathComponents p = splitDirectories (pkgPath p)

targetForPath :: FilePath -> Maybe DirAction
targetForPath dir = fst <$> lookupPath (splitDirectories dir) targetsTrie
