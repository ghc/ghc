...
So, while regular bindists are usual stay-on-same-arch compilers or
cross-compilers, stage3 bindists are cross-compiled compilers.
...
compilers. The additional build dir (bindist-stage3) let's us keep both, such
...
From now on, we call the directory containing the uncompressed bindist
bindist-dir, regardless of the stage. While we refer to the concrete directory
name in case there is a difference between stage2 and stage3 compilers.
...
- copy the specific binaries which should be in the bindist to the
  ...
  to
    <build root/<bindist-dir>/ghc-<X>.<Y>.<Z>-<arch>-<os>/bin/xxxx-<VER>
  where executable-stage-dir is:
...
buildBinDistDir root conf@BindistConfig{..} = do
    ...
    when (verbosity >= Verbose) $ do
      let libNames = map (pkgName . fst) lib_targets
          pgmNames = map (pkgName . fst) bin_targets
