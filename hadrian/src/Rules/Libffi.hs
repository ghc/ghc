...
explanation of "indicative input". The definition is copied here for
your convenience:
...
and so the archive file is still a valid choice sole indicating input of
...
dependant libffi rules i.e. to generate vital inputs as is noted on the
wiki).
...
-- See [Libffi indicating inputs]. as the archive is the sole indicating
-- input it is safe to `trackAllow` the libffi build dir.
