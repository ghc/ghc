module Rules.Libffi (
    needLibffi, libffiRules, libffiLibrary, libffiHeaderFiles,
...
-- | Need the (locally built) libffi library. Returns the list of dynamic.
-- library files (all but one of which should be sym links).
needLibffi :: Stage -> Action [FilePath]
needLibffi stage = readFileLines =<< dynLibManifest stage   -- TODO cache this
