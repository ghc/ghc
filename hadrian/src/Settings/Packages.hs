...
rtsPackageArgs = package rts ? do
    ...
    let ghcArgs = mconcat
          [ arg "-Irts"
          , arg $ "-I" ++ path
          , flag WithLibdw ? mconcat
              [ if not (null libdwIncludeDir) then arg ("-I" ++ libdwIncludeDir) else mempty
              , if not (null libdwLibraryDir) then arg ("-L" ++ libdwLibraryDir) else mempty
              ]
