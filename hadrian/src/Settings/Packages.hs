    ...
    mconcat
        ...
        , package compiler ? mconcat
          ...
          , builder (Cabal Setup) ? mconcat
            ...
            -- When building stage 0, use thread safe RTS functions if the
            -- bootstrapping compiler provides a threaded RTS way.
        ...
        , package ghc ? mconcat
          ...
          , builder (Cabal Flags) ? mconcat
            ...
            , ifM stage0
                  -- We build a threaded stage 0 if the bootstrapping compiler
                  -- supports it.
                  (ifM threadedBootstrapper
                       (arg "threaded")
                       (arg "-threaded"))
                  -- We build a threaded stage 1 if the configuration calls
                  -- for it.
