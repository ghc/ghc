...
primops also have internal implementations under certain conditions. In
fact, the interal implementation must be used when they do. Now that
`foreign import prim` exists, only those primops which have both
internal and external implementations ought to be this file. The rest
aren't really primops, since they don't need bespoke compiler support
but just a general way to interface with C--. They are just foreign
calls.
...
https://gitlab.haskell.org/ghc/ghc/merge_requests/1330#note_211467 has a
table of which external-only primops are blocked by which of these.
Hopefully those restrictions are relaxed so the rest of those can be
moved over.

'module GHC.Prim.Ext is a temporarily "holding ground" for primops that
were formally in here, until they can be given a better home. Likewise,
their underlying C-- implementation need not live in the RTS either.
Best case (in my view), both the C-- and `foreign import prim` can be
moved to a small library tailured to the features being implemented and
dependencies of those features.
