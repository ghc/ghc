...
exprArity e = go e
  where
    ...
    go (Cast e co)                 = trim_arity (go e) (coercionRightKind co)
...
arityType env (Cast e co)
  = case arityType env e of
      ...
      ABot n | co_arity < n -> ATop (replicate co_arity noOneShotInfo)
             | otherwise    -> ABot n
  where
    co_arity = length (typeArity (coercionRightKind co))
...
etaInfoAppTy _  (EtaCo co : eis) = etaInfoAppTy (coercionRightKind co) eis
