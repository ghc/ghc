...
isExprLevPoly = go
  where
   ...
   go_app (Cast _ co)     = resultIsLevPoly (coercionRightKind co)
...
mkCast (Coercion e_co) co
  | isCoVarType (coercionRightKind co)
...
mkCast (Cast expr co2) co
  = WARN(let { from_ty = coercionLeftKind co;
               to_ty2  = coercionRightKind co2 } in
...
mkCast expr co
  = let from_ty = coercionLeftKind co in
