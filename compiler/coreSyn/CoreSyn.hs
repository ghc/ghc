...
module CoreSyn (
        ...
        applyTypeToArg, applyTypeToArgs,
...
-- This used to live in CoreUtils, but we need it for the App type synonym.
-- We could move this and the type synonym to CoreUtils, but then we would also
-- have to move collectArgs and mkApps there.
...
exprType _other = pprTrace "exprType" (empty {- import loop :-( (pprCoreExpr other) -}) alphaTy
...
applyTypeToArgs _e op_ty args
  ...
  where
    ...
    panic_msg = text "temporary removed due to import cycles"
    {-
