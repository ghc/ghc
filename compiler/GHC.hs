...
setUnitDynFlagsNoCheck uid dflags1 = do
  ...
  (cache, (dbs,unit_state,home_unit,mconstants)) <- liftIO $ initUnitsWithCache logger dflags1 index cached_unit_dbs (hsc_all_home_unit_ids hsc_env) emptySharedProviders
  ...
  modifySession $ \h -> h{ hsc_unit_env  = unit_env1 { ue_index = newUnitIndex cache }
                         }
