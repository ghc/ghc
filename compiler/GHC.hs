...
setSessionDynFlags dflags0 = do
  ...
  hsc_env <- getSession
  (dbs,unit_state,home_unit,mconstants) <- liftIO $ initUnits logger dflags1 (hsc_unit_dbs hsc_env)
...
setProgramDynFlags_ invalidate_needed dflags = do
  ...
  if changed
    then do
        hsc_env <- getSession
        (dbs,unit_state,home_unit,mconstants) <- liftIO $ initUnits logger dflags0 (hsc_unit_dbs hsc_env)
