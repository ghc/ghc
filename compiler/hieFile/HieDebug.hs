...
diffFile :: PrimOpCache -> Diff HieFile
diffFile primOpCache = diffAsts primOpCache eqDiff `on` (getAsts . hie_asts)

diffAsts :: (Outputable a, Eq a) => PrimOpCache -> Diff a -> Diff (M.Map FastString (HieAST a))
diffAsts primOpCache f = diffList (diffAst primOpCache f) `on` M.elems

diffAst :: (Outputable a, Eq a) => PrimOpCache -> Diff a -> Diff (HieAST a)
diffAst primOpCache diffType (Node info1 span1 xs1) (Node info2 span2 xs2) =
    infoDiff ++ spanDiff ++ diffList (diffAst primOpCache diffType) xs1 xs2
  where
    spanDiff
    ...
    diffIdents a b = (diffList diffIdent `on` normalizeIdents primOpCache) a b
...
normalizeIdents primOpCache = sortOn fst . map (first $ toHieName primOpCache) . M.toList
  where
