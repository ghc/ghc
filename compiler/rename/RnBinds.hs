...
rnMethodBindLHS is_cls_decl cls located@(L loc bind) rest = case bind of
  FunBind { fun_id = name } ->
    mkValid name $ \name' -> bind { fun_id = name', fun_ext = noExtField }
  PatBind { pat_lhs = VarPat x name, pat_rhs = grhss, pat_ext = pat_fvs } -> do
    mod <- getModule
    (grhss', rhs_fvs) <- rnGRHSs PatBindRhs rnLExpr grhss
    let
      all_fvs = rhs_fvs
      fvs'    = filterNameSet (nameIsLocalOrFrom mod) all_fvs
    mkValid name $ \name' -> bind { pat_lhs = VarPat x name'
                                  , pat_ext = fvs'
                                  }
  PatBind { pat_lhs = XPat (L loc' (VarPat x name)), pat_rhs = grhss, pat_ext = pat_fvs } -> do
    mod <- getModule
    (grhss', rhs_fvs) <- rnGRHSs PatBindRhs rnLExpr grhss
    let
      all_fvs = rhs_fvs
      fvs'    = filterNameSet (nameIsLocalOrFrom mod) all_fvs
    mkValid name $ \name' -> bind { pat_lhs = XPat (L loc' (VarPat x name'))
                                  , pat_ext = fvs'
                                  }
  _ -> do
    let
