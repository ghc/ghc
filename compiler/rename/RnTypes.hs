...
-- | 'extractHsTyRdrTyVars' finds the kind/type variables
-- of a HsType'/HsKind.
...
-- | 'extractHsTyRdrTyVarsDups' finds the kind/type variables
-- of a HsType'/HsKind.
-- It's used when making the @forall@s explicit.
-- Does not return any wildcards.
...
-- Note [Ordering of implicit variables].
extractHsTyRdrTyVarsKindVars :: LHsType GhcPs -> TcRn FreeKiTyVarsNoDups
extractHsTyRdrTyVarsKindVars ty = do
  polykinds <- xoptM LangExt.PolyKinds
  return $ if polykinds then extractHsTyRdrTyVars ty
                        else []
...
--
--  * data T a (b :: k1) :: k2 -> k1 -> k2 -> Type   -- result: [k1,k2]
