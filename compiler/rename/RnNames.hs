...
reportUnusedNames _export_decls gbl_env
  = do  { dus <- readMutVar (tcg_dus gbl_env)
        ; traceRn "RUN" (ppr dus)
        ; warnUnusedImportDecls gbl_env
        ; warnUnusedTopBinds $ unused_locals dus
        ; warnMissingSignatures gbl_env }
  where
    used_names :: DefUses -> NameSet
    used_names dus = findUses dus emptyNameSet
    ...
    unused_locals :: DefUses -> [GlobalRdrElt]
    unused_locals dus =
      let -- Note that defined_and_used, defined_but_not_used
          ...
          (_defined_and_used, defined_but_not_used)
              = partition (gre_is_used (used_names dus)) defined_names
