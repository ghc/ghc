...
lookupImpDeprec iface gre
  = mi_warn_fn (mi_exts iface) (greOccName gre) `mplus`  -- Bleat if the thing,
    case gre_par gre of                      -- or its parent, is warn'd
       ParentIs  p              -> mi_warn_fn (mi_exts iface) (nameOccName p)
       FldParent { par_is = p } -> mi_warn_fn (mi_exts iface) (nameOccName p)
       NoParent                 -> Nothing
