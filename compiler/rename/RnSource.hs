...
rnTyClDecls tycl_ds
  = do { -- Rename the type/class, instance, and role declaraations
       ...
       ; let tycl_sccs = depAnalTyClDecls rdr_env tlks_fv_env tycls_w_fvs
             role_annot_env = mkRoleAnnotEnv role_annots
             (tlks_env, tlks_fv_env) = mkTLKS_fv_env (tlkss_w_fvs ++ cusks_w_fvs)
...
-- | Free variables of top-level kind signatures.
newtype TLKS_FV_Env = TLKS_FV_Env (NameEnv FreeVars)

lookupTLKS_FV_Env :: TLKS_FV_Env -> Name -> FreeVars
lookupTLKS_FV_Env (TLKS_FV_Env e) name
  = fromMaybe emptyFVs (lookupNameEnv e name)

-- | Top-level kind signatures.
type TLKS_Env = NameEnv (LTopKindSig GhcRn)

mkTLKS_fv_env :: [(LTopKindSig GhcRn, FreeVars)] -> (TLKS_Env, TLKS_FV_Env)
mkTLKS_fv_env tlkss_w_fvs = (tlks_env, tlks_fv_env)
  where
    tlks_env = mapNameEnv fst compound_env
    tlks_fv_env = TLKS_FV_Env (mapNameEnv snd compound_env)
    compound_env :: NameEnv (LTopKindSig GhcRn, FreeVars)
      = mkNameEnvWith (tlksName . unLoc . fst) tlkss_w_fvs

getTLKSs :: [Name] -> TLKS_Env -> [LTopKindSig GhcRn]
getTLKSs bndrs tlks_env = mapMaybe (lookupNameEnv tlks_env) bndrs

rnTLKSs :: NameSet  -- names of types and classes in the current TyClGroup
        -> [LTopKindSig GhcPs]
        -> RnM [(LTopKindSig GhcRn, FreeVars)]
rnTLKSs tc_names tlkss
  = do { let (no_dups, dup_tlkss) = removeDups (compare `on` get_name) tlkss
             get_name = tlksName . unLoc
       ; mapM_ dupTLKS_Err dup_tlkss
       ; mapM (wrapLocFstM (rnTLKS tc_names)) no_dups
       }

rnTLKS :: NameSet  -- names of types and classes in the current TyClGroup
       -> TopKindSig GhcPs
       -> RnM (TopKindSig GhcRn, FreeVars)
rnTLKS tc_names (TopKindSig _ prov v ki)
  = do  { tlks_ok <- xoptM LangExt.TopLevelKindSignatures
        ; unless tlks_ok $ addErr tlksErr
        ; new_v <- lookupSigCtxtOccRn (TopSigCtxt tc_names) (text "top-level kind signature") v
...
depAnalTyClDecls rdr_env tlks_fv_env ds_w_fvs
    ...
    edges = [ DigraphNode d name (map (getParent rdr_env) (nonDetEltsUniqSet deps))
            | (d, fvs) <- ds_w_fvs,
              let { name = tcdName (unLoc d)
                  ; tlks_fvs = lookupTLKS_FV_Env tlks_fv_env name
                  ; deps = fvs `plusFV` tlks_fvs
                  }
            ]
