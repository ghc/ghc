...
extendPatSynEnv val_decls local_fix_env thing = do {
  ...
  where
    ...
    new_ps (ValBindsIn _ binds _) = foldrBagM new_ps' [] binds
    ...
    new_ps' bind names
      ...
      = do
          ...
          let rnames = map recordPatSynSelectorId as
              ...
              mkFieldOcc (L l name) = L l (FieldOcc PlaceHolder (L l name))
...
add_bind b (ValBindsIn x bs sigs) = ValBindsIn x (bs `snocBag` b) sigs
add_bind _ (ValBindsOut {})       = panic "RdrHsSyn:add_bind"

add_sig :: LSig a -> HsValBinds a -> HsValBinds a
add_sig s (ValBindsIn x bs sigs) = ValBindsIn x bs (s:sigs)
