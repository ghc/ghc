...
garbage collection happens during the call to `hello`, it may be that
the array is freed while in use by the foreign function.

For a long time we offered the `touch#` primop to give the users the tools to
prevent this. For instance, the user might modify `callForeign` as:
...
In this case the Simplifier can may realize that (forever $ hello barr)
...
To address this, we introduced a new primop, keepAlive#, which has similar
semantics to `touch#` but "scopes over" the computation needing the kept-alive
value:
...
There are a few things to note here:
 ...
 - we are RuntimeRep-polymorphic in the result value since we don't want to
   force the user to use 

We implement this operation by desugaring to touch# during CorePrep. Specifically,
...
pointer to `x` and entering `k s0`. This is compilation strategy is safe
...
keepAlive#. However, we did explored (and eventually gave up on)
one additional optimisation, which we describe below.


# Recovery CPR unboxing
