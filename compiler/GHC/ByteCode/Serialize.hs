...
addSerializableNameWriter HscEnv {..} bh' = do
  nc <- evaluate hsc_NC
  pure $ flip addWriterToUserData bh' $ BinaryWriter $ \bh (SerializableName nm) ->
    if
      | isExternalName nm -> do
          updateNameCache nc (nameModule nm) (occName nm) $ \cache -> evaluate (extendOrigNameCache cache (nameModule nm) (occName nm) nm, ())
          putByte bh 0
          put_ bh nm
      | otherwise -> do
          putByte bh 1
