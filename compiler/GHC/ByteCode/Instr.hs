...
import GHC.Runtime.Heap.Layout ( StgWord, fromStgWord )
...
-- | The layout of the frame a loop's safepoint leaves on the stack when it
-- fires: the saved pc and the words above the loop's sequel, in the form of an
-- @StgLargeBitmap@ -- a size in words and the bits, with a 0 bit for each
-- pointer slot. 'GHC.ByteCode.Asm' puts it in the BCO's literals and the
-- interpreter copies it into the BCO the frame carries.
-- See Note [Join points as loops] in GHC.StgToByteCode.
data ResumeBitmap = ResumeBitmap
  { rb_size :: !Word       -- ^ the number of words the bitmap describes
  , rb_bits :: ![StgWord]  -- ^ the bits themselves, least significant first
  }

instance Outputable ResumeBitmap where
  ppr (ResumeBitmap size bits) =
    ppr size <+> text "words" <+> brackets (hsep (map pprHexWord bits))
    where pprHexWord w = text "0x" <> text (showHex (fromStgWord w) "")
...
data BCInstr
   ...
   -- yield if the thread should stop, and fall through otherwise. The bitmap
   -- describes the resume frame the interpreter leaves behind when it does.
   -- See Note [Join points as loops] in GHC.StgToByteCode.
   | YIELD_CHECK      !ResumeBitmap
...
instance Outputable BCInstr where
   ...
   ppr (YIELD_CHECK bm)      = text "YIELD_CHECK" <+> ppr bm
...
bciStackUse JMP{}                 = 0
bciStackUse YIELD_CHECK{}         = 3 -- the resume frame it may push
bciStackUse ENTER{}               = 0
