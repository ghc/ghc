...
-- The linking plan for each module
...
data LoadHowMuch
   ...
   | LoadUpTo (ModuleName, UnitId)
     -- ^ Load only the given module and its dependencies.
   | LoadDependenciesOf (ModuleName, UnitId)
...
load' cache how_much mHscMessage mod_graph = do
    ...
    -- (see msDeps)
    let all_home_mods =
          Set.fromList [ (ms_mod_name s, ms_unitid s)
        ...
        checkMod m and_then
            ...
            | otherwise = do
                    liftIO $ errorMsg logger
                        (text "no such module:" <+> quotes (ppr (snd m) <> colon <> ppr (fst m)))
                    return (Failed, [])
...
topSortModules :: Bool -> [ModuleGraphNode] -> Maybe (ModuleName, UnitId) -> [SCC ModuleGraphNode]
...
--
-- Chase downwards from the specified root set, returning module graph nodes
...
-- unchanged.
--
-- The returned list of [ModuleGraphNode] nodes has one node for each home-package
-- module, plus one for any hs-boot files.  The imports of these nodes
-- are all there, including the imports of non-home-package modules.
--
-- Downsweep is where all the dependencies are worked out for a module graph.
-- It shouldn't be necessary at this point to perform any recursive traversals to
-- work out properties of the build graph. You should instead construct the ModuleGraph
-- which is constructed from the returned ModuleGraphNodes.
downsweep :: HscEnv
          -> [ModSummary]
          -- ^ Old summaries
