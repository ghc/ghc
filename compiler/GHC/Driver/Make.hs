...
upsweep mHscMessage old_hpt_var nodes = do
  ...
  let
    runComp _ [] _ = return []
    runComp !mod_idx (m:ms) hsc_env1 = do
      (mod, (result, hpt)) <- runOne mod_idx m hsc_env1
      let hsc_env2 = hscUpdateHPT (const hpt) hsc_env1
      if succeeded result
        then ((Just (mod, hpt)) :) <$> runComp (mod_idx + 1) ms hsc_env2
        else (Nothing :) <$> (if keep_going then runComp mod_idx ms hsc_env2 else return [])

    runOne !mod_idx node lcl_hsc_env = do
      res <- prettyPrintGhcErrors logger $ do
        upsweep_one node
          mHscMessage
          lcl_hsc_env old_hpt_var
          mod_idx num_nodes
  ...
  liftIO $ runComp 1 nodes hsc_env
