    ...
    msg
      ...
      =
        hang
          ...
          (sep (map ppr missing))
    warn =
      mkPlainMsgEnvelope (SevWarning (WarnReason Opt_WarnMissingHomeModules)) noSrcSpan msg
...
warnUnusedPackages = do
    ...
    let warn =
          mkPlainMsgEnvelope (SevWarning $ WarnReason Opt_WarnUnusedPackages) noSrcSpan msg
      ...
      where read_msgs = do
            ...
            print_loop (x:xs) = case x of
                Just (msgClass,srcSpan,msg) -> do
                    putLogMsg dflags msgClass srcSpan msg
                    print_loop xs
...
withDeferredDiagnostics f = do
  ...
  then f
    ...
    let deferDiagnostics _dflags !msgClass !srcSpan !msg = do
          let action = putLogMsg dflags msgClass srcSpan msg
          case msgClass of
            MCDiagnostic (SevWarning {})
                    -> atomicModifyIORef' warnings $ \i -> (action: i, ())
            MCDiagnostic (SevError {})
