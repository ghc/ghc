   ...
   = do
     ...
     where
        calcDeps x =
          ...
          [ (modName, hsc_currentPackage hsc_env, isBoot)
          | (modName, isBoot) <- msDeps x
          ]
        ...
        loop :: [(Located ModuleName, Unit, IsBoot)]
...
summariseFile hsc_env old_summaries src_fn package mb_phase obj_allowed maybe_buf
