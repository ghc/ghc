...
warnMissingHomeModules hsc_env mod_graph =
  ...
  where
    ...
    warn = singleMessage $ mkDriverMessage (hsc_dflags hsc_env) noSrcSpan
...
warnUnusedPackages = do
    ...
    let warn = singleMessage $ mkDriverMessage dflags noSrcSpan (DriverUnusedPackages unusedArgs)
...
warnUnnecessarySourceImports sccs = do
  ...
  where check dflags ms =
        ...
        warn dflags (L loc mod) =
          GhcDriverMessage <$> mkDriverMessage dflags loc (DriverUnnecessarySourceImports mod)
           ...
           = do exists <- liftIO $ doesFileExist file
                if exists || isJust maybe_buf
                    then summariseFile hsc_env old_summaries file mb_phase
                                ...
                                $ mkDriverErrorMessage noSrcSpan (DriverFileNotFound file)
    ...
    new_summary location mod src_fn src_timestamp
      = runExceptT $ do
        ...
        when (pi_mod_name /= wanted_mod) $
                throwE $ singleMessage $ mkDriverErrorMessage pi_mod_name_loc
        ...
        when (hsc_src == HsigFile && isNothing (lookup pi_mod_name (homeUnitInstantiations home_unit))) $
            let suggestions = suggestInstantiatedWith pi_mod_name (homeUnitInstantiations home_unit)
                buildingCabalPkg = BuildingCabalPackage (gopt Opt_BuildingCabalPackage dflags)
            in throwE $ singleMessage $ mkDriverErrorMessage pi_mod_name_loc
...
noHsFileErr loc path
  = singleMessage $ mkDriverErrorMessage loc (DriverFileNotFound path)

moduleNotFoundErr :: ModuleName -> DriverMessages
moduleNotFoundErr mod
  = singleMessage $ mkDriverErrorMessage noSrcSpan (DriverModuleNotFound mod)

multiRootsErr :: [ModSummary] -> IO ()
multiRootsErr [] = panic "multiRootsErr"
multiRootsErr summs@(summ1:_)
  = throwOneError $ fmap GhcDriverMessage $
    mkDriverErrorMessage noSrcSpan $ DriverDuplicatedModuleDeclaration mod files
  where
    mod = ms_mod summ1
