...
import GHC.Types.Error ( mkMessages, getMessages, singleMessage )
...
parUpsweep_one mod home_mod_map comp_graph_loops lcl_dflags home_unit mHscMessage cleanup par_sem
    ...
    if not deps_ok
      then return Failed
        ...
        let logger err = printBagOfErrors lcl_dflags (getMessages $ srcErrorMessages err)
           ...
           = do exists <- liftIO $ doesFileExist file
                if exists || isJust maybe_buf
                    then summariseFile hsc_env old_summaries file mb_phase
                           ...
                           fmap ghcUnknownMessage $ mkPlainMsgEnvelope noSrcSpan $
      ...
      = runExceptT $ do
        ...
        when (pi_mod_name /= wanted_mod) $
                throwE $ singleMessage . fmap ghcUnknownMessage $
                  mkPlainMsgEnvelope pi_mod_name_loc $
...
getPreprocessedImports hsc_env src_fn mb_phase maybe_buf = do
  ...
  (pi_srcimps, pi_theimps, L pi_mod_name_loc pi_mod_name)
      <- ExceptT $ do
          ...
          return (first (fmap ghcUnknownMessage . mkMessages . fmap pprError) mimps)
...
noModError hsc_env loc wanted_mod err
  = fmap ghcUnknownMessage $ mkPlainMsgEnvelope loc $ cannotFindModule hsc_env wanted_mod err

noHsFileErr :: SrcSpan -> String -> MsgEnvelope GhcMessage
noHsFileErr loc path
  = fmap ghcUnknownMessage $ mkPlainMsgEnvelope loc $ text "Can't find" <+> text path
...
multiRootsErr summs@(summ1:_)
  = throwOneError . fmap ghcUnknownMessage $ mkPlainMsgEnvelope noSrcSpan $
