            ...
            handleSourceError (\err -> do logger err; return Nothing) $ do
                ...
                lcl_hsc_env'' <- case finish_loop of
                    ...
                    Just loop -> typecheckLoop lcl_dflags lcl_hsc_env' $
                                 filter (/= moduleName (mwib_module this_build_mod)) $
                                 map (moduleName . mwib_module) loop
          ...
          = do mb_s <- summariseModule hsc_env old_summary_map
               ...
               case mb_s of
                   ...
                   Just (Right s)-> do
                     ...
                     loop ss new_map
          where
            GenWithIsBoot { gwib_mod = L loc mod, gwib_isBoot = is_boot } = s
...
msDeps s = [ d
           | m <- ms_home_srcimps s
           , d <- [ GenWithIsBoot { gwib_mod = m, gwib_isBoot = IsBoot }
                  , GenWithIsBoot { gwib_mod = m, gwib_isBoot = NotBoot }
                  ]
           ]
        ++ [ GenWithIsBoot { gwib_mod = m, gwib_isBoot = NotBoot }
           | m <- ms_home_imps s
...
summariseModule hsc_env old_summary_map is_boot (L loc wanted_mod)
  ...
  | Just old_summary <- Map.lookup
      (ModuleNameWithIsBoot wanted_mod is_boot)
      old_summary_map
