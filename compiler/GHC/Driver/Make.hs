...
data ModuleGraphNodeWithBootFile
  = ModuleGraphNodeWithBootFile ModuleGraphNode [NodeKey]
...
pruneCache hpt summ
  ...
  where prune (CachedIface { cached_modiface = iface
          ...
          where
           ...
           linkable' | Just ms <- lookupUFM ms_map modl
...
interpretBuildPlan hug mhmi_cache old_hpt plan = do
  ...
  where
    collect_results build_map = do
      ...
      waits <- mapM (\br -> collect_result (fst <$> resultVar br)) (M.elems build_map)
      -- 2. Block waiting for all to finish
      return (sequence waits)

      where

      collect_result res_var = return (runMaybeT (waitResult res_var))
    ...
    buildSingleModule rehydrate_nodes origin mod = do
      ...
      let direct_deps = nodeDependencies False mod
          -- Really important to force this or the whole buildMap is retained and hence all the
          -- result variables so they can't be collected after they are no longer used.
    ...
    buildOneLoopyModule (ModuleGraphNodeWithBootFile mn deps) = do
      ...
      rehydrate_action <- rehydrateAction Rehydrated (HasBoot (mkNodeKey mn) : map NoBoot deps)
    ...
    buildModuleLoop ms = do
      ...
      let extract (Left mn) = NoBoot (mkNodeKey mn)
          extract (Right (ModuleGraphNodeWithBootFile mn _)) = HasBoot (mkNodeKey mn)
  ...
  -- An action which rehydrates the given keys
    rehydrateAction :: ResultLoopOrigin -> [HasBoot NodeKey] -> BuildM MakeAction
    rehydrateAction origin deps = do
      hug_var <- gets hug_var
      !build_map <- getBuildMap
      res_var <- liftIO newEmptyMVar
      let
          !build_deps = getDependencies (map getNK deps) build_map
      let loop_action = do
            (hug, tdeps) <- wait_deps_hug hug_var build_deps
            hsc_env <- asks hsc_env
            let new_hsc = setHUG hug hsc_env
                mns :: [ModuleName]
                mns = mapMaybe (nodeKeyModName . getNK) deps
      ...
      -- module loop will see the updated interfaces for all the identifiers in the loop.
          ...
          update_module_pipeline (m, i) =
            case m of
              NoBoot m -> setModulePipeline m (mkBuildResult (Loop origin) (fanout i))
              HasBoot m -> do
                setModulePipeline m (mkBuildResult (Loop origin) (fanout i))
                -- SPECIAL: Anything outside the loop needs to see A rather than A.hs-boot
                setModulePipeline (boot_key m) (mkBuildResult (Loop origin) (fanout i))
    ...
    checkRehydrationInvariant hmis deps =
        let mk_key mi = NodeKey_Module (ModNodeKeyWithUid (mi_mnwib mi) (moduleUnitId $ mi_module mi))
            hmi_names = map (mk_key . hm_iface) hmis
            start = map getNK deps
        in massertPpr (hmi_names /= start) $ (ppr hmi_names $$ ppr start)


data HasBoot a = HasBoot { getNK :: a } | NoBoot { getNK :: a }
