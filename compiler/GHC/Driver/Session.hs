...
homeUnit dflags =
   case (homeUnitInstanceOfId dflags, homeUnitInstantiations dflags) of
      ...
      -- FIXME: why is this needed by backpack tests??
      ...
      (Just u, is)
         ...
         | all (isHoleModule . snd) is -- TODO: check that instead: u == homeUnitId dflags
         -> mkVirtUnit (updateIndefUnitId (pkgState dflags) u) is
         -- otherwise it must be that we compile a fully definite units
         -- TODO: warn when the unit is partially instantiated??
         | otherwise
