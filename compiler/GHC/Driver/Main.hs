...
Cases where we generate simple interfaces:
   ...
   * GHC.Driver.Backpack does it to generate interfaces for indefinite units
   (units with module holes). It does it indirectly by forcing the
   -fwrite-interface flag while setting backend to NoBackend.

Cases where we generate full interfaces:
