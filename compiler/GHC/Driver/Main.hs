...
An interface files consists of two parts

* The `ModIface` which ends up getting written to disk
* and the `ModDetails` which provides a more structured view that is suitable for usage during compilation.
...
interface file, and regenerate the ModDetails from the ModIface. The key part about
the loading is that the ModDetails is regenerated lazily from the ModIface, so that
there's only a detailed in-memory representation for declarations which are actually used
from the interface. This mode is also used when reading interface files from
external packages.
...
at hand, we create the `ModDetails` from the `ModIface`. Doing this means that
