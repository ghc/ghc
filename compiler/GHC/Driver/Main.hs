...
hscTypecheckAndGetWarnings hsc_env summary = do
  ...
  let rnfWarnMsgs = (foldl' (\_ w -> rnfOne w) () warnings)
      rnfOne w = rnf (errMsgSpan w) `seq` errMsgDiagnostic w `seq` errMsgSeverity w
                `seq` errMsgContext w `seq` ()

  return $ seq rnfWarnMsgs (tc_result, warnings)
