...
searchPackageId :: DynFlags -> PackageId -> [UnitInfo]
searchPackageId dflags pid = filter ((pid ==) . unitPackageId)
          ...
          where upd_pkg pkg
                  ...
                  = let fs = installedUnitIdFS (unDefUnitId wiredInUnitId)
                    in pkg {
                      ...
                      unitInstanceOfId = ComponentId fs
                    }
                  | otherwise
                  = pkg
                upd_deps pkg = pkg {
...
-}
  ...
  let pkgname_map = foldl' add Map.empty pkgs2
        where add pn_map p
                = Map.insert (unitPackageName p) (unitInstanceOfId p) pn_map
...
componentIdString dflags cid = do
    ...
    return $
        case unitComponentName conf of
            Nothing -> unitPackageIdString conf
            Just (PackageName libname) ->
                unitPackageNameString conf
                    ++ "-" ++ showVersion (unitPackageVersion conf)
                    ++ ":" ++ unpackFS libname

displayInstalledUnitId :: DynFlags -> InstalledUnitId -> Maybe String
displayInstalledUnitId dflags uid =
    fmap unitPackageIdString (lookupInstalledPackage dflags uid)
