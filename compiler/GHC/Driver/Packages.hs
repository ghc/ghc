...
matching (UnitIdArg (UnitId (DefUnitId uid)))  = matchingId uid
...
upd_wired_in_uid wiredInMap (UnitId def_uid) =
    UnitId (upd_wired_in wiredInMap def_uid)
...
updateVisibilityMap wiredInMap vis_map = foldl' f vis_map (Map.toList wiredInMap)
  where f vm (from, to) = case Map.lookup (UnitId from) vis_map of
                    Nothing -> vm
                    Just r -> Map.insert (UnitId to) r
                                (Map.delete (UnitId from) vm)
...
unwireUnitId dflags uid@(UnitId def_uid) =
    maybe uid UnitId (Map.lookup def_uid (unwireMap (pkgState dflags)))
...
improveUnitId _ uid@(UnitId _) = uid -- short circuit
