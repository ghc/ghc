...
findImportedModule hsc_env mod_name mb_pkg =
  ...
  where
    ...
    prebuilt_import = findExposedPackageModule hsc_env mod_name mb_pkg

    non_current_import = any_home_import `orIfNotFound` prebuilt_import

    unqual_import = current_home_import `orIfNotFound` non_current_import
...
findExactModule hsc_env mod =
  case M.lookup iuid (hsc_internalUnitEnv hsc_env) of
    Just unitEnv -> findInstalledHomeModule
      (hsc_FC hsc_env) iuid
      (internalUnitEnv_dflags unitEnv) (installedModuleName mod)
    Nothing -> findPackageModule hsc_env mod
  where
    iuid = installedModuleUnitId mod
...
addHomeModuleToFinder :: HscEnv -> ModuleName -> ModLocation -> InstalledUnitId -> IO Module
...
findHomeModule hsc_env dflags mod_name = do
 ...
 where
  iuid = thisInstalledUnitId dflags
