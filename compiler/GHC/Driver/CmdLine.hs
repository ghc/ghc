...
type Warn = MsgEnvelope DriverMessage

type Errs  = Bag Err
type Warns = Messages DriverMessage
...
-- addWarn and addFlagWarn take DiagOpts
-- because we need to delay the message filtering
-- See #12056
addWarn :: Monad m => DiagOpts -> String -> EwM m ()
addWarn opts msg = addFlagWarn opts $ DriverUnknownMessage $ UnknownDiagnostic $
  mkPlainDiagnostic WarningWithoutFlag noHints $ text msg

addFlagWarn :: Monad m => DiagOpts -> DriverMessage -> EwM m ()
addFlagWarn opts msg = EwM
  (\(L loc _) es ws -> return (es, addMessage (mkPlainMsgEnvelope opts loc msg) ws, ()))
