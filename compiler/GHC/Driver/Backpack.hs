...
hsunitModuleGraph unit = do
    ...
    let get_decl (L _ (DeclD hsc_src lmodname mb_hsmod)) = do
          Just `fmap` summariseDecl pn (homeUnitId home_unit) hsc_src lmodname mb_hsmod
...
hsModuleToModSummary pn hsc_src modname
    ...
    this_mod <- liftIO $ addHomeModuleToFinder hsc_env (hsc_home_unit hsc_env) modname location
    return ModSummary {
            ms_mod = this_mod,
            ms_hsc_src = hsc_src,
            ms_location = location,
