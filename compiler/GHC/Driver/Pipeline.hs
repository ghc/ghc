...
compileOne' m_tc_result mHscMessage
 ...
 = do
   ...
   (frontend_status, plugin_hsc_env) <- hscIncrementalCompileFrontend
     always_do_basic_recompilation_check
     m_tc_result mHscMessage
     hsc_env summary source_modified mb_old_iface (mod_index, nmods)
   -- Use an HscEnv updated with the plugin info
   let hsc_env' = plugin_hsc_env
       hsc_lang = backend (hsc_dflags hsc_env')
   ...
   case frontend_status of

     HscFrontendStatus_UpToDate iface hmi_details ->
       -- TODO recomp014 triggers this assert. What's going on?!
       -- ASSERT( isJust mb_old_linkable || isNoLink (ghcLink dflags) )
       return $! HomeModInfo iface hmi_details mb_old_linkable

     HscFrontendStatus_Recomp iface hmi_details summary tc_result mb_old_hash -> case src_flavour of
       HsBootFile -> do
         case hsc_lang of
           NoBackend -> pure ()
           Interpreter -> pure ()
           _ -> touchObjectFile logger dflags object_filename
         return $! HomeModInfo iface hmi_details Nothing
       HsigFile -> do
         o_time <- case hsc_lang of
           NoBackend -> pure $ ms_hs_date summary
           Interpreter -> pure $ ms_hs_date summary
           _ -> do
             ...
             _ <- runPipeline StopLn hsc_env'
               ( output_fn
               , Nothing
               , Just $ HscOut src_flavour mod_name $ HscPhase_FrontendOut frontend_status
               )
               (Just basename)
               Persistent
               (Just location)
         ...
         let !linkable = LM o_time this_mod []
         return $! HomeModInfo iface hmi_details (Just linkable)
       HsSrcFile -> do
         (final_iface, final_details, linkable) <- if hsc_lang == NoBackend || hsc_lang == Interpreter
           then do
             -- We do this in the NoBackend case because we still want to catch the
             -- errors that come from desugaring.
             middle_status <-
               runHsc hsc_env $ hscIncrementalCompileMiddle summary tc_result mb_old_hash
             case (middle_status, hsc_lang) of
               (_, ViaC) -> panic "compileOne impossible, should be other branch of if"
               (_, NCG) -> panic "compileOne impossible, should be other branch of if"
               (_, LLVM) -> panic "compileOne impossible, should be other branch of if"
               (HscMiddleStatus_Recomp {}, NoBackend) ->
                 panic "compileOne 'NoBackend' means we should always get 'HscMiddleStatus_NotGeneratingCode'"
               (HscMiddleStatus_NotGeneratingCode, Interpreter) ->
                 panic "compileOne 'HscNotGeneratingCode' means we should have been 'NoBackend'"
               (HscMiddleStatus_NotGeneratingCode, NoBackend) -> do
                 let linkable = LM (ms_hs_date summary) this_mod []
                 pure (iface, hmi_details, linkable)
               ( HscMiddleStatus_Recomp
                ...
                , Interpreter
                 ...
                 let !linkable = LM unlinked_time (ms_mod summary)
                                (hs_unlinked ++ stub_o)
                 return (final_iface, hmi_details, linkable)
             ...
             (_, _, Just (final_iface, details)) <- runPipeline StopLn hsc_env'
               ( output_fn
               , Nothing
               , Just $ HscOut src_flavour mod_name $
                   HscPhase_FrontendOut frontend_status
               )
               (Just basename)
               Persistent
               (Just location)
             ...
             return (final_iface, details, linkable)
         return $! HomeModInfo final_iface final_details (Just linkable)
 ...
 = do   -- normal Hsc mode, not mkdependHS
  ...
  -- run the compiler!
        ...
        (result, plugin_hsc_env) <-
          liftIO $ hscIncrementalCompileFrontend True Nothing (Just msg) hsc_env'
        ...
        return (HscOut src_flavour mod_name $ HscPhase_FrontendOut result,
                panic "HscOut doesn't have an input filename")

runPhase (HscOut src_flavour mod_name (HscPhase_FrontendOut result)) _ = do
        dflags <- getDynFlags
        logger <- getLogger
        location <- getLocation src_flavour mod_name
        setModLocation location

        let hsc_lang = backend dflags
            o_file = ml_obj_file location -- The real object file

        case result of
          HscFrontendStatus_UpToDate _ _ -> do
            when (hsc_lang /= NoBackend) $ do
              liftIO $ touchObjectFile logger dflags o_file
              -- The .o file must have a later modification date
              -- than the source file (else we wouldn't get Nothing)
              -- but we touch it anyway, to keep 'make' happy (we think).
            return (RealPhase StopLn, o_file)
          HscFrontendStatus_Recomp _ _ summary tc_result mb_old_hash -> case src_flavour of
            HsBootFile -> do
              when (hsc_lang /= NoBackend) $ do
            ...
            HsSrcFile -> do
              PipeState {hsc_env = hsc_env'} <- getPipeState
              middle_status <- liftIO $ runHsc hsc_env' $
                hscIncrementalCompileMiddle summary tc_result mb_old_hash
              pure ( HscOut src_flavour mod_name $ HscPhase_MiddleOut middle_status
                   , o_file
                   )

runPhase (HscOut src_flavour mod_name (HscPhase_MiddleOut result)) _ = do
        location <- getLocation src_flavour mod_name
        setModLocation location
        dflags <- getDynFlags
        logger <- getLogger
        ...
        case result of
            HscMiddleStatus_NotGeneratingCode -> pure
              ( RealPhase StopLn
