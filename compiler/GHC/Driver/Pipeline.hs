...
import GHC.Types.Basic ( ForeignSrcLang(..) )
...
preprocessPipeline pipe_env hsc_env input_fn = do
  ...
  liftIO (handleFlagWarnings (hsc_logger hsc_env) dflags3 warns3)
...
hscGenBackendPipeline pipe_env hsc_env mod_sum result = do
  let mod_name = moduleName (ms_mod mod_sum)
      ...
      dflags = hsc_dflags hsc_env
<<<<<<< HEAD
  -- This call to getLocation should be nuked once the `ModLocation` also contains
  -- the location of the `dyn_o` file.
=======
