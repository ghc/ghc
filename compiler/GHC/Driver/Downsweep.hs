...
downsweepInteractiveImports hsc_env ic = unsafeInterleaveIO $ do
  ...
  let interactive_node  = case expectJust $ M.lookup key graph of
        NSuccess r -> r
        NFailed e -> pprPanic "downsweepInteractiveImports" (text "NFailed:" <+> ppr e)
...
downsweepFromRootNodes hsc_env summ_cache imps_cache maybe_base_graph excl_mods allow_dup_roots mode root_nodes root_uids = do
     ...
     deps' <- runDownsweepM env  $ do
        ...
        return deps'
     let downsweep_els   = M.elems deps'
         downsweep_errs  = [ e | NFailed e  <- downsweep_els ]
         downsweep_nodes = [ s | NSuccess s <- downsweep_els ]
...
type MGRes = NResult DriverMessages
...
data NResult e v
  ...
  = NSuccess v
  -- | Failed to compute the node payload because of an error.
  -- We cache the failure to expand this node, but keep traversing the graph
  -- without exiting.
  | NFailed e
  -- | Skip a node! This means this node doesn't produce a payload and we can
  -- just ignore it if we ever come across it (this skip is also cached)
  | NSkip
...
dfsBuild base_map roots key expand = go roots (fromMaybe Map.empty base_map)
  where
    ...
    go (s:ss) visited
      ...
      = do r <- expand s
           case r of
             NSkip       -> go ss (Map.insert k NSkip         visited) -- Skip!
             NFailed err -> go ss (Map.insert k (NFailed err) visited) -- Keep going despite the failure!
             NSuccess (v,ns) ->
