...
data Backend
   ...
   | NoBackend     -- ^ Disabled backend (used by Haddock, -fno-code, ghc-api, etc.)
...
platformDefaultBackend platform = if
      ...
      | otherwise                       -> LLVM
   where
      ncgValidOs = case platformOS platform of
         OSAIX    -> False -- we want to use LLVM by default on AIX
         OSDarwin -> False -- and IOS/Darwin
         _        -> True
...
platformNcgSupported platform = if
      | platformUnregisterised platform -> False
        -- NCG doesn't support unregisterised ABI
      | ncgValidArch                    -> True
      | otherwise                       -> False
   where
...
backendRetainsAllBindings _           = False
