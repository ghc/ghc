...
bcPrepExpr fvs (StgTick bp@(Breakpoint tick_ty _ _) rhs)
  | isLiftedTypeKind (typeKind tick_ty) = do
      ...
      let fvs' = fvs `unionDVarSet` exprArgs rhs
          expr' = StgTick bp rhs'
...
bcPrepAlt fvs (ac, bndrs, expr) = (,,) ac bndrs <$> bcPrepExpr fvs expr
