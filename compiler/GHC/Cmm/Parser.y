...
importName
        ...
        | 'extern' NAME
        { ($2, mkForeignLabel $2 ForeignLabelInExternalPackage ForeignLabelIsFunction) }
        ...
        | 'extern' 'DATA' NAME
        { ($3, mkForeignLabel $3 ForeignLabelInExternalPackage ForeignLabelIsData) }
        ...
        | STRING NAME
        { ($2, mkForeignLabel $2 (ForeignLabelInPackage (UnitId (mkFastString $1))) ForeignLabelIsFunction) }

        -- A data label imported from the shared library for a Haskell package
        -- with the given UnitId. Such labels behave as local when used within
        -- the specified unit, or as extern otherwise.
        | STRING 'DATA' NAME
        { ($3, mkForeignLabel $3 (ForeignLabelInPackage (UnitId (mkFastString $1))) ForeignLabelIsData) }
