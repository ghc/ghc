...
import GHC.LanguageExtensions.Type ( Extension (LinearTypes) )
...
nameToMachOp name =
  case lookupUFM machOps name of
        Nothing -> failMsgPD $ \span -> mkParserErrorMessage span $ PsCmmParserError (CmmUnknownPrimitive name)
        Just m  -> return m
...
parseSafety str      = failMsgPD $ \span -> mkParserErrorMessage span $
                                              PsCmmParserError (CmmUnrecognisedSafety str)

parseCmmHint :: String -> PD ForeignHint
parseCmmHint "ptr"    = return AddrHint
parseCmmHint "signed" = return SignedHint
parseCmmHint str      = failMsgPD $ \span -> mkParserErrorMessage span $
                                               PsCmmParserError (CmmUnrecognisedHint str)
...
stmtMacro fun args_code = do
  case lookupUFM stmtMacros fun of
    Nothing -> failMsgPD $ \span -> mkParserErrorMessage span $ PsCmmParserError (CmmUnknownMacro fun)
...
foreignCall conv_string results_code expr_code args_code safety ret
  = do  conv <- case conv_string of
          ...
          _         -> failMsgPD $ \span -> mkParserErrorMessage span $
                                              PsCmmParserError (CmmUnknownCConv conv_string)
        return $ do
          platform <- getPlatform
