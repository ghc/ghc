...
cmmMachOpFold
    :: Platform
    -> MachOp a                 -- The operation from an CmmMachOp
    -> SizedTupleGADT a CmmExpr -- The optimized arguments
    -> CmmExpr
...
cmmMachOpFoldM _ (MO_V_Broadcast lg _w) exprs =
  case exprs of
    TupleG1 (CmmLit l) -> Just $! CmmLit (CmmVec $ replicate lg l)
    _ -> Nothing
cmmMachOpFoldM _ (MO_VF_Broadcast lg _w) exprs =
  case exprs of
    TupleG1 (CmmLit l) -> Just $! CmmLit (CmmVec $ replicate lg l)
    _ -> Nothing
cmmMachOpFoldM _ op (TupleG1 (CmmLit (CmmInt x rep)))
  | MO_WF_Bitcast width <- op = case width of
      W32 | res <- castWord32ToFloat (fromInteger x)
          ...
          , not (res == 0 || isNaN res || isInfinite res)
  ...
  = Just $! case op of
      ...
      MO_SF_Round from to
        -> CmmLit (CmmFloat (fromInteger $ narrowS from x) to)
...
cmmMachOpFoldM _ op (TupleG2 _shiftee (CmmLit (CmmInt shift _)))
  | Just width <- isShift op
  , shift >= fromIntegral (widthInBits width)
  = Just $! CmmLit (CmmInt 0 width) -- TODO: rubbish
  where
    isShift :: MachOp a -> Maybe Width
...
cmmMachOpFoldM _ (MO_SS_Conv rep1 rep2) (TupleG1 x) | rep1 == rep2 = Just x
cmmMachOpFoldM _ (MO_UU_Conv rep1 rep2) (TupleG1 x) | rep1 == rep2 = Just x
cmmMachOpFoldM _ (MO_XX_Conv rep1 rep2) (TupleG1 x) | rep1 == rep2 = Just x

-- Eliminate nested conversions where possible
cmmMachOpFoldM platform conv_outer (TupleG1 (CmmMachOp conv_inner (TupleG1 x)))
  ...
  = case () of
      ...
      _ | rep1 < rep2 && rep1 == rep3 -> Just x
        ...
        | rep1 < rep2 && rep2 > rep3 ->
            Just $! cmmMachOpFold platform (intconv signed1 rep1 rep3) (TupleG1 x)
        -- Nested widenings: collapse if the signedness is the same
        | rep1 < rep2 && rep2 < rep3 && signed1 == signed2 ->
            Just $! cmmMachOpFold platform (intconv signed1 rep1 rep3) (TupleG1 x)
        -- Nested narrowings: collapse
        | rep1 > rep2 && rep2 > rep3 ->
            Just $! cmmMachOpFold platform (MO_UU_Conv rep1 rep3) (TupleG1 x)
        | otherwise ->
            Nothing
  where
    -- TODO: This could handle MO_XX_Conv better.
        isIntConversion :: MachOp a -> Maybe (Width, Width, Bool)
...
cmmMachOpFoldM platform mop (TupleG2 (CmmLit (CmmInt x xrep)) (CmmLit (CmmInt y _yrep)))
  = case mop of
        ...
        MO_U_Quot r
          | y /= 0 -> Just $! CmmLit (CmmInt (x_u `quot` y_u) r)
          | otherwise -> Nothing -- TODO: we could do more with undefined behavior
        MO_U_Rem  r
          | y /= 0 -> Just $! CmmLit (CmmInt (x_u `rem`  y_u) r)
          | otherwise -> Nothing
        MO_S_Quot r
          | y /= 0 -> Just $! CmmLit (CmmInt (x_s `quot` y_s) r)
          | otherwise -> Nothing
        MO_S_Rem  r
          | y /= 0 -> Just $! CmmLit (CmmInt (x_s `rem`  y_s) r)
          | otherwise -> Nothing
        ...
        MO_S_MulMayOflo r
          | mul_res <- x_s * y_s
          , mul_res /= narrowS r mul_res -> Just $! CmmLit (CmmInt 1 r)
          -- some platforms may introduce false positives
          -- so we don't try constant-folding this to zero
          | otherwise -> Nothing

        MO_F_Add _ -> invalidArgsPanic
        MO_F_Sub _ -> invalidArgsPanic
        MO_F_Mul _ -> invalidArgsPanic
        MO_F_Quot _ -> invalidArgsPanic
        MO_F_Eq _ -> invalidArgsPanic
        MO_F_Ne _ -> invalidArgsPanic
        MO_F_Ge _ -> invalidArgsPanic
        MO_F_Le _ -> invalidArgsPanic
        MO_F_Gt _ -> invalidArgsPanic
        MO_F_Lt _ -> invalidArgsPanic
        MO_F_Min _ -> invalidArgsPanic
        MO_F_Max _ -> invalidArgsPanic
        MO_V_Extract _ _ -> invalidArgsPanic
        MO_V_Add _ _ -> invalidArgsPanic
        MO_V_Sub _ _ -> invalidArgsPanic
        MO_V_Mul _ _ -> invalidArgsPanic
        MO_VS_Quot _ _ -> invalidArgsPanic
        MO_VS_Rem _ _ -> invalidArgsPanic
        MO_VU_Quot _ _ -> invalidArgsPanic
        MO_VU_Rem _ _ -> invalidArgsPanic
        MO_V_Shuffle _ _ _ -> invalidArgsPanic
        MO_VF_Shuffle _ _ _ -> invalidArgsPanic
        MO_VF_Extract _ _ -> invalidArgsPanic
        MO_VF_Add _ _ -> invalidArgsPanic
        MO_VF_Sub _ _ -> invalidArgsPanic
        MO_VF_Mul _ _ -> invalidArgsPanic
        MO_VF_Quot _ _ -> invalidArgsPanic
        MO_VS_Min _ _ -> invalidArgsPanic
        MO_VS_Max _ _ -> invalidArgsPanic
        MO_VU_Min _ _ -> invalidArgsPanic
        MO_VU_Max _ _ -> invalidArgsPanic
        MO_VF_Min _ _ -> invalidArgsPanic
        MO_VF_Max _ _ -> invalidArgsPanic

   where
        x_u = narrowU xrep x
        y_u = assert (xrep == _yrep) $ narrowU xrep y
        x_s = narrowS xrep x
        y_s = assert (xrep == _yrep) $ narrowS xrep y
        invalidArgsPanic = pprPanic "cmmMachOpFoldM" $
              text "Found" <+> pprMachOp mop
                <+> text "illegally applied to two int literals"
...
cmmMachOpFoldM platform op (TupleG2 x@(CmmLit _) y)
   | not (isLit y) && isCommutableMachOp op
   = Just $! (cmmMachOpFold platform op (TupleG2 y x))
...
--
cmmMachOpFoldM platform mop1 (TupleG2 (CmmMachOp mop2 (TupleG2 arg1 arg2)) arg3)
   | mop2 `associates_with` mop1
     && not (isLit arg1) && not (isPicReg arg1)
   = Just $! cmmMachOpFold platform mop2 (TupleG2 arg1 (cmmMachOpFold platform mop1 (TupleG2 arg2 arg3)))
   where
...
cmmMachOpFoldM platform mop1@(MO_Add{}) (TupleG2 (CmmMachOp mop2@(MO_Sub{}) (TupleG2 arg1 arg2)) arg3)
   | not (isLit arg1) && not (isPicReg arg1)
   = Just $! cmmMachOpFold platform mop1 (TupleG2 arg1 (cmmMachOpFold platform mop2 (TupleG2 arg3 arg2)))
...
-- PicBaseReg + label.
--
cmmMachOpFoldM _ MO_Add{} (TupleG2
                            (CmmMachOp op@MO_Add{} (TupleG2 pic (CmmLit lit)))
                            (CmmLit (CmmInt n rep)))
  | isPicReg pic
  = Just $! CmmMachOp op (TupleG2 pic (CmmLit $ cmmOffsetLit lit off))
...
cmmMachOpFoldM _ (MO_Add _) (TupleG2 (CmmReg reg) (CmmLit (CmmInt n rep)))
  | validOffsetRep rep
  = Just $! cmmRegOff reg (fromIntegral (narrowS rep n))
cmmMachOpFoldM _ (MO_Add _) (TupleG2 (CmmRegOff reg off) (CmmLit (CmmInt n rep)))
  | validOffsetRep rep
  = Just $! cmmRegOff reg (off + fromIntegral (narrowS rep n))
cmmMachOpFoldM _ (MO_Sub _) (TupleG2 (CmmReg reg) (CmmLit (CmmInt n rep)))
  | validOffsetRep rep
  = Just $! cmmRegOff reg (- fromIntegral (narrowS rep n))
cmmMachOpFoldM _ (MO_Sub _) (TupleG2 (CmmRegOff reg off) (CmmLit (CmmInt n rep)))
  | validOffsetRep rep
  = Just $! cmmRegOff reg (off - fromIntegral (narrowS rep n))

-- Fold label(+/-)offset into a CmmLit where possible

cmmMachOpFoldM _ (MO_Add _) (TupleG2 (CmmLit lit) (CmmLit (CmmInt i rep)))
  | validOffsetRep rep
  = Just $! CmmLit (cmmOffsetLit lit (fromIntegral (narrowU rep i)))
cmmMachOpFoldM _ (MO_Add _) (TupleG2 (CmmLit (CmmInt i rep)) (CmmLit lit))
  | validOffsetRep rep
  = Just $! CmmLit (cmmOffsetLit lit (fromIntegral (narrowU rep i)))
cmmMachOpFoldM _ (MO_Sub _) (TupleG2 (CmmLit lit) (CmmLit (CmmInt i rep)))
  | validOffsetRep rep
...
cmmMachOpFoldM platform cmp (TupleG2 (CmmMachOp conv (TupleG1 x)) (CmmLit (CmmInt i _)))
  ...
  = Just $! cmmMachOpFold platform narrow_cmp (TupleG2 x (CmmLit (CmmInt i rep)))
 where
    maybe_conversion :: MachOp a -> Maybe (Width, Bool, Width -> Integer -> Integer)
    ...
    maybe_comparison :: MachOp a -> Width -> Bool -> Maybe (MachOp a)
...
cmmMachOpFoldM platform mop (TupleG2 x y@(CmmLit (CmmInt 0 _)))
  = case mop of
        -- Arithmetic
...
cmmMachOpFoldM platform mop (TupleG2 x (CmmLit (CmmInt 1 rep)))
  = case mop of
...
cmmMachOpFoldM platform mop (TupleG2 x (CmmLit (CmmInt n _)))
  = case mop of
        MO_Mul rep
           | Just p <- exactLog2 n ->
                 Just $! cmmMachOpFold platform (MO_Shl rep) (TupleG2 x (CmmLit (CmmInt p $ wordWidth platform)))
        MO_U_Quot rep
           | Just p <- exactLog2 n ->
                 Just $! cmmMachOpFold platform (MO_U_Shr rep) (TupleG2 x (CmmLit (CmmInt p $ wordWidth platform)))
        MO_U_Rem rep
           | Just _ <- exactLog2 n ->
                 Just $! cmmMachOpFold platform (MO_And rep) (TupleG2 x (CmmLit (CmmInt (n - 1) rep)))
        MO_S_Quot rep
           | Just p <- exactLog2 n,
             CmmReg _ <- x ->   -- We duplicate x in signedQuotRemHelper, hence require
                                -- it is a reg.  FIXME: remove this restriction.
                Just $! cmmMachOpFold platform (MO_S_Shr rep)
                  (TupleG2 (signedQuotRemHelper rep p) (CmmLit (CmmInt p $ wordWidth platform)))
        MO_S_Rem rep
           | Just p <- exactLog2 n,
             CmmReg _ <- x ->   -- We duplicate x in signedQuotRemHelper, hence require
                                -- it is a reg.  FIXME: remove this restriction.
                ...
                Just $! cmmMachOpFold platform (MO_Sub rep)
                    (TupleG2 x (cmmMachOpFold platform (MO_And rep)
                      (TupleG2 (signedQuotRemHelper rep p) (CmmLit (CmmInt (- n) rep)))))
        _ -> Nothing
  where
    ...
    signedQuotRemHelper rep p = CmmMachOp (MO_Add rep) (TupleG2 x x2)
      where
        bits = fromIntegral (widthInBits rep) - 1
        shr = if p == 1 then MO_U_Shr rep else MO_S_Shr rep
        x1 = CmmMachOp shr (TupleG2 x (CmmLit (CmmInt bits $ wordWidth platform)))
        x2 = if p == 1 then x1 else
             CmmMachOp (MO_And rep) (TupleG2 x1 (CmmLit (CmmInt (n-1) rep)))
