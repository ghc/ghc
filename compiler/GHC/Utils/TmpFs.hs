...
data TmpFs = TmpFs
    ...
    , tmp_dirs_to_clean  :: IORef (Map FilePath FilePath)
        -- These have to be IORefs, because the defaultCleanupHandler needs to know
        -- what to clean when an exception happens
...
-- | Initialise an empty TmpFs sharing unique numbers with the given TmpFs
...
mergeTmpFsInto src dst = do
    src_files <- readIORef (tmp_files_to_clean src)
    src_dirs  <- readIORef (tmp_dirs_to_clean src)
    atomicModifyIORef' (tmp_files_to_clean dst) (\s -> (mergeFilesToClean src_files s, ()))
    atomicModifyIORef' (tmp_dirs_to_clean dst) (\s -> (Map.union src_dirs s, ()))
