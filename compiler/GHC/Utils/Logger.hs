...
-- hooks. The compiler itself uses this in multithreaded code and it is also
-- probably used by ghc-api users (IDEs, etc.). We could have used a "IsLogger"
-- type-class instead but having a constraint on every function using the logger
...
--
-- Finally, in many cases the LogFlags are expected to be stable in the
-- compiler. We should only need to change them when we read the command-line
-- flags, when we parse OPTIONS_GHC pragmas or when they are explicitly changed
-- via ghc-api. That's why we have the "data Log = Log Logger LogFlags" type to
-- bundle a Logger and some LogFlags.
--
-- If it appears that LogFlags don't change much (just like hooks), we could
-- perhaps merge Logger and Log.  Historically there was no logger abstraction
-- and all the logging functions took DynFlags as an argument: it was less
-- painful to first extract from the DynFlags the Logger (and its hooks) and
-- then the LogFlags, instead of extracting everything related to logging in one
-- patch. This is one thing to revisit when the ghc-api will be stabilized.
--
