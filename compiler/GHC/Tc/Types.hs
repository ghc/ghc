...
--
-- Please see GHC.Tc.Monad as well for operations on these types. You probably
...
*                                                                      *
    ...
    All the combinators for the monad can be found in GHC.Tc.Monad
*                                                                      *
************************************************************************
...
--        in GHC.Iface.{Make,Recomp}, and tcLookupGlobal in GHC.Tc.Monad.Env
--
...
type ThBindEnv = NameEnv (TopLevelFlag, ThLevel)
   ...
   -- bound at top level!  See Note [Template Haskell levels] in GHC.Tc.Splice
...
data ThStage    -- See Note [Template Haskell state diagram]
                -- and Note [Template Haskell levels] in GHC.Tc.Splice
    -- Start at:   Comp
  ...
  | RunSplice (TcRef [ForeignRef (TH.Q ())])
      ...
      --
      -- See Note [Collecting modFinalizers in typed splices] in "GHC.Tc.Splice".
...
type ThLevel = Int
    -- NB: see Note [Template Haskell levels] in GHC.Tc.Splice
...
-- See 'GHC.Tc.Monad.Env' for how to retrieve a 'TyThing' given a 'Name'.
data TcTyThing
...
data PromotionErr
  ...
  | ConstrainedDataConPE PredType
                     ...
                     --           non-equality contexts] in GHC.Tc.HsType
  | PatSynPE         -- Pattern synonyms
                     -- See Note [Don't promote pattern synonyms] in GHC.Tc.Monad.Env
...
-- a) for static forms in GHC.Tc.Expr.checkClosedInStaticForm and
-- b) to figure out when a nested binding can be generalised,
--    in GHC.Tc.Binds.decideGeneralisationPlan.
--
...
This test is made in GHC.Tc.Expr.checkClosedInStaticForm.
...
data TcIdSigInst
  = TISI { sig_inst_sig :: TcIdSigInfo
         ...
         , sig_inst_skols :: [(Name, TcTyVar)]
               ...
               -- See Note [Binding scoped type variables] in GHC.Tc.Sigs
               --
         ...
         , sig_inst_wcx   :: Maybe TcType
               ...
               -- only from the return value of GHC.Tc.HsType.tcAnonWildCardOcc
         }
...
data TcPatSynInfo
  = TPSI {
        ...
        patsig_implicit_bndrs :: [TyVarBinder], -- Implicitly-bound kind vars (Inferred) and
          ...
          -- See Note [The pattern-synonym signature splitting rule] in GHC.Tc.PatSyn
