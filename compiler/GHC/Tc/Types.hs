...
module GHC.Tc.Types(
        ...
        Env(..),
        TcGblEnv(..), TcLclEnv(..),
        TcRnError(..), tcRnErrorDoc,
        NameSuggestions(..), HowInScope,
        ImportSuggestion(..), ExtensionSuggestion(..),
        OutOfScopeSuggestions(..), noOutOfScopeSuggestions,
...
tcRnErrorDoc (TcRnBadTelescope telescope sorted_tvs context) =
  ...
  where m = hang (text "These kind and type variables:" <+> telescope $$
