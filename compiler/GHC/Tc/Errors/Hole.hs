     ...
     ; if isEmptyWC wanted && isEmptyBag th_relevant_cts
       then do { traceTc "}" empty
               ...
               ; let outermost_first = reverse th_implics
                     toFinal cts = foldr (setWCAndBinds fresh_binds) cts outermost_first
               -- To fix #16875,  We add a fast-path: we check if there are any
               -- extra constraints,  and if so, we make sure to first check if
               -- the types can match at all, and if so, then we check the
               -- relevant constraints and conditions on the types as well (as
               -- these are more expensive). By doing this fail fast check first,
               -- we avoid doing a lot of unneccessary work in the later check.
               ; impossible <- case wanted of
                    -- If the bag is not a singleton, then there are some
                    -- constraints on the type as well, like e.g. Foldable.
                    ...
                    _ -> return False
              ; res <- if impossible then return False else
                  do { -- We add the cloned relevants to the wanteds generated by
