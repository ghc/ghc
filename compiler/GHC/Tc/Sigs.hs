...
import GHC.Tc.Unify( tcSkolemise, unifyType )
import GHC.Tc.Instantiate( topInstantiate )
import GHC.Tc.Monad.Env( tcLookupId )
...
* tcInstSig: TcIdSigInfo -> TcIdSigInst
  ...
  binder.  You can see this in GHC.Tc.Binds.tcLhsId.
...
(see GHC.Tc.Binds.tcValBinds).  Moreover, because of top-level
...
a Type (not a TcType), in GHC.Tc.PatSyn.tc_patsyn_finish, and that
...
-- See Note [Recipe for checking a signature] in GHC.Tc.HsType
tcPatSynSig name sig_ty
  ...
  = do {  traceTc "tcPatSynSig 1" (ppr sig_ty)
       ...
       -- If you don't do this, the debugging checks fail in GHC.Tc.PatSyn.
  ...
  = setSrcSpan loc $  -- Set the binding site of the tyvars
    do { traceTc "Staring partial sig {" (ppr hs_sig)
       ; (wcs, wcx, tv_prs, theta, tau) <- tcHsPartialSigType ctxt hs_ty
         -- See Note [Checking partial type signatures] in GHC.Tc.HsType
...
impedance-matching check in GHC.Tc.Binds.mkExport

See Note [Signature skolems] in GHC.Tc.Utils.Type

None of this applies to a function binding with a complete
signature, which doesn't use tcInstSig.  See GHC.Tc.Binds.tcPolyCheck.
-}
