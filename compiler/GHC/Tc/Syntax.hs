...
import GHC.Tc.Env   ( tcLookupGlobalOnly )
...
import {-# SOURCE #-} GHC.Tc.Splice (runTopSplice)
...
-- Confused by zonking? See Note [What is zonking?] in GHC.Tc.Utils.Monadic.
...
is justified by the use of newUnique in GHC.Tc.Utils.Monadic.instSkolTyCoVarX.)
...
Problem:
    ...
    In GHC.Tc.Utils.Monadic.zonkTcTyVar, we short-circuit (Indirect ty) to
    (Indirect zty), see Note [Sharing in zonking] in GHC.Tc.Utils.Monadic. But we
    ...
    the same path as Note [Sharing in zonking] in GHC.Tc.Utils.Monadic, we'll
    update alpha to
