...
import GHC.Tc.Utils.Monadic   as TcM
import GHC.Tc.Monad as TcM
...
-- monad. See GHC.Tc.Monad Note [Constraints and errors]
...
captureTopConstraints thing_inside
  = do { static_wc_var <- TcM.newTcRef emptyWC ;
       ...
       -- See GHC.Tc.Monad Note [Constraints and errors]
       ...
       ; case mb_res of
           ...
           Nothing  -> do { _ <- simplifyTop lie; failM } }
                ...
                -- this function is here instead of GHC.Tc.Monad
...
How is this implemented? It's complicated! So we'll step through it all:
 ...
 4) `GHC.Tc.Simplify.simplifyTop`:
       ...
       * In the case of `warnAllUnsolved` for resolved, but unsafe
         ...
         message (pop it) and call `GHC.Tc.Monad.recordUnsafeInfer` to
 ...
 6) `GHC.Tc.Monad.recordUnsafeInfer` -- Save the unsafe result and reason in an
...
growThetaTyVars theta tcvs
  ...
  where
    ...
    (ips, non_ips) = partition isIPPred theta
                         -- See Note [Inheriting implicit parameters] in GHC.Tc.Utils.Type
...
* Wrinkle 4 (#14479)
  ...
  refrain from bogusly quantifying, in GHC.Tc.Simplify.decideMonoTyVars.  We
  report the error later, in GHC.Tc.Binds.chooseInferredQuantifiers.
...
GHC.Tc.Binds.tcPolyBinds, which gives all the binders in the group the type
...
solveImplication imp@(Implic { ic_tclvl  = tclvl
  ...
  where
    ...
    -- Note [TcLevel and untouchable type variables] in GHC.Tc.Utils.Type),
...
-- See Note [Checking telescopes] in GHC.Tc.Utils.Constraint
...
* We don't report redundant givens for *every* implication; only
  for those which reply True to GHC.Tc.Simplify.warnRedundantGivens:
   ...
   - More subtly, in a function definition
     ...
     other set False in
        - GHC.Tc.Binds.tcSpecPrag
        - GHC.Tc.Binds.tcTySig
...
-- invariant (WantedInv) in Note [TcLevel and untouchable type variables] in GHC.Tc.Utils.Type
...
defaultTyVarTcS the_tv
  ...
  , not (isTyVarTyVar the_tv)
    ...
    -- never with a type; c.f. GHC.Tc.Utils.Monadic.defaultTyVar
...
GHC.Tc.Simplify prior to Oct 2014).
