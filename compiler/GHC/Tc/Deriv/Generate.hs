...
gen_Eq_binds loc tycon inst_tys = do
    -- See Note [Auxiliary binders]
    con2tag_RDR <- new_con2tag_rdr_name loc tycon

    return (method_binds con2tag_RDR, aux_binds con2tag_RDR)
  where
    all_cons = filter (not . dataConCannotMatch inst_tys) $ tyConDataCons tycon
...
gen_Ord_binds loc tycon inst_tys = do
    -- See Note [Auxiliary binders]
    con2tag_RDR <- new_con2tag_rdr_name loc tycon
  ...
  where
    ...
    tycon_data_cons = filter (not . dataConCannotMatch inst_tys) $ tyConDataCons tycon
...
gen_Read_binds get_fixity loc tycon inst_tys
  ...
  where
    ...
    data_cons = filter (not . dataConCannotMatch inst_tys) $ tyConDataCons tycon
...
gen_Show_binds get_fixity loc tycon inst_tys
...
gen_Data_binds loc rep_tc inst_tys
  ...
  where
    data_cons  = filter (not . dataConCannotMatch inst_tys) $ tyConDataCons rep_tc
