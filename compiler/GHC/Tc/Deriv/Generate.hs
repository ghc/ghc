...
nlConWildPat con = noLocA $ ConPat
  ...
  , pat_args = RecCon noAnn $ HsRecFields
      { rec_ext = noAnn
      , rec_flds = []
      , rec_dotdot = Nothing }
  }
...
--      | is_record = mk_alt prefix_parser (mk_parser record_prec record_stmts body)
      ...
      where
        ...
        con_arity    = dataConSourceArity data_con
        labels       = map (mkFastStringShortText . field_label . flLabel) $ dataConFieldLabels data_con
  ...
  where
    ...
    gen_bind (DerivDataConstr dc dataC_RDR dataT_RDR)
      ...
      where
        ...
        labels   = map (nlHsLit . mkHsStringFS . mkFastStringShortText . field_label . flLabel)
