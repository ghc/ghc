...
module GHC.Tc.Typeable(mkTypeableBinds, tyConIsTypeable) where
...
1. Generate a binding for each module p:M
   (done in GHC.Tc.Typeable by mkModIdBindings)
...
* GHC.Prim doesn't have any associated object code, so we need to put the
  ...
  place to be GHC.Types. GHC.Tc.Typeable.mkPrimTypeableBinds is responsible for
