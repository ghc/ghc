...
module GHC.Tc.Deriv.Typeable(mkTypeableBinds, tyConIsTypeable) where
...
import GHC.Tc.Env
import GHC.Tc.Evidence ( mkWpTyApps )
import GHC.Tc.Monad
...
1. Generate a binding for each module p:M
   (done in GHC.Tc.Deriv.Typeable by mkModIdBindings)
...
* GHC.Prim doesn't have any associated object code, so we need to put the
  ...
  place to be GHC.Types. GHC.Tc.Deriv.Typeable.mkPrimTypeableBinds is responsible for
...
-- 'tcRnSrcDecls'.
--
-- See Note [Grand plan for Typeable] in GHC.Tc.Deriv.Typeable.
