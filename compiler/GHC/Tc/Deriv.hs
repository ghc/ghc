...
warnUselessTypeable
  = do { warn <- woptM Opt_WarnDerivingTypeable
       ; when warn $ addDiagnosticTc (WarnReasonWithFlag Opt_WarnDerivingTypeable)
               ...
               CanDeriveAnyClass -> do
                 ...
                 when (newtype_deriving && deriveAnyClass) $
                   lift $ whenWOptM Opt_WarnDerivingDefaults $
                     addDiagnosticTc (WarnReasonWithFlag Opt_WarnDerivingDefaults) $ sep
  ...
  = do { traceTc "doDerivInstErrorChecks2" (ppr clas_inst)
       ...
       ; case wildcard of
           ...
           Just span -> setSrcSpan span $ do
             ...
             diagnosticTc (WarnReasonWithFlag Opt_WarnPartialTypeSignatures)
