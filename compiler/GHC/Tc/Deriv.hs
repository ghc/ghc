  ...
  = discardWarnings $
    ...
    do  {
        ...
        ; let (aux_binds, aux_sigs) = unzipBag bagBinds
              aux_val_binds = ValBinds noAnn (map VbBind (bagToList aux_binds) ++ map VbSig (bagToList aux_sigs))
    ...
    do  { (rn_aux, dus_aux) <- rnLocalValBindsRHS (mkNameSet bndrs) (makeRnValBinds noAnn binds' sigs')
