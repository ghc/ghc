...
exports_from_avail (Just (L _ rdr_items)) rdr_env imports this_mod
  ...
  where
    ...
    exports_from_item expacc@ExportAccum{
                        ...
                        expacc_warn_spans = export_warn_spans,
                        expacc_dont_warn  = dont_warn_export
                      } (L loc ie@(IEWholeNamespace (warn_txt_ps, ns_spec, _)))
      ...
      = do { let { mod            = moduleName this_mod
            ...
            ; return ( ExportAccum { expacc_exp_occs   = occs'
                                   ...
                                   , expacc_warn_spans = export_warn_spans'
                                   , expacc_dont_warn  = dont_warn_export' }
                     , Just (L loc (IEWholeNamespace (warn_txt_rn, ns_spec, exported_names)), new_exports) ) }
    ...
    lookup_ie_kids_all ie (L _loc rdr) gre =
      do { let name = greName gre
         ...
         ; when (null kids_0) $
            if isTyConName name
            then addTcRnDiagnostic (TcRnDodgyExports (DodgyExportsEmptyParent gre))
