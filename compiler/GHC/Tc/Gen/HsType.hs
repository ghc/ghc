...
zipBinders = zip_binders [] emptyTCvSubst
  where
    -- subst: we substitute as we go eo ensure that the resulting
    ...
    zip_binders acc subst ki bs
      ...
      , let (subst', tb') = substTyBinder subst tb
...
substTyBinder :: TCvSubst -> TyBinder -> (TCvSubst, TyBinder)
substTyBinder subst (Anon af ty)          = (subst, Anon af (substScaledTy subst ty))
substTyBinder subst (Named (Bndr tv vis)) = (subst', Named (Bndr tv' vis))
                                          where
