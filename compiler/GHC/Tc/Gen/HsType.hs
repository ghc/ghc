...
* If delta{L+1} is unconstrained, we /must not/ quantify over it!
  ...
  becuase that is ill-kinded: the kind of the body of the forall,
...
where F :: Type -> RuntimeRep.   Now it's alpah that is free in the kind
and it /won't/ be defaulted.

So we use doNotQuantifyTyVars to either default the free vars of
exp_kind (if possible), or promote them (if not).
