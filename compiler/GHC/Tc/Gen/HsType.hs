...
tcHsTypeApp wc_ty kind
  ...
  = do { mode <- mkMode TypeLevel
       ; let mode' = mode { mode_holes = False }
          ...
          do { traceTc "tcInferTyApps (vis kind app)"
             ...
             ; let exp_kind = substTy subst $ tyBinderType ki_binder
                   arg_mode = mode { mode_tyki  = KindLevel
                                   , mode_holes = False }
                   -- mode_holes: see Note [Wildcards in visible kind application]
...
tcAnonWildCardOcc (TcTyMode { mode_lvl = lvl, mode_holes = emit_holes })
                  ty exp_kind
...
And whenever we see a '@', we set mode_holes to False, so that
