...
data ContextKind = TheKind Kind   -- ^ a specific kind
                 ...
                 | OpenBoxedKind  -- ^ something of the form @TYPE (BoxedRep _)@
...
newExpectedKind OpenBoxedKind = newOpenBoxedTypeKind
...
--
-- Superficially similar to 'ContextKind', but it lacks 'AnyKind' and instead
-- of @'TheKind' liftedTypeKind@ provides 'LiftedKind', which is much simpler
-- to match on and handle in 'isAllowedDataResKind'.
  ...
  where
    ...
    allowed_kind dflags
      ...
      | is_datatype && xopt LangExt.UnliftedDatatypes dflags
        ...
        -- which reduces to `TYPE (BoxedRep l)`.
        -- See Note [Implementation of UnliftedDatatypes] in TcTyClsDecls.
      = AnyBoxedKind
      | otherwise
      = LiftedKind
