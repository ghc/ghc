...
tcClassSigType :: [Located Name] -> LHsSigType GhcRn -> TcM (LHsSigType GhcTc)
...
tc_lhs_sig_type skol_info (L loc (HsSig { sig_bndrs = hs_outer_bndrs
  ...
  = setSrcSpan loc $
    do { (tc_lvl, wanted, (outer_bndrs, ty))
       ...
       ; let
           (L loc' hs_ty') = hs_ty
           sig_ty = L loc (HsSig noExtField outer_bndrs (L loc' $ XHsType (HsTypeTc (mkInfForAllTys kvs ty1) hs_ty')))
...
bindExplicitTKBndrsX skol_mode@(SM { sm_parent = check_parent, sm_kind = ctxt_kind
  ...
  where
    ...
    tc_hs_bndr lcl_env (UserTyVar _ flag (L l name))
      | check_parent
      , Just (ATyVar _ tv) <- lookupNameEnv lcl_env name
      = return (UserTyVar noExtField flag (L l tv), tv)
      | otherwise
      = do { kind <- newExpectedKind ctxt_kind
           ; var <- newTyVarBndr skol_mode name kind
           ; return $ (UserTyVar noExtField flag (L l var), var)}

    tc_hs_bndr lcl_env (KindedTyVar _ flag (L l name) lhs_kind@(L l' hs_kind))
      | check_parent
      , Just (ATyVar _ tv) <- lookupNameEnv lcl_env name
      = do { kind <- tc_lhs_kind_sig tc_ki_mode (TyVarBndrKindCtxt name) lhs_kind
           ; discardResult $
             unifyKind (Just (ppr name)) kind (tyVarKind tv)
                          ...
                          --    class C (m :: * -> *) where
                          --      type F (m :: *) = ...
           ; return $ (KindedTyVar noExtField flag (L l tv) (L l' $ XHsType (HsTypeTc kind hs_kind)), tv) }

      | otherwise
      = do { kind <- tc_lhs_kind_sig tc_ki_mode (TyVarBndrKindCtxt name) lhs_kind
           ; var <- newTyVarBndr skol_mode name kind
           ; return (KindedTyVar noExtField flag (L l var) (L l' $ XHsType (HsTypeTc kind hs_kind)), var)
           }
    ...
    do { mode <- mkHoleMode TypeLevel HM_Sig
      ...
      --     here because we don't have a complete type to check
       ...
       ; let
           ...
           sig_ty' = HsWC sig_wcs (L l (HsSig noExtField outer_bndrs (L ty_loc (XHsType (HsTypeTc tau body_ty')))))
