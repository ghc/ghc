...
tcApp rn_expr exp_res_ty
  ...
  = do { (tc_fun, fun_sigma) <- tcInferAppHead rn_fun rn_args
      ...
      ; quickLook impred delta inst_args app_res_ty exp_res_ty
       ...
       ; app_res_ty <- zonkQuickLook app_res_ty
...
tcArgs fun args
  ...
  where
    ...
    tc_arg n (HsETypeArg l hs_ty ty) = return (n, HsETypeArg l hs_ty ty)

    tc_arg n (HsEValArg l arg arg_ty)
      = do { arg_ty <- zonkQuickLook arg_ty   -- Key step; exploit QL
...
tcInstFun rn_fun fun_sigma rn_args
  ...
  where
    ...
    go delta acc so_far fun_ty args
      | case args of { [] -> True; HsEValArg {} : _ -> True; _ -> False }
      , not (isRhoTy fun_ty)
      = do { (wrap, fun_rho) <- topInstantiate fun_orig fun_ty
           ; go delta (addArgWrap wrap acc) so_far fun_rho args }
    ...
    go delta acc so_far fun_ty (HsETypeArg loc hs_ty_arg _ : args)
      ...
      | otherwise
      = do { (wrap1, ty_arg, inst_ty) <- tcVTA fun_orig fun_ty hs_ty_arg
           ; let acc' = HsETypeArg loc hs_ty_arg ty_arg : addArgWrap wrap1 acc
    ...
    go delta acc so_far fun_ty (HsEValArg loc arg _ : args)
      ...
      = -- Function type was of form   f :: forall a b. t1 -> t2 -> b
        ...
        do { nu1 <- newOpenFlexiTyVar
           ; nu2 <- newOpenFlexiTyVar
           ; let delta' = (delta `delVarSet` kappa) `extendVarSetList` [nu1,nu2]
                 arg_ty = mkTyVarTy nu1
                 res_ty = mkTyVarTy nu2
                 acc'   = HsEValArg loc arg arg_ty : acc
           ; writeMetaTyVar kappa (mkVisFunTy arg_ty res_ty)
           ; go delta' acc' (arg_ty : so_far) res_ty args }

      | otherwise   -- Use ordinary unification
       ...
       ; case tcSplitForAllTy_maybe upsilon_ty of
           Just (tvb, inner_ty)
             ...
             do { let tv   = binderVar tvb
                ...
                ; return (wrap1, ty_arg, insted_ty) }

           _ -> ty_app_err upsilon_ty hs_ty }
...
quickLook impred delta inst_args app_res_ty exp_res_ty
 ...
 = return ()  -- Short cut
 | otherwise
 = do { delta <- quickLookArgs delta inst_args
      ; case exp_res_ty of
          Infer {}     -> return ()
          Check res_ty -> qlMatch True delta app_res_ty res_ty }

zonkQuickLook :: TcType -> TcM TcType
-- This applies big-Theta to a type, simply by zonking
-- We have to do this aggressively (not, say, delayed until constraint
-- solving) to expose the polymoprhism to subsequent instantiation
zonkQuickLook ty = zonkTcType ty
...
quickLookArg delta (HsEValArg _ arg arg_ty)
  | (fun, args)           <- collectHsArgs (unLoc arg)
  , HsVar _ (L _ id_name) <- fun
  = do { id_ty <- tcLookupFunTy id_name
       ; (fun_delta, inst_args, app_res_ty) <- tcInstFun fun id_ty args
...
quickLookArg delta _
  = return delta

tcLookupFunTy :: Name -> TcM TcSigmaType
-- An extremely cut-down form of tcInferId
-- Returns only the type; does none of the extra checks
tcLookupFunTy name
  = do { thing <- tcLookup name
       ; case thing of
           ATcId { tct_id = id } -> return (idType id)
           AGlobal (AnId id)     -> return (idType id)
           AGlobal (AConLike cl) -> case cl of
             RealDataCon dc -> return (dataConUserType dc)
             PatSynCon ps   -> case patSynBuilderOcc ps of
               Just (_, ty) -> return ty
               Nothing      -> nonBidirectionalErr name
...
qlUnify delta ty1 ty2
  = go emptyVarSet ty1 ty2
  where
    go :: TyVarSet -> TcType -> TcType -> TcM ()
    -- The TyVarSet gives the variables bound by
    -- an enclosing forall; don't unify with these
    go bvs (TyVarTy tv) ty2
      | tv `elemVarSet` delta = go_kappa bvs tv ty2

    go bvs ty1 (TyVarTy tv)
      | tv `elemVarSet` delta = go_kappa bvs tv ty1
    ...
    go bvs (FunTy { ft_arg = arg1, ft_res = res1 })
       (FunTy { ft_arg = arg2, ft_res = res2 })
      = do { go bvs arg1 arg2; go bvs res1 res2 }
        -- ToDo: do we need to consider those implicit rep args?

    go bvs (AppTy t1a t1b) (AppTy t2a t2b)
      = do { go bvs t1a t2a; go bvs t1b t2b }

    go bvs (ForAllTy bv1 ty1) (ForAllTy bv2 ty2)
      = go bvs' ty1 ty2
      where
       bvs' = bvs `extendVarSet` binderVar bv1
                  `extendVarSet` binderVar bv2
              -- This is a bit conservative; we could
              -- keep a bound-var set for each side
    ...
    go_flexi bvs kappa ty2  -- ty2 is zonked
      | ty2_tvs `intersectsVarSet` bvs   -- We really only need shallow here
