...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
tcApp typecheck application chains (f e1 @ty e2) specially:
...
The main workhorse is tcApp, which does this
...
3. Use tcInstFun to instantiate the function, Quick-Looking
   as we go.  This implements the |-inst judgement of the QL paper:
   Fig 4, plus the modification in Fig 5.
...
tcInstFun do_ql inst_final rn_fun fun_sigma rn_args
  ...
  where
    ...
    go1 delta acc so_far fun_ty args@( ETypeArg { eva_loc = loc, eva_hs_ty = hs_ty }
                                     : rest_args )
...
the QL paper, in Fig 5, rather directly.
