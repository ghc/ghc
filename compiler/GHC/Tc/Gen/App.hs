...
zonkArg eva@(EValArg { eva_ty = Scaled m ty })
  = do { ty' <- zonkTcType ty
       ; return (eva { eva_ty = Scaled m ty' }) }
...
tcInferSigma inst le@(L loc rn_expr)
  | (rn_fun, rn_args, _) <- splitHsApps rn_expr
  = addExprCtxt le $
    setSrcSpan loc $
    do { do_ql <- doQuickLook rn_fun
       ; (tc_fun, fun_sigma) <- tcInferAppHead rn_fun rn_args
       ; (_delta, inst_args, app_res_sigma) <- tcInstFun do_ql False rn_fun fun_sigma rn_args
       ; _tc_args <- tcValArgs do_ql tc_fun inst_args
       ; maybe_instantiate app_res_sigma }
  where
    orig = lexprCtOrigin le
    maybe_instantiate res_ty
      | inst      = do { (_,rho) <- topInstantiate orig res_ty
                       ; return rho }
      | otherwise = return res_ty
...
tcValArgs quick_look fun args
  ...
  where
    ...
    tc_arg n eva@(EValArg { eva_arg = arg, eva_ty = Scaled mult arg_ty })
...
doQuickLook :: HsExpr GhcRn -> TcM Bool
-- GHC switches on impredicativity all the time for ($)
doQuickLook (HsVar _ f) | unLoc f `hasKey` dollarIdKey = return True
doQuickLook _                                          = xoptM LangExt.ImpredicativeTypes
...
----------------
quickLookArg :: Delta -> LHsExpr GhcRn -> Scaled TcSigmaType
...
quickLookArg delta arg (Scaled _ arg_ty)
