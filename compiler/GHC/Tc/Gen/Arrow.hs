...
import GHC.Types.Id( mkLocalId, IdBinding(..) )
...
tcArrDoStmt env ctxt (RecStmt { recS_stmts = L l stmts, recS_later_ids = later_names
  ...
  = do  { let tup_names = rec_names ++ filterOut (`elem` rec_names) later_names
        ...
        ; let tup_ids = zipWith (\n p -> mkLocalId n (LambdaBound ManyTy) p) tup_names tup_elt_tys -- ROMES:TODO: LambdaBound or LetBound, for recursive do binders?
