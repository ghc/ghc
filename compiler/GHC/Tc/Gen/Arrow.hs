...
data CmdEnv = CmdEnv
    ...
    , cmd_names :: CmdSyntaxTable GhcTc -- ^ Arrow methods (arr, (>>>), ...)
    }
...
tcCmdTop env (L loc (HsCmdTop _names cmd)) cmd_ty@(cmd_stk, res_ty)
  = setSrcSpan loc $
    do  { let names' = cmd_names env
        ; cmd' <- tcCmd env cmd cmd_ty
