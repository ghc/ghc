...
tc_pat pat_ty penv ps_pat thing_inside = case ps_pat of
  ...
  ViewPat mbInv expr pat -> do
        ...
        ; isSeeThrough <- isSeeThroughView mbInv pat_ty (unLoc expr)
        ; return $ ( ,res ) $
          if isSeeThrough
          then unLoc pat'
          else ViewPat pat_ty (mkLHsWrap expr_wrap expr') pat' }

        where
          -- See Note [Desugaring overloaded list patterns] (special case)
          isSeeThroughView :: Maybe (HsExpr GhcRn)
                           -> Type
                           -> HsExpr GhcRn
                           -> TcM Bool
          isSeeThroughView mbInv pat_ty expr = 
