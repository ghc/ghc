...
tcConValArgs con_like arg_tys penv con_args thing_inside = case con_args of
  PrefixCon arg_pats -> do
        ...
        ; unless (null invis_arg_pats) $ do
            forM_ invis_arg_pats $ \case
              L loc (InvisPat _ tp) ->
                setSrcSpanA loc $
    ...
    where
      ...
      (invis_arg_pats, vis_arg_pats) = partition (isInvisArgPat . unLoc) (dropHsConPatTyArgs arg_pats)
...
takeHsConPatTyArgs :: [LPat GhcRn] -> [HsTyPat GhcRn]
takeHsConPatTyArgs (L _ (InvisPat _ tp) : ps) = tp : takeHsConPatTyArgs ps
takeHsConPatTyArgs _ = []

dropHsConPatTyArgs :: [LPat GhcRn] -> [LPat GhcRn]
dropHsConPatTyArgs = dropWhile (isInvisArgPat . unLoc)
