...
* Why put [InvisTVBinder] in ConLikeTc, when we only need [TyVar] to
  desguar?  It's a bit of a toss-up, but having [InvisTvBinder] supports
...
* Note that the [InvisTvBinder] is strictly redundant anyway; it's
  ...
  fresh multiplicity variables.  But it seems simpler to do the work
  here, once for all, and record the results for the desugarer.
