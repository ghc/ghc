...
tcRuleTmBndrs rule_name (L _ (RuleBndr _ (L _ name)) : rule_bndrs)
  = do  { ty <- newOpenFlexiTyVarTy
        ...
        ; return (tyvars, mkLocalId name (LambdaBound ManyTy) ty : tmvars) } -- ROMES:TODO: LambdaBound or LetBound?
...
--   error for each out-of-scope type variable used
  = do  { let ctxt = RuleSigCtxt rule_name name
        ...
        ; let id  = mkLocalId name (LambdaBound ManyTy) id_ty -- ROMES:TODO: LambdaBound or LetBound?
