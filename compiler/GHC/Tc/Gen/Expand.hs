...
tcExpand e@(HsOverLabel (_, Rebindable rs_table) v)
  | Just fromLabelName <- lookup fromLabelClassOpOcc rs_table
...
tcExpand e@(HsIf (Rebindable rs_table) p b1 b2)
  | Just ifThenElseName <- lookup ifThenElseIdOcc rs_table
  = return $ Just $
    HSE { hse_ctxt = ExprCtxt e
...
tcExpand e@(HsGetField (Rebindable rs_table) expr f)
 | Just getFieldName <- lookup getFieldClassOpOcc rs_table
 = return $ Just $
   HSE { hse_ctxt = ExprCtxt e
...
tcExpand e@(HsProjection (Rebindable rs_table) fs)
 | Just getFieldName <- lookup getFieldClassOpOcc rs_table
 , Just circName <- lookup composeIdOcc rs_table
 = return $ Just $
   HSE { hse_ctxt = ExprCtxt e
...
tcExpand e@(RecordUpd (Rebindable rs_table) (L l expr) (OverloadedRecUpdFields { olRecUpdFields = us}))
 | Just getFieldName <- lookup getFieldClassOpOcc rs_table
 , Just setFieldName <- lookup setFieldClassOpOcc rs_table
 = return $ Just $
   HSE { hse_ctxt = ExprCtxt e
...
tcExpand e@(SectionR _ op expr)
  ...
  -- These are not rebindable, but never the less they are expanded here
  -- hence no need to refer to a lookup table
  = return $ Just $
    HSE { hse_ctxt = ExprCtxt e
...
tcExpand e@(ExplicitList (Rebindable rs_table) exps)
  | Just fromListNName <- lookup fromListNClassOpOcc rs_table
  = do { loc <- getSrcSpanM -- See Note [Source locations for implicit function calls]
       ; let lit_n    = mkIntegralLit (length exps)
             ...
             exp_list = genHsApps' (wrapGenSpan' loc fromListNName)
