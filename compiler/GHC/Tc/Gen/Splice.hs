...
When a variable is used at level n+1 and bound at level n, the renamer will decide
in `checkThLocalNameWithLift` to attempt to implicitly lift the variable to
level n+1. This is done by attempting to replace the occurence of `x` with
`$(lift x)`.

This decision is stored in the extension point 'XXUntypedSplice' and 'XXTypedSplice'.

When the typechecker encounters an XXUntypedSplice it will check that the variable
has a 'Lift' instance and can be lifted. Otherwise a level error is reported to
the user.
...
tcUntypedSplice (QuoteWrapper _ m_var) splice_name (HsQuasiQuote (HsQuasiQuoteExt flavour) quoter s) = do
   ...
   -- 2. The quasi-quoter has type Q Exp/Pat/Dec etc
   ...
   -- 3. Lookup the relevant selector and apply it to the quasi-quoter
...
tcUntypedSplice q splice_name (XUntypedSplice (HsImplicitLiftSplice bind_lvl use_lvl mgre id_name))
  = do {   id_ty <- newOpenFlexiTyVarTy
         ...
         ; lift <- setSrcSpan (getLocA id_name) $
                    newMethodFromName (ImplicitLiftOrigin bind_lvl use_lvl mgre (getName id_name))
...
tcPendingSpliceTyped q@(QuoteWrapper _ m_var) splice_name (HsTypedSpliceExpr _ expr) res_ty
  = do { res_ty <- expTypeToType res_ty
       ...
       ; return (PendingTcSplice splice_name expr'') }
tcPendingSpliceTyped q splice_name (XTypedSplice (HsImplicitLiftSplice bind_lvl use_lvl mgre id_name)) res_ty
  = do { res_ty <- expTypeToType res_ty
       ; let rep = getRuntimeRep res_ty
       ; let v_expr = noLocA (HsVar noExtField id_name)
    ...
    setSrcSpan loc    $ do
      ...
      case lvl of
        Brack _ (TcPending ps_var lie_var q) -> do
          ...
          -- TODO: stubNestedSplice is the best thing to do here (since tooling working with typed checked expressions)
          -- will not be able to identify where the nested splices are.
          -- Instead we should modify the extension point of HsTypedSplice to propagate the
