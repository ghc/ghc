...
instance TH.Quasi TcM where
  ...
  qAddDoc docArg s = do
    ...
    unless isLocal $ failWithTc $ text
      "Can't add documentation to" <+> pprLoc docArg <+> text "as it isn't inside the current module"
    ...
    where
      ...
      pprLoc (TH.InstDoc n i) = text "the instance" <+> ppr_th (TH.AppT (TH.ConT n) i)
  ...
  case Map.lookup nm declDocs of
    ...
    Nothing -> do
      ...
      case nameModule_maybe nm of
        ...
        Just mod -> do
          ModIface { mi_decl_docs = DeclDocMap dmap
  ...
  case Map.lookup nm argDocs of
    ...
    Nothing -> do
      ...
      case nameModule_maybe nm of
        Nothing -> pure Nothing
        Just mod -> do
          ModIface { mi_arg_docs = ArgDocMap amap
                   } <- liftIO $ hscGetModuleInterface hsc_env mod
...
handleTHMessage msg = case msg of
  ...
  AddDoc l s -> wrapTHResult $ TH.qAddDoc l s
