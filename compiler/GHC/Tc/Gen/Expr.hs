...
tcCheckExprNC (L loc expr) res_ty
  ...
  where setLocAndCtxt l e m = do
          tunnel <- inGeneratedCode
          if tunnel && not (isGeneratedSrcSpan l)
...
tcExpr (XExpr (HsExpanded a b)) t
  = fmap (XExpr . ExpansionExpr . HsExpanded a . unLoc) $
      tcLExpr (L generatedSrcSpan b) t
