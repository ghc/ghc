      ...
      Just mindef -> do
        ...
        when (tcg_src tcg_env /= HsigFile) $
            whenIsJust (isUnsatisfied (mindef `impliesAtom`) defMindef) $
                       (\bf -> addDiagnosticTc WarnReason (warningMinimalDefIncomplete bf))
        return mindef
  where
...
warnMissingAT name
  = do { warn <- woptM Opt_WarnMissingMethods
       ...
       ; diagnosticTc (WarnReasonWithFlag Opt_WarnMissingMethods) (warn && hsc_src == HsSrcFile)
