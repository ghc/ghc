...
reasons:
  ...
  - When typechecking a class decl, say
    ...
    and kinds generated by GHC.Tc.Type.
...
in addition to type variables! As a result, there are some places in GHC.Tc.Utils.Type
...
-- See Note [ExpType] in GHC.Tc.Utils.Monadic, where you'll also find manipulators.
...
data InferResult
  = IR { ir_uniq :: Unique  -- For debugging only
       ...
       , ir_lvl  :: TcLevel -- See Note [TcLevel of ExpType] in GHC.Tc.Utils.Monadic
       ...
       , ir_inst :: Bool
         ...
         -- See Note [Deep instantiation of InferResult] in GHC.Tc.Unify
...
--
-- This is defined here to avoid defining it in GHC.Tc.Expr boot file.
data SyntaxOpType
  = SynAny     -- ^ Any type
...
data TcTyVarDetails
  = SkolemTv      -- A skolem
       TcLevel    -- Level of the implication that binds it
                  -- See GHC.Tc.Unify Note [Deeper level on the left] for
...
We place this function here in GHC.Tc.Utils.Type, not in GHC.Core.TyCo.FVs,
...
-- called from GHC.Tc.Simplify, which itself is imported by GHC.Tc.Validity
...
--     (see Note [Remove redundant provided dicts] in GHC.Tc.PatSyn)
--
...
mkMinimalBySCs get_pred xs = go preds_with_scs []
 where
   ...
   go (work_item@(p,_,_) : work_list) min_preds
     ...
     , t1 `tcEqType` t2   -- See GHC.Tc.PatSyn
