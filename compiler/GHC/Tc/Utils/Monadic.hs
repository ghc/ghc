...
-- others:
import GHC.Tc.Monad        -- TcType, amongst others
...
Note [Case branches must never infer a non-tau type] of GHC.Tc.Match.
...
-- actual data definition is in GHC.Tc.Utils.Type
...
d) The resulting skolems are
   ...
   See GHC.Tc.Deriv.Constraints Note [Overlap and deriving] for an example
...
-- the unifier; see GHC.Tc.Unify.nicer_to_update_tv1, and
...
writeMetaTyVarRef tyvar ref ty
  ...
  = do { meta_details <- readMutVar ref;
       ...
       ; let kind_check_ok = tcIsConstraintKind zonked_tv_kind
             ...
             -- Hack alert! tcIsConstraintKind: see GHC.Tc.Type
...
(according to Note [Recipe for checking a signature] in GHC.Tc.Type).
...
in GHC.Tc.Utils.Type), but they also allow us to determine which variables to
...
example, the code in GHC.Tc.Simplify.decideMonoTyVars and in
GHC.Tc.Type.kindGeneralizeSome, both of which exclude certain otherwise-eligible
...
defaultTyVar default_kind tv
  ...
  | isTyVarTyVar tv
    ...
    -- on TyVarTvs; see Note [Signature skolems] in GHC.Tc.Utils.Type.
  ...
  where
    ...
    default_kind_var kv
      ...
      = do { addErr (vcat [ text "Cannot default kind variable" <+> quotes (ppr kv')
           ...
           -- promote, skolemise, or zap-to-Any, to satisfy GHC.Tc.Type
...
which is propagated up to the toplevel (see GHC.Tc.Simplify.tcSimplifyInferCheck).
...
simplifier by GHC.Tc.Module.tcRnSrcDecls calling GHC.Tc.Simplify.tcSimplifyTop.
...
zonkTyCoVar tv | isTcTyVar tv = zonkTcTyVar tv
   ...
   -- GHC.Tc.Type.bindTyClTyVars, but it seems
...
Why?, for example:
- For CDictCan, the @GHC.Tc.Simplify.expandSuperClasses@ step, which runs after the
...
But c.f Note [Sharing when zonking to Type] in GHC.Tc.Syntax.
