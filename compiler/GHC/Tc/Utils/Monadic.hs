...
writeMetaTyVarRef tyvar ref ty
  ...
  = do { meta_details <- readMutVar ref;
       ...
       ; let kind_check_ok = tcIsConstraintKind zonked_tv_kind
             ...
             -- Hack alert! tcIsConstraintKind: see GHC.Tc.HsType
...
(according to Note [Recipe for checking a signature] in GHC.Tc.HsType).
...
GHC.Tc.HsType.kindGeneralizeSome, both of which exclude certain otherwise-eligible
  ...
  where
    ...
    default_kind_var kv
      ...
      = do { addErr (vcat [ text "Cannot default kind variable" <+> quotes (ppr kv')
           ...
           -- promote, skolemise, or zap-to-Any, to satisfy GHC.Tc.HsType
...
zonkTyCoVar tv | isTcTyVar tv = zonkTcTyVar tv
   ...
   -- GHC.Tc.HsType.bindTyClTyVars, but it seems
