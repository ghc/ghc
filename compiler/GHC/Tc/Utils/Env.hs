...
tcCheckUsage name id_mult thing_inside
  = do { (local_usage, result) <- tcCollectingUsage thing_inside
    ...
    promote_mult m = do { let fvs = tyCoVarsOfType m
                        ; zonked_fvs <- zonkTyCoVarsAndFV fvs
                        ; tclvl <- getTcLevel
                          -- Non-determinism is OK because order of promotion doesn't matter
                        ; forM_ (nonDetEltsUniqSet zonked_fvs) $ \tv -> do
                            when (isMetaTyVar tv) $ do { _ <- promoteMetaTyVarTo tclvl tv
                                                       ; return () }
                        ; zonkTcType m
                        }
