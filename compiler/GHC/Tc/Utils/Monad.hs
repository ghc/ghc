...
initTc :: HscEnv
       ...
       -> RealSrcSpan
       -> TcM r
       -> IO (Messages DecoratedMessage, Maybe r)
...
initTcInteractive :: HscEnv -> TcM a -> IO (Messages DecoratedMessage, Maybe a)
...
getErrsVar :: TcRn (TcRef (Messages DecoratedMessage))
getErrsVar = do { env <- getLclEnv; return (tcl_errs env) }

setErrsVar :: TcRef (Messages DecoratedMessage) -> TcRn a -> TcRn a
...
addMessages :: Messages DecoratedMessage -> TcRn ()
addMessages msgs1
...
mkLongErrAt :: SrcSpan -> SDoc -> SDoc -> TcRn (MsgEnvelope DecoratedMessage)
mkLongErrAt loc msg extra
  = do { printer <- getPrintUnqualified ;
         unit_state <- hsc_units <$> getTopEnv ;
         let msg' = pprWithUnitState unit_state msg in
         return $ mkLongMsgEnvelope ErrReason loc printer msg' extra }
...
mkDecoratedSDocAt :: DiagnosticReason
                  ...
                  -> TcRn (MsgEnvelope DecoratedMessage)
...
capture_messages :: TcM r -> TcM (r, Messages DecoratedMessage)
...
-----------------------
tryTc :: TcRn a -> TcRn (Maybe a, Messages DecoratedMessage)
...
warnIfFlag warn_flag is_bad msg
  = do { warn_on <- woptM warn_flag
       ; when (warn_on && is_bad) $
         addDiagnostic (WarnReasonWithFlag warn_flag) msg }

-- | Display a warning if a condition is met.
warnIf :: Bool -> SDoc -> TcRn ()
warnIf is_bad msg
  = when is_bad (addDiagnostic WarnReason msg)
