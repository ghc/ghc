...
-----------------------
addWildCards :: TcTyVar -> TcM ()
addWildCards tv
  = do { wcs_var <- getLclEnv >>= return . tcl_wcs
       ; wcs_vars <- readTcRef wcs_var
       ; writeTcRef wcs_var (tv : wcs_vars) }

captureWildCards :: TcM r -> TcM ([TcTyVar], r)
-- capture_wildCards simply captures and returns the wildcards type variables
--                   constraints generated by thing_inside
-- Precondition: thing_inside must not throw an exception!
-- Reason for precondition: an exception would blow past the place
-- where we read the lie_var, and we'd lose the constraints altogether
captureWildCards thing_inside
  = do { wcs_var <- newTcRef []
