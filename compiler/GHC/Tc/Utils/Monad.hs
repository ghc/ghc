...
checkNoErrs main
  = do  { (res, errs_found) <- askErrsFound main
        ; when errs_found failM
        ; return res }
...
-----------------------
-- (askErrsFound m) runs m
-- If m fails,
--    then (askErrsFound m) fails, propagating only
--         insoluble constraints
--
-- If m succeeds with result r,
--    then (askErrsFound m) succeeds with result (r, b),
--         where b is True iff m generated errors
--
-- Regardless of success or failure,
--   propagate any errors/warnings generated by m
askErrsFound :: TcRn a -> TcRn (a, Bool)
askErrsFound thing_inside
