...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
* setSrcSpan sets tcl_provenance to RebindableSyntaxCP when it
  gets an UnhelpfulSrcSpan from rebindable syntax,
  and back to OtherCP when we get a useful SrcSpan.
* tcMethod in Tc/TyCl/Instance.hs sets tcl_provenance to
  DerivingCP when the method is generated by deriving.

This provenance information is currently used in mkGivenWarningReporter
in Tc/Errors.hs where we disable reporting inaccessible code warnings
for code that was generated by deriving clauses.
-}
...
-- Do not update the context if we are in code generated by rebindable syntax
-- See Note [Rebindable syntax and HsExpansion] in GHC.Hs.Expr
updCtxt ctxt env@(TcLclEnv { tcl_ctxt = ctxts, tcl_provenance = prov })
  | RebindableSyntaxCP <- prov = env
