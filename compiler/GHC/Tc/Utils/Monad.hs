...
import GHC.Types.Basic( TopLevelFlag, TypeOrKind(..), isTopLevel )
import Maybes
...
checkCrossStageLifting mk_expr sp_id top_lvl id_name id
                        (Brack st (TcPending ps_var zs_var _wrap))
  | isTopLevel top_lvl
  ...
  =     -- Nested identifiers, such as 'x' in
    ...
    do  { -- Important to set the stage so that any constraints generated by mk_expr 
          -- end up having the right level.
        e <- setStage st (mk_expr id_name id)
        ; case e of
