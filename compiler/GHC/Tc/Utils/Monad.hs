...
module GHC.Tc.Utils.Monad(
  ...
  mkDecoratedSDocAt, reportDiagnostic, reportDiagnostics,
...
initTc :: HscEnv
       ...
       -> RealSrcSpan
       -> TcM r
       -> IO (Messages TcRnDsMessage, Maybe r)
...
initTcInteractive :: HscEnv -> TcM a -> IO (Messages TcRnDsMessage, Maybe a)
...
getErrsVar :: TcRn (TcRef (Messages TcRnDsMessage))
getErrsVar = do { env <- getLclEnv; return (tcl_errs env) }

setErrsVar :: TcRef (Messages TcRnDsMessage) -> TcRn a -> TcRn a
...
addErrAt loc msg = do { ctxt <- getErrCtxt
                      ...
                      ; add_long_err_at loc msg err_info }
...
addMessages :: Messages TcRnDsMessage -> TcRn ()
addMessages msgs1
...
add_long_err_at :: SrcSpan -> SDoc -> SDoc -> TcRn ()
add_long_err_at loc msg extra = mk_long_err_at loc msg extra >>= reportDiagnostic
  where
    mk_long_err_at :: SrcSpan -> SDoc -> SDoc -> TcRn (MsgEnvelope TcRnMessage)
    mk_long_err_at loc msg extra
...
reportDiagnostic msg
  = do { traceTc "Adding diagnostic:" (pprLocMsgEnvelope msg) ;
         ...
         writeTcRef errs_var (fmap (mkTcRnDsMessage . Right) msg `addMessage` msgs) }
...
capture_messages :: TcM r -> TcM (r, Messages TcRnDsMessage)
...
-----------------------
tryTc :: TcRn a -> TcRn (Maybe a, Messages TcRnDsMessage)
...
add_err_tcm tidy_env err_msg loc ctxt
 = do { err_info <- mkErrInfo tidy_env ctxt ;
        add_long_err_at loc err_msg err_info }
...
recordUnsafeInfer :: Messages TcRnDsMessage -> TcM ()
