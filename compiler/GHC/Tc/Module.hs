...
tcRnSrcDecls explicit_mod_hdr export_ies decls
 = do { -- Do all the declarations
      ...
      -- The export list must "see" the declarations created by the finalizers
      ; tcg_env <- setEnvs (tcg_env, tcl_env) $
                   rnExports explicit_mod_hdr export_ies

      --------- Emit the ':Main.main = runMainIO main' declaration ----------
      -- Do this /after/ rnExports, so that it can consult
      -- the tcg_exports that rnExports creates
      ...
      ; main_ev_binds <- simplifyTop lie_main
...
setMainCtxt main_name io_ty thing_inside
  = setSrcSpan (getSrcSpan main_name) $
    ...
    checkConstraints skol_info [] []  $  -- Builds an implication if necy
...
* rnExports: checks the export list.  We need to do this after running
  any finalisers, which may add new declarations.
