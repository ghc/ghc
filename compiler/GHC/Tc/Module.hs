...
In addition if the user specifies the main module in the export list,
(s)he can specify it qualified (eg `Main.main`) or unqualified (`main`).
...
In all cases check, that the `Main` module exports exatly on `main` function.
...
in Main module  |  Ambigous   |             |  Ambigous   |             |  `main` not |            |
...
* The module `Sub` contains the following functions:
     main :: IO ()
     foo :: IO ()
* 1) Here the header is `Main (foo)`.
* 2) Here we have no extra test case. It would exercise the same code path as `T19397E4`.
* 3) Here the header is `Main (Sub.foo)`.
