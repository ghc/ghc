...
import {-# SOURCE #-}   GHC.Tc.Expr( tcSyntaxOp, tcSyntaxOpGen, tcInferSigma )

import GHC.Hs
import GHC.Tc.Zonk
import GHC.Tc.Sigs( TcPragEnv, lookupPragEnv, addInlinePrags )
import GHC.Tc.Monad
...
tcPatBndr penv@(PE { pe_ctxt = LetPat { pc_lvl    = bind_lvl
  ...
  -- Note [Typechecking pattern bindings] in GHC.Tc.Binds
...
-- See Note [Typechecking pattern bindings], item (4) in GHC.Tc.Binds
--
...
like the situation in Note [Case branches must be taus] in GHC.Tc.Match.
...
See also Note [Typechecking pattern bindings] in GHC.Tc.Binds
-}
