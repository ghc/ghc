...
-- question. (See GHC.Tc.Expr.tcArgs.) This argument is the reversed list of
...
* Implications. We may decide to build an implication for the whole
  ...
  and GHC.Tc.Unify.alwaysBuildImplication checks the UserTypeCtxt.
...
-- See also Note [ExpType] in GHC.Tc.Utils.Monadic
...
1. Consider this (which uses visible type application):
   ...
   We'll call GHC.Tc.Expr.tcInferFun to infer the type of the (let .. in f)
...
emitResidualTvConstraint skol_info m_telescope skol_tvs tclvl wanted
  ...
  , isNothing m_telescope || skol_tvs `lengthAtMost` 1
    ...
    -- wanted constraints. See Note [Checking telescopes] in GHC.Tc.Utils.Constraint.
...
The question is answered by swapOverTyVars, which is use
  - in the eager unifier, in GHC.Tc.Unify.uUnfilledVar1
...
* Priority.  If the levels are the same, look at what kind of
  ...
  in preference to SkolemTv or RuntimeUnkTv:
     ...
     b) Even if it's not touchable, GHC.Tc.Simplify.floatEqualities
...
Note [Non-TcTyVars in GHC.Tc.Unify]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
