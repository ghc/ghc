...
import TysPrim
import GHC.Types.Name.RdrName( lookupLocalRdrOcc )
...
tcHsSigType ctxt sig_ty
  = addSigCtxt ctxt (hsSigType sig_ty) $
    do { traceTc "tcHsSigType {" (ppr sig_ty)
       ...
       ; when insol failM
       -- See Note [Fail fast if there are insoluble kind equalities] in GHC.Tc.Simplify
...
tc_hs_sig_type skol_info hs_sig_type ctxt_kind
  ...
  = do { (tc_lvl, (wanted, (spec_tkvs, ty)))
       ...
       -- This bit is very much like decideMonoTyVars in GHC.Tc.Simplify,
...
-- promoted. (See [Promoting unification variables] in GHC.Tc.Simplify
  ...
  = addSigCtxt ctxt hs_ty $
    do { (implicit_tvs, (explicit_tvs, (wcs, wcx, theta, tau)))
            <- solveLocalEqualities "tcHsPartialSigType"    $
                 ...
                 -- insoluble equalities. See GHC.Tc.Simplify
