...
import {-# SOURCE #-} GHC.Tc.Match ( tcGRHSsPat, tcMatchesFun )
import {-# SOURCE #-} GHC.Tc.Expr  ( tcMonoExpr )
import {-# SOURCE #-} GHC.Tc.PatSyn ( tcPatSynDecl, tcPatSynBuilderBind )
...
-- type environment; see the invariant on GHC.Tc.Monad.Env.tcExtendIdEnv
    ...
    choose_psig_context psig_qtvs annotated_theta (Just wc_var_ty)
      = do { let free_tvs = closeOverKinds (growThetaTyVars inferred_theta seed_tvs)
           ...
           ; case tcGetCastedTyVar_maybe wc_var_ty of
               ...
               -- comes from the checkExpectedKind in GHC.Tc.HsType.tcAnonWildCardOcc. So, to
...
We typecheck pattern bindings as follows.  First tcLhs does this:
  ...
  3. Invoke tcLetPat to typecheck the pattern.
     ...
     - We pass in the current TcLevel.  This is captured by
       GHC.Tc.Pat.tcLetPat, and put into the pc_lvl field of PatCtxt, in
       PatEnv.
     ...
     - When we come to a binder (GHC.Tc.Pat.tcPatBndr), it looks it up
