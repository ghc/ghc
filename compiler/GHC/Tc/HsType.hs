...
import GHC.Tc.Instantiate ( tcInstInvisibleTyBinders, tcInstInvisibleTyBinder )
...
the surrounding context, we must obey the following dictum:
  ...
  Every metavariable in a type must either be
    ...
    (C) a cause to error    See Note [Naughty quantification candidates] in GHC.Tc.Utils.Monadic
...
in GHC.Tc.Utils.Type. Note that promotion is identical in effect to generalizing
...
-- See Note [Extended defaults]" in GHC.Tc.Monad.Env
  ...
  where
    ...
    go (L _ hs_tv : hs_tvs)
       = do { tv <- tc_tv hs_tv
            ...
            -- See GHC.Tc.Utils.Monadic Note [Cloning for tyvar binders]
...
* In GHC.Tc.Sigs.tcUserSigType we return a PartialSig, which (unlike
...
* GHC.Tc.Binds.chooseInferredQuantifiers fills in that hole TcTyVar
...
* I make an /ordinary/ tuple of the constraints, in
  GHC.Tc.Binds.chooseInferredQuantifiers. This is ill-kinded because
