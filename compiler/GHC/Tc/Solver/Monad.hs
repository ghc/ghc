...
mightEqualLater (IS { inert_cycle_breakers = cbvs })
  ...
  where
    ...
    can_unify lhs_tv CycleBreakerTv _rhs_ty
      | Just tyfam_app <- lookup lhs_tv cbvs
      = anyFreeVarsOfType isMetaTyVar tyfam_app
      | otherwise
      = pprPanic "mightEqualLater finds an unbound cbv" (ppr lhs_tv $$ ppr cbvs)
    can_unify _lhs_tv _other _rhs_ty
      = True
