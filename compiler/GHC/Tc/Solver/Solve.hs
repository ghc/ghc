         ...
         ; if isSolvedWC solved_wc
           then
             ...
             do { traceTc "solveCompletelyIfRequired: unsolved }" $
                ...
                ; ( `unTcS` env ) $ case ct of
                    CDictCan dict_ct ->
                      updInertDicts dict_ct
                    _ ->
                      -- We get here when calling 'solveCompletelyIfRequired'
                      -- from 'solveForAll'; just stash the unsolved quantified
                      -- constraint in the irreds.
                      updInertIrreds (IrredCt (ctEvidence ct) IrredShapeReason)
