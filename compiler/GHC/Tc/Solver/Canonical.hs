...
type variable binder affect GHC's notion of typechecker equality. That is, we
have the following:
...
GHC.Tc.Solver.Canonical.can_eq_nc' and GHC.Tc.Utils.TcType.tcEqType, which
implement typechecker equality. sameVis puts both forms of invisible type
variable binders into the same equivalence class.

Note that we do /not/ use sameVis in GHC.Core.Type.eqType, which implements
a slighty more coarse-grained notion of equality that does not consider the
ArgFlag of ForAllTys at all. That is, eqType would equate all of
forall k. <...>, forall {k}. <...>, and forall k -> <...>.
-}
