...
module GHC.Tc.Solver.Canonical(
     ...
     solveCallStack    -- For GHC.Tc.Simplify
  ) where
...
import GHC.Tc.Unify( swapOverTyVars, metaTyVarUpdateOK )
...
-- Also called from GHC.Tc.Simplify when defaulting call stacks
...
1. Eagerly generate superclasses for given (but not wanted)
   ...
   termination condition: see Note [Expanding superclasses] in GHC.Tc.Utils.Type.
...
3. If we have any remaining unsolved wanteds
        (see Note [When superclasses help] in GHC.Tc.Utils.Constraint)
   try harder: take both the Givens and Wanteds, and expand
   superclasses again.  See the calls to expandSuperClasses in
   GHC.Tc.Simplify.simpl_loop and solveWanteds.
...
-- The loop-breaking here follows Note [Expanding superclasses] in GHC.Tc.Utils.Type
...
(by GHC.Tc.Simplify.simplifyInfer).  A function can only have a
...
GHC.Tc.Simplify.applyTyVarDefaulting will find a variable
...
canEqTyVar ev eq_rel swapped tv1 ps_xi1 xi2 ps_xi2
  ...
  -- Note [Flattening] in GHC.Tc.Solver.Flatten gives us (F2), which says that
...
canEqTyVarHomo ev eq_rel swapped tv1 ps_xi1 xi2 _
  ...
  = do { traceTcS "canEqTyVar swapOver" (ppr tv1 $$ ppr tv2 $$ ppr swapped)
         -- FM_Avoid commented out: see Note [Lazy flattening] in GHC.Tc.Solver.Flatten
...
Note [Wanteds do not rewrite Wanteds] in GHC.Tc.Utils.Constraint: note that the
new `co` is a Wanted.
