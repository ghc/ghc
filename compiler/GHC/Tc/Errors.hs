...
import Control.Monad    ( when, foldM, forM_, unless)
...
reportHoles tidy_cts ctxt holes
  = do
      ...
      forM_ holes' $ \hole -> do { msg_mb <- mkHoleError lcl_name_cache tidy_cts ctxt' hole
...
-- | This function zonks all the tcl_bndrs in a 'TcLclEnv' and returns a
-- cache so that relevant_bindings can avoid zonking and tidying many
-- times.
...
mkHoleError :: NameEnv Type -> [Ct] -> ReportErrCtxt -> Hole -> TcM (Maybe (MsgEnvelope DiagnosticMessage))
    ...
    go dflags n_left tvs_seen docs discards (tc_bndr : tc_bndrs)
      ...
      where
        discard_it = go dflags n_left tvs_seen docs
                        discards tc_bndrs
        go2 id_name top_lvl
          = do { let tidy_ty = lookupNameEnv_NF lcl_name_env id_name
