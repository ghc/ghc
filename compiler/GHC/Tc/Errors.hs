...
reportWanteds ctxt tc_lvl (WC { wc_simple = simples, wc_impl = implics
  ...
  = do { traceTc "reportWanteds" (vcat [ text "Simples =" <+> ppr simples
       ...
       ; (_, no_out_of_scope) <- askNoErrs $
                                 reportOutOfScopes ctxt_for_scope_errs out_of_scope
...
reportOutOfScopes :: ReportErrCtxt -> [Hole] -> TcM ()
reportOutOfScopes ctxt
  = mapM_ $ \hole -> do { err <- mkOutOfScopeError hole
                        ; maybeReportOutOfScopeError ctxt err
...
maybeReportOutOfScopeError :: ReportErrCtxt -> ErrMsg -> TcM ()
maybeReportOutOfScopeError ctxt err
...
maybeAddDeferredHoleBinding ctxt err (Hole { hole_ty = hole_ty
                                           , hole_sort = ExprHole ev_id })
-- Only add bindings for holes in expressions
-- not for holes in partial type signatures
-- cf. addDeferredBinding
  | deferringAnyBindings ctxt
  = do { dflags <- getDynFlags
       ; let err_tm = mkErrorTerm dflags hole_ty err
...
----------------
mkOutOfScopeError :: Hole -> TcM ErrMsg
mkOutOfScopeError (Hole { hole_occ = occ
                        , hole_ty = hole_ty
                        , hole_loc = ct_loc })
  = do { dflags  <- getDynFlags
