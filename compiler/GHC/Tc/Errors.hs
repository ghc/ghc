...
import GHC.Tc.Unify( occCheckForErrors, MetaTyVarUpdateResult(..) )
import GHC.Tc.Env( tcInitTidyEnv )
...
import GHC.Tc.EvTerm
import GHC.Hs.Binds ( PatSynBind(..) )
import GHC.Types.Name
import GHC.Types.Name.RdrName ( lookupGRE_Name, GlobalRdrEnv, mkRdrUnqual )
...
import {-# SOURCE #-} GHC.Tc.Hole ( findValidHoleFits )
...
we run into a type mismatch in GHC.Tc.Unify, we normally just emit an error. But it
...
--      (see Note [Fail fast on kind errors] in GHC.Tc.Simplify)
  ...
  = do { traceTc "reportUnsolved {" $
       ...
       ; let err_ctxt = CEC { cec_encl  = []
                            ...
                            , cec_suppress = insolubleWC wanted
                                 ...
                                 -- in GHC.Tc.Simplify.setImplicationStatus
...
-- See Note [Tracking redundant constraints] in GHC.Tc.Simplify
