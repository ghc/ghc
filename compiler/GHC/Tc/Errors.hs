...
mkGivenWarningReporter ctxt cts
  = do { (ctxt, binds_msg, ct) <- relevantBindings True ctxt ct
       ...
       ; if tcl_deriving (ctLocEnv (ctLoc ct))
            then return () -- Don't report a warning if the code we're checking was generated by a deriving clause
            else reportWarning (Reason Opt_WarnInaccessibleCode) err
       }
  where
