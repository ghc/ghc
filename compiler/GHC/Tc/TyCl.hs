...
kcConArgTys :: ConArgKind                      -- Expected kind of the argument(s)
            -> [HsConFieldSpec GhcRn]          -- User-written argument types
            -> TcM ()
kcConArgTys exp_kind arg_tys
  = forM_ arg_tys $ \(CFS _ _ _ mult ty _) ->
    do { _ <- tcCheckLHsTypeInContext ty exp_kind
       ; tcMult mult }
...
tcConArg :: ConArgKind   -- expected kind for args; always OpenKind for datatypes,
         ...
         -> HsConFieldSpec GhcRn -> TcM (Scaled TcType, HsSrcBang)
