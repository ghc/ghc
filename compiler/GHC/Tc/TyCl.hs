...
import GHC.Tc.Unify ( checkTvConstraints )
import GHC.Tc.Type
...
4.  tyConScopedTyVars.  A challenging piece in all of this is that we
    ...
    These tyvars are brought into scope in GHC.Tc.Type.bindTyClTyVars.
...
swizzleTcTyConBndrs tc_infos
  | all no_swizzle swizzle_prs
    ...
    -- See Note [Non-cloning for tyvar binders] in GHC.Tc.Type
...
* Step 1: inferInitialKinds, and in particular kcInferDeclHeader.
  ...
  These unification variables
    ...
    - Have complete fresh Names; see GHC.Tc.Utils.Monadic
...
* Step 3: generaliseTcTyCon. Generalise each TyCon in turn.
  ...
  All this is very similar at the level of terms: see GHC.Tc.Binds
...
* Do not default TyVarTvs.  We always want to kind-generalise over
  ...
  variable. Hence the check in GHC.Tc.Simplify.defaultTyVarTcS, and
  GHC.Tc.Utils.Monadic.defaultTyVar.  Here's another example (#14555):
...
GHC.Tc.Type defeats that approach.) addVDQNote is used in kcTyClDecl,
...
kcExplicitTKBndrs and kcImplicitTKBndrs in GHC.Tc.Type, separate from their
tc... variants.
...
Then:
  ...
  * During GHC.Tc.Type.tcTyVar we look in the *local* env, to get the
    fully-known, not knot-tied TcTyCon for T.

  * Then, in GHC.Tc.Syntax.zonkTcTypeToType (and zonkTcTyCon in particular)
...
7. Two return kinds for instances: If an instance has two return kinds,
   ...
   kind. GHC.Tc.Type.checkExpectedKind normally does this check for types, but
...
See also Note [Re-quantify type variables in rules] in GHC.Tc.Rule, which explains
...
tcTyFamInstEqnGuts fam_tc mb_clsinfo imp_vars exp_bndrs hs_pats hs_rhs_ty
  = do { traceTc "tcTyFamInstEqnGuts {" (ppr fam_tc)
       ...
       -- in GHC.Tc.Type.kcCheckDeclHeader_cusk
...
checkValidClass cls
  ...
  where
    ...
    -- Note [TyVars and TcTyVars during type checking] in GHC.Tc.Utils.Type
