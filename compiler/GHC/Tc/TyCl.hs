...
* Consider a data type, rather than a data instance, declaration
  ...
  data types (DDVanilla) only for data instances (DDInstance); in
  the latter the family constructor is not knot-tied so there is no
  problem.

* Consider this (fromn GHC itself):
   ...
   for datatypes like GhcPass, we would not be able to exand the type
   synonym (it'd still be a TcTyCon).  So again, we don't do uinfyType
