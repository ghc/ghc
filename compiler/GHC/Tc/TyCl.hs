...
kcConArgs :: NewOrData -> Kind -> [HsScaled GhcRn (LHsType GhcRn)] -> TcM ()
kcConArgs new_or_data res_kind arg_tys = do
...
kcConH98Args new_or_data res_kind con_args = case con_args of
  PrefixCon tys     -> kcConArgs new_or_data res_kind tys
  InfixCon ty1 ty2  -> kcConArgs new_or_data res_kind [ty1, ty2]
  RecCon (L _ flds) -> kcConArgs new_or_data res_kind $
...
kcConGADTArgs new_or_data res_kind con_args = case con_args of
  PrefixConGADT tys     -> kcConArgs new_or_data res_kind tys
  RecConGADT (L _ flds) -> kcConArgs new_or_data res_kind $
                           map (hsLinear . cd_fld_type . unLoc) flds
