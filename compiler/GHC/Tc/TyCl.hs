...
4.  tyConScopedTyVars.  A challenging piece in all of this is that we
    ...
    GHC.Tc.HsType.splitTelescopeTvs!)
    ...
    These tyvars are brought into scope in GHC.Tc.HsType.bindTyClTyVars.
...
swizzleTcTyConBndrs tc_infos
  | all no_swizzle swizzle_prs
    ...
    -- See Note [Non-cloning for tyvar binders] in GHC.Tc.HsType
...
GHC.Tc.HsType defeats that approach.) addVDQNote is used in kcTyClDecl,
...
kcExplicitTKBndrs and kcImplicitTKBndrs in GHC.Tc.HsType, separate from their
tc... variants.
...
Then:
  ...
  * During GHC.Tc.HsType.tcTyVar we look in the *local* env, to get the
    fully-known, not knot-tied TcTyCon for T.

  * Then, in GHC.Tc.Zonk.zonkTcTypeToType (and zonkTcTyCon in particular)
...
7. Two return kinds for instances: If an instance has two return kinds,
   ...
   kind. GHC.Tc.HsType.checkExpectedKind normally does this check for types, but
...
tcTyFamInstEqnGuts fam_tc mb_clsinfo imp_vars exp_bndrs hs_pats hs_rhs_ty
  = do { traceTc "tcTyFamInstEqnGuts {" (ppr fam_tc)
       ...
       -- in GHC.Tc.HsType.kcCheckDeclHeader_cusk
