...
module GHC.Tc.Rule ( tcRules ) where
...
import GHC.Tc.Expr
import GHC.Tc.Env
import GHC.Tc.Unify( buildImplicationFor )
import GHC.Tc.Evidence( mkTcCoVarCo )
...
Note [Naughty quantification candidates] in GHC.Tc.Utils.Monadic. This is what would happen
...
tcRule (HsRule { rd_ext  = ext
  ...
  = addErrCtxt (ruleCtxt name)  $
    do { traceTc "---- Rule ------" (pprFullRuleName rname)
       ...
       -- c.f. GHC.Tc.Simplify.simplifyInfer
       ...
       -- during zonking (see GHC.Tc.Syntax.zonkRule)
...
--   error for each out-of-scope type variable used
  = do  { let ctxt = RuleSigCtxt name
        ...
        ; let id  = mkLocalId name id_ty
                    -- See Note [Pattern signature binders] in GHC.Tc.Type
...
--     see GHC.Tc.Unify.implicationNeeded. Not doing so caused #14732.
--
