...
import GHC.Tc.Monad
import GHC.Tc.Sigs( emptyPragEnv, completeSigFromId )
import GHC.Tc.Env
...
After that, Note [Naughty quantification candidates] in GHC.Tc.Utils.Monadic takes
over and errors.
...
See also Note [Lift equality constraints when quantifying] in GHC.Tc.Utils.Type
  ...
  = addPatSynCtxt lname $
    do { let decl_arity = length arg_names
       ...
       ; checkTc (null bad_tvs) $
         ...
         -- See Note [The pattern-synonym signature splitting rule] in GHC.Tc.Sigs
