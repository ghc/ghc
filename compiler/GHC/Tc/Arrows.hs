...
module GHC.Tc.Arrows ( tcProc ) where

import GhcPrelude

import {-# SOURCE #-}   GHC.Tc.Expr( tcMonoExpr, tcInferRho, tcSyntaxOp, tcCheckId, tcPolyExpr )

import GHC.Hs
import GHC.Tc.Match
import GHC.Tc.Syntax( hsLPatType )
...
tc_cmd env cmd@(HsCmdArrApp _ fun arg ho_app lr) (_, res_ty)
  ...
  where
       ...
       -- See Note [Escaping the arrow scope] in GHC.Tc.Utils
