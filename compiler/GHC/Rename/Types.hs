...
mkConOpPatRn op2 fix2 p1@(L loc (ConPat op1 ts (InfixCon p11 p12) NoExtField)) p2
  = do  { fix1 <- lookupFixityRn (unLoc op1)
        ...
        ; if nofix_error then do
                ...
                ; return (ConPat op2 ts (InfixCon p1 p2) NoExtField) }

          else if associate_right then do
                { new_p <- mkConOpPatRn op2 fix2 p12 p2
                ; return (ConPat op1 ts (InfixCon p11 (L loc new_p)) NoExtField) }
                -- XXX loc right?
          else return (ConPat op2 ts (InfixCon p1 p2) NoExtField) }
...
not_op_pat (ConPat _ _ (InfixCon _ _) NoExtField) = False
not_op_pat _                                      = True
...
checkPrec op (ConPat op1 _ (InfixCon _ _) NoExtField) right = do
