...
renameSig ctxt sig@(ClassOpSig _ is_deflt vs ty)
  ...
  where
    ...
    ty_ctxt = GenericCtx (text "a class method signature for"
                          <+> quotes (ppr v1))
    inf_msg = if is_deflt
      then Just (text "A default type signature should not contain inferred type variables")
      else Nothing
...
renameSig _ (SpecInstSig _ src ty)
  ...
  where
    inf_msg = Just (text "A SPECIALISE signature should not contain inferred type variables")
