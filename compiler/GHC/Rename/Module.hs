...
rnFamInstEqn doc atfi rhs_kvars
  ...
  = do { let mb_cls = case atfi of
       ...
       ; let bndrs = fromMaybe [] mb_bndrs
         ...
         -- Implicitly bound variables. This is empty if we have an explicit
         -- 'forall', which means ignoring the rhs_kvars function parameter. See
         -- Note [forall-or-nothing rule] in GHC.Rename.HsType.
         --
         -- > type instance forall a b c. .... = .... => []
       ...
       ; all_imp_vars <- forAllOrNothing (isJust mb_bndrs) $ pure $
         ...
         -- explicit_bndrs' case) because there must be none if we're going to
         -- implicitly bind anything, per the previous comment.
...
rnConDecl decl@(ConDeclGADT { con_names   = names
  ...
  = do  { mapM_ (addLocM checkConName) names
        ...
        ; let explicit_tkvs = hsQTvExplicit qtvs
          ...
          -- See #14808.
        ; implicit_bndrs <- forAllOrNothing explicit_forall $ pure
