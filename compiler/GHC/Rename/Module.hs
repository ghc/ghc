  ...
  where
    ...
    new_ps' bind names
      ...
      = do
          ...
          let con_info = mkConInfo ConIsPatSyn (con_details_arity length (RecCon as)) flds
          return ((PatSynName bnd_name, con_info) : names)
      | L bind_loc (PatSynBind _ (PSB { psb_id = L _ n, psb_args = as })) <- bind
      = do
        bnd_name <- newTopSrcBinder (L (l2l bind_loc) n)
        let con_info = mkConInfo ConIsPatSyn (con_details_arity length as) []
        return ((PatSynName bnd_name, con_info) : names)
      | otherwise
      = return names

    con_details_arity :: (rec -> Arity) -> HsConDetails arg rec -> Arity
    con_details_arity recToArity = \case
