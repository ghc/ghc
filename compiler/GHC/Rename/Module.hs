...
(1) Rename the type/class declarations, standalone kind signatures,
    role declarations, and instances individually
...
(3) Attach roles and kind signatures to the appropriate SCC

(4) Create one singleton "SCC" per instance and put them at the end,
    where their lexical dependencies are surely satisfied.
    See Note [Put instances at the end]
...
rnTyClDecls tycl_ds
  = do { -- Rename the type/class, instance, and role declarations
       ...
       ; let tycl_sccs = depAnalTyClDecls rdr_env tc_names kisig_fv_env tycls_w_fvs
             ...
             tycl_groups = map (mk_group role_annot_env kisig_env) tycl_sccs
...
* deps = toParents rdr_env inst_fvs `intersectFVs` tc_names   -- in mkInstGroups
* deps = toParents rdr_env node_fvs `intersectFVs` tc_names   -- in depAnalTyClDecls
* deps = delFVs node_bndrs (plusFVs nodes_deps)               -- in rnTyClGroups
