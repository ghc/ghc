...
rnWcBody ctxt nwc_rdrs hs_ty
  ...
  where
    ...
    rn_ty env hs_ty@(HsForAllTy { hst_tele = tele, hst_body = hs_body })
      = bindLHsForAllTelescope (rtke_ctxt env) (Just $ inTypeDoc hs_ty) tele $ \ tele' ->
...
check_inferred_vars ctxt (Just msg) ty =
  ...
  where
    ...
    forallty_bndrs (L _ ty) = case ty of
      ...
      HsForAllTy { hst_tele = L _ (HsForAllInvis { hsf_invis_bndrs = tvs }) }
                    -> map unLoc tvs
      _             -> []
...
rnHsTyKi env ty@(HsForAllTy { hst_tele = tele, hst_body = tau })
  = do { checkPolyKinds env ty
       ; bindLHsForAllTelescope (rtke_ctxt env) (Just $ inTypeDoc ty)
                                tele $ \ tele' ->
...
bindLHsForAllTelescope :: HsDocContext
                       -> Maybe SDoc -- Just d => check for unused tvs
                                     --   d is a phrase like "in the type ..."
                       -> LHsForAllTelescope GhcPs
                       -> (LHsForAllTelescope GhcRn -> RnM (a, FreeVars))
                       -> RnM (a, FreeVars)
bindLHsForAllTelescope doc mb_in_doc tele thing_inside =
  case tele of
    L l (HsForAllVis { hsf_vis_bndrs = bndrs }) ->
      bindLHsTyVarBndrs doc mb_in_doc Nothing bndrs $ \bndrs' ->
        thing_inside $ L l $ mkHsForAllVisTele bndrs'
    L l (HsForAllInvis { hsf_invis_bndrs = bndrs }) ->
      bindLHsTyVarBndrs doc mb_in_doc Nothing bndrs $ \bndrs' ->
        thing_inside $ L l $ mkHsForAllInvisTele bndrs'
...
extract_hs_for_all_telescope :: LHsForAllTelescope GhcPs
                             -> FreeKiTyVarsWithDups -- Accumulator
                             -> FreeKiTyVarsWithDups -- Free in body
                             -> FreeKiTyVarsWithDups
extract_hs_for_all_telescope (L _ tele) acc_vars body_fvs =
  case tele of
