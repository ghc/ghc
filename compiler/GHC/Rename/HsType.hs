...
module GHC.Rename.HsType (
        ...
        extractConGADTBodyTyVars, extractDataDefnKindVars,
...
extractConGADTBodyTyVars :: ConGADTBody GhcPs -> FreeKiTyVars
extractConGADTBodyTyVars body = case body of
  PrefixConGADT body'       -> extract_prefix_con_gadt_body body'
  RecConGADT (L _ flds) res -> extract_ltys (map (cd_fld_type . unLoc) flds) $
                               extractHsTyRdrTyVars res

extract_prefix_con_gadt_body :: PrefixConGADTBody GhcPs -> FreeKiTyVars
extract_prefix_con_gadt_body prefix_body = go prefix_body []
  where
    go :: PrefixConGADTBody GhcPs -> FreeKiTyVars -> FreeKiTyVars
    go (PCGResult res_ty)    acc = extract_lty res_ty acc
    go (PCGAnonArg arg body) acc = extract_scaled_lty arg (go body acc)
