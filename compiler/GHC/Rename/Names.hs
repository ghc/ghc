  ...
  = setSrcSpan loc $ do
    ...
    case imp_details of
        ...
        _  | implicit   -> return () -- Do not bleat for implicit imports
           ...
           | otherwise  -> whenWOptM Opt_WarnMissingImportList $
                           addWarn (WarnReason Opt_WarnMissingImportList)
    ...
    whenWOptM Opt_WarnWarningsDeprecations (
       case (mi_warns iface) of
          WarnAll txt -> addWarn (WarnReason Opt_WarnWarningsDeprecations)
...
warnUnqualifiedImport decl iface =
    ...
    $ when bad_import
    $ addWarnAt (WarnReason Opt_WarnCompatUnqualifiedImports) loc warning
  where
    mod = mi_module iface
    ...
    lookup_lie (L loc ieRdr)
        ...
        where
            ...
            emit_warning (DodgyImport n) = whenWOptM Opt_WarnDodgyImports $
              addWarn (WarnReason Opt_WarnDodgyImports) (dodgyImportWarn n)
            emit_warning MissingImportList = whenWOptM Opt_WarnMissingImportList $
              addWarn (WarnReason Opt_WarnMissingImportList) (missingImportListItem ieRdr)
            emit_warning (BadImportW ie) = whenWOptM Opt_WarnDodgyImports $
              addWarn (WarnReason Opt_WarnDodgyImports) (lookup_err_msg (BadImport ie))
                ...
                where
                  ...
                  add_warn name msg
                    = when (name `elemNameSet` sig_ns && export_check name)
                           (addWarnAt (WarnReason flag) (getSrcSpan name) msg)
...
warnUnusedImport flag fld_env (L loc decl, used, unused)
  ...
  | null used
  = addWarnAt (WarnReason flag) loc msg1
  ...
  = addWarnAt (WarnReason flag) loc msg2
