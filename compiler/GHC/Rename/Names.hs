  ...
  = setSrcSpan loc $ do
    ...
    case imp_details of
        ...
        _  | implicit   -> return () -- Do not bleat for implicit imports
           ...
           | otherwise  -> whenWOptM Opt_WarnMissingImportList $
                           addDiagnostic (WarnReasonWithFlag Opt_WarnMissingImportList)
    ...
    whenWOptM Opt_WarnWarningsDeprecations (
       case (mi_warns iface) of
          WarnAll txt -> addDiagnostic (WarnReasonWithFlag Opt_WarnWarningsDeprecations)
...
warnUnqualifiedImport decl iface =
    ...
    $ when bad_import
    $ addDiagnosticAt (WarnReasonWithFlag Opt_WarnCompatUnqualifiedImports) loc warning
  where
    mod = mi_module iface
    ...
    lookup_lie (L loc ieRdr)
        ...
        where
            ...
            emit_warning (DodgyImport n) = whenWOptM Opt_WarnDodgyImports $
              addDiagnostic (WarnReasonWithFlag Opt_WarnDodgyImports) (dodgyImportWarn n)
            emit_warning MissingImportList = whenWOptM Opt_WarnMissingImportList $
              addDiagnostic (WarnReasonWithFlag Opt_WarnMissingImportList) (missingImportListItem ieRdr)
            emit_warning (BadImportW ie) = whenWOptM Opt_WarnDodgyImports $
              addDiagnostic (WarnReasonWithFlag Opt_WarnDodgyImports) (lookup_err_msg (BadImport ie))
                ...
                where
                  ...
                  add_warn name msg
                    = when (name `elemNameSet` sig_ns && export_check name)
                           (addDiagnosticAt (WarnReasonWithFlag flag) (getSrcSpan name) msg)
...
warnUnusedImport flag fld_env (L loc decl, used, unused)
  ...
  | null used
  = addDiagnosticAt (WarnReasonWithFlag flag) loc msg1
  ...
  = addDiagnosticAt (WarnReasonWithFlag flag) loc msg2
