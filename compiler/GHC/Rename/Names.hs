...
warnMissingKindSignatures gbl_env
  = do { warn_missing_kind_sigs  <- woptM Opt_WarnMissingKindSignatures
       ; when (warn_missing_kind_sigs) (mapM_ add_ty_warn tcs)
       }
  where
    tcs = tcg_tcs gbl_env
    ksig_ns = tcg_ksigs gbl_env

    add_ty_warn :: TyCon -> IOEnv (Env TcGblEnv TcLclEnv) ()
    add_ty_warn tyCon = when (name `elemNameSet` ksig_ns) $
        addWarnAt (Reason Opt_WarnMissingKindSignatures) (getSrcSpan name) $
            hang (text "Top-level tycon with no kind signature:")
               2 (text "type" <+> pprPrefixName name <+> dcolon <+> ki_msg)
      where
