...
getMinimalImports = fmap combine . mapM mk_minimal
  where
    ...
    combine  =  mapMaybe merge . groupBy ((==) `on` getKey) . sortOn getKey
    ...
    merge :: [LImportDecl GhcRn] -> Maybe (LImportDecl GhcRn)
    merge []                   = Nothing
    merge decls@((L l decl):_) = Just $ L l (decl { ideclHiding = Just (False, L l lies) })
      where
        lies = concatMap (unLoc . snd) $ mapMaybe (ideclHiding . unLoc) decls
