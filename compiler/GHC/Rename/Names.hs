...
rnImports imports = do
    ...
    -- Update imp_source_mods if imp_direct_mods mentions any of them
  ...
  where
    clobberSourceImports imp_avails =
      imp_avails { imp_source_mods = imp_source_mods' }
      where
        imp_source_mods' = mergeUFM combJ id (const mempty)
                            (imp_source_mods imp_avails)
...
calculateAvails home_unit iface mod_safe' want_boot imported_by =
  let imp_mod    = mi_module iface
      ...
      dep_source_mods_map = mkModDeps (dep_source_mods deps)

      source_mods
        -- If we are looking for a boot module, it must be HPT
        | IsBoot <- want_boot = addToUFM dep_source_mods_map (moduleName imp_mod) (GWIB (moduleName imp_mod) IsBoot)
        -- Now we are importing A properly, so don't go looking for
        -- A.hs-boot
        | isHomeUnit home_unit pkg = dep_source_mods_map
