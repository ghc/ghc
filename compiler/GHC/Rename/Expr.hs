...
rnStmt ctxt rnBody (L loc (BindStmt _ pat body)) thing_inside
  = do  { (body', fv_expr) <- rnBody body
        ...
        { (thing, fvs3) <- thing_inside (collectDefaultPatBinders pat')
...
rnRecStmtsAndThen ctxt rnBody s cont
  = do  { -- (A) Make the mini fixity env for all of the stmts
        ...
        ; let bound_names = collectDefaultLStmtsBinders (map fst new_lhs_and_fv)
...
rn_rec_stmts_lhs fix_env stmts
  = do { ls <- concatMapM (rn_rec_stmt_lhs fix_env) stmts
       ; let boundNames = collectDefaultLStmtsBinders (map fst ls)
...
rn_rec_stmt ctxt rnBody _ (L loc (BindStmt _ pat' body), fv_pat)
  = do { (body', fv_expr) <- rnBody body
       ...
       ; let bndrs = mkNameSet (collectDefaultPatBinders pat')
 ...
 where
   ...
   stmtTreeArg ctxt tail_fvs tree = do
     let stmts = flattenStmtTree tree
         pvarset = mkNameSet (concatMap (collectDefaultStmtBinders.unLoc.fst) stmts)
...
segments stmts = map fst $ merge $ reverse $ map reverse $ walk (reverse stmts)
  where
    allvars = mkNameSet (concatMap (collectDefaultStmtBinders.unLoc.fst) stmts)
    ...
    stmtRefs stmt fvs
      ...
      where fvs' = fvs `intersectNameSet` allvars
            pvars = mkNameSet (collectDefaultStmtBinders (unLoc stmt))
 ...
 where
  ...
  go lets indep bndrs ((L loc (BindStmt xbs pat body), fvs): rest)
    ...
    = go lets ((L loc (BindStmt xbs pat body), fvs) : indep)
         bndrs' rest
    where bndrs' = bndrs `unionNameSet` mkNameSet (collectDefaultPatBinders pat)
