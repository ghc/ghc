...
because of a refutable pattern, in order for the types to work out.

-}

isRefutablePattern :: LPat (GhcPass p) -> Bool
isRefutablePattern lpat =
  case unLoc lpat of
    WildPat{}       -> False
    VarPat{}        -> False
    LazyPat{}       -> False
    AsPat _ _ p     -> isRefutablePattern p
    ParPat _ p      -> isRefutablePattern p
    ViewPat _ _ p   -> isRefutablePattern p
    SigPat _ p _    -> isRefutablePattern p
    BangPat _ p     -> isRefutablePattern p
    ListPat{}       -> True
    TuplePat _ ps _ -> any isRefutablePattern ps
    SumPat{}        -> True
    ConPatIn{}      -> True
    ConPatOut{}     -> True
    LitPat{}        -> True
