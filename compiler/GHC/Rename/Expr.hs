...
* It's a bit painful to transform `OpApp e1 op e2` to a
  `HsExpansion` form, because the renamer does precedence
...
* leftSection and rightSection must be linearity-polymorphic.
...
* leftSection is just ($) really; but unlike ($) it is
  levity polymorphic, so we can write `(x +#)`, say.
