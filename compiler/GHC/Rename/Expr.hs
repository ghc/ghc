...
mkSetField set_field a (L _ (FieldLabelString field)) b =
  genHsApp (genHsApp (genHsVar set_field `genAppType` genHsTyLit (mkFastStringTextUTF8 field)) b) a
...
mkGet get_field l@(r :| _) (L _ (FieldLabelString field)) =
  wrapGenSpan (genHsApp (genHsVar get_field `genAppType` genHsTyLit (mkFastStringTextUTF8 field)) r) NE.<| l
...
mkProjection getFieldName circName (field :| fields) = foldl' f (proj field) fields
  where
    ...
    proj (FieldLabelString f) = genHsVar getFieldName `genAppType` genHsTyLit (mkFastStringTextUTF8 f)
