...
mkGetField get_field arg field =
  HsApp noExtField (mkGetFieldExpr get_field field) arg
...
mkProjection getFieldName circName (field :| fields) =
  unLoc $ foldl' f (mkGetFieldExpr getFieldName field) fields
  where
    f :: LHsExpr GhcRn -> XRec GhcRn FieldLabelString -> LHsExpr GhcRn
    f acc field = wrapGenSpan $ genHsApps circName [mkGetFieldExpr getFieldName field, acc]

mkGetFieldExpr :: Name -> XRec GhcRn FieldLabelString -> LHsExpr GhcRn
mkGetFieldExpr getFieldName (L ann (FieldLabelString f)) =
