...
genOpApp :: HsExpr GhcRn -> HsExpr GhcRn -> HsExpr GhcRn -> HsExpr GhcRn
genOpApp x op = OpApp (Fixity NoSourceText minPrecedence InfixL) (wrapGenSpan x) (wrapGenSpan op) . wrapGenSpan
...
mkProjection getFieldName circName (field : fields) = foldl' f (proj field) fields
  where
    ...
    f acc field = genOpApp (proj field) (genHsVar circName) acc
