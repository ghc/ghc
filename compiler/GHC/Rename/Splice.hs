...
mkQuasiQuoteExpr flavour quoter q_span' quote
  = L q_span $ HsApp noComments (L q_span
             $ HsApp noComments (L q_span
                    (HsVar noExtField (N q_span quote_selector)))
                                quoterExpr)
                    quoteExpr
  where
    q_span = noAnnSrcSpan q_span'
    quoterExpr = L q_span $! HsVar noExtField $! (N q_span quoter)
...
rnSplice (HsTypedSplice x hasParen splice_name expr)
  = do  { loc  <- getSrcSpanM
        ; n' <- newLocalBndrRn (N (noAnnSrcSpan loc) splice_name)
...
rnSplice (HsQuasiQuote x splice_name quoter q_loc quote)
  = do  { loc  <- getSrcSpanM
        ; splice_name' <- newLocalBndrRn (N (noAnnSrcSpan loc) splice_name)
