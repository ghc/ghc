...
nameSpacesRelated what_looking ns ns'
  = ns' `elem` ns : [ons | ons <- other_namespaces, WL_Anything <- [what_looking]]
  where
    other_namespaces
      | isVarNameSpace ns     = [dataName]
      | isDataConNameSpace ns = [varName]
      | isTvNameSpace ns      = [tcClsName]
      | isTcClsNameSpace ns   = [tvName]
