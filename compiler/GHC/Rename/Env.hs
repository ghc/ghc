...
module GHC.Rename.Env (
        ...
        lookupIfThenElse, lookupReboundOrQualName,
...
lookupReboundOrQualName :: Module -> FastString -> RnM Name
lookupReboundOrQualName mod ns = do
  mName <- lookupRebound ns
  case mName of
    Just (L _ name) -> pure name -- Rebindable on; name is the one in scope.
    Nothing -> -- Rebindable off; use the name from mod.
      lookupOccRn (mkOrig mod (mkVarOccFS ns)) -- Errors out if the name isn't there.
  where
