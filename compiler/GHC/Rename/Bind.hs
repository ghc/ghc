...
findDupSigs sigs
  ...
  where
    ...
    mtch (InlineSig _ _ prag1)        (InlineSig _ _ prag2)
      -- If any of the two prags is INLINEABLE we leave it to `addInlinePrags` to
      -- figure out if it's a valid combination.
      |  inlineable1 || inlineable2                = False
      | otherwise                                  = True
      where
        inlineable1 = isInlinablePragma prag1
        inlineable2 = isInlinablePragma prag2
