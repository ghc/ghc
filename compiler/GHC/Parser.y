...
rhs     :: { Located (GRHSs GhcPs (LHsExpr GhcPs)) }
        : '=' exp wherebinds    {% runPV (unECP $2) >>= \ $2 ->
                                  do { let loc = (comb3 $1 (reLoc $2) (adaptWhereBinds $3))
                                     ; acs (\cs ->
                                       sL loc (GRHSs NoExtField (unguardedRHS (ApiAnn (Anchor $ rs loc) (GrhsAnn Nothing (mj AnnEqual $1)) cs) loc $2)
...
tup_exprs :: { forall b. DisambECP b => PV (SumOrTuple b) }
           ...
           | commas tup_tail
                 { $2 >>= \ $2 ->
                   do { let {cos = map (\ll -> (Left (ApiAnn (Anchor $ rs ll) (rs ll) []))) (fst $1) }
                      ; return (Tuple (cos ++ $2)) } }
...
commas_tup_tail : commas tup_tail
        { $2 >>= \ $2 ->
          do { let {cos = map (\l -> (Left (ApiAnn (Anchor $ rs l) (rs l) []))) (tail $ fst $1) }
...
transformqual :: { Located (RealSrcSpan -> [LStmt GhcPs (LHsExpr GhcPs)] -> Stmt GhcPs (LHsExpr GhcPs)) }
    ...
    : 'then' exp              {% runPV (unECP $2) >>= \ $2 ->
                                 acs (\cs->
                                 sLLlA $1 $> (\r ss -> (mkTransformStmt (ApiAnn (Anchor r) [mj AnnThen $1] cs) ss $2))) }
    | 'then' exp 'by' exp     {% runPV (unECP $2) >>= \ $2 ->
                                 runPV (unECP $4) >>= \ $4 ->
                                 acs (\cs -> sLLlA $1 $> (
                                                     \r ss -> (mkTransformByStmt (ApiAnn (Anchor r) [mj AnnThen $1,mj AnnBy $3] cs) ss $2 $4))) }
    | 'then' 'group' 'using' exp
            {% runPV (unECP $4) >>= \ $4 ->
               acs (\cs -> sLLlA $1 $> (
                                   \r ss -> (mkGroupUsingStmt (ApiAnn (Anchor r) [mj AnnThen $1,mj AnnGroup $2,mj AnnUsing $3] cs) ss $4))) }

    | 'then' 'group' 'by' exp 'using' exp
            {% runPV (unECP $4) >>= \ $4 ->
               runPV (unECP $6) >>= \ $6 ->
               acs (\cs -> sLLlA $1 $> (
                                   \r ss -> (mkGroupByUsingStmt (ApiAnn (Anchor r) [mj AnnThen $1,mj AnnGroup $2,mj AnnBy $3,mj AnnUsing $5] cs) ss $4 $6))) }
