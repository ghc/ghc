        ...
        : 'pattern' pattern_synonym_lhs '=' pat
         {%      let (name, args,as ) = $2 in
                 acsA (\cs -> sLL $1 (reLoc $>) . ValD noExtField $ mkPatSynBind name args $4
                      ...
                      (ApiAnn (glR $1) (AnnsModule (as ++ [mj AnnPattern $1, mj AnnEqual $3]) (AnnList Nothing Nothing [] [])) cs)) }

        | 'pattern' pattern_synonym_lhs '<-' pat
         {%    let (name, args, as) = $2 in
               acsA (\cs -> sLL $1 (reLoc $>) . ValD noExtField $ mkPatSynBind name args $4 Unidirectional
                       (ApiAnn (glR $1) (AnnsModule (as ++ [mj AnnPattern $1,mu AnnLarrow $3]) (AnnList Nothing Nothing [] [])) cs)) }
            ...
            {% do { let (name, args, as) = $2
                  ...
                  ; acsA (\cs -> sLL $1 $> . ValD noExtField $
                           mkPatSynBind name args $4 (ExplicitBidirectional mg)
                            (ApiAnn (glR $1) (AnnsModule (as ++ [mj AnnPattern $1,mu AnnLarrow $3]) (fst $ unLoc $5)) cs))
                   }}
...
where_decls :: { Located (AnnList
                         , Located (OrdList (LHsDecl GhcPs))) }
        : 'where' '{' decls '}'       { sLL $1 $> (AnnList (Just $ moc $2) (Just $ mcc $4) [mj AnnWhere $1]
                                            (fst $ unLoc $3),sL1 $3 (snd $ unLoc $3)) }
        | 'where' vocurly decls close { L (comb2 $1 $3) (AnnList Nothing Nothing [mj AnnWhere $1] (fst $ unLoc $3)
                                          ,sL1 $3 (snd $ unLoc $3)) }
...
aexp    :: { ECP }
        ...
        | 'do' stmtlist              { ECP $
                                        $2 >>= \ $2 ->
                                        mkHsDoPV (comb2 $1 (reLoc $2)) $2
                                               (\cs -> ApiAnn (glR $1) (hsDoAnn $1 AnnDo) cs)}
        | 'mdo' stmtlist            {% runPV $2 >>= \ $2 ->
                                       fmap ecpFromExp $
                                       acsA (\cs -> L (comb2 $1 (reLoc $2))
                                              (mkHsDoAnns MDoExpr $2
                                           (ApiAnn (glR $1) (hsDoAnn $1 AnnMdo) cs))) }
        | 'proc' aexp '->' exp
...
alt     :: { forall b. DisambECP b => PV (LMatch GhcPs (LocatedA b)) }
           : pat alt_rhs  { $2 >>= \ $2 ->
                            acs (\cs -> sLL (reLoc $1) $> (Match { m_ext = (ApiAnn (glAR $1) [] cs)
                                                  , m_ctxt = CaseAlt
                                                  , m_pats = [$1]
                                                  , m_grhss = unLoc $2 }))}
...
alt_rhs :: { forall b. DisambECP b => PV (Located (GRHSs GhcPs (LocatedA b))) }
        : ralt wherebinds           { $1 >>= \alt ->
                                      return $ sLL alt (reLoc $>) (GRHSs noExtField (unLoc alt) $2) }
...
--   let l = comb2A x y
--   amsr (L (SrcSpanAnn (ann la) l) b) bs
...
-- amsrlr :: MonadP m => LocatedL a -> [AddApiAnn] -> m (LocatedL a)
-- amsrlr a@(L l _) bs = do
--   cs2 <- addAnnsAt (locA l) []
--   let ApiAnn a (AnnList o c r) cs = ann l
--   return (reAnnL bs cs a)
...
addTrailingVbarA  la span = addTrailingAnnA la (AddVbarAnn (rs span))
...
addTrailingSemiA  la span = addTrailingAnnA la (AddSemiAnn (rs span))

addTrailingCommaA :: MonadP m => LocatedA a -> SrcSpan -> m (LocatedA a)
addTrailingCommaA  la span = addTrailingAnnA la (AddCommaAnn (rs span))

addTrailingAnnA :: MonadP m => LocatedA a -> TrailingAnn -> m (LocatedA a)
addTrailingAnnA (L (SrcSpanAnn anns l) a) ta = do
  cs <- addAnnsAt l []
  -- AZ:TODO: generalise updating comments into an annotation
  let anns' = addTrailingAnnToA l ta anns
  return (L (SrcSpanAnn anns' l) a)
