...
importdecl :: { LImportDecl GhcPs }
        : 'import' maybe_src maybe_safe optqualified maybe_pkg modid optqualified maybeas maybeimpspec
                {% do {
                  ...
                  ; let loc = (comb5 $1 (reLoc $6) $7 (snd $8) $9);
...
inst_decl :: { LInstDecl GhcPs }
       ...
       {% do { (binds, sigs, _, ats, adts, _) <- cvBindsAndSigs (snd $ unLoc $4)
             ...
             ; let cid cs = ClsInstDecl
                                     { cid_ext = (EpAnn (spanAsAnchor (comb3 $1 $3 $4)) anns cs, NoAnnSortKey)
...
injectivity_cond :: { LInjectivityAnn GhcPs }
        : tyvarid '->' inj_varids
           {% acsI (\cs -> sLL $1 $> (InjectivityAnn (EpAnn (glNR $1) [mu AnnRarrow $2] cs) $1 (reverse (unLoc $3)))) }
...
opt_tyfam_kind_sig :: { Located ([AddEpAnn], LFamilyResultSig GhcPs) }
        :              { noLoc     ([]               , noLocI     (NoSig    noExtField)   )}
        | '::' kind    { sLL $1 $> ([mu AnnDcolon $1], sLLi $1 (reLoc $>) (KindSig  noExtField $2))}
        | '='  tv_bndr {% do { tvb <- fromSpecTyVarBndr $2
                             ; return $ sLL $1 $> ([mj AnnEqual $1], sLLi $1 (reLoc $>) (TyVarSig noExtField tvb))} }
...
stand_alone_deriving :: { LDerivDecl GhcPs }
  : 'deriving' deriv_standalone_strategy 'instance' overlap_pragma inst_type
                {% do { let { err = text "in the stand-alone deriving instance"
                                    <> colon <+> quotes (ppr $5) }
                      ; acsA (\cs -> sLL $1 $>
                                 (DerivDecl (EpAnn (glEE $1 $>) [mj AnnDeriving $1, mj AnnInstance $3] cs) (mkHsWildCardBndrs $5) $2 $4)) }}
...
ctype   :: { LHsType GhcPs }
        ...
        | ipvar '::' ctype            {% acsA (\cs -> sLL $1 $> (HsIParamTy (EpAnn (glEE $1 $>) [mu AnnDcolon $2] cs) (reLocE $1) $3)) }
        | type                        { $1 }
        ...
        | MDO stmtlist             {% hintQualifiedDo $1 >> runPV $2 >>= \ $2 ->
                                       ...
                                       acsA (\cs -> L (comb2 $1 $2)
                                              (mkHsDoAnns (MDoExpr $
                                                          ...
                                                          $2
                                           (EpAnn (glEE $1 $>) (AnnList (Just $ glIR $2) Nothing Nothing [mj AnnMdo $1] []) cs) )) }
        | 'proc' aexp '->' exp
                       {% (checkPattern <=< runPV) (unECP $2) >>= \ p ->
                           runPV (unECP $4) >>= \ $4@cmd ->
                           fmap ecpFromExp $
                           acsA (\cs -> sLL $1 $> $ HsProc (EpAnn (glEE $1 $>) [mj AnnProc $1,mu AnnRarrow $3] cs) p (sLLa $1 (reLoc $>) $ HsCmdTop noExtField cmd)) }
...
projection
        ...
        : projection TIGHT_INFIX_PROJ field
                             {% acs (\cs -> sLL $1 $> ((sLLi $2 (reLocN $>) $ DotFieldOcc (EpAnn (glEE $1 $>) (AnnFieldLabel (Just $ glAA $2)) cs) $3) `NE.cons` unLoc $1)) }
        | PREFIX_PROJ field  {% acs (\cs -> sLL $1 $> ((sLLi $1 (reLocN $>) $ DotFieldOcc (EpAnn (glEE $1 $>) (AnnFieldLabel (Just $ glAA $1)) cs) $2) :| [])) }
...
texp :: { ECP }
        ...
        | infixexp qop
                             {% runPV (unECP $1) >>= \ $1 ->
                                ...
                                runPV $2 >>= \ $2 ->
                                return $ ecpFromExp $
                                reLocA $ sLL $1 $> $ SectionL noAnn $1 (la2la $2) }
        | qopm infixexp      { ECP $
                                superInfixOp $
                                unECP $2 >>= \ $2 ->
                                $1 >>= \ $1 ->
        ...
        | field TIGHT_INFIX_PROJ fieldToUpdate
                        { do
                            let top =  sL1i $1 $ DotFieldOcc noAnn $1
                                ...
                                l = comb2 $1 $3
                                isPun = True
                            var <- mkHsVarPV (L (noAnnSrcSpanN $ getHasLoc final) (mkRdrUnqual . mkVarOccFS . field_label . unLoc . dfoLabel . unLoc $ final))
...
fieldToUpdate
        ...
        : fieldToUpdate TIGHT_INFIX_PROJ field   {% getCommentsFor (getLocA $3) >>= \cs ->
                                                     return (sLL $1 $> ((sLLi $2 $> (DotFieldOcc (EpAnn (glR $2) (AnnFieldLabel $ Just $ glAA $2) cs) $3)) : unLoc $1)) }
        | field       {% getCommentsFor (getHasLoc $1) >>= \cs ->
                        return (sL1 $1 [sL1i $1 (DotFieldOcc (EpAnn (glNR $1) (AnnFieldLabel Nothing) cs) $1)]) }
...
dbind   : ipvar '=' exp                {% runPV (unECP $3) >>= \ $3 ->
                                          acsA (\cs -> sLL $1 $> (IPBind (EpAnn (glEE $1 $>) [mj AnnEqual $2] cs) (reLocE $1) $3)) }
...
sL1i x = sL (noAnnSrcSpanI $ getHasLoc x)   -- #define sL1   sL (getLoc $1)

{-# INLINE sL1n #-}
sL1n :: HasLoc a => a -> b -> LocatedN b
sL1n x = L (noAnnSrcSpanN $ getHasLoc x)   -- #define sL1   sL (getLoc $1)
...
sLLa :: (Monoid t, HasLoc a, HasLoc b) => a -> b -> c -> LocatedAnS t c
