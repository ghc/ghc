...
aexp    :: { ECP }
        ...
        | 'let' binds 'in' exp          {  ECP $
                                           ...
                                           amms (mkHsLetPV (comb2 $1 $>) (snd (unLoc $2)) $4)
                                               (mj AnnLet $1:mj AnnIn $3
                                                 :(fst $ unLoc $2)) }
        | 'let' binds 'in' error         {% hintParsingErrorWithContext $3 "missing body in 'let/in' clause" "'in' is here" "missingLetBody"}
        | 'let' binds error        {% hintParsingErrorWithContext $1 "missing 'in' in 'let/in' clause" "'let' is here" "missingIn"}
        | '\\' 'lcase' altslist
            {  ECP $ $3 >>= \ $3 ->
        ...
        | 'if' exp optSemi 'then' exp optSemi 'else' error        {% hintParsingErrorWithContext $7 "parse error in 'else' clause" "'else' is here" "parseError"}
        | 'if' exp optSemi 'then' exp optSemi error        {% hintParsingErrorWithContext $4 "missing 'else' clause" "'then' is here" "missingElse"}
        | 'if' exp optSemi 'then' error        {% hintParsingErrorWithContext $4 "parse error in 'then' clause" "'then' is here" "parseError"}
        | 'if' exp optSemi error        {% hintParsingErrorWithContext $1 "missing 'then' clause" "'if' is here" "missingThen"}
        | 'if' error        {% hintParsingErrorWithContext $1 "parse error in 'if' condition" "'if' is here" "parseError"}
                                         ...
                                         return $ ECP $
                                           ...
                                           amms (mkHsCasePV (comb3 $1 $3 $4) $2 (mkMatchGroup
                                               ...
                                               (mj AnnCase $1:mj AnnOf $3
                                                  :(fst $ unLoc $4)) }
        | 'case' exp error  {% do
            hintParsingErrorWithContext $1 "missing 'of' in 'case' clause" "'case' is here" "missingOf"
        }
...
--                                       (getSTRING $1) noExtField) }
        ...
        | '(' texp error                  {% hintParsingErrorWithContext $1 "missing closing brace ')'" "opening brace is here" "unclosedBracedExpression"}
        | '(' tup_exprs ')'             { ECP $
                                           $2 >>= \ $2 ->
                                           amms (mkSumOrTuplePV (comb2 $1 $>) Boxed (snd $2))
                                                ((mop $1:fst $2) ++ [mcp $3]) }

        | '(' tup_exprs error {% hintParsingErrorWithContext $1 "missing closing tuple brace ')'" "opening brace is here" "unclosedTuple"}
        | '(#' texp '#)'                { ECP $
                                           unECP $2 >>= \ $2 ->
                                           amms (mkSumOrTuplePV (comb2 $1 $>) Unboxed (Tuple [L (gl $2) (Just $2)]))
                                                [mo $1,mc $3] }
        | '(#' texp error {% hintParsingErrorWithContext $1 "missing closing unboxed tuple/sum brace '#)'" "opening brace is here" "unclosedUnboxedTupleSum"}
        | '(#' tup_exprs '#)'           { ECP $
                                           $2 >>= \ $2 ->
                                           amms (mkSumOrTuplePV (comb2 $1 $>) Unboxed (snd $2))
                                                ((mo $1:fst $2) ++ [mc $3]) }

        | '(#' tup_exprs error {% hintParsingErrorWithContext $1 "missing closing unboxed tuple brace '#)'" "opening brace is here" "unclosedUnboxedTuple"}
        | '[' list ']'      { ECP $ $2 (comb2 $1 $>) >>= \a -> ams a [mos $1,mcs $3] }
        | '[' list  error   {% hintParsingErrorWithContext $1 "missing closing list bracket ']'" "opening bracket is here" "unclosedList"}
        ...
        | '[|' exp error {% hintParsingErrorWithContext $1 "missing closing TH quote '|]'" "opening quote is here" "unclosedTHQuote" }
        | '[||' exp '||]'     {% runPV (unECP $2) >>= \ $2 ->
                                 fmap ecpFromExp $
                                 ams (sLL $1 $> $ HsBracket noExtField (TExpBr noExtField $2))
                                      (if (hasE $1) then [mj AnnOpenE $1,mc $3] else [mo $1,mc $3]) }
        | '[||' exp error {% hintParsingErrorWithContext $1 "missing typed TH quote '|]]'" "opening quote is here" "unclosedTTHQuote"}
        | '[t|' ktype '|]'    {% fmap ecpFromExp $
                                 ams (sLL $1 $> $ HsBracket noExtField (TypBr noExtField $2)) [mo $1,mu AnnCloseQ $3] }
        | '[t|' ktype error {% hintParsingErrorWithContext $1 "missing closing TH type quote '|]'" "opening quote is here" "unclosedTHTypeQuote"}
        | '[p|' infixexp '|]' {% (checkPattern <=< runPV) (unECP $2) >>= \p ->
                                      fmap ecpFromExp $
                                      ams (sLL $1 $> $ HsBracket noExtField (PatBr noExtField p))
                                          [mo $1,mu AnnCloseQ $3] }
        | '[p|' infixexp error {% hintParsingErrorWithContext $1 "missing closing TH pattern quote '|]'" "opening quote is here" "unclosedTHPatternQuote"}
        | '[d|' cvtopbody '|]' {% fmap ecpFromExp $
                                  ams (sLL $1 $> $ HsBracket noExtField (DecBrL noExtField (snd $2)))
                                      (mo $1:mu AnnCloseQ $3:fst $2) }
        | '[d|' cvtopbody error {% hintParsingErrorWithContext $1 "missing closing TH definition quote '|]'" "opening quote is here" "unclosedTHDefinitionQuote"}
        ...
        | '(|' aexp cmdargs error {% hintParsingErrorWithContext $1 "missing closing arrow notation '|)'" "opening arrow notation is here" "unclosedArrowNotation"}
...
hintParsingErrorWithContext :: Located Token -> String -> String -> String -> P ECP
hintParsingErrorWithContext tokOpen messageError messageHint varName = do
    loc <- getRealSrcLoc

    -- Note: The error message location is on the opening token of the failing expression
    addError $ PsError (PsErrParseErrorWithContext (text messageHint)) [] (RealSrcSpan (realSrcLocSpan loc) Nothing)
    addError $ PsError (PsErrParseErrorWithContext (text messageError)) [] (getLoc tokOpen)
