...
maybemodwarning :: { Maybe (LocatedP WarningTxt) }
    : '{-# DEPRECATED' strings '#-}'
                      {% fmap Just $ amsrp (sLL $1 $> $ DeprecatedTxt (getDEPRECATED_PRAGs $1) (snd $ unLoc $2))
                              (AnnPragma (mo $1) (mc $3) (fst $ unLoc $2)) }
    | '{-# WARNING' strings '#-}'
                         {% fmap Just $ amsrp (sLL $1 $> $ WarningTxt (getWARNING_PRAGs $1) (snd $ unLoc $2))
                                 (AnnPragma (mo $1) (mc $3) (fst $ unLoc $2))}
    |  {- empty -}                  { Nothing }
...
tycl_hdr_inst :: { Located ([AddApiAnn],(Maybe (LHsContext GhcPs), Maybe [LHsTyVarBndr () GhcPs], LHsType GhcPs)) }
        : 'forall' tv_bndrs '.' context '=>' type   {% hintExplicitForall $1
                                                       >> fromSpecTyVarBndrs $2
                                                         >>= \tvbs ->
                                                             (acs (\cs -> (sLL $1 (reLoc $>) ([mu AnnForall $1, mj AnnDot $3]
                                                                                  , (Just (addTrailingDarrowC $4 $5 cs), Just tvbs, $6)))))
                                                    }
        ...
        | context '=>' type         {% acs (\cs -> (sLLAA $1 $>([], (Just (addTrailingDarrowC $1 $2 cs), Nothing, $3)))) }
        ...
        : namelist strings
                {% fmap unitOL $ acsA (\cs -> sLL $1 $>
                     (Warning (ApiAnn (glR $1) (fst $ unLoc $2) cs) (unLoc $1)
                              (WarningTxt NoSourceText $ snd $ unLoc $2))) }
...
deprecation :: { OrdList (LWarnDecl GhcPs) }
        : namelist strings
             {% fmap unitOL $ acsA (\cs -> sLL $1 $> $ (Warning (ApiAnn (glR $1) (fst $ unLoc $2) cs) (unLoc $1)
                                          (DeprecatedTxt NoSourceText $ snd $ unLoc $2))) }
...
type :: { LHsType GhcPs }
        ...
        | btype mult '->' ctype        {% hintLinear (getLoc $2) >>
                                          acsA (\cs -> sLL (reLoc $1) (reLoc $>)
                                           $ HsFunTy (ApiAnn (glAR $1) (mau $3) cs) ((unLoc $2) (toUnicode $3)) $1 $4) }

        | btype '->.' ctype            {% hintLinear (getLoc $2) >>
                                          acsA (\cs -> sLL (reLoc $1) (reLoc $>)
                                            $ HsFunTy (ApiAnn (glAR $1) (mau $2) cs) (HsLinearArrow UnicodeSyntax) $1 $3) }
                                              -- [mu AnnLollyU $2] }

mult :: { Located (IsUnicodeSyntax -> HsArrow GhcPs) }
        : PREFIX_PERCENT atype          { sLL $1 (reLoc $>) (\u -> mkMultTy u $2) }
...
deriv_clause_types :: { LDerivClauseTys GhcPs }
        : qtycon              { let { tc = sL1a (reLoc $1) (HsTyVar noAnn NotPromoted $1) } in
                                sL1a (reLoc $1) (DctSingle noExtField (mkLHsSigType tc)) }
