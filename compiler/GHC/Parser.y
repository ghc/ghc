...
%token
 ...
 '{-# DYSFUNCTIONAL'      { L _ (ITdysfunctional_prag _) }
...
inst_decl :: { LInstDecl GhcPs }
        : 'instance' overlap_pragma dysfunctional_pragma inst_type where_inst
       {% do { (binds, sigs, _, ats, adts, _) <- cvBindsAndSigs (snd $ unLoc $5)
             ...
             ; ams (L (comb3 $1 $4 $5) (ClsInstD { cid_d_ext = noExtField, cid_inst = cid }))
...
dysfunctional_pragma :: { Maybe (Located ()) }
  : '{-# DYSFUNCTIONAL' '#-}' {% ajs (sL1 $1 ()) [mo $1, mc $2] }
  | {- empty -}               { Nothing }
