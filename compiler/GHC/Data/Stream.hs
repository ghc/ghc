...
module GHC.Data.Stream (
    ...
    map, mapM, mapAccumL_-- mapAccumL, mapAccumL_
  ) where
...
-- new value.
--
newtype Stream m a b = Stream { runStreamInternal :: forall r' r . (a -> m r') -> (b -> StreamS m r' r) -> StreamS m r' r }
...
collect str = go [] (runStreamInternal str pure Done)
 where
...
consume str l f = go (runStreamInternal str pure Done)
  where
    go (Done r) = return r
...
-- | Note this is not very efficient, avoid using it if you can.
...
mapAccumL_ f c str = Stream $ \f h -> go c f h (runStream str)
  ...
  where
    ...
    go c f1 h1 (Yield a p) = Effect (f c a >>= (\(c', b) -> f1 b >>= \r' -> return $ Yield r' (go c' f1 h1 p)))
