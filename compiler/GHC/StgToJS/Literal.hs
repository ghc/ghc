...
genLit = \case
  ...
  LitLabel (CLabelSpec name ForeignLabelIsFunction _)
                             -> return [ ApplExpr hdMkFunctionPtr
                                                  [global (mkRawSymbol True (mkFastStringShortText name))]
                                       , ValExpr (JInt 0)
                                       ]
  LitLabel (CLabelSpec name ForeignLabelIsData _)
                             -> return [ toJExpr (global (mkRawSymbol True (mkFastStringShortText name)))
                                       , ValExpr (JInt 0)
                                       ]
  LitRubbish _ rr_ty ->
...
genStaticLit = \case
  ...
  LitLabel (CLabelSpec name fod _)
                  -> return [ LabelLit (fod == ForeignLabelIsFunction)
                                       (mkRawSymbol True (mkFastStringShortText name))
                            , IntLit 0 ]
  LitRubbish _ rep ->
