...
data Dependencies = Deps
   ...
   , dep_source_mods :: [ModuleName]
      -- ^ Transitive closure of hs-boot files below this one which are
      -- visible from the module. If both A and A[boot] are visible then
      -- A will not appear in this list. This list is just for things we
      -- want to load the boot interface file for. This information is only
      -- used to populate the eps_is_boot field.
