...
data ModuleGraph = ModuleGraph
  ...
  , mg_zero_graph :: (ReachabilityIndex ZeroSummaryNode, Either (ModNodeKeyWithUid, ImportLevel) UnitId -> Maybe ZeroSummaryNode)
...
mgQueryZero :: ModuleGraph
            -> Either (ModNodeKeyWithUid, ImportLevel) UnitId
            -> Either (ModNodeKeyWithUid, ImportLevel) UnitId
            -> Bool
...
mgQueryManyZero :: ModuleGraph -- ^ @g@
            -> [Either (ModNodeKeyWithUid, ImportLevel) UnitId] -- ^ @roots@
            -> Either (ModNodeKeyWithUid, ImportLevel) UnitId -- ^ @b@
            -> Bool -- ^ @b@ is reachable from @roots@
...
mkTransZeroDeps :: [ModuleGraphNode] -> (ReachabilityIndex ZeroSummaryNode, (Either (ModNodeKeyWithUid, ImportLevel) UnitId) -> Maybe ZeroSummaryNode)
...
type ZeroSummaryNode = Node Int (Either (ModNodeKeyWithUid, ImportLevel) UnitId)

zeroSummaryNodeKey :: ZeroSummaryNode -> Int
zeroSummaryNodeKey = node_key

zeroSummaryNodeSummary :: ZeroSummaryNode -> Either (ModNodeKeyWithUid, ImportLevel) UnitId
...
moduleGraphNodesZero ::
     [ModuleGraphNode]
  -> (Graph ZeroSummaryNode, Either (ModNodeKeyWithUid, ImportLevel) UnitId -> Maybe ZeroSummaryNode)
moduleGraphNodesZero summaries =
  ...
  where
    ...
    (nodes) = mapMaybe go numbered_summaries
      ...
      where
        ...
        go (s, key) = normal_case s
          where
           normal_case :: (ModuleGraphNode, ImportLevel)  -> Maybe ZeroSummaryNode
           normal_case (((ModuleNode nks ms), s)) = Just $
                  DigraphNode (Left (mnKey ms, s)) key $ out_edge_keys $
    ...
    classifyDeps s (ModuleNodeEdge il (NodeKey_Module k)) | s == il = Just (Left k)
    classifyDeps s (ModuleNodeEdge il (NodeKey_ExternalUnit u)) | s == il = Just (Right (u))
    ...
    lookup_node :: Either (ModNodeKeyWithUid, ImportLevel) UnitId -> Maybe ZeroSummaryNode
    lookup_node key = Map.lookup key node_map

    lookup_key :: Either (ModNodeKeyWithUid, ImportLevel) UnitId -> Maybe Int
    lookup_key = fmap zeroSummaryNodeKey . lookup_node

    node_map :: Map.Map (Either (ModNodeKeyWithUid, ImportLevel) UnitId) ZeroSummaryNode
    node_map =
      Map.fromList [ (s, node)
                   | node <- nodes
    ...
    out_edge_keys = mapMaybe lookup_key . map (bimap (, NormalLevel) id)
        -- If we want keep_hi_boot_nodes, then we do lookup_key with
        -- IsBoot; else False
