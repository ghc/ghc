...
import GHC.Types.SourceFile ( hscSourceString, isHsigFile, HscSource(..), hscSourceToIsBoot )
...
data ModuleNodeType = MN_Fixed | MN_Compile | MN_Other
...
moduleNodeType (ModuleNode _ (ModuleNodeFixed _ _ _)) = MN_Fixed
moduleNodeType _ = MN_Other
...
-}
data ModuleNodeInfo = ModuleNodeFixed ModNodeKeyWithUid HscSource ModLocation
                    | ModuleNodeCompile ModSummary

-- | Extract the Module from a ModuleNodeInfo
moduleNodeInfoModule :: ModuleNodeInfo -> Module
moduleNodeInfoModule (ModuleNodeFixed key _ _) =
  mkModule (RealUnit $ Definite $ mnkUnitId key) (gwib_mod $ mnkModuleName key)
...
moduleNodeInfoModNodeKeyWithUid (ModuleNodeFixed key _ _) = key
moduleNodeInfoModNodeKeyWithUid (ModuleNodeCompile ms) = msKey ms

-- | Extract the HscSource from a ModuleNodeInfo
moduleNodeInfoHscSource :: ModuleNodeInfo -> HscSource
moduleNodeInfoHscSource (ModuleNodeFixed _ src _) = src
moduleNodeInfoHscSource (ModuleNodeCompile ms) = ms_hsc_src ms

-- | Extract the ModLocation from a ModuleNodeInfo
moduleNodeInfoLocation :: ModuleNodeInfo -> ModLocation
moduleNodeInfoLocation (ModuleNodeFixed _ _ loc) = loc
...
isBootModuleNodeInfo (ModuleNodeFixed _ src _) = hscSourceToIsBoot src
...
mapMG f mg@ModuleGraph{..} = mg
  { mg_mss = flip fmap mg_mss $ \case
      ...
      ModuleNode deps (ModuleNodeFixed key src loc)  -> ModuleNode deps (ModuleNodeFixed key src loc)
...
mnKey (ModuleNodeFixed key _ _) = key
...
showModMsg dflags recomp (ModuleNode _ (ModuleNodeCompile mod_summary)) =
...
showModMsg dflags _ (ModuleNode _ mn@(ModuleNodeFixed {})) =
  if gopt Opt_HideSourcePaths dflags
      then text mod_str
      else hsep $
         [ text (mod_str ++ replicate (max 0 (16 - length mod_str)) ' ')
         , char '('
         , text (op $ ml_hi_file (moduleNodeInfoLocation mn)) <> char ','
         , message, char ')' ]
  where
    op       = normalise
    mod_str  = moduleNameString (moduleNodeInfoModuleName mn) ++
               hscSourceString (moduleNodeInfoHscSource mn)
    message = text "fixed"
