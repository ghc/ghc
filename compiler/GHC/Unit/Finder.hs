...
flushFinderCaches hsc_env =
 ...
 where
        ...
        home_units   = hsc_allHomeUnits hsc_env
...
findHomeModuleInUnit hsc_env mod_name _mb_pkg = do
  foldl' orIfNotFound (pure notFound)
    ...
    $ (current_home_unit : deps)
  where
    currentUnit = hsc_currentUnit hsc_env
    deps = map internalUnitEnv_home_unit $ hsc_homeUnitDependencies_unitEnv hsc_env currentUnit
    notFound = NotFound [] Nothing [] [] [] []
    current_home_unit = hsc_home_unit hsc_env
