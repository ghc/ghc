...
import GHC.Driver.Env (HscEnv(..), hsc_home_unit, hsc_dflags)
...
findImportedModule hsc_env mod fs =
  ...
  in do
    findImportedModuleNoHsc fc fopts (hsc_unit_env hsc_env) home_unit mod fs
...
findImportedModuleNoHsc
  ...
  -> PkgQual
  -> IO FindResult
findImportedModuleNoHsc fc fopts ue home_unit mod_name mb_pkg =
  case mb_pkg of
    NoPkgQual  -> unqual_import
    ThisPkg uid | homeUnitId home_unit == uid -> home_import
                | Just os <- lookup uid other_fopts -> home_pkg_import (uid, os)
                | otherwise -> pprPanic "findImportModule" (ppr mod_name $$ ppr mb_pkg $$ ppr (homeUnitId home_unit) $$ ppr uid $$ ppr (map fst all_opts))
    OtherPkg _ -> pkg_import
  where
    all_opts = ((homeUnitId home_unit, fopts): other_fopts)
    ...
    home_pkg_import (uid, opts)
      ...
      | mod_name `Set.member` finder_reexportedModules opts =
        findImportedModuleNoHsc fc opts ue (DefiniteHomeUnit uid Nothing) mod_name NoPkgQual
    ...
    units     = homeUnitEnv_units $ ue_findHomeUnitEnv (homeUnitId home_unit) ue
    hpt_deps :: [UnitId]
...
findExactModule :: FinderCache -> FinderOpts ->  UnitEnvGraph FinderOpts -> UnitState -> HomeUnit -> InstalledModule -> IO InstalledFindResult
