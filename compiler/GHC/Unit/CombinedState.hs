...
lookupSimpleIface (CombinedState eps hug) mod =
  HUG.lookupHugByModule mod hug >>= \case
    Just hm -> pure $! Just $! hm_simple_iface hm
...
lookupDependencies (CombinedState eps hug) mod =
  HUG.lookupHugByModule mod hug >>= \case
    Just hm -> pure $! Just $! mi_simple_info_deps (hm_simple_iface hm)
...
lookupIfaceHash (CombinedState eps hug) mod =
  HUG.lookupHugByModule mod hug >>= \case
    Just hm -> pure $! Just $! mi_simple_info_iface_hash (hm_simple_iface hm)
...
lookupSafeHaskell (CombinedState eps hug) mod =
  HUG.lookupHugByModule mod hug >>= \case
    Just hm -> pure $! Just $! (mi_simple_trust_info (mi_simple_info_public (hm_simple_iface hm)), mi_simple_trust_pkg (mi_simple_info_public (hm_simple_iface hm)))
...
lookupHashFunction (CombinedState _eps hug) mod =
  HUG.lookupHugByModule mod hug >>= \case
    Just hm -> pure $! Just $! mi_cache_hash_fn (mi_simple_caches (mi_simple_info_public (hm_simple_iface hm)))
...
lookupFreeHoles (CombinedState eps hug) mod =
  HUG.lookupHugByModule mod hug >>= \case
    Just hm -> pure $! Just $! mi_simple_free_holes (hm_simple_iface hm)
