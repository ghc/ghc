...
pprUnitInfo GenericUnitInfo {..} =
    vcat [
      ...
      field "import-dirs"          (fsep (map stext unitImportDirs)),
      field "library-dirs"         (fsep (map stext unitLibraryDirs)),
      field "dynamic-library-dirs" (fsep (map stext unitLibraryDynDirs)),
      field "bytecode-library-dirs" (fsep (map stext unitLibraryBytecodeDirs)),
      field "hs-libraries"         (fsep (map stext unitLibraries)),
      field "extra-libraries"      (fsep (map stext unitExtDepLibsSys)),
      field "extra-ghci-libraries" (fsep (map stext unitExtDepLibsGhc)),
      field "include-dirs"         (fsep (map stext unitIncludeDirs)),
      field "includes"             (fsep (map stext unitIncludes)),
      field "depends"              (fsep (map ppr  unitDepends)),
      field "cc-options"           (fsep (map stext unitCcOptions)),
      field "ld-options"           (fsep (map stext unitLinkerOptions)),
      field "framework-dirs"       (fsep (map stext unitExtDepFrameworkDirs)),
      field "frameworks"           (fsep (map stext unitExtDepFrameworks)),
      field "haddock-interfaces"   (fsep (map stext unitHaddockInterfaces)),
      field "haddock-html"         (fsep (map stext unitHaddockHTMLs))
    ]
  where
