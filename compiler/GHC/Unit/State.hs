...
mungeUnitInfo top_dir pkgroot =
    mungeDynLibFields
  . mungeUnitInfoPaths (BS8.pack top_dir) (BS8.pack pkgroot)
...
collectIncludeDirs ps = map BS8.unpack $ ordNub (filter (not . BS8.null) (concatMap unitIncludeDirs ps))
...
collectLinkOpts dflags ps =
    (
        ...
        concatMap (map ("-l" ++) . map BS8.unpack . unitExtDepLibsSys) ps,
        concatMap (map BS8.unpack . unitLinkerOptions) ps
    )
...
collectArchives dflags pc =
  ...
  where searchPaths = ordNub . filter notNull . libraryDirsForWay (ways dflags) $ pc
        libs        = packageHsLibs dflags pc ++ (map BS8.unpack $ unitExtDepLibsSys pc)
...
packageHsLibs dflags p = map (mkDynName . addSuffix . BS8.unpack) (unitLibraries p)
  where
        ways0 = ways dflags
...
libraryDirsForWay ws ui
  | WayDyn `elem` ws = map BS8.unpack $ unitLibraryDynDirs ui
  | otherwise        = map BS8.unpack $ unitLibraryDirs ui
...
getUnitExtraCcOpts ctx unit_state home_unit pkgs = do
  ...
  return $ map BS8.unpack (concatMap unitCcOptions ps)
...
getUnitFrameworkPath ctx unit_state home_unit pkgs = do
  ps <- getPreloadUnitsAnd ctx unit_state home_unit pkgs
  return $ map BS8.unpack (ordNub (filter (not . BS8.null) (concatMap unitExtDepFrameworkDirs ps)))
...
getUnitFrameworks ctx unit_state home_unit pkgs = do
  ps <- getPreloadUnitsAnd ctx unit_state home_unit pkgs
  return $ map BS8.unpack (concatMap unitExtDepFrameworks ps)
