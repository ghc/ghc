...
updateIndexDefault ref logger cfg raw_dbs other_flags = do
  ...
  atomicModifyIORef' ref $ \ UnitIndexBackend {..} -> let
      updated = UnitIndexBackend {
        -- TODO this favors older entries, which seems sensible because they are
        -- most likely to be caught in retainers?
        moduleNameProviders = mod_map Semigroup.<> moduleNameProviders,
