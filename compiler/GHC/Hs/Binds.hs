...
data HsBindLR idL idR
  = -- | Function-like Binding
    ...
    --
    -- Reason 1: Special case for type inference: see 'GHC.Tc.Binds.tcMonoBinds'.
    --
...
variables.  The action happens in GHC.Tc.Binds.mkExport.
...
a) Dependency analysis prior to type checking
    (see GHC.Tc.Binds.tc_group)

b) Deciding whether we can do generalisation of the binding
    (see GHC.Tc.Binds.decideGeneralisationPlan)

c) Deciding whether the binding can be used in static forms
    (see GHC.Tc.Expr.checkClosedInStaticForm for the HsStatic case and
     GHC.Tc.Binds.isClosedBndrGroup).
