...
-- @
--
-- Note that this function is only used for pretty-printing in error messages
-- (see @GHC.Rename.Module.rnConDecl.split_prefix_gadt_ty@).
...
splitNestedLHsSigmaTysInvis ty
    ...
    -- underneath it.
  | Just (arg_tys, tvs1, theta1, rho1) <- deep_split_lhs_sigma_ty_invis_maybe ty
  ...
  where
    -- The uses of noLoc below are OK, since the main purpose of this function
    -- is for pretty-printing. As a result, the lack of source locations is
    ...
    -- Decompose a function type's arguments, followed by any @forall@s and
    -- contexts that appear after the last function arrow. For example:
    --
    -- @
    -- 'deep_split_lhs_sigma_ty_invis_maybe' (Int -> Char -> forall a. Show a => a -> b)
    --   = ([Int, Char], [a], [Show a], [a -> b])
    -- @
    deep_split_lhs_sigma_ty_invis_maybe ::
         LHsType pass
      -> Maybe ( [LHsType pass], [LHsTyVarBndr Specificity pass]
               , LHsContext pass, LHsType pass )
    deep_split_lhs_sigma_ty_invis_maybe ty
      | Just (arg_ty, res_ty)           <- splitLHsFunTy_maybe ty
      , Just (arg_tys, tvs, theta, rho) <- deep_split_lhs_sigma_ty_invis_maybe res_ty
