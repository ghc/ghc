...
splitHsFunType ::
  ...
  -> ( ([EpToken "("], [EpToken ")"]) , EpAnnComments -- The locations of any parens and
                                  -- comments discarded
     , [HsConFieldSpec GhcPs], LHsType GhcPs)
...
pprHsConFieldSpecWith :: (OutputableBndrId p)
                      ...
                      -> HsConFieldSpec (GhcPass p) -> SDoc
pprHsConFieldSpecWith ppr_mult (CFS _ prag mark mult ty doc) =
  pprMaybeWithDoc doc (ppr_mult mult (ppr prag <+> ppr mark <> ppr ty))

pprHsConFieldSpecNoMult :: (OutputableBndrId p) => HsConFieldSpec (GhcPass p) -> SDoc
pprHsConFieldSpecNoMult = pprHsConFieldSpecWith (\_ d -> d)

hsPlainTypeField :: LHsType GhcPs -> HsConFieldSpec GhcPs
hsPlainTypeField = mkConFieldSpec (HsUnannotated OnConField (EpColon noAnn))

mkConFieldSpec :: HsMultAnnOn (LHsType GhcPs) GhcPs -> LHsType GhcPs -> HsConFieldSpec GhcPs
mkConFieldSpec mult (L _ (HsDocTy _ ty lds)) = (mkConFieldSpec mult ty) { cfs_doc = Just lds }
mkConFieldSpec mult (L _ (XHsType (HsBangTy ann (HsSrcBang srcTxt unp str) t))) = CFS (ann, srcTxt) unp str mult t Nothing
