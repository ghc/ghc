...
module GHC.Hs.Type (
        ...
        mkHsQTVisvs, hsQTVisvExplicit, emptyLHsQTVisvs,
        isHsKindedTyVar, tyBinderIgnoreVis_, tyBinderIgnoreVis,
        tyBinderAnyInvisible, hsTtvbAllKinded,
...
Additionally some declarations accepting invisible patterns at the type level
have an analogous rule. For example in all of:
...
fastidiousness of the programmer. But there is also additional precedent from
ScopedTypeVariables where only explicit 'forall' binds over the body. A dual way
of looking at that is the user-written free type variables in a term-level
definition are only bound on the LHS as implicit vars explicitly, never
implicitly. So why should free variables in a type-level definition be bound
over there implicitly either?
