...
compilation. The downside is that it defeats cross module inlining.

How does that work? We have TakesForever.hs and some module Depends.hs
that imports it. Now typically Depends.hs would only compile after
TakesForever.hs has been fully compiled. Now we might have a build graph
that looks like:
...
The downside is that we can't inline *anything* from TakesForever.hs so it is fine
to use this for expensive long running functions or types that are not performance
critical. But if `Depends.hs` or any of the Step2-*.hs modules depends crucially on
inlining things from TakesForever.hs or needs to know the exact representation of
