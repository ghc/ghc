...
unguardedGRHSs rhs@(L loc _)
  = GRHSs noExtField (unguardedRHS noAnn (locA loc) rhs) (noLocA emptyLocalBinds)
    ...
    go ty@(FunTy { ft_af = af, ft_mult = mult, ft_arg = arg, ft_res = res })
      = case af of
          ...
          InvisArg | (theta, tau) <- tcSplitPhiTy ty
                  -> noLocA (HsQualTy { hst_ctxt = Just (noLocA (map go theta))
                                      , hst_xqual = noAnn
                                      , hst_body = go tau })
...
mkPatSynBind :: LocatedN RdrName -> HsPatSynDetails (LocatedN RdrName)
             -> LPat GhcPs -> HsPatSynDir GhcPs -> ApiAnn' AnnsModule -> HsBind GhcPs
