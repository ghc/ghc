...
mkHsPar e = L (getLoc e) (HsPar noAnn noHsTok e noHsTok)
...
mkLHsPar le@(L loc e)
  | hsExprNeedsParens appPrec e = L loc (HsPar noAnn noHsTok le noHsTok)
  | otherwise                   = le

mkParPat :: IsPass p => LPat (GhcPass p) -> LPat (GhcPass p)
mkParPat lp@(L loc p)
  | patNeedsParens appPrec p = L loc (ParPat noAnn noHsTok lp noHsTok)
  | otherwise                = lp

nlParPat :: LPat (GhcPass name) -> LPat (GhcPass name)
nlParPat p = noLocA (ParPat noAnn noHsTok p noHsTok)
...
nlHsPar e     = noLocA (HsPar noAnn noHsTok e noHsTok)
...
mkMatch ctxt pats expr binds
  ...
  where
    ...
    paren lp@(L l p)
      | patNeedsParens appPrec p = L l (ParPat noAnn noHsTok lp noHsTok)
      | otherwise                = lp
