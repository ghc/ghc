...
--
-- This type is used as a boolean
-- but also to pass an evidence that the AST has been typechecked when we want
-- to collect evidence binders.
--
...
2. In the desugarer, most the time we don't want to collect evidence binders, so
   we also use CollNoDictBinders flag.
