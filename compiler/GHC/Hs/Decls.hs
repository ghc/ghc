...
tyClDeclLName :: ( XRec pass (IdP pass) ~ Located (IdP pass)
                 ) => TyClDecl pass -> Located (IdP pass)
tyClDeclLName (FamDecl { tcdFam = FamilyDecl { fdLName = ln } }) = ln
tyClDeclLName decl = tcdLName decl

-- tcdName is commonly used by both GHC and third-party tools, so it
-- needs to be polymorphic in the pass
tcdName :: ( XRec pass (IdP pass) ~ Located (IdP pass)
           ) => TyClDecl pass -> IdP pass
...
type HsConDeclDetails pass
   = HsConDetails (LBangType pass) (XRec pass [LConDeclField pass])
...
hsConDeclArgTys :: HsConDeclDetails (GhcPass p) -> [LBangType (GhcPass p)]
...
data RuleBndr pass
  ...
  | RuleBndrSig (XRuleBndrSig pass) (XRec pass (IdP pass)) (LHsSigWcType pass)
