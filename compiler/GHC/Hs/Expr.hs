...
import GHC.Tc.Utils.Type (TcType)
import {-# SOURCE #-} GHC.Tc.Utils (TcLclEnv)
...
data HsSplice id
   ...
   | HsQuasiQuote        -- See Note [Quasi-quote overview] in GHC.Tc.Splice
        (XQuasiQuote id)
        (IdP id)         -- Splice point
        (IdP id)         -- Quoter
...
-- These are the arguments that are passed to `GHC.Tc.Splice.runTopSplice`
data DelayedSplice =
  DelayedSplice
