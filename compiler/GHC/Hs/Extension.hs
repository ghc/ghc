...
{-# LANGUAGE UndecidableSuperClasses #-}  -- for IsPass
...
IsPass has a superclass constraint of IsPass (NoGhcTcPass pass); this allows
us to print types that are protected by NoGhcTc. The other superclass constraint
stating that NoGhcTcPass is idempotent is needed to avoid infinite regress
of superclasses. See also Note [NoGhcTc].
...
type OutputableBndrId pass =
  ( OutputableBndr (NameOrRdrName (IdGhcP pass))
  , OutputableBndr (IdGhcP pass)
  , OutputableBndr (NameOrRdrName (IdGhcP (NoGhcTcPass pass)))
