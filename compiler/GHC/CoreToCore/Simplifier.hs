...
module GHC.CoreToCore.Simplifier ( core2core, simplifyExpr ) where
...
import GHC.Core.Lint      ( endPass, lintPassResult, dumpPassResult, lintAnnots )
...
import VarSet
import VarEnv
import GHC.CoreToCore.CallArity      ( callArityAnalProgram )
import GHC.CoreToCore.CSE            ( cseProgram )
import GHC.CoreToCore.DmdAnal        ( dmdAnalProgram )
import GHC.CoreToCore.Exitify        ( exitifyProgram )
import GHC.CoreToCore.FloatIn        ( floatInwards )
import GHC.CoreToCore.FloatOut       ( floatOutwards )
import GHC.CoreToCore.LiberateCase   ( liberateCase )
import GHC.CoreToCore.Monad
import GHC.CoreToCore.Simplify.Env
import GHC.CoreToCore.Simplify.Monad
import GHC.CoreToCore.Simplify       ( simplTopBinds, simplExpr, simplRules )
import GHC.CoreToCore.Simplify.Utils ( simplEnvForGHCi, activeRule, activeUnfolding )
import GHC.CoreToCore.SpecConstr     ( specConstrProgram)
import GHC.CoreToCore.Specialise     ( specProgram)
import GHC.CoreToCore.StaticArgs     ( doStaticArgs )
import GHC.CoreToCore.WorkWrap       ( wwTopBinds )
import SrcLoc
import Util
import Module
