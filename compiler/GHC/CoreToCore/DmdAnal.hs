...
module GHC.CoreToCore.DmdAnal ( dmdAnalProgram ) where
...
import DynFlags
import GHC.CoreToCore.WorkWrapLib ( findTypeShape, deepSplitProductType_maybe )
...
import DataCon
import Id
import GHC.Core.Utils        ( exprIsHNF, exprType, exprIsTrivial, exprOkForSpeculation )
 ...
 * Any strict binders with product type, can use
   ...
   in WorkWrapLib. But then we don't want box it up again when returning it! We want
