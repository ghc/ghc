...
A pointer is Evaluated and Properly Tagged (EPT) when the pointer
  ...
  (a) points directly to the value, and
...
~~~~~~~~~~~~~~~~~~~~~~
The goal of EPT enforcement is to mark as many binders as possible as EPT
(see Note [Evaluated and Properly Tagged]).
To find more EPT binders, it establishes the following

EPT INVARIANT:
> Any binder of a strict field or a strict worker argument is EPT.
...
example using strict worker functions (see Note [CBV Function Ids]), e.g.
...
is the above example translated to use a strict worker function.

Implementation
--------------
EPT enforcement encompasses an analysis as well as a transformation.
* EPT analysis infers which binders are EPT, attaching the result to /binders/.
  This is implemented in GHC.Stg.EnforceEpt.inferTags.
  Crucially, the analysis assumes that the EPT invariant defined above is in
  effect, because it improves the precision of the analysis quite significantly.
* The EPT rewriter establishes the EPT invariant. That is, if
    (a) a binder x is used to
          * construct a strict field (`SP x y`), or
          * passed as a strict worker argument (`$wf x`),
        and
  ...
  and on strict worker arguments as well (Note [CBV Function Ids]).
* We also export the tag sigs of top level bindings to allow this optimisation
  to work across module boundaries.
  NB: Note that the EPT Invariant *must* be upheld, regardless of the
  optimisation level; hence the tag sig is practically part of the internal ABI
  of a strict data constructor or strict worker function.
  Note [CBV Function Ids] contains the details.
...
This could be fixed by replicating the floating capabilities of CorePrep, but
that is a large price to pay for curing such a rare regression.
...
effectively be lazy. That is because the EPT rewriter arranges that only EPT
arguments go into strict fields, and an eval on EPT things is a no-op.
