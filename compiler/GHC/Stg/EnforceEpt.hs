...
Note that any strict function can in theory be chosen as a CBV function;
...
Implementation
--------------
EPT enforcement works like contract checking in Design by Contract, where
the "contract" is the EPT invariant and "contract checking" corresponds to
inserting evals where necessary.

Dynamic contract checks can be elided if their outcome is known statically,
leading to more efficient code.
The same is true for EPT enforcement, which encompasses a transformation to
insert "dynamic checks" (evals) as well as an analysis that tries to elide
evals that can statically be proven redundant.
We run the analysis *before* the transformation, so that unnecessary evals are
...
* The EPT rewriter establishes the EPT invariant by inserting evals. That is, if
    (a) a binder x is used to
        ...
        and
    (b) x was not inferred EPT (see "EPT analysis" below),
...
* EPT analysis infers which binders are EPT, attaching the result to /binders/.
  ...
  Crucially, as typical in contract checking, the analysis assumes that the EPT
  invariant defined above is in effect, which improves the precision of the
...
* We also export the tag sigs of top level bindings to allow this optimisation
  ...
  of a strict data constructor or CBV function.
