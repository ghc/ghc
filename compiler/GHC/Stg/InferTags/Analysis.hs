...
solveConstraints :: HasDebugCallStack => Logger -> DynFlags -> AM ()
solveConstraints logger dflags = do
        ...
        seq (unsafePerformIO $ GHC.Utils.Logger.dumpIfSet_dyn
                logger
                dflags
                Opt_D_dump_stg_tag_nodes
                "STG Infered tags"
                FormatText
                (dumpNodes resultNodes)
                ) (return ())
        return ()
  where
...
nodeRhs :: HasDebugCallStack => Module -> ContextStack -> TopLevelFlag
        -> Id -> StgRhs
        -> AM (InferStgRhs)
nodeRhs this_mod ctxt topFlag binding (StgRhsCon _ ccs con cn ticks args)
  | null args = do
        ...
        markDone $ node
        return $! (StgRhsCon (node_id,RhsCon) ccs con cn ticks args)
  | otherwise = do
        ...
        return $! (StgRhsCon (node_id,remainsConRhs) ccs con cn ticks args)
  where
...
nodeConApp this_mod ctxt (StgConApp _ext con cn args tys) = do
    node_id <- mkUniqueId
    mapM_ (addImportedNode this_mod) [v | StgVarArg v <- args]
    ...
    return $! (StgConApp node_id con cn args tys, node_id)
