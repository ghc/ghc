...
{-# NOINLINE findTags #-}
findTags :: Logger -> DynFlags -> Module -> [StgTopBinding] -> ([TgStgTopBinding], [(Id,EnterLattice)])
-- findTags this_mod us binds = passTopBinds binds
findTags logger dflags this_mod binds =
    let state = FlowState {
            fs_idNodeMap = mempty,
    ...
    -- Run the analysis
        ...
        analysis = do
            ...
            {-# SCC "solveConstraints" #-} (solveConstraints logger dflags)
