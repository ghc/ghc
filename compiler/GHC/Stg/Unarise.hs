...
data UnariseEnv = UnariseEnv
   { unenvVals   :: !(VarEnv UnariseVal)  -- ^ rho environment
...
mapRho f env = env { unenvVals = f (unenvVals env) }
...
unariseExpr env e@(StgApp f [])
  = case lookupVarEnv (unenvVals env) f of
...
unariseExpr env e@(StgApp f args)
  ...
  where
    f' = case lookupVarEnv (unenvVals env) f of
...
unariseExpr env (StgCase scrut bndr alt_ty alts)
  ...
  | StgApp v [] <- scrut
  , Just (MultiVal xs) <- lookupVarEnv (unenvVals env) v
...
For arguments (StgArg) and binders (Id) we have two kind of unarisation:
  ...
  - When unarising case alternative binders we remove void binders, but we
    ...
    least one non-void thing. So in the env we will have something like:

      x :-> MultiVal [xu1, xu2]

    Now, after we eliminate void binders in the pattern, we get exactly the same
    number of binders, and extend env again with these:
...
unariseFunArg env (StgVarArg x) =
  case lookupVarEnv (unenvVals env) x of
