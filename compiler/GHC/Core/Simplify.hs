...
module GHC.Core.Simplify ( simplTopBinds, simplExpr, simplRules ) where
...
import GHC.Core.Pretty  ( pprCoreExpr )
import GHC.Core.Unfold
import GHC.Core.Utils
import GHC.Core.Opt     ( pushCoTyArg, pushCoValArg
...
the simplifier is in GHC.Core.Simplifier.
...
simplTopBinds env0 binds0
  = do  {       -- Put all the top-level binders into scope at the start
                ...
                -- See note [Glomming] in GHC.Core.OccurAnal.
...
Note [Binder swap] in GHC.Core.OccurAnal.
...
[Case of cast] in GHC.Core.OccurAnal may then apply to the result.
...
See Note [seq# magic] in GHC.Core.ConstantFold.  Example (#15226):
...
NB: take great care when doing strictness analysis;
    see Note [Lambda-bound unfoldings] in GHC.Core.DmdAnal.
...
See Note [Forming Rec groups] in GHC.Core.OccurAnal
-}
