...
import GHC.Core.WorkWrapLib ( isWorkerSmallEnough, mkWorkerArgs )
import DataCon
...
import Id
import GHC.Core.Pretty  ( pprParendExpr )
  ...
  = do  { mb_pats <- mapM (callToPats env bndr_occs) calls
        ...
        ; let have_boring_call = any isNothing mb_pats
              ...
              too_big (vars,_) = not (isWorkerSmallEnough (sc_dflags env) vars)
                  ...
                  -- See Note [Limit w/w arity] in WorkWrapLib
