...
{-# LANGUAGE CPP #-}
module GHC.Core.WorkWrap ( wwTopBinds ) where
 ...
 * We want them to be still there at the end of GHC.Core.DmdAnal, so that
...
Note [Final Demand Analyser run] in GHC.Core.DmdAnal).
...
in w/w-ing because we'll simply add (\y:Void#), see WorkWrapLib.mkWorerArgs.
