...
dataConUserTyVarBindersNeedWrapper dc@(MkData { dcUnivTyVars = univ_tvs
                                              , dcExTyCoVars = ex_tvs
                                              , dcEqSpec = eq_spec })
  ...
  where
    ...
    need_reorder   = worker_tvs /= binderVars wrapper_tvbs
    need_flag_cast = any (not . eqForAllVis coreTyLamForAllTyFlag)
                         (binderFlags wrapper_tvbs)
          -- Workers always use 'Specified' foralls (coreTyLamForAllTyFlag),
          -- wrappers may use 'Inferred', 'Specified', or 'Required' foralls.
          --
          -- We do /not/ need a cast to convert 'Inferred' to 'Specified', but
          -- we do need one for 'Required'.
          --
          -- See Note [ForAllTy and type equality], Note [Comparing visibility],
          -- and [Required foralls in Core].

    worker_tvs   = univ_tvs ++ ex_tvs
    wrapper_tvbs = dataConUserTyVarBinders dc
