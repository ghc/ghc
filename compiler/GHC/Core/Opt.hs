...
import GHC.Core.Pretty ( pprCoreBindings, pprRules )
import GHC.Core.OccurAnal( occurAnalyseExpr, occurAnalysePgm )
...
-- This is not possible with user-supplied empty literals,
-- GHC.Core.Make.mkStringExprFS turns those into [] automatically, but just in
-- case something else in GHC generates a string literal directly.
