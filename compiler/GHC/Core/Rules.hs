...
match renv subst e1 (Cast e2 co2) mco
  = match renv subst e1 e2 (checkReflexiveCastCo (TCC (exprType e2) (mkTransCastCo co2 mco)))
...
matchTemplateCast renv subst e1 co1 e2 mco
  | -- Check that the coercion does not mention any of the template variables
    not $ anyFreeVarsOfCastCo (`elemVarSet` rv_tmpls renv) substed_co
  = -- This is the good path
    -- See Note [Casts in the template] wrinkle (CT0)
    match renv subst e1 e2 (checkReflexiveCastCo (TCC substed_ty (mkTransCastCo mco (mkSymCastCo substed_ty substed_co))))
