...
import GHC.Core.TyCoRep( TyCoBinder(..), TyBinder )
...
-- as per the invariants of 'CoreExpr': see Note [Core let/app invariant]
...
simplifier loop described in Note [Cascading inlines] in GHC.Core.OccurAnal.
Suppose x occurs just once; GHC.Core.OccurAnal.occAnalNonRecRhs decides x will
...
in GHC.Core.ConstantFold convert it (a ==# 3#) to
   case a of { DEFAULT -> 0#; 3# -> 1# }
for excellent reasons described in
  GHC.Core.ConstantFold Note [The litEq rule: converting equality to case].
...
-- happen: see Note [Core let/app invariant]. This invariant states that arguments of
