...
mkFunResMCo :: Mult -> Type -> MCoercionR -> MCoercionR
mkFunResMCo _ _         MRefl    = MRefl
mkFunResMCo mult arg_ty (MCo co) = MCo (mkFunResCo Representational mult arg_ty co)
...
mkPiCo r v co | isTyVar v = mkHomoForAllCos [v] co
              | isCoVar v = assert (not (v `elemVarSet` tyCoVarsOfCo co)) $
                  ...
                  -- now (Aug 2018) v won't occur in co.
                            mkFunResCo r (varMult v) (varType v) co
              | otherwise = mkFunResCo r (varMult v) (varType v) co

mkFunResCo :: Role -> Mult -> Type -> Coercion -> Coercion
-- Given res_co :: res1 -> res2,
--   mkFunResCo r m arg res_co :: (arg -> res1) ~r (arg -> res2)
-- Reflexive in the multiplicity argument
mkFunResCo role mult arg_ty res_co
