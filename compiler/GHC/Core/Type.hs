...
getConv_maybe rinfo 
  | TyConApp rinfo [rep, conv] <- coreFullView rinfo
  , rinfo `hasKey` runtimeInfoDataConKey    = Just conv
  | rinfo <- coreFullView rinfo             = Just $ mkTyConApp getConvTyCon [rinfo]  
  | rep <- coreFullView rinfo
  , isCallingConvTy rep                     = Just rep
  | otherwise                               = Nothing
