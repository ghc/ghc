...
--
-- Previously we used a wired-in closure
-- `base:Control.Exception.Base.absentSumFieldError` But it introduced a
-- dependency on `base` for any code using unboxed sums and it became an issue
-- when we wanted to use unboxed sums in boot libraries used by base (see
-- #17791). Now we use the following indirections:
--
-- * a wired-in closure `ghc-prim:GHC.Prim.Ext.raiseAbsentSumField`: it's
--   defined in ghc-prim so we can depend on it
--
