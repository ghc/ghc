...
--
-- We need a rubbish value (called `absentSumFieldError`) for GC PtrSlot and it
-- should be a valid (no NULL pointer) non-CAFFY closure:
--
--    1. it can't take arguments because it's used in unarise and applying an
--       argument would require allocating a thunk.
--
--    2. it can't be CAFFY because that would mean making some non-CAFFY
--       definitions that use unboxed sums CAFFY in unarise.
--
--       Getting this wrong causes hard-to-debug runtime issues, see #15038.
--
--    3. it can't be defined in `base` package.
--
...
--       `base`, see #17791.
--
--    4. it is never used in Core, only in STG; and even then only for filling a
...
--
-- Currently `absentSumFieldError` is defined in ghc-prim:GHC.Prim.Panic.  It
-- does not throw a Haskell exception; instead, it calls `panic#`, which
-- immediately halts execution.
--
-- Note that a runtime invocation of `absentSumFieldError` indicates a GHC bug.
-- Unlike (say) pattern-match errors, it cannot be caused by a user error.
-- That's why it is OK for it to be un-catchable.
--
