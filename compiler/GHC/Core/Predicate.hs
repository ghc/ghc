...
TODO: Not sure this note is right. In particular if we have a type family:

    type family MaybeShowNum a n :: Constraint where
        MaybeShowNum a n = (Show a, Num n)

then `MaybeShowNum Bool Int` *will* determine the value! However
typeDeterminesValue `MaybeShowNum Bool Int` would give `False` since
the type family is opaque.

Similarly I tried making specialize go wrong when we allow specialization on
implicit params it just seems to work.

I will update the note more before merging.
-}
