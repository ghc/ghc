...
negOp dflags (LitNumber nt i)
   | litNumIsSigned nt = Just (Lit (mkLitNumberWrap dflags nt (-i)))
negOp _      _                = Nothing

complementOp :: DynFlags -> Literal -> Maybe CoreExpr  -- Binary complement
complementOp dflags (LitNumber nt i) =
   Just (Lit (mkLitNumberWrap dflags nt (complement i)))
...
intOp2' :: (Integral a, Integral b)
        ...
        -> DynFlags -> Literal -> Literal -> Maybe CoreExpr
intOp2' op dflags (LitNumber LitNumInt i1) (LitNumber LitNumInt i2) =
  let o = op dflags
...
intOpC2 :: (Integral a, Integral b)
        => (a -> b -> Integer)
        -> DynFlags -> Literal -> Literal -> Maybe CoreExpr
intOpC2 op dflags (LitNumber LitNumInt i1) (LitNumber LitNumInt i2) =
...
wordOp2 :: (Integral a, Integral b)
        => (a -> b -> Integer)
        -> DynFlags -> Literal -> Literal -> Maybe CoreExpr
wordOp2 op dflags (LitNumber LitNumWord w1) (LitNumber LitNumWord w2)
...
wordOpC2 :: (Integral a, Integral b)
        => (a -> b -> Integer)
        -> DynFlags -> Literal -> Literal -> Maybe CoreExpr
wordOpC2 op dflags (LitNumber LitNumWord w1) (LitNumber LitNumWord w2) =
  ...
  = do { dflags <- getDynFlags
       ...
       ; case e1 of
           _ | shift_len == 0
             ...
             -> return $ Lit $ mkLitNumberWrap dflags LitNumInt 0
           ...
           Lit (LitNumber nt x)
             ...
             -> let op = shift_op dflags
                ...
                in  liftMaybe $ Just (Lit (mkLitNumberWrap dflags nt y))
...
isMinBound dflags (LitNumber nt i) = case nt of
...
isMaxBound dflags (LitNumber nt i) = case nt of
...
builtinBignumRules :: DynFlags -> [CoreRule]
builtinBignumRules dflags
...
match_integerBit dflags id_unf _ [arg]
  | Just (LitNumber LitNumWord x) <- exprIsLiteral_maybe id_unf arg
  , x >= 0
