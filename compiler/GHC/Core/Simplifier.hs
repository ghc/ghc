...
\section[Simplifier]{Driver for simplifying @Core@ programs}
-}

{-# LANGUAGE CPP #-}

module GHC.Core.Simplifier ( core2core, simplifyExpr ) where
...
import GHC.Core
import HscTypes
import GHC.Core.CSE     ( cseProgram )
import GHC.Core.Rules   ( mkRuleBase, unionRuleBase,
                          extendRuleBaseList, ruleCheckProgram, addRuleInfo,
                          getRules )
import GHC.Core.Pretty  ( pprCoreBindings, pprCoreExpr )
import GHC.Core.OccurAnal ( occurAnalysePgm, occurAnalyseExpr )
import IdInfo
...
import GHC.Core.Simplify.Utils ( simplEnvForGHCi, activeRule, activeUnfolding )
import GHC.Core.Simplify.Env
import GHC.Core.Simplify.Monad
import GHC.Core.Monad
import qualified ErrUtils as Err
import GHC.Core.FloatIn  ( floatInwards )
import GHC.Core.FloatOut ( floatOutwards )
...
import VarSet
import VarEnv
import GHC.Core.LiberateCase ( liberateCase )
import GHC.Core.SAT          ( doStaticArgs )
import GHC.Core.Specialise   ( specProgram)
import GHC.Core.SpecConstr   ( specConstrProgram)
import GHC.Core.DmdAnal      ( dmdAnalProgram )
import GHC.Core.CallArity    ( callArityAnalProgram )
import GHC.Core.Exitify      ( exitifyProgram )
import GHC.Core.WorkWrap     ( wwTopBinds )
import SrcLoc
import Util
import Module
...
getCoreToDo dflags
  ...
  where
    ...
    -- after this before anything else
        ...
        -- has run at all. See Note [Final Demand Analyser run] in GHC.Core.DmdAnal
