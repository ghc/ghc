...
traverse_FamInstEnv :: Monad m => (FamInst -> m ()) -> FamInstEnv -> m ()
traverse_FamInstEnv f (FamIE _ rm) = traverse_RM f rm
...
familyInstances (pkg_fie, home_fie) fam
  ...
  where
    ...
    get (FamIE _ env) = lookupRM [LookupKnownTc (tyConName fam)] env
...
lookupFamInstEnvByTyCon (pkg_ie, home_ie) fam_tc
  ...
  where
    get (FamIE _ rm) = lookupRM [LookupKnownTc (tyConName fam_tc)] rm
