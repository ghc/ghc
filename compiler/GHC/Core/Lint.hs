...
lintVarOcc var nargs
  = addLoc (OccOf var) $
    do  { checkL (isNonCoVarId var)
        ...
        -- as the type of the binding site.  We lint the type so that
        -- we apply the substitution.  Actually this will get into trouble
        -- with  /\a. \(x::Maybe a). /\a. ...(x::Maybe a)...
        -- The "::Maybe a" on the occurrence is referring to the /outer/ a.
        -- But this naive check will treat it like the inner one.
...
lintType (TyVarTy tv)
  ...
  = do { subst <- getTCvSubst
       ; case lookupTyVar subst tv of
           ...
           Nothing -> -- lintTyBndr always extends the substitition
                      failWithL $
...
data LintEnv
  = LE { le_flags :: LintFlags       -- Linting the result of this pass
       ...
       , le_ids   :: IdSet     -- In-scope Ids; all Linted
