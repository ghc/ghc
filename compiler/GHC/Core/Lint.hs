...
lintCoreBindings dflags pass local_in_scope binds
  = initL dflags flags local_in_scope $
    ...
    do { checkL (null dups) (dupVars dups)
       ...
       ; lintRecBindings TopLevel Recursive all_pairs $
         return () }
  where
...
lintRecBindings :: TopLevelFlag -> RecFlag -> [(Id, CoreExpr)]
                -> LintM a -> LintM a
lintRecBindings top_lvl rec_flag pairs thing_inside
...
-- Binder's type has already been linted; check other invariants
...
-- NB: the Id can be Linted or not -- it's only
lintRhs bndr rhs
...
lintCoreExpr e@(Let (Rec pairs) body)
  = do  { -- Check that the list of pairs is non-empty
        ...
        ; lintRecBindings NotTopLevel Recursive pairs $
...
lintCoreArgs  :: LintedType -> [CoreArg] -> LintM OutType
...
-----------------
lintValApp :: CoreExpr -> LintedType -> LintedType -> LintM OutType
lintValApp arg fun_ty arg_ty
...
-------------------
lintType :: OutType -> LintM LintedType
...
lintType ty@(ForAllTy (Bndr tcv vis) body_ty)
  = lintTyCoBndr tcv $ \tcv' ->
    do { body_ty' <- lintType body_ty
       ...
       ; lintL (isTyCoVar tcv) $
         text "Non-Tyvar or Non-Covar bound in type:" <+> ppr ty
       ; when (isCoVar tcv) $
         lintL (tcv' `elemVarSet` tyCoVarsOfType body_ty') $
         text "Covar does not occur in the body:" <+> ppr ty

       ; let k = typeKind body_ty'
       ; checkValueKind k (text "the body of forall:" <+> ppr ty)
       ; case occCheckExpand [tcv'] k of  -- See Note [Stupid type synonyms]
           Just {} -> return (ForAllTy (Bndr tcv' vis) body_ty')
           Nothing -> failWithL (hang (text "Variable escape in forall:")
                                    2 (vcat [ text "type:" <+> ppr body_ty'
                                            , text "kind:" <+> ppr k ]))
    }

{- Strange bit of old code
...
lintStarCoercion g
  = do { g' <- lintCoercion g
       ...
       ; checkValueKind (typeKind t1) (text "the kind of the left type in" <+> ppr g)
       ; checkValueKind (typeKind t2) (text "the kind of the right type in" <+> ppr g)
...
lintCoercion co@(TyConAppCo r tc cos)
  ...
  = do { checkTyCon tc
       ...
       ; sequence_ [ lintRole co' r (coercionRole co')
                   | (r, co') <- zip (tyConRolesX r tc) cos' ]
...
lintCoercion co@(AppCo co1 co2)
  ...
  = do { co1' <- lintCoercion co1
       ...
       ; lint_co_app co (typeKind (coercionLKind co1')) [coercionLKind co2']
       ; lint_co_app co (typeKind (coercionRKind co1')) [coercionRKind co2']

       ; let r1 = coercionRole co1
             r2 = coercionRole co2
       ; if r1 == Phantom
...
----------
lintCoercion co@(ForAllCo tcv1 kind_co body_co)
  = do { when (isCoVar tcv1) $
         lintL (almostDevoidCoVarOfCo tcv1 body_co)
               (text "Covar can only appear in Refl and GRefl: " <+> ppr co)
               -- What is this all about?
       ; kind_co' <- lintStarCoercion kind_co
       ; lintTyCoBndr tcv1 $ \tcv1' ->
    do { body_co' <- lintCoercion body_co

       ; ensureEqTys (varType tcv1') (coercionLKind kind_co') $
         text "Kind mis-match in ForallCo" <+> ppr co
       ; return (ForAllCo tcv1' kind_co' body_co') } }
...
lintCoercion co@(TransCo co1 co2)
  = do { co1' <- lintCoercion co1
       ; co2' <- lintCoercion co2
       ; let Pair ty1a ty1b = coercionKind co1'
             Pair ty2a ty2b = coercionKind co2'
...
lintCoercion this@(AxiomRuleCo ax cos)
  = do { cos' <- mapM lintCoercion cos
       ; lintRoles 0 (coaxrAsmpRoles ax) cos'
  ...
  lintRoles n (e : es) (co : cos)
    | e == coercionRole co = lintRoles (n+1) es cos
...
addInScopeId id m
  = LintM $ \ env@(LE { le_ids = id_set, le_joins = join_set }) errs ->
    unLintM m (env { le_ids   = extendVarSet id_set id
