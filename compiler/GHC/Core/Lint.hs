...
import GHC.CoreToCore.Monad as CoreMonad
import Bag
import Literal
import DataCon
import TysWiredIn
...
import RepType
import GHC.Core.TyCoRep       -- checks validity of types/coercions
...
import GHC.Core.OptCoercion ( checkAxInstCo )
import UniqSupply
...
lintSingleBinding top_lvl_flag rec_flag (binder,rhs)
  = addLoc (RhsOf binder) $
    ...
    do { ty <- lintRhs binder rhs
       ...
       -- Note [idArity varies independently of dmdTypeDepth] in GHC.Core.DmdAnal
...
lintTyApp fun_ty arg_ty
  ...
  = do  { lintTyKind tv arg_ty
        ...
        -- See Note [The substitution invariant] in GHC.Core.TyCoSubst
  ...
  = do { lintL (isCoVar cv)
    ...
    do { k <- lintType ty
       ...
       ; return liftedTypeKind
           ...
           -- See Note [NthCo and newtypes] in GHC.Core.TyCoRep
    }}
...
lint_app doc kfn kas
    = do { in_scope <- getInScope
         ...
         -- Note [The substitution invariant] in GHC.Core.TyCoSubst
...
Note [Rules and join points] in GHC.Core.OccurAnal for further discussion.
-}
...
Pointer Table from the top-level bindings. See Note [Grand plan for static
forms] in GHC.Core.Simplifier.
