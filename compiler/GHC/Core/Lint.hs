...
lintType ty@(ForAllTy (Bndr tcv vis) body_ty)
  ...
  = lintTyBndr tcv $ \tv' ->
    do { body_ty' <- lintType body_ty
       ...
       ; case occCheckExpand [tv'] k of  -- See Note [Stupid type synonyms]
           Just {} -> return (ForAllTy (Bndr tv' vis) body_ty')
  ...
  = lintCoBndr tcv $ \cv' ->
    do { body_ty' <- lintType body_ty
       ...
       ; lintL (cv' `elemVarSet` tyCoVarsOfType body_ty') $
       ...
       -- need to check for skolem-escape, as we must do in the tyvar case

       ; return (ForAllTy (Bndr cv' vis) body_ty') }
