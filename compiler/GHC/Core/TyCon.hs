...
import {-# SOURCE #-} GHC.Core.TyCoRep    ( Kind, Type, PredType, mkForAllTy, mkFunTy )
...
* In a PromotedDataCon whose kind has an equality constraint:
  ...
  See Note [Constraints in kinds] in GHC.Core.TyCoRep, and
...
in GHC.Core.TyCoRep), so we change it to Specified when making MkT's TyCoBinders
-}
...
* See Note [VarBndrs, TyCoVarBinders, TyConBinders, and visibility] in GHC.Core.TyCoRep
