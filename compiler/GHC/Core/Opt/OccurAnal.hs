...
(BS2) When doing a lookup in occ_bs_env, we may need to iterate,
      ...
      At the first case addBndrSwap will extend occ_bs_env with
          [x :-> a]
      At the second we occ-anal the scrutinee 'x', which lookup up
        'x in occ_bs_env, returning 'a', it should.
        Then addBndrSwap will add [b :-> a] to occ_bs_env.
      At the third case we'll again look up 'x' which returns 'a'.
      But we don't want to stop there, else we'll end up with
                 case x of a { 1# -> e1; DEFAULT ->
                 case a of b { 2# -> e2; DEFAULT ->
                 case a of c { 3# -> e3; DEFAULT -> ..a..b..c.. }}}
      Instead, we want iterate the lookup in addBndrSwwap, to give
                 case x of a { 1# -> e1; DEFAULT ->
                 case a of b { 2# -> e2; DEFAULT ->
...
addBndrSwap scrut case_bndr
  ...
  , let rng_vars' = rng_vars `extendVarSet` case_bndr'
