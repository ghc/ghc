...
specExpr env expr@(App {})
  = do { let (fun, args) = collectArgs expr
       ; (args, uds_args) <- mapAndCombineSM (specExpr env) args
       ; let (fun', args') = rewrite_class_ops fun args
       ; (fun',  uds_fun)  <- case fun' of
           Var f -> case specVar env f of
             Var f' -> return (Var f', mkCallUDs env f' args')
             e'     -> return (e', emptyUDs) -- I don't expect this!
           other -> specExpr env other
       ; return (fun' `mkApps` args', uds_fun `plusUDs` uds_args)
       }
  where
...
Here, specialise `f` at `Bool`, `()`, `Maybe Integer` alright, but we used to
fail to specialise the `C Bool b` instance's method `m` at `b ~ ()` and
`c ~ Maybe Integer`. That's because the code for `$sf` looks like

  $dC = $fCBoolb @() GHC.Show.$fShow()
  $dShow = GHC.Show.$fShowMaybe @Integer GHC.Show.$fShowInteger
  $sf (a::Bool) (b::()) (c::Maybe Integer) =
        ... (m @Bool @() $dC @(Maybe Integer) $dShow a b c) ...

And `m` is just a DictSel. There's nothing to specialise! The next Simplifier
run will expose the rewritten instance method:

  ... $fCBoolb_$cm @() $fShow() @(Maybe Integer) $dShow a b c ...

But it would be better not to miss this specialisation opportunity. This is our
