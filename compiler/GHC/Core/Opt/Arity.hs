...
Note [Optimistic arity in fixed-point iteration]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
leads to a cascade of further transformations that ultimately optimise the
counting loop into a recursive thunk, for which a <<loop>> error will be
emitted. This happens for T18231, for example. More details in
https://gitlab.haskell.org/ghc/ghc/-/merge_requests/4207#note_304817.
Divergence is later on discovered independently by strictness analysis anyway,
so starting from @ATop@ will still discover functions that call erroring or
diverging functions after strictness analysis and is what GHC has been doing
since forever.
-}
