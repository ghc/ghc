...
data AnalysisMode
  ...
  | ArityAnalysis { aa_dflags :: !DynFlags, aa_sigs :: !(IdEnv ArityType) }
  -- ^ Used for regular arity analysis.
...
initArityEnv dflags
  = AE { ae_mode  = ArityAnalysis { aa_dflags = dflags, aa_sigs = emptyVarEnv }
...
pedanticBottoms AE{ ae_mode = mode } = case mode of
  BotStrictness                       -> True
  ArityAnalysis{ aa_dflags = dflags } -> gopt Opt_PedanticBottoms dflags
...
myExprIsCheap AE{ae_mode = mode} e mb_ty = case mode of
  BotStrictness                                     -> False
  ArityAnalysis{aa_dflags = dflags, aa_sigs = sigs} ->
    cheap_dict || exprIsCheapX (myIsCheapApp sigs) e
    where
      cheap_dict = gopt Opt_DictsCheap dflags && fmap isDictTy mb_ty == Just True
