...
mkWwBodies opts fun_id arg_vars res_ty demands res_cpr
  = do  { massertPpr (filter isId arg_vars `equalLength` demands)
        ...
        ; let (work_lam_args, work_call_args) = mkWorkerArgs fun_id (wo_fun_to_thunk opts)
              ...
              call_rhs fn_rhs = mkVarAppsBeta fn_rhs fn_args
...
-- PRECONDITION: The arg expressions don't are not free in any of the lambdas.
mkVarAppsBeta :: CoreExpr -> [CoreArg] -> CoreExpr
-- The precondition holds for our call site in mkWwBodies, because all the FVs
-- of fn_args are fresh.
mkVarAppsBeta (Lam b body) (a:as) = bindNonRec b a $! mkVarAppsBeta body as
mkVarAppsBeta f            as     = mkApps f as
...
function for the worker:
  ...
  * Lambda-bound unfoldings: When doing case-of-case, we might get a join binder
    ...
    Zapping lambda-bound unfoldings is especially important for
    unboxed arguments; see Note [Zap unfolding when beta-reducing] in
    GHC.Core.Opt.Simplify and see #13890
...
mkWWstr_one opts inlineable_flag arg =
  case wantToUnboxArg fam_envs inlineable_flag arg_ty arg_dmd of
    ...
    DropAbsent
      | Just filler <- mkAbsentFiller opts arg
         -- Absent case.  We can't always handle absence for arbitrary
         -- unlifted types, so we need to choose just the cases we can
         -- (that's what mkAbsentFiller does)
      -> return (True, [], nop_fn, filler)
...
mkAbsentFiller opts arg
  ...
  , not (isCoVar arg) -- Simon is preparing a patch that explains more
  = Just (mkTyApps (Lit (mkLitRubbish prim_reps)) [arg_ty])  -- Catch all: Either @arg_ty@ wasn't of form @TYPE rep@ or @rep@ wasn't mono rep.
