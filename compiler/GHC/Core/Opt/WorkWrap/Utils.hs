...
wantToUnbox fam_envs has_inlineable_prag ty dmd =
  case deepSplitProductType_maybe fam_envs ty of
    Just dcac@DataConAppContext{ dcac_dc = dc }
      | isStrUsedDmd dmd
...
-- | Like 'dataConInstArgTys', but
--
--   * handles existentials (by cloning)
--   * and returns fresh 'Id's for the instantiated argument types
--
-- The cloning and freshening needs 'MonadUnique'.
dataConInstExAndArgVars :: MonadUnique m => FastString -> DataCon -> [Type] -> m ([TyCoVar], [Id])
dataConInstExAndArgVars prefix dc tc_args = do
  let subst_univ = zipTvSubst (dataConUnivTyVars dc) tc_args
  (subst, ex_tvs') <- cloneTyVarBndrs subst_univ (dataConExTyCoVars dc) <$> getUniqueSupplyM
  -- the following line is straight from 'dataConInstArgTys'
  let arg_tys = mapScaledType (substTy subst) <$> dataConRepArgTys dc
  let mk_id (Scaled m ty) uniq = mkSysLocalOrCoVar prefix uniq m ty
  arg_id_uniqs <- getUniquesM
  let arg_ids = zipWithEqual "dataConInstExAndArgVars" mk_id arg_tys arg_id_uniqs
  pure (ex_tvs', arg_ids)
...
unbox_one dflags fam_envs arg cs
          DataConAppContext { dcac_dc = dc, dcac_tc_args = tc_args
                            , dcac_co = co }
  = do { (ex_tvs', arg_ids) <- dataConInstExAndArgVars ww_prefix dc tc_args
       ; case_bndr_uniq <- getUniqueM
       ; let   str_marks = dataConRepStrictness dc
               arg_ids1  = zipWithEqual "unbox_one" (flip setCaseBndrEvald) arg_ids str_marks
               -- See Note [Add demands for strict constructors]
               cs'       = addDataConStrictness dc cs
               arg_ids2  = zipWithEqual "unbox_one" setIdDemandInfo         arg_ids1 cs'
               ...
               con_app    = mkConApp2 dc tc_args (ex_tvs' ++ arg_ids2) `mkCast` mkSymCo co
        ; (_, worker_args, wrap_fn, work_fn) <- mkWWstr dflags fam_envs False (ex_tvs' ++ arg_ids2)
