...
module GHC.Core.Opt.WorkWrap.Utils
   ...
   , DataConPatContext(..), deepSplitProductType_maybe, wantToUnbox
   , findTypeShape
   , isWorkerSmallEnough
   )
where
...
-- | Context for a 'DataCon' pattern wrapped in a cast, where we know the type
-- arguments of the 'TyCon' but not any of the arguments to the 'DataCon' (type
-- or term).
--
...
-- Example:
--
-- > DataConPatContext Right [Int, Bool] (co :: Right Int Bool ~ NT Char)
--
-- represents
--
-- > (Right ... :: Either Int Bool) |> co :: NT Char
--
...
--
-- See Note [Which types are unboxed?].
deepSplitProductType_maybe :: FamInstEnvs -> Type -> Maybe DataConPatContext
...
--
-- See Note [Which types are unboxed?].
deepSplitCprType_maybe :: FamInstEnvs -> ConTag -> Type -> Maybe DataConPatContext
...
dubiousDataConInstArgTys dc tc_args = arg_tys
  where
    ...
    arg_tys  = substTy subst . scaledThing <$> dataConRepArgTys dc
...
The respective tests are in 'deepSplitProductType_maybe' and
'deepSplitCprType_maybe', respectively.
...
mkWWcpr opt_CprAnal fam_envs body_ty cpr
  ...
  = case asConCpr cpr of
       ...
       Just con_tag | Just dcpc <- deepSplitCprType_maybe fam_envs con_tag body_ty
                    -> mkWWcpr_help dcpc
                    |  otherwise
