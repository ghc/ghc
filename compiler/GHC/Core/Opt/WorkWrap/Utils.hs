...
isRecDataCon fam_envs fuel dc
  ...
  where
    ...
    go_tc_app fuel rec_tc tc tc_args
      ...
      |  isPrimTyCon tc
      || isPromotedDataCon tc -- we only care about value level recursion
      = Just NonRecursive
      ...
      | Just (_tvs, rhs, _co) <- unwrapNewTyConEtad_maybe tc
          -- NB 1: No recursion into _co
          -- NB 2: We discard length tc_args. Even undersaturated is OK; think
          --       of `StateT Int (ST s)` where we want to unfold the ST.
      , Just rec_tc' <- checkRecTc rec_tc tc
