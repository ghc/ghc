...
mkWwBodies opts fun_id arg_vars res_ty demands res_cpr
  = do  { massertPpr (filter isId arg_vars `equalLength` demands)
        ...
        ; let arg_fvs = exprsFreeVars $ map varToCoreExpr arg_vars -- Free tyvars, in particular
              empty_subst = mkEmptySubst (mkInScopeSet arg_fvs)
              zapped_arg_vars = map zap_var arg_vars
              (_subst, cloned_arg_vars) = cloneBndrs empty_subst uniq_supply zapped_arg_vars
  ...
  where
    ...
    zap_info info -- See Note [Zap IdInfo on worker args]
      = info `setUnfoldingInfo` zapUnfolding unf
      where
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
We have to zap unfoldings when re-using arg variables of the original function
for the worker. Here's why:
When doing case-of-case, we might get a join binder with an unfolding like

  join {f (x::Char) y[Unf=T x] = ... case x of C# c -> ... } in ...

When we worker/wrapper that join point to unbox x, we'll get

  join {$wf (ww::Char#) y[Unf=T x] = let x = C# ww in ... case x of ...} in

but now the unfolding for y is ill-scoped! Yuck. Therefore we simply zap the
unfolding (but keep evaluatedness, see 'zapUnfolding').

We zap in mkWwBodies because we need the zapped variables both when binding them
in mkWWstr (mk_absent_let, specifically) and in mkWorkerArgs, where they occur
in the call to the fun body.
...
fun RHS because they might shadow each other. E.g.
  ...
  $wf :: Int# -> Int
  $wf n = let x = error "absent" :: Char; let x = I# n :: Int in
          (\x x -> x + 1) x x
  -- type mismatch here   ^   expected: Char, actual: Int

  f :: Char -> Int -> Int
  f x x = case x of I# n -> $wf n

Note that this may apply equally to TyVars, in a type like f :: forall a. a ->
forall a. a->a. In #12562, we even saw a type variable in the worker shadowing
an outer term-variable binding.

We avoid the issue by freshening the argument variables from the
original fun RHS through 'substBndrs', which will also take care of subsitution
in binder types.

Fortunately, it's sufficient to pick the FVs of the arg vars as in-scope set,
so that we don't need to do a FV traversal over the whole body of the original
function.
...
Needless to say, there are some wrinkles:
  ...
  2. We also mustn't put an error-thunk (that fills in for an absent value of
     ...
     either, just in case it ends up in a strict fields!
