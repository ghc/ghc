...
import GHC.Types.Literal ( rubbishLit )
...
Then f gets a strictness sig of <S(L,A,A)><A>. We make a worker $wf thus:
  ...
  $wf xs = case ps of MkT xs _ _ -> <body mentioning xs>
    where
      ...
      zs = LitRubbish[LiftedRep] @[Int]
      ps = MkT xs ys zs
      w  = LitRubbish[IntRep] @Int#
...
The story would end here, if it wasn't for two issues:

  1. In case we have a absent, /lazy, and lifted/ arg, we use an error-thunk
     instead. If the binding is used (buggily, of course, e.g. #11126), then we
     get a nice panic message instead of undefined runtime behavior (segfault).

     But we can't use an error-thunk if the demand is both absent /and strict/:
     The code would force the panic after creating it. You may ask: How can
     the demand be both absent and strict? That's exactly what happens for
     'zs': it is not used, so its demand is Absent, but then during w/w, in
     addDataConStrictness, we strictify the demand. So it gets cardinality
     'C_10', the empty interval.

     We don't want an error-thunk for 'zs' because MkT's third argument
     has a bang, and hence should always be evaluated. This turned out
     to be important when fixing #16970, which establishes the invariant
     that strict constructor arguments are always evaluated. So we stick to
     a rubbish literal -- see #19133.

  2. We can only emit a RubbishLit if the arg's type @arg_ty@ is mono-rep, e.g.
...
While (1) is simply an optimisation in terms of compiler debugging experience,
(2) should be irrelevant in most programs, if not all.
...
mk_absent_let dflags arg dmd
  ...
  , not (isStrictDmd dmd)
  , let lifted_arg = arg `setIdStrictness` botSig `setIdCprInfo` mkCprSig 0 botCpr
        -- Note in strictness signature that this is bottoming
        -- (for the sake of the "empty case scrutinee not known to
        -- diverge for sure lint" warning)
  = Just (Let (NonRec lifted_arg panic_rhs))

  -- The default case for mono rep: Bind @__RUBBISH[prim_reps] \@arg_ty@
  -- See Note [Absent fillers], the main part
  | Just prim_reps <- mb_mono_prim_reps
  = Just (bindNonRec arg (mkTyApps (Lit (rubbishLit prim_reps)) [arg_ty]))

  -- Catch all: Either @arg_ty@ wasn't of form @TYPE rep@ or @rep@ wasn't mono rep.
  -- See (2) in Note [Absent fillers]
