...
--
-- See Note [CPR for recursive data structures] for which recursive DataCons
-- we want to flag.
...
isRecDataCon fam_envs fuel dc
  ...
  where
    ...
    go_ty fuel rec_tc ty
      ...
      -- = go_ty fuel rec_tc _arg_ty <||> go_ty fuel rec_tc _res_ty
          -- Plausible, but unnecessary for CPR.
          -- See Note [CPR for recursive data structures], point (1)
      = Just NonRecursive

      | Just (_tcv, ty') <- splitForAllTyCoVar_maybe ty
      = go_ty fuel rec_tc ty'
          -- See Note [CPR for recursive data structures], point (2)
    ...
    go_tc_app fuel rec_tc tc tc_args
      ...
      |  isPrimTyCon tc
      || isPromotedDataCon tc -- See Note [CPR for recursive data structures], point (2)
      = Just NonRecursive

      | Nothing <- checkRecTc rec_tc tc
      = Just NonRecursive -- we expanded this TyCon once already, no need to test it multiple times

      | Just (_tvs, rhs, _co) <- unwrapNewTyConEtad_maybe tc
          -- See Note [CPR for recursive data structures], points (2) and (3)
      , Just rec_tc' <- checkRecTc rec_tc tc
      = go_ty fuel rec_tc' rhs
      ...
      , Just rec_tc' <- checkRecTc rec_tc tc
      = if fuel >= 0
          -- See Note [CPR for recursive data structures], point (4)
          then combineRecFlags (map (\dc -> go_dc (subWithInf fuel 1) rec_tc' dc) dcs)
          else Nothing -- that's why we track fuel!

      | otherwise
      = warnPprTrace True (text "isRecDataCon: No idea" $$ ppr tc $$ ppr tc_args) $ Just Recursive
