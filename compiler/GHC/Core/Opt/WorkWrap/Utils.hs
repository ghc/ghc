...
Then f gets a strictness sig of <S(L,A,A)><A>. We make a worker $wf thus:
  ...
  $wf xs = case ps of MkT xs _ _ -> <body mentioning xs>
    where
      ...
      zs = __RUBBISH[LiftedRep] @[Int]
      ps = MkT xs ys zs
      w  = __RUBBISH[IntRep] @Int#
...
The story would end here, if it wasn't for two issues:
  ...
  1. In case we have a absent, /lazy/, and /lifted/ arg, we use an error-thunk
     ...
     We also mustn't put thunks (that fill in for an absent value of lifted rep)
     in strict fields, because #16970 establishes the invariant that strict
     fields are always evaluated. That's the reason why 'zs' binds a rubbish
     literal instead of an error-thunk, see #19133.
     ...
     interval, rather than 'C_00'. Hence the 'isStrictDmd' check: No error-thunk
     if it's put in a strict field.
