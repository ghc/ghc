...
mkAbsentFiller mod arg str
  -- We never make a filler for a dictionary type: it might be speculatively
...
(AF4) We never make an absent filler for a dictionary type.
...
evaluated guaranteed-non-bottom values: see
...
must not make an absent filler for `u` either (#27627)!!

TL;DR: we play safe: we never make an absent filler for any dictionary type,
using `isDictTy` rather than `isTerminatingType` to decide:

  * `mkAbsentFiller` returns `Nothing` for any `isDictTy` type, so worker/wrapper
