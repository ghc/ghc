...
When equality ops are replaced with case, we might first need to extend
them. It's a bit ugly we are adapting for a C-- restriction in core, but
...
litEq is_eq = msum
  [ do [Lit lit, expr] <- getArgs
       (expr', lit') <- extend_if_needed lit expr
       platform <- getPlatform
  ...
  where
    extend_if_needed lit expr = do
      LitNumber nt0 number <- pure lit
      (mOp, nt) <- case nt0 of
        LitNumInteger -> mzero -- lifted type
        LitNumNatural -> mzero -- lifted type
        LitNumInt     -> pure (Nothing,             LitNumInt)
        LitNumInt8    -> pure (Just Int8ToIntOp,    LitNumInt)
        LitNumInt16   -> pure (Just Int16ToIntOp,   LitNumInt)
        LitNumInt32   -> pure (Just Int32ToIntOp,   LitNumInt)
        LitNumInt64   -> mzero -- too big
        LitNumWord    -> pure (Nothing,             LitNumWord)
        LitNumWord8   -> pure (Just Word8ToWordOp,  LitNumWord)
        LitNumWord16  -> pure (Just Word16ToWordOp, LitNumWord)
        LitNumWord32  -> pure (Just Word32ToWordOp, LitNumWord)
        LitNumWord64  -> mzero -- too big
      pure ( case mOp of
               Nothing -> expr
               Just op -> OpVal op `App` expr
           , LitNumber nt number
           )
