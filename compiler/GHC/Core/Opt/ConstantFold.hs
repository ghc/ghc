...
seqRule = do
  ...
  -- evaluate uses `seq#` like `seq# (lazy x) s`, and `exprIsHNF` does not look
  -- through `lazy`, so we do it manually here.
  let unwrap_lazy (Var lazy `App` Type {} `App` e)
        | lazy `hasKey` lazyIdKey = e
      unwrap_lazy e               = e
  guard $ exprIsHNF (unwrap_lazy a)
