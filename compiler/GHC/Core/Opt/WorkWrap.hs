...
tryWW dflags fam_envs is_rec fn_id rhs
  ...
  | is_fun && is_eta_exp
  = splitFun dflags fam_envs new_fn_id fn_info wrap_dmds div forced_cpr rhs
  ...
  where
    ...
    (_tm, forced_cpr) = forceCpr seqDmd cpr
...
splitFun dflags fam_envs fn_id fn_info wrap_dmds div cpr rhs
  = WARN( not (wrap_dmds `lengthIs` arity), ppr fn_id <+> (ppr arity $$ ppr wrap_dmds $$ ppr cpr) ) do
    ...
    stuff <- mkWwBodies (initWwOpts dflags fam_envs) rhs_fvs fn_id wrap_dmds use_cpr
    case stuff of
