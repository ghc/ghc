...
tryWW dflags fam_envs is_rec fn_id rhs
  ...
  | is_fun && is_eta_exp
  = splitFun dflags fam_envs new_fn_id fn_info wrap_arg_dmds div term_sig cpr_sig rhs
  ...
  where
    ...
    term_sig             = termInfo fn_info
    cpr_sig              = cprInfo fn_info
...
---------------------
splitFun :: DynFlags -> FamInstEnvs -> Id -> IdInfo -> [Demand] -> Divergence -> TermSig -> CprSig -> CoreExpr
         -> UniqSM [(Id, CoreExpr)]
splitFun dflags fam_envs fn_id fn_info wrap_arg_dmds div term_sig cpr_sig rhs
  ...
  where
    ...
    -- CPR and Term sig describe the fun body, are after applying to idArity args
    body_cpr        = getSig cpr_sig
    body_term       = getSig term_sig
    ...
    (work_term_sig, work_cpr_sig)
      | isJoinId fn_id = (term_sig, cpr_sig)
