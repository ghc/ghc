...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Suppose we see (local or at top-level, join or let)

  f xs = let binds in g as;
  rest

where `xs` don't occur in `binds` and `as` do not mention `binds` and `xs` is
not empty. This pattern commonly arises after Note [Join point loopification].
...
NB: Denesting reverts Note [Join point loopification]; there, `f` is a regular
let and `binds` is a joinrec. But if no specialisation happens, occurrence
analysis will immediately transform back to the loopified form, so no harm done.
