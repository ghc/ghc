...
Wrinkle [trimJoinCont for quasi join points]
  ...
  so we must preserve the continuation (instead of trimming it). Hence the
  special logic in 'trimJoinCont' for quasi join points.

Wrinkle [Casts and join point result types]

  For a quasi join point, the result type of the binder and the body type
  ...
  where
    ...
    -- and Note [Simplifying rules] in GHC.Core.Opt.Simplify.Utils.
                 ...
                 join_ok = case mb_new_id of
                             Just id | JoinPoint { joinPointArity = join_arity } <- idJoinPointHood id
