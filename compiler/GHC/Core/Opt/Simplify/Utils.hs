...
tryEtaExpandRhs mode bndr rhs
  ...
  , want_eta rhs
  = pprTrace "eta expand" (vcat [ ppr bndr <+> equals <+> ppr rhs
                                , ppr arity_type ]) $
  ...
  where
    ...
    -- See Note [Do not eta-expand trivial expressions]
    want_eta (Cast e _)        = want_eta e
    want_eta (Tick _ e)        = want_eta e
    want_eta (App e (Type {})) = want_eta e
    want_eta (Var _)           = False

    -- Eta-expand if there is a value lambda already
    want_eta (Lam b e) = isId b || want_eta e

    -- Otherwise don't a expand PAPs/thunks, unless it
    -- is headed with a one-shot lambda
