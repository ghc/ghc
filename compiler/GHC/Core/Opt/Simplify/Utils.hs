...
This RhsCtxt gives a little bit incentive for small functions to inline.
That incentive is what is needed in case (4).

A good example of a function where this is important is GHC.Num.Integer
where we ended up with calls like this:
...
and the Simplifier sees
    f (integerSigNum a b)
Becuase `f` scrutines `x`, the unfolding guidance for f gives a discount
