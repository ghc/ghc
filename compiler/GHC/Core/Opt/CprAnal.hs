...
extendSigEnv env id sig
  | isTopCprSig sig
  = env { ae_sigs = delVarEnv (ae_sigs env) id } -- See Note [No Top CprSig in SigEnv]
  | otherwise
...
But we have to careful with shadowing: When there already is an entry for 'x'
in the SigEnv and 'x' is supposed to get a Top signature, then we have to
delete the old entry for 'x'.
