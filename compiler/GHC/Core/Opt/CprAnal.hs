...
cprTransformDataConWork fam_envs con args
  ...
  , isRecDataCon fam_envs fuel con /= Just Recursive -- See Note [CPR for recursive data structures]
...
con and the answers of our function:
  ...
  data F = F (F -> Int)                        NonRec  (see point (2))
  data G = G (Int -> G)                        NonRec  (see point (2))
