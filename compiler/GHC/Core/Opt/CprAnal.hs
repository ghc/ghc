...
cprAnalApp env fn args
  ...
  where
    ...
    -- See Note [Nested CPR] for the exprIsHNF
    arg_cprs     = [ if exprIsHNF arg' then ty else topCprType
                   | (ty, arg') <- cprs_w_args
...
cprTransformDataConWork :: DataCon -> [CprType] -> CprType
cprTransformDataConWork con args
  ...
  where
    ...
    extract_cpr (CprType 0 cpr) = cpr
    extract_cpr _               = topCpr -- intervening lambdas
