 ...
 * Any strict binder with product type will have the CPR property according
   ...
   But we can go a little further and improve field binder CPR. Consider

      data T a = MkT !(Int, Int) Int
 ...
 * It's a bit delicate because we're brittly anticipating worker/wrapper here.
   If the case above is scrutinising something other than an argument to the
 ...
 * We used to give the case binder the CPR property unconditionally instead
   of deriving it from the case scrutinee.
