...
cprAnal' env (Case scrut case_bndr ty alts)
  ...
  where
    ...
    mb_field_dmds      = optimisticFieldCprDemands env scrut scrut_ty
    (alt_tys, alts')   = mapAndUnzip (cprAnalAlt env' mb_field_dmds) alts
...
cprAnalAlt
  :: AnalEnv
  -> Maybe [Demand] -- ^ Demand on the fields if the scrutinee is a variable and
                    -- Note [Optimistic field binder CPR] applies
  -> Alt Var        -- ^ current alternative
  -> (CprType, Alt Var)
cprAnalAlt env mb_var_scrut_dmd (Alt con bndrs rhs)
  = (rhs_ty, Alt con bndrs rhs')
  where
    env_alt
      | DataAlt _dc <- con, Just dmds <- mb_var_scrut_dmd
      ...
      = foldl' do_one_field env (zipEqual "cprAnalAlt" (filter isId bndrs) dmds)
      | otherwise
      = env
...
-- Tests (A) and (B) of Note [Optimistic field binder CPR] and
-- returns the alleged demand of the function on field binders,
-- from which 'giveStrictFieldsCpr' infers their CPR property.
optimisticFieldCprDemands :: AnalEnv -> CoreExpr -> CprType -> Maybe [Demand]
optimisticFieldCprDemands env scrut scrut_ty
  | case_will_cancel
  , Just id <- as_id scrut
  = fst <$> wantToUnboxArg env id (idDemandInfo id)
  | otherwise
  = Nothing
  where
    ...
    -- variable!
    as_id (Var id)   | isLocalId id = Just id
    as_id (Cast e _)                = as_id e
    as_id _                         = Nothing
...
extendSigEnvForDemand env id dmd
  | Just (_, DataConPatContext { dcpc_dc = dc }) <- wantToUnboxArg env id dmd
  = extendSigEnv env id (CprSig (conCprType (dataConTag dc)))
  | otherwise
  = env

wantToUnboxArg :: AnalEnv -> Id -> Demand -> Maybe ([Demand], DataConPatContext)
wantToUnboxArg env id dmd
...
Lacking Nested CPR, we have to guess a bit, by looking for
  ...
  (C) Strict demand on the field `y` as per the demand on the scrutinee

((A) and (B) are tested in 'optimisticFieldCprDemands'.)

While (A) is a necessary condition to give a field the CPR property and (C)
is accurate as long as the scrutinee really is a parameter, (B) is too lax.
That may lead to unsound analysis results and thus reboxing in the wrapper,
like in
...
Lacking Nested CPR and multiple levels of unboxing, only the outer box of `z`
will be available and a case on `y` won't actually cancel away. But it's simple,
and nothing terrible happens if we get it wrong. e.g. #10694.
