...
Hence we don't analyse or annotate data structures in 'cprAnalBind', which has
a special case for the 'isDataStructure' case, which is triggered for bindings
which satisfy
...
instead we keep on cprAnal'ing through *expandable* unfoldings for these data
