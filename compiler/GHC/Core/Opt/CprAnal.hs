...
isTerminates :: TermFlag -> Bool
isTerminates Terminates   = True
isTerminates MightDiverge = False
...
cprTransformDataConWork con args
  ...
  where
    ...
    extract_cpr (tf, CprType n cpr) str
      | n == 0, term_or_strict tf cpr = cpr -- doesn't terminate or intervening lambdas
      | otherwise                     = topCpr
