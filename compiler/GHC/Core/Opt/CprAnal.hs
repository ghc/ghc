...
cprAnalAlt env scrut_ty (Alt con bndrs rhs)
  ...
  where
    env_alt
      | DataAlt dc <- con
      -- We used to give field binders the CPR property if they are demanded
      -- strictly. See Historic Note [Optimistic field binder CPR]
...
Note that
  ...
  * We can even give fields of parameters the CPR property if they unbox. E.g.
