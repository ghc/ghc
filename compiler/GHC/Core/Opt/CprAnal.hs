...
cprTransform env args id
  ...
  where
    sig
      ...
      = transformSig (idUnique id) str_sig sig args
      ...
      | isGlobalId id
      = transformSig (idUnique id) (idStrictness id) (getAnalAnnotation id) args
      | otherwise
      = top
...
class (Eq l, Outputable l) => ForwardLattice l where
  ...
  transformSig :: Unique -> StrictSig -> Sig l -> [l] -> l
...
instance ForwardLattice CAT where
  ...
  transformSig uniq str_sig (Sig (CAT sig_t sig_c)) args =
    CAT (transformSig    uniq str_sig (Sig sig_t) arg_terms)
        (cprTransformSig uniq         (Sig sig_c) arg_cprs)
    where
