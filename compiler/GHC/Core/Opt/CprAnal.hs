...
cprAnal' env args (Lam var body)
  ...
  where
    ...
    env'                 = extendAnalEnvForDemand env var (idDemandInfo var) arg_ty'
...
cprFix top_lvl env str orig_pairs
  ...
  where
    init_sig id = mkCprSig (idArity id) recFunTerm botCpr
...
pruneSig d (CprSig cpr_ty)
  = CprSig $ cpr_ty { ct_cpr  = pruneDeepCpr  d (ct_cpr cpr_ty)
                    -- TODO: We need the lubTerm with the initial termination because
                    --       of functions like iterate, which we would CPR
                    --       multiple levels deep, thereby changing termination
                    --       behavior.
                    , ct_term = pruneDeepTerm d (ct_term cpr_ty `lubTerm` recFunTerm) }
