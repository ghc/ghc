...
dmdAnal' env dmd (Case scrut case_bndr ty [Alt alt bndrs rhs])
  ...
  = let
        ...
        -- FIXME NB: If case_bndr_dmd is absDmd, boxity will say Unboxed, which is
        ...
        !(!bndrs', !scrut_sd)
          ...
          = (setBndrsDemandInfo bndrs id_dmds, mkProd Unboxed id_dmds)
          | otherwise
...
because we were inferring a too optimistic boxity before.
...
thing for now is to put an unboxing demand on recursive function bodies and a
boxed demand on non-recursive ones.
-}
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
You might ask whether lazy, unboxed demands like `L!P(..)` should generally be
forbidden. They should not, as the following example should convince you of:
...
A related variant of this is when `h` comes from a strict field of a record that
`foo` is strict in:
...
This is exactly the situation of Note [Add demands for strict constructors] and
will be dealt with by 'finaliseBoxity'.
