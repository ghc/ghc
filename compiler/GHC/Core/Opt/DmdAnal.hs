...
-- precise excpetions.
-- See Note [Only track precise exceptions from the RealWorld].
...
{- Note [Only track precise exceptions from the RealWorld]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Demand analysis infers whether or not an expression throws a precise exception
in its Divergence, to preserve precise exception semantics.
See Note [Precise exceptions and strictness analysis] in GHC.Types.Demand.

But while conservative, it is too imprecise! It will fail to infer that 'error'
can't throw a precise exception, for example, giving it 'exnDiv'.
As for why, you have to follow the Exception instance of the thrown imprecise
ErrorCall exception, until it finally calls 'GHC.Fingerprint.getFileHash' under
an unsafePerformIO umbrella, which may very well throw a precise exception when
it can't find the file.
See https://gitlab.haskell.org/ghc/ghc/-/merge_requests/2525#note_260024 for
more details.

Why is that bad? Well, consider

  let err = error "boom" -- is inferred exnDiv
  in case writeMutVar# var err of s -> x

Is this strict in 'x'? I'd Yes, very much! But without any further action,
'exnDiv' will "taint" the whole IO action and the expression is inferred lazy
in 'x'.

Our countermeasure is to limit taint based on type: We assume that any
expression with a type that doesn't need to produce a new 'State# Realworld' if
scrutinised in a Case can't actually throw a precise exception. This assumption
ignores black magic like unsafePerformIO and manual fabrication of the
RealWorld token, but we don't give any precise exception guarantees in this
scenario anyway.

In the implementation, when annotating 'error' with its demand signature (for
which we have inferred Divergence 'exnDiv', for the sake of the example) we
call 'setIdStrictnessClearExn'. That will consult 'tryClearPreciseException' to
see if we can relax 'exnDiv' to 'botDiv', by checking its return type 'a'
with 'forcesRealWorld'. That type clearly does not embed a 'State# RealWorld'
token, so we clear the 'exnDiv' to 'botDiv'. Then we will successfully infer
'botDiv' for 'err' and ultimately the expression in question is strict in 'x'.

We also limit taint based on type of the scrutinee when analysing Case
expressions.
-}
...
setIdStrictnessClearExn :: AnalEnv -> Id -> StrictSig -> Id
-- See Note [Only track precise exceptions from the RealWorld]
setIdStrictnessClearExn env id sig
