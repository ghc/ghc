...
In the following analysis "flowchart" we abbreviate
  T(op) = may (or may not) throw precise exception
  B(ot) = may not throw precise exception
For an expression @f a1 ... an :: ty@ we determine that
  1. B if ty is *not* @State# RealWorld@ or an unboxed tuple thereof.
     This check is done by 'forcesRealWorld'.
     (Why not simply unboxed pairs as above? This is motivated by T13380{d,e}.)
  2. B if f is a PrimOp, and it is *not* raiseIO#
  3. B If f is an unsafe FFI call, then
  _. T otherwise "give up"

It is sound to return B in those cases, because
...
We *need* to return B in those cases, because
