...
dmdAnalBindLetUp top_lvl env id rhs anal_body = S3 final_ty (NonRec <$> id' <*> rhs') body'
  where
    ...
    S2 body_ty' id_dmd = findBndrDmd env notArgOfDfun body_ty id
...
dmdAnalBindLetDown top_lvl env dmd bind anal_body = case bind of
  ...
  where
    push_taint_to_bind (Clean (S3 ty bind a)) = S3 ty (Clean bind) a
    push_taint_to_bind (Dirty (S3 ty bind a)) = S3 ty (Dirty bind) a
    do_rest env' lazy_fv pairs' build_bind = push_taint_to_bind $ do -- Tainted.do
      let S2 body_ty body' = anal_body env'
      -- see Note [Lazy and unleashable free variables]
      let dmd_ty = addLazyFVs body_ty lazy_fv
      -- pairs' has updated idDmdSig. Now annotate the idDemandInfo:
      pairs_w_sig <- pairs'
      let S2 final_ty id_dmds = findBndrsDmds env' dmd_ty (strictMap fst pairs_w_sig)
      let do_one dmd (id1, !rhs1) = do
            !id2 <- setBindIdDemandInfo top_lvl id1 dmd
            pure (id2, rhs1)
      -- Important to force this as build_bind might not force it.
      !pairs2 <- strictZipWithM do_one id_dmds pairs_w_sig
      pure (S3 final_ty (build_bind pairs2) body')
...
-- Two common cases of the analysis function:

trivialCase :: DmdType -> CoreExpr -> SPair DmdType (Chgd CoreExpr)
trivialCase ty e = S2 ty (Clean e)

congruentCase :: DmdType -> CoreExpr -> (CoreExpr -> CoreExpr) -> Chgd CoreExpr -> SPair DmdType (Chgd CoreExpr)
congruentCase ty e wrap body' = S2 ty (setWhenClean e $ wrap <$> body')
...
dmdAnal' _ _ e@Lit{}         = trivialCase nopDmdType e
dmdAnal' _ _ e@Type{}        = trivialCase nopDmdType e -- Doesn't happen, in fact
dmdAnal' _ _ e@(Coercion co) = trivialCase (unitDmdType (coercionDmdEnv co)) e
dmdAnal' env dmd e@(Var var) = trivialCase (dmdTransform env var dmd) e

dmdAnal' env dmd e@(Cast body co)
  = congruentCase (dmd_ty `plusDmdType` mkPlusDmdArg (coercionDmdEnv co)) e (flip Cast co) body'
  where
    S2 dmd_ty body' = dmdAnal env dmd body

dmdAnal' env dmd e@(Tick t body)
  = congruentCase dmd_ty e (Tick t) body'
  where
    S2 dmd_ty body' = dmdAnal env dmd body

dmdAnal' env dmd e@(App fun ty@Type{})
  = congruentCase fun_ty e (flip App ty) fun'
  where
...
--         , text "overall res dmd_ty =" <+> ppr (res_ty `bothDmdType` arg_ty) ])
    S2 (res_ty `plusDmdType` arg_ty) (setWhenClean e $ App <$> fun' <*> arg')
...
dmdAnal' env dmd e@(Lam var body)
  ...
  = let
    ...
    in
    congruentCase body_ty e (Lam var) body'
  ...
  = let (n, body_dmd)    = peelCallDmd dmd
        ...
        S2 lam_ty var'   = annotateLamIdBndr env notArgOfDfun body_ty var
        new_dmd_type = multDmdType n lam_ty
    in
    S2 new_dmd_type (setWhenClean e $ Lam <$> var' <*> body')
...
dmdAnal' env dmd e@(Case scrut case_bndr ty alts@[Alt alt bndrs rhs])
  ...
  = let -- See Note [Space Leaks in Demand Analysis] for abundant use of S2 and bangs
        ...
        S2 alt_ty2 case_bndr_dmd = findBndrDmd env False alt_ty1 case_bndr
...
dmdAnal' env dmd l@(Let bind body)
  = S2 final_ty (setWhenClean l $ Let <$> bind' <*> body')
  where
...
dmdAnalSumAlt env dmd case_bndr alt@(Alt con bndrs rhs)
  ...
  = S2 alt_ty (setWhenClean alt $ Alt con <$> new_ids <*> rhs')
...
setBndrsDemandInfo :: [Var] -> [Demand] -> Chgd [Var]
...
annotateBndr env dmd_ty var
  ...
  where
    ...
    S2 dmd_ty' dmd = findBndrDmd env False dmd_ty var
  ...
  where
    ...
    final_ty = case maybeUnfoldingTemplate (idUnfolding id) of
                 ...
                 Just unf -> main_ty `plusDmdType` unf_ty
                          where
                             S2 unf_ty _ = dmdAnalStar env dmd unf

    main_ty = addDemand dmd dmd_ty'
    S2 dmd_ty' dmd = findBndrDmd env arg_of_dfun dmd_ty id
  ...
  where
    ...
    go dmd_ty (b:bs)
      | isId b    = let S2 dmd_ty1 dmds = go dmd_ty bs
                        S2 dmd_ty2 dmd  = findBndrDmd env False dmd_ty1 b
                    in S2 dmd_ty2  (dmd : dmds)
      | otherwise = go dmd_ty bs

findBndrDmd :: AnalEnv -> Bool -> DmdType -> Id -> SPair DmdType Demand
...
When either fun' or arg' is 'Dirty', then soll will the new 'App'. If both
...
Note that `rhs1` is quite likely to be different than `rhs2`! How do we know
it has reached a fixed-point? Consider what would happen in the third iteration:
The arguments to `dmdAnal` are *exactly the same* as in the second iteration
(modulo rhs1/rhs2, which just changes because of Note [Initialising strictness]),
so we'll get exactly the same result! I.e., `rhs2` = `rhs3`, `dt2` = `dt3`.
That's why we can ignore whether or not `rhs2` is still dirty when detecting the
fixed-point.
