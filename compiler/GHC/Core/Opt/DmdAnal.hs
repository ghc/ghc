...
which were all ultimatey forced at the end of the analysis.

This was because the return type of the analysis was a lazy pair. There were quite
a few places which needed to be forced so that eventually in dmdAnal', in the
`Let` case, the result of analysing the bound expression was forced before analysing
the body.
...
Further to this, the updating of Ids was not forced because the result of updating
...
back into core expressions so that we can collect the environments more eagerly.

The net result of all these improvements is the peak live memory usage of compiling
jsaddle-dom decreases about 4GB, a bunch of bytes allocated benchmarks also
decrease because we allocate a lot less thunks which we immediately overwrite and
