...
In the following analysis "flowchart" we abbreviate
  T(rue) = may (or may not) throw precise exception. Top of the lattice.
  F(alse) = may not throw precise exception. Bottom of the lattice.
For an expression @f a1 ... an :: ty@ we determine that
  1. F if ty is *not* @State# RealWorld@ or an unboxed tuple thereof.
     This check is done by 'forcesRealWorld'.
     (Why not simply unboxed pairs as above? This is motivated by T13380{d,e}.)
  2. F if f is a PrimOp, and it is *not* raiseIO#
  3. F If f is an unsafe FFI call, then
  _. T otherwise "give up"

It is sound to return F in those cases, because
...
We *need* to return F in those cases, because
