...
-- | A very syntactic analysis of whether an expression might throw a precise
-- exception when evaluated. Only true for an IO action that
--   * is not a 'PrimOp' application other than 'raiseIO#', OR
--   * is not an unsafe foreign call
--
...
exprMayThrowPreciseException envs e
  ...
  = case collectArgs e of
      (Var f, _)
        ...
        -> op == RaiseIOOp
        -- We regard unsafe foreign calls as "may not throw precise exception".
        -- So safe (and interruptible) foreign calls may throw.
        | Just fcall <- isFCallId_maybe f
        -> isSafeForeignCall fcall
      _ -> True
