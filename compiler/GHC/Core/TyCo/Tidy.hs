...
tidy_tycovar_upd tidy_env var = (# tidy_env', upd_var #)
  where
    ...
    -- extend the TidyOccEnv and potentially update the OccName
    !(# occ_env', upd_occ #) = tidy_OccName_upd occ_env (getHelpfulOccName var)
    -- potentially update the var name and type
    upd_name = tidyNameOcc_upd (varName var) upd_occ
    upd_var  = rebuild var (setVarName var) (# upd_name #)
                `bindUpd` updateVarType_upd (tidy_type_upd tidy_env)
            
    subst' = case upd_var of
      NoUpdate _ -> subst
      Update v'  -> extendVarEnv subst var v'
