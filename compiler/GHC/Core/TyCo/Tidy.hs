...
tidyVarBndr tidy_env var = case tidy_tycovar_upd tidy_env var of
  (# tidy_env', var' #) -> (tidy_env', unUpd var' )
...
tidy_tycovar_upd tidy_env var = (# tidy_env', upd_var #)
  where
    ...
    subst'           = extendVarEnv subst var (unUpd upd_var)
    ...
    !(# occ_env', tidy_occ #) = tidy_OccName_upd occ_env (unUpd helpful_occ)
    upd_occ = current_occ <~~ const
                          <~$ tidy_occ
                          <~* helpful_occ

    -- tidy the Name with the given OccName and finally update the Var
    upd_name = tidyNameOcc_upd (varName var) upd_occ
    upd_var  = var <~~ (\name ty -> (setVarName var name) { varType = ty })
                   <~$ upd_name
                   <~* tidy_type_upd tidy_env (varType var)
...
avoidNameClashes tvs (occ_env, subst)
  ...
  where
    occs = map (\tv -> unUpd (getHelpfulOccName tv)) tvs
...
updateVarType_upd upd var = var <~~ (\t -> var { varType = t})
                                <~$ upd (varType var)
---------------
...
tidyTypes env tys = Upd.runUpd (tidy_types_upd env) tys
...
tidyType env ty = Upd.runUpd (tidy_type_upd env) ty
...
tidy_type_upd = go
  where
    go env t = case t of
      LitTy _             -> NoUpdate t
      TyVarTy tv          -> t <~~ TyVarTy
                               <~$ tidy_TyCoVar_upd env tv
      TyConApp _ []       -> NoUpdate t
      TyConApp tycon tys  -> t <~~ TyConApp tycon
                               <~$ tidy_types_upd env tys
      AppTy fun arg       -> t <~~ AppTy
                               <~$ go env fun
                               <~* go env arg
      FunTy af w arg res  -> t <~~ FunTy af
                               <~$ go env w
                               <~* go env arg
                               <~* go env res
      CastTy ty co        -> t <~~ CastTy
                               <~$ go env ty
                               <~* tidy_co_upd env co
      CoercionTy co       -> t <~~ CoercionTy
                               <~$ tidy_co_upd env co
...
tidyCo env co = Upd.runUpd (tidy_co_upd env) co
...
tidy_co_upd = go
  where
    ...
    go env@(_, subst) c = case c of
      Refl ty                 -> c <~~ Refl
                                   <~$ tidy_type_upd env ty
      GRefl r ty mco          -> c <~~ GRefl r
                                   <~$ tidy_type_upd env ty
                                   <~* go_mco env mco
      TyConAppCo r tc cos     -> c <~~ TyConAppCo r tc
                                   <~$ Upd.updateList (go env) cos
      AppCo co1 co2           -> c <~~ AppCo
                                   <~$ go env co1
                                   <~* go env co2
      ForAllCo tv h co        -> c <~~ ForAllCo
                                   <~$ tvp
                                   <~* go env h
                                   <~* tidy_co_upd envp co
      ...
      HoleCo _                -> NoUpdate c
      AxiomInstCo con ind cos -> c <~~ AxiomInstCo con ind
                                   <~$ Upd.updateList (go env) cos
      UnivCo p r t1 t2        -> c <~~ UnivCo
                                   <~$ go_prov env p
                                   <~* NoUpdate r
                                   <~* tidy_type_upd env t1
                                   <~* tidy_type_upd env t2
      SymCo co                -> c <~~ SymCo
                                   <~$ go env co
      TransCo co1 co2         -> c <~~ TransCo
                                   <~$ go env co1
                                   <~* go env co2
      NthCo r d co            -> c <~~ NthCo r d
                                   <~$ go env co
      LRCo lr co              -> c <~~ LRCo lr
                                   <~$ go env co
      InstCo co ty            -> c <~~ InstCo
                                   <~$ go env co
                                   <~* go env ty
      KindCo co               -> c <~~ KindCo
                                   <~$ go env co
      SubCo co                -> c <~~ SubCo
                                   <~$ go env co
      AxiomRuleCo ax cos      -> c <~~ AxiomRuleCo ax
                                   <~$ Upd.updateList (go env) cos

    go_prov env p = case p of
      PhantomProv co          -> p <~~ PhantomProv    <~$ go env co
      ProofIrrelProv co       -> p <~~ ProofIrrelProv <~$ go env co
      PluginProv _            -> NoUpdate p
      CorePrepProv            -> NoUpdate p
