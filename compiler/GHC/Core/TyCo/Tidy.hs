...
We used to use `strictMap` to implement tidyTypes or tidyCos (for example) but
it's better to write specialised functions, otherwise the mapping function (e.g.
"tidyType env") or its parameters (e.g. the TidyEnv) may be allocated, defeating
what we obtained with worker/wrapper.
