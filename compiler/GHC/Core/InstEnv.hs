...
(IL1) Find all instances `I` that *match* the target constraint (that is, the target
      ...
      the /candidates/ (all_matches in lookupInstEnv).  Also find all non-incoherent
      instances that *unify with but do not match* the target constraint
      (all_unifs in lookupInstEnv).
...
(IL6) Otherwise there is exactly one candidate remaining.
      Now find all non-incoherent instances that unify with the target constraint,
      but do not match it. Such non-candidate instances might match
      when the target constraint is further instantiated.  These are returned
      from lookupInstEnv as final_unifs :: PotentialUnifiers.
...
but we also need to report whether the match was canonical
as described in Note [Coherence and specialisation: overview].

For this, we look *only* at the OverlapFlag of the instance being matched.
For example:
