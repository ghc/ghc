...
data ArgSummary
  ...
  | ValueArg      -- Arg is a con-app or PAP
                  ...
                  --                          in GHC.Core.Opt.Simplify.Inline
  | ConArg !DataCon
  ...
  where
    ...
    size_up !depth arg_comps (Case e _ _ alts)
        ...
        = let
            ...
            -- NB: sizeZero is a unit for addAltSize, so folding with it is
            -- equivalent to foldr1, and TooBig still bails out early.
            added_alt_sizes = (foldr addAltSize sizeZero alt_sizes)
    ...
    size_up_call !arg_comps fun val_args voids
       = case idDetails fun of
           ...
           ClassOpId cls _               -> classOpSize opts cls arg_comps val_args
           _ | fun `hasKey` buildIdKey   -> buildSize
             | fun `hasKey` augmentIdKey -> augmentSize
             | otherwise                 -> funSize opts arg_comps fun (length val_args) voids
...
classOpSize :: UnfoldingOpts -> Class -> UnVarSet -> [CoreExpr] -> ExprSize
-- See (IA1) in Note [Interesting arguments] in GHC.Core.Opt.Simplify.Inline
...
classOpSize opts cls !top_args (dict_arg:other_val_args)
  ...
  where
    ...
    arg_discount (Tick _ arg)   = arg_discount arg
    arg_discount (Var dict) | dict `elemUnVarSet` top_args
                                = unitVarEnv dict (classOpArgDiscount dict_discount)
    arg_discount _              = mempty
