...
* A scaling of >= 15 is required to avoid large regressions on some nofib benchmarks.
  (>100% allocation increases for)
...
tryUnfolding dflags !case_depth id lone_variable
             ...
             is_wf is_exp guidance
 = case guidance of
     UnfNever -> pprTraceM "UnfNever" (ppr id) >> traceInline dflags id str (text "UnfNever") Nothing
