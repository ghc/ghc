...
checkProcessArgsResult flags
  = when (notNull flags) $
    ...
    where mkMsg (L loc flag)
              = mkPlainMsgEnvelope (SevError NoErrReason) loc $
...
optionsErrorMsgs unhandled_flags flags_lines _filename
  ...
  where unhandled_flags_lines :: [Located String]
        ...
        mkMsg (L flagSpan flag) =
            mkPlainMsgEnvelope (SevError NoErrReason) flagSpan $
...
throwErr loc doc =
  throw $ mkSrcErr $ unitBag $ mkPlainMsgEnvelope (SevError NoErrReason) loc doc
