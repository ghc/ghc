...
string_qual_inter_begin :: Action
string_qual_inter_begin span buf len buf2 = do
  let modName = ModuleName $ lexemeToFastString buf modNameLen
  let meta = defaultStrMeta{strMetaQualified = Just modName}
  L span (ITstringInterBegin meta) <$ string_inter_begin span buf len buf2
  where
    -- Module name length = total length minus length of `.s"`
    modNameLen = len - 3
...
data LayoutContext
  = NoLayout
  | InterStringLayout -- FIXME(bchinn): is this the best place?
  | Layout !Int !GenSemic
  deriving Show
