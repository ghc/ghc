...
lexString = lexStringWith (fromRight . processCharsSingle)
...
fromRawLexedStringMulti s =
  case processCharsInterMulti $ mapRaw unRawLexedString s of
    Just s' -> s'
...
lexMultilineString = lexStringWith processCharsMulti

processCharsMulti :: String -> Maybe String
processCharsMulti = fmap from . processCharsInterMulti . to
  where
...
--
-- `src` and `x` here will probably be instantiated as `SourceText` and
...
-- multiline strings.
processCharsInterMulti :: InterMultiString src x -> Maybe (InterMultiString src x)
processCharsInterMulti =
...
rmCommonWhitespacePrefix s0 =
  ...
  -- the prefix). So we can use a plain `mapRaw` to just process the string parts, because
...
collapseOnlyWsLines = \case
  ...
  Left (src, s) : rest ->
    case rest of
      -- If there are two string parts in a row, we can't know if we can
      -- collapse this part without analyzing the next part. This shouldn't
      -- happen anyway, so just panic for now.
...
rmFirstNewline = go0
  where
    go0 = \case
      Left (src, s) : ps -> Left (src, go1 s) : ps
      ps -> ps
    go1 = \case
      '\n' : s -> s
      s -> s
