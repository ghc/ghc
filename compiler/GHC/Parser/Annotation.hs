...
data Anchor = Anchor        { anchor :: RealSrcSpan  }
                                 ...
                                 -- the annotations.
            | MovedAnchor   { anchor :: RealSrcSpan, origAnchor :: RealSrcSpan }
            | DeletedAnchor { anchor :: RealSrcSpan, deletedAnchor :: RealSrcSpan }
        deriving (Data, Eq, Show)

spanAsAnchor :: SrcSpan -> Anchor
spanAsAnchor s  = Anchor (realSrcSpan s)
...
noComments ::ApiAnnCO
noComments = ApiAnn (Anchor placeholderRealSpan) NoApiAnns []
...
comment loc cs = ApiAnn (Anchor loc) NoApiAnns cs
...
addAnns ApiAnnNotUsed as cs = ApiAnn (Anchor placeholderRealSpan) as cs
...
widenAnchor (Anchor s) as = Anchor (widenRealSpan s as)
widenAnchor _ _ = panic "widenAnchor"

widenAnchorR :: Anchor -> RealSrcSpan -> Anchor
widenAnchorR (Anchor s) r = Anchor (combineRealSrcSpans s r)
widenAnchorR _ _ = panic "widenAnchor"
...
instance Semigroup Anchor where
  Anchor r1 <> Anchor r2 = Anchor (combineRealSrcSpans r1 r2)
  l <> r = panic $ "Anchor semigroup" ++ show (l,r)
...
instance Outputable Anchor where
  ppr (Anchor a)          = text "Anchor" <+> ppr a
  ppr (MovedAnchor a b)   = text "MovedAnchor" <+> ppr a <+> ppr b
  ppr (DeletedAnchor a b) = text "DeletedAnchor" <+> ppr a <+> ppr b
