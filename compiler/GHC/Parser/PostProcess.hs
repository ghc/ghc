...
checkPat loc (L l e@(PatBuilderVar (L ln c))) tyargs args
  ...
  | not (null tyargs) || (not (null args) && patIsRec c) =
      patFail (locA l) . PsErrParseErrorInPat e . Just $ PsParseErrorInPatDetails {
              peipd_tyargs     = tyargs
...
checkPat loc (L l e) [] [] = do
  p <- checkAPat loc e
  return (L l p)
checkPat loc e _ _ = patFail (locA loc) (PsErrParseErrorInPat (unLoc e) Nothing)
...
checkAPat loc e0 = do
 ...
 case e0 of
   ...
   _           -> patFail (locA loc) (PsErrParseErrorInPat e0 Nothing)
...
instance DisambECP (PatBuilder GhcPs) where
  ...
  mkHsNegAppPV l (L lp p) anns = do
    lit <- case p of
      ...
      _ -> patFail l $ PsErrParseErrorInPat p (Just $ PsParseErrorInPatDetails [] 0 False True)
    cs <- getCommentsFor l
    let an = EpAnn (spanAsAnchor l) anns cs
