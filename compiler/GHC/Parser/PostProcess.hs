...
addUnpackednessP (L lprag (UnpackednessPragma anns prag unpk)) t = do
  let l' = combineSrcSpans lprag (getLoc t)
      t' = addUnpackedness (prag, unpk) t
  addAnnsAt l' anns
  return (L l' t')
...
class DisambTD b where
  ...
  mkInfixTyAppPV :: LHsType GhcPs -> Located RdrName -> LHsType GhcPs -> PV (Located b)
  mkInfixTyUnpackednessPV :: Located UnpackednessPragma -> Located b -> PV (Located b)
...
instance DisambTD (HsType GhcPs) where
  ...
  mkInfixTyAppPV t1 op t2 = return (mkLHsOpTy t1 op t2)
  mkInfixTyUnpackednessPV = addUnpackednessP
...
--
--   Then it will be finalized into a DataConDef.
data DataConBuilder
...
finalizeDataConBuilder :: DataConBuilder -> DataConDef

-- Detect when the record syntax is used:
--   data T = MkT { ... }
finalizeDataConBuilder (PrefixDataConBuilder flds data_con)
  | [L l_t (HsRecTy _ fields)] <- toList flds
  = DataConDef data_con (RecCon (L l_t fields))

-- Normal prefix constructor, e.g.  data T = MkT A B C
finalizeDataConBuilder (PrefixDataConBuilder flds data_con)
  = DataConDef data_con (PrefixCon (map hsLinear (toList flds)))

-- Infix constructor, e.g. data T = Int :! Bool
finalizeDataConBuilder (InfixDataConBuilder lhs data_con rhs)
  = DataConDef data_con (InfixCon (hsLinear lhs) (hsLinear rhs))
...
instance DisambTD DataConBuilder where
  ...
  mkInfixTyAppPV lhs (L l_tc tc) rhs = do
  ...
  mkInfixTyUnpackednessPV unpk constr_stuff
