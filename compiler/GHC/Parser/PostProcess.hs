...
mkMultField :: EpToken "%" -> LHsType GhcPs -> TokDcolon -> LHsType GhcPs -> HsConFieldSpec GhcPs
mkMultField pct (L _ (HsTyLit _ (HsNumTy (SourceText (unpackFS -> "1")) 1))) col t
  -- See #18888 for the use of (SourceText "1") above
  = mkConFieldSpec (HsLinearAnn (EpPct1 pct1 (EpColon col))) t
  where
    -- The location of "%" combined with the location of "1".
    pct1 :: EpToken "%1"
    pct1 = epTokenWidenR pct (locA (getLoc t))
mkMultField pct mult col t = mkConFieldSpec (HsExplicitMult (pct, EpColon col) mult) t
