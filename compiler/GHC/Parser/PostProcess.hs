...
add_where an@(AddApiAnn _ rs) ApiAnnNotUsed
  = ApiAnn (Anchor rs) (AnnList Nothing Nothing [an] []) []
  ...
  where
    ...
    newAnns (SrcSpanAnn ApiAnnNotUsed l) (ApiAnn as (AnnParen _ o c) cs) =
      let
        ...
        an = (ApiAnn (Anchor lr) (NameAnn NameParens o (realSrcSpan l) c []) cs)
    ...
    newAnns (SrcSpanAnn (ApiAnn ap (AnnListItem ta) csp) l) (ApiAnn as (AnnParen _ o c) cs) =
      let
        ...
        an = (ApiAnn (Anchor lr) (NameAnn NameParens o (realSrcSpan l) c ta) (csp ++ cs))
      in SrcSpanAnn an (RealSrcSpan lr Nothing)
