  ...
  where
    ...
    recFieldToProjUpdate (L l (HsRecField anns (L _ (FieldOcc _ (L loc rdr))) arg pun)) =
        ...
        let f = occNameFS . rdrNameOcc $ rdr
            fl = HsFieldLabel noAnn f -- AZ: what about the ann?
        in mkRdrProjUpdate l (L (locA loc) [L (locA loc) fl]) (punnedVar f) pun anns
        where
...
mkRdrGetField :: SrcSpan -> LHsExpr GhcPs -> Located (HsFieldLabel GhcPs)
  -> ApiAnnCO -> LHsExpr GhcPs
mkRdrGetField loc arg field anns =
  L (noAnnSrcSpan loc) HsGetField {
      gf_ext = anns
    , gf_expr = arg
    , gf_field = field
    }
