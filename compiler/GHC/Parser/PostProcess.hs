...
add_where an@(AddEpAnn _ (EpaSpan rs _rb)) (EpAnn a (AnnList anc o c r t) cs) cs2
  | valid_anchor a
...
add_where an@(AddEpAnn _ (EpaSpan rs rb)) EpAnnNotUsed cs
  = EpAnn (EpaSpan rs rb)
           (AnnList (Just $ EpaSpan rs rb) Nothing Nothing [an] []) cs
