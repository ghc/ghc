...
import GHC.LanguageExtensions (Extension(RecursiveDo, OverloadedRecordUpdate))
...
mkStandaloneKindSig loc lhs rhs anns =
  ...
  where
    check_lhs_name v@(unLoc->name) =
      ...
      then return v
      else addFatalError $ mkParserErrorMessage (getLocA v) $
             PsMalformedExprMessage (PsUnexpectedQualifiedConstructor (unLoc v))
    check_singular_lhs vs =
      case vs of
        [] -> panic "mkStandaloneKindSig: empty left-hand side"
        [v] -> return v
        _ -> addFatalError $ mkParserErrorMessage (getLoc lhs) $
               PsExtensionMessage (PsMultipleNamesInStandaloneKindSignature vs)
    ...
    parse_role (L loc_role (Just role))
      = case lookup role possible_roles of
          ...
          Nothing         ->
            ...
            in
            addFatalError $ mkParserErrorMessage loc_role $
              PsMalformedExprMessage (PsIllegalRoleName role nearby)
...
fromSpecTyVarBndr bndr = case bndr of
  ...
  where
    ...
    check_spec InferredSpec  loc = addFatalError $ mkParserErrorMessage (locA loc) $
                                     PsMalformedExprMessage PsInferredTypeVarNotAllowed
...
cvBindsAndSigs fb = do
  ...
  where
    ...
    drop_bad_decls (L l (SpliceD _ d) : ds) = do
      addError $ mkParserErrorMessage (locA l) $ PsExtensionMessage (PsDeclSpliceNotAtTopLevel d)
      drop_bad_decls ds
...
tyConToDataCon (L loc tc)
  ...
  | otherwise
  = Left $ mkParserErrorMessage (locA loc) $ PsMalformedExprMessage (PsNotADataCon tc)
  where
    occ = rdrNameOcc tc
...
mkPatSynMatchGroup (L loc patsyn_name) (L ld decls) =
  ...
  where
    ...
    extraDeclErr loc decl =
        addFatalError $ mkParserErrorMessage loc $
          PsExtensionMessage (PsNoSingleWhereBindInPatSynDecl patsyn_name decl)

    wrongNameBindingErr loc decl =
      addFatalError $ mkParserErrorMessage loc $
          PsExtensionMessage (PsInvalidWhereBindInPatSynDecl patsyn_name decl)

    wrongNumberErr loc =
      addFatalError $ mkParserErrorMessage loc $
        PsExtensionMessage (PsEmptyWhereInPatSynDecl patsyn_name)

recordPatSynErr :: SrcSpan -> LPat GhcPs -> P a
recordPatSynErr loc pat =
    addFatalError $ mkParserErrorMessage loc $
      PsExtensionMessage (PsRecordSyntaxInPatSynDecl pat)
...
checkTyVars pp_what equals_or_where tc tparms
  ...
  where
    check (HsTypeArg _ ki@(L loc _)) = addFatalError $ mkParserErrorMessage (locA loc) $
                                         PsMalformedExprMessage (PsUnexpectedTypeAppInDecl ki pp_what (unLoc tc))
    check (HsValArg ty) = chkParens [] emptyComments ty
    check (HsArgPar sp) = addFatalError $ mkParserErrorMessage sp $
                            PsMalformedExprMessage (PsMalformedDecl pp_what (unLoc tc))
    ...
    chk _ _ t@(L loc _)
        = addFatalError $ mkParserErrorMessage (locA loc) $
            PsMalformedExprMessage (PsUnexpectedTypeInDecl t pp_what (unLoc tc) tparms equals_or_where)
...
checkDatatypeContext (Just c)
    = do allowed <- getBit DatatypeContextsBit
         unless allowed $ addError $ mkParserErrorMessage (getLocA c) $
                                       PsMalformedExprMessage (PsIllegalDataTypeContext c)
...
checkRuleTyVarBndrNames = mapM_ (check . fmap hsTyVarName)
  where check (L loc (Unqual occ)) =
          ...
          when ((occNameString occ ==) `any` ["forall","family","role"])
            (addFatalError $ mkParserErrorMessage (locA loc) $
               PsMalformedExprMessage (PsParseErrorOnInput occ))
        check _ = panic "checkRuleTyVarBndrNames"

checkRecordSyntax :: (MonadP m, Outputable a) => LocatedA a -> m (LocatedA a)
checkRecordSyntax lr@(L loc r)
    = do allowed <- getBit TraditionalRecordSyntaxBit
         unless allowed $ addError $ mkParserErrorMessage (locA loc) $
                                       PsMalformedExprMessage (PsIllegalTraditionalRecordSyntax (ppr r))
         return lr
...
checkEmptyGADTs gadts@(L span (_, []))           -- Empty GADT declaration.
    = do gadtSyntax <- getBit GadtSyntaxBit   -- GADTs implies GADTSyntax
         unless gadtSyntax $ addError $ mkParserErrorMessage span $
                                          PsMalformedExprMessage PsIllegalWhereInDataDecl
         return gadts
...
checkTyClHdr is_cls ty
  ...
  where
    ...
    go _ (HsParTy an (L l (HsStarTy _ isUni))) acc ann' fix
      = do { addPsMessage (locA l) (PsExtensionMessage PsStarBinderWithStarIsType)
    ...
    go l _ _ _ _
      = addFatalError $ mkParserErrorMessage l $
          PsMalformedExprMessage (PsMalformedTyOrClDecl ty)
...
(checkExpBlockArguments, checkCmdBlockArguments) = (checkExpr, checkCmd)
  where
    ...
    checkExpr expr = case unLoc expr of
      HsDo _ (DoExpr m) _  -> check (PsMalformedExprMessage . PsDoInFunAppExpr m)     expr
      HsDo _ (MDoExpr m) _ -> check (PsMalformedExprMessage . PsMDoInFunAppExpr m)    expr
      HsLam {}             -> check (PsMalformedExprMessage . PsLambdaInFunAppExpr)     expr
      HsCase {}            -> check (PsMalformedExprMessage . PsCaseInFunAppExpr)       expr
      HsLamCase {}         -> check (PsMalformedExprMessage . PsLambdaCaseInFunAppExpr) expr
      HsLet {}             -> check (PsMalformedExprMessage . PsLetInFunAppExpr)        expr
      HsIf {}              -> check (PsMalformedExprMessage . PsIfInFunAppExpr)         expr
      HsProc {}            -> check (PsMalformedExprMessage . PsProcInFunAppExpr)       expr
      _                    -> return ()

    checkCmd :: LHsCmd GhcPs -> PV ()
    checkCmd cmd = case unLoc cmd of
      HsCmdLam {}  -> check (PsMalformedExprMessage . PsLambdaCmdInFunAppCmd) cmd
      HsCmdCase {} -> check (PsMalformedExprMessage . PsCaseCmdInFunAppCmd)   cmd
      HsCmdIf {}   -> check (PsMalformedExprMessage . PsIfCmdInFunAppCmd)     cmd
      HsCmdLet {}  -> check (PsMalformedExprMessage . PsLetCmdInFunAppCmd)    cmd
      HsCmdDo {}   -> check (PsMalformedExprMessage . PsDoCmdInFunAppCmd)     cmd
      _            -> return ()

    check err a = do
      blockArguments <- getBit BlockArgumentsBit
      unless blockArguments $
        addError $ mkParserErrorMessage (getLocA a) $ (err a)
...
checkAPat loc e0 = do
 ...
 case e0 of
   ...
   PatBuilderOpApp _ op _ _ | opIsAt (unLoc op) -> do
     addError $ mkParserErrorMessage (getLocA op) $ PsMalformedExprMessage PsAtInPatternPosition
...
patFail loc e = addFatalError $ mkParserErrorMessage loc $ PsMalformedExprMessage (PsParseErrorInPat e)
...
checkValSigLhs lhs@(L l _)
  = addFatalError $ mkParserErrorMessage (locA l) $ PsMalformedExprMessage (PsInvalidTypeSignature lhs)

checkDoAndIfThenElse
  :: (Outputable a, Outputable b, Outputable c)
  => (a -> Bool -> b -> Bool -> c -> PsMalformedExprMessage)
...
checkDoAndIfThenElse err guardExpr semiThen thenExpr semiElse elseExpr
 | semiThen || semiElse = do
      ...
      unless doAndIfThenElse $ addError (mkParserErrorMessage loc $ PsMalformedExprMessage e)
  | otherwise = return ()
...
instance DisambInfixOp RdrName where
  ...
  mkHsInfixHolePV l _ = addFatalError $ mkParserErrorMessage l $ PsLayoutMessage PsInvalidInfixHole
...
instance DisambECP (HsCmd GhcPs) where
  ...
  mkHsProjUpdatePV l _ _ _ _ = addFatalError $ mkParserErrorMessage l $
                                                 PsExtensionMessage PsOverloadedRecordDotInvalid
  mkHsLamPV l mg = do
    cs <- getCommentsFor l
  ...
  mkHsIfPV l c semi1 a semi2 b anns = do
    checkDoAndIfThenElse PsSemiColonsInCondCmd c semi1 a semi2 b
    cs <- getCommentsFor l
  ...
  mkHsDoPV l (Just m)    _ _ = addFatalError $ mkParserErrorMessage l $ PsExtensionMessage (PsQualifiedDoInCmd m)
  ...
  mkHsRecordPV _ l _ a (fbinds, ddLoc) _ = do
    ...
    if not (null ps)
      then addFatalError $ mkParserErrorMessage l $ PsExtensionMessage PsOverloadedRecordDotInvalid
...
cmdFail loc e = addFatalError $ mkParserErrorMessage loc $ PsMalformedExprMessage (PsParseErrorInCmd e)

instance DisambECP (HsExpr GhcPs) where
  type Body (HsExpr GhcPs) = HsExpr
  ecpFromCmd' (L l c) = do
    addError $ mkParserErrorMessage (locA l) $ PsMalformedExprMessage (PsArrowCmdInExpr c)
  ...
  mkHsIfPV l c semi1 a semi2 b anns = do
    checkDoAndIfThenElse PsSemiColonsInCondExpr c semi1 a semi2 b
    cs <- getCommentsFor l
  ...
  mkHsViewPatPV l a b _ = addError (mkParserErrorMessage l $ PsMalformedExprMessage (PsViewPatInExpr a b))
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkHsAsPatPV l v e   _ = addError (mkParserErrorMessage l $ PsLayoutMessage (PsTypeAppWithoutSpace (unLoc v) e))
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkHsLazyPatPV l e   _ = addError (mkParserErrorMessage l $ PsLayoutMessage (PsLazyPatWithoutSpace e))
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkHsBangPatPV l e   _ = addError (mkParserErrorMessage l $ PsLayoutMessage (PsBangPatWithoutSpace e))
                          >> return (L (noAnnSrcSpan l) (hsHoleExpr noAnn))
  mkSumOrTuplePV = mkSumOrTupleExpr
  rejectPragmaPV (L _ (OpApp _ _ _ e)) =
    -- assuming left-associative parsing of operators
    rejectPragmaPV e
  rejectPragmaPV (L l (HsPragE _ prag _)) = addError $ mkParserErrorMessage (locA l) $
                                                         PsImportOrPragmaMessage (PsUnallowedPragma prag)
  rejectPragmaPV _                        = return ()
...
instance DisambECP (PatBuilder GhcPs) where
  ...
  ecpFromCmd' (L l c)    = addFatalError $ mkParserErrorMessage (locA l) $ PsMalformedExprMessage (PsArrowCmdInPat c)
  ecpFromExp' (L l e)    = addFatalError $ mkParserErrorMessage (locA l) $ PsMalformedExprMessage (PsArrowExprInPat e)
  mkHsLamPV l _          = addFatalError $ mkParserErrorMessage l $ PsMalformedExprMessage PsLambdaInPat
  mkHsLetPV l _ _ _      = addFatalError $ mkParserErrorMessage l $ PsMalformedExprMessage PsLetInPat
  mkHsProjUpdatePV l _ _ _ _ = addFatalError $ mkParserErrorMessage l $ PsExtensionMessage PsOverloadedRecordDotInvalid
  ...
  mkHsCasePV l _ _ _     = addFatalError $ mkParserErrorMessage l $ PsMalformedExprMessage PsCaseInPat
  mkHsLamCasePV l _ _    = addFatalError $ mkParserErrorMessage l $ PsMalformedExprMessage PsLambdaCaseInPat
  type FunArg (PatBuilder GhcPs) = PatBuilder GhcPs
  superFunArg m = m
  mkHsAppPV l p1 p2      = return $ L l (PatBuilderApp p1 p2)
  mkHsAppTypePV l p la t = return $ L l (PatBuilderAppType p la (mkHsPatSigType t))
  mkHsIfPV l _ _ _ _ _ _ = addFatalError $ mkParserErrorMessage l $ PsMalformedExprMessage PsIfThenElseInPat
  mkHsDoPV l _ _ _       = addFatalError $ mkParserErrorMessage l $ PsMalformedExprMessage PsDoNotationInPat
  ...
  mkHsRecordPV _ l _ a (fbinds, ddLoc) anns = do
    let (fs, ps) = partitionEithers fbinds
    if not (null ps)
     then addFatalError $ mkParserErrorMessage l $
                            PsExtensionMessage PsOverloadedRecordDotInvalid
     else do
       cs <- getCommentsFor l
...
checkUnboxedStringLitPat (L loc lit) =
  case lit of
    HsStringPrim _ _  -- Trac #13260
      -> addFatalError $ mkParserErrorMessage loc $
                           PsMalformedExprMessage (PsIllegalUnboxedStringInPat lit)
    _ -> return ()
...
mkPatRec p _ _ =
  addFatalError $ mkParserErrorMessage (getLocA p) $
                    PsMalformedExprMessage (PsInvalidRecordCon (unLoc p))
...
instance DisambTD DataConBuilder where
  ...
  mkHsAppKindTyPV lhs l_at ki =
    addFatalError $ mkParserErrorMessage l_at $
                      PsMalformedExprMessage (PsUnexpectedKindAppInDataCon (unLoc lhs) (unLoc ki))
  ...
  mkHsOpTyPV lhs tc rhs = do
    ...
    where
      ...
      check_no_ops (HsOpTy{}) =
        addError $ mkParserErrorMessage (locA l) $
                     PsMalformedExprMessage (PsInvalidInfixDataCon (unLoc lhs) (unLoc tc) (unLoc rhs))
  ...
  mkUnpackednessPV unpk constr_stuff
    ...
    = -- When the user writes  data T = {-# UNPACK #-} Int :+ Bool
      ...
      do lhs' <- addUnpackednessP unpk lhs
         ...
         return $ L l (InfixDataConBuilder lhs' data_con rhs)
    | otherwise =
      do addError $ mkParserErrorMessage (getLoc unpk) $
                      PsMalformedExprMessage PsUnpackDataCon
         return constr_stuff
...
tyToDataConBuilder t =
  addFatalError $ mkParserErrorMessage (getLocA t) $
                    PsMalformedExprMessage (PsInvalidDataCon (unLoc t))
...
checkPrecP (L l (_,i)) (L _ ol)
 ...
 | otherwise = addFatalError $ mkParserErrorMessage l $
                                 PsMalformedExprMessage (PsPrecedenceOutOfRange i)
  where
...
mkRecConstrOrUpdate _ (L _ (HsVar _ (L l c))) _lrec (fbinds,dd) anns
  ...
  = do
      let (fs, ps) = partitionEithers fbinds
      if not (null ps)
        then addFatalError $ mkParserErrorMessage (getLocA (head ps)) $
                               PsExtensionMessage PsOverloadedRecordDotInvalid
        else return (mkRdrRecordCon (L l c) (mk_rec_fields fs dd) anns)
mkRecConstrOrUpdate overloaded_update exp _ (fs,dd) anns
  | Just dd_loc <- dd = addFatalError $ mkParserErrorMessage dd_loc $
                                          PsMalformedExprMessage PsDotsInRecordUpdate
...
mkRdrRecordUpd overloaded_on exp@(L loc _) fbinds anns = do
  ...
  case overloaded_on of
    False | not $ null ps ->
      ...
      addFatalError $ mkParserErrorMessage (locA loc) $
        PsExtensionMessage (PsSyntaxUsedButNotEnabled OverloadedRecordUpdate (SuggestExtension OverloadedRecordUpdate))
    False ->
    ...
    True -> do
      ...
      if not $ null qualifiedFields
        then
          addFatalError $ mkParserErrorMessage (getLoc (head qualifiedFields)) $
            PsExtensionMessage PsOverloadedRecordUpdateNoQualifiedFields
  ...
  where
    ...
    mkCImport = do
      ...
      case parseCImport cconv safety (mkExtName (unLoc v)) e (L loc esrc) of
        Nothing         -> addFatalError $ mkParserErrorMessage loc $
                             PsMalformedExprMessage PsMalformedEntityString
         ...
         if allowed
          then
            ...
            in (\newName
               ...
               <$> nameT
          else addFatalError $ mkParserErrorMessage (locA l) $
                 PsExtensionMessage PsIllegalPatSynExport
  where
    name = ieNameVal specname
    nameT =
      if isVarNameSpace (rdrNameSpace name)
        then addFatalError $ mkParserErrorMessage (locA l) $
               PsMalformedExprMessage (PsVarForTyCon name)
...
mkTypeImpExp name =
  do allowed <- getBit ExplicitNamespacesBit
     unless allowed $ addError $ mkParserErrorMessage (getLocA name) $
                                   PsExtensionMessage PsIllegalExplicitNamespace
...
checkImportSpec ie@(L _ specs) =
  ...
  where
    importSpecError l =
      addFatalError $ mkParserErrorMessage l $ PsImportOrPragmaMessage PsIllegalImportBundleForm
...
warnPrepositiveQualifiedModule span =
  addPsMessage span (PsImportOrPragmaMessage PsImportPreQualified)

failOpNotEnabledImportQualifiedPost :: SrcSpan -> P ()
failOpNotEnabledImportQualifiedPost loc =
  addError $ mkParserErrorMessage loc $ PsImportOrPragmaMessage PsImportPostQualified

failOpImportQualifiedTwice :: SrcSpan -> P ()
failOpImportQualifiedTwice loc =
  addError $ mkParserErrorMessage loc $ PsImportOrPragmaMessage PsImportQualifiedTwice

warnStarIsType :: SrcSpan -> P ()
warnStarIsType span = addPsMessage span (PsExtensionMessage PsStarIsTypeWithoutExtension)

failOpFewArgs :: MonadP m => LocatedN RdrName -> m a
failOpFewArgs (L loc op) =
  do { star_is_type <- getBit StarIsTypeBit
     ; addFatalError $ mkParserErrorMessage (locA loc) $
         PsMalformedExprMessage (PsOpTooFewArgs (StarIsType star_is_type) op) }
...
instance MonadP PV where
  addError err = -- FIXME(adn) How to deal with accumulated hints?
...
hintBangPat span e = do
    ...
    unless bang_on $
      addError $ mkParserErrorMessage span $ PsExtensionMessage (PsIllegalBangPattern e)
...
mkSumOrTupleExpr l Boxed a@Sum{} _ =
    addFatalError $ mkParserErrorMessage (locA l) $ PsMalformedExprMessage (PsUnsupportedBoxedSumExpr a)
  ...
  where
    ...
    toTupPat p = case p of
      Left _ -> addFatalError $
                  mkParserErrorMessage (locA l) $ PsMalformedExprMessage PsTupleSectionInPattern
      Right p' -> checkLPat p'
...
mkSumOrTuplePat l Boxed a@Sum{} _ =
    addFatalError $
      mkParserErrorMessage (locA l) $ PsMalformedExprMessage (PsUnsupportedBoxedSumPattern a)
