...
class b ~ (Body b) GhcPs => DisambECP b where
  ...
  mkHsDoPV
    :: SrcSpan -> LocatedL [LStmt GhcPs (LocatedA b)]
    -> (ApiAnnComments -> ApiAnn' AnnList)
    -> PV (LocatedA b)
...
instance DisambECP (HsCmd GhcPs) where
  ...
  mkHsDoPV l stmts anns = do
    cs <- addAnnsAt l []
    return $ L (noAnnSrcSpan l) (HsCmdDo (anns cs) stmts)
...
instance DisambECP (HsExpr GhcPs) where
  ...
  mkHsDoPV l stmts anns = do
    cs <- addAnnsAt l []
    return $ L (noAnnSrcSpan l) (HsDo (anns cs) DoExpr stmts)
...
instance DisambECP (PatBuilder GhcPs) where
  ...
  mkHsOpAppPV l p1 op p2 = do
    ...
    return $ L (noAnnSrcSpan l) $ PatBuilderOpApp p1 (reLocL op) p2 anns
  mkHsCasePV l _ _ _
