...
mkRdrRecordUpd overloaded_on exp@(L loc _) fbinds = do
  ...
  where
    ...
    toProjUpdates =
      map (\case { Right p -> p
                 ; Left f -> recUpdFieldToProjUpdate f
                 } )

    -- Convert a top-level field update like {foo=2} or {bar} (punned)
    -- to a projection update.
    recUpdFieldToProjUpdate :: LHsRecField GhcPs  (LHsExpr GhcPs) -> LHsRecUpdProj GhcPs
    recUpdFieldToProjUpdate fld =
        let (L l (HsRecField occ arg pun)) =
              fmap mk_rec_upd_field fld -- Make this RecField into a RecUpdField.
            -- The idea here is to convert the label to a singleton `[FastString]`.
            (loc, f) = field occ
        in mkRdrProjUpdate l (L loc [L loc f]) (punnedVar pun f arg) pun -- Now this RecUpdField into a RecUpdProj.
        where
          -- If punning, compute mkVar f otherwise just arg. This has
          -- the effect that sentinel `HsVar "pun-rhs"` is replaced by
          -- `HsVar "f"` here, before the update is written to a
          -- setField expressions.
          punnedVar :: Bool -> FastString -> LHsExpr GhcPs -> LHsExpr GhcPs
          punnedVar pun f arg = if pun then mkVar f else arg

          field :: Located (AmbiguousFieldOcc GhcPs) -> (SrcSpan, FastString)
          field = \case
              L _ (Ambiguous _ (L loc lbl)) ->  (loc, occNameFS . rdrNameOcc $ lbl)
              L _ (Unambiguous _ (L loc lbl)) -> (loc, occNameFS . rdrNameOcc $ lbl)

          mkVar :: FastString -> LHsExpr GhcPs
          mkVar = noLoc . HsVar noExtField . noLoc . mkRdrUnqual . mkVarOccFS
