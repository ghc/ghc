...
instance Diagnostic PsMessage where
  diagnosticMessage = \case
    ...
    PsErrParseErrorInPat s Nothing
      -> mkSimpleDecorated $ parse_error_in_pat <+> ppr s
    PsErrParseErrorInPat s (Just PsParseErrorInPatDetails{..})
  ...
  diagnosticHints  = \case
    ...
    PsErrParse token PsErrParseDetails{..}        ->
      case null token of
        True  -> []
        False -> catMaybes [sug_th, sug_rdo, sug_do, sug_let, sug_pat]
      where
        sug c s = if c then Just s else Nothing
        sug_th  = sug (not ped_th_enabled && token == "$")            (SuggestExtension LangExt.TemplateHaskell) -- #7396
        sug_rdo = sug (token == "<-" && ped_mdo_in_last_100)          (SuggestExtension LangExt.RecursiveDo)
        sug_do  = sug (token == "<-" && not ped_mdo_in_last_100)       SuggestDo
        sug_let = sug (token == "=" && ped_do_in_last_100)             SuggestLetInDo         -- #15849
        sug_pat = sug (not ped_pat_syn_enabled && ped_pattern_parsed) (SuggestExtension LangExt.PatternSynonyms) -- #12429
    ...
    PsErrDoNotationInPat{}                        -> [SuggestDo]
    PsErrIfThenElseInPat                          -> noHints
    PsErrLambdaCaseInPat                          -> noHints
    PsErrCaseInPat                                -> [SuggestMissingDo]
    ...
    PsErrParseErrorInPat _ Nothing                -> [SuggestMissingDo]
    PsErrParseErrorInPat _ (Just PsParseErrorInPatDetails{..}) ->
      if | peipd_args_num /= 0 && peipd_pat_is_rec -> [SuggestExtension LangExt.RecursiveDo]
         | otherwise                               -> noHints
