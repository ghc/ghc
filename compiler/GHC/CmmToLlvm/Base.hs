...
varInsert s t = modifyEnv $ \env -> env { envVarMap = addToUFM_U (envVarMap env) s t }
funInsert s t = modifyEnv $ \env -> env { envFunMap = addToUFM_U (envFunMap env) s t }

-- | Lookup variables or functions in the environment.
varLookup, funLookup :: Uniquable key => key -> LlvmM (Maybe LlvmType)
varLookup s = getEnv (flip lookupUFM_U s . envVarMap)
funLookup s = getEnv (flip lookupUFM_U s . envFunMap)
