...
staticLitsToWords platform = go
  where
    ...
    -- Decompose a multi-word or floating-point literal into multiple
    -- single-word literals
    isMultiWord :: CmmLit -> Maybe [CmmLit]
    isMultiWord (CmmFloat n W64)
      -- This will produce a W64 integer, which will then be broken up further
      -- on the next iteration on 32-bit platforms.
      = Just [doubleToWord platform n]
    isMultiWord (CmmFloat n W32)
      = Just [floatToWord platform n]
    isMultiWord (CmmInt n W64)
      | W32 <- wordWidth platform
      = Just [CmmInt hi W32, CmmInt lo W32]
      where
        hi = n `shiftR` 32
        lo = n .&. 0xffffffff
    isMultiWord _ = Nothing
...
floatToWord platform r
  = runST $ do
        ...
        w32 <- readArray arr' 0
        return (CmmInt (toInteger w32) (wordWidth platform))
  where
    castFloatToWord32Array :: STUArray s Int Float -> ST s (STUArray s Int Word32)
    castFloatToWord32Array = U.castSTUArray

doubleToWord :: Platform -> Rational -> CmmLit
doubleToWord platform r
  = runST $ do
        arr <- newArray_ ((0::Int),1)
        writeArray arr 0 (fromRational r)
        arr' <- castDoubleToWord64Array arr
        w64 <- readArray arr' 0
        return $ CmmInt (toInteger w64) (wordWidth platform)
  where
