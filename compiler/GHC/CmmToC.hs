...
pprTop platform = \case
  (CmmProc infos clbl _in_live_regs graph) ->
    ...
    (vcat [
           ...
           (if (externallyVisibleCLabel clbl)
                    then mkFN_ else mkIF_) (pdoc platform clbl) <+> lbrace,
           nest 8 temp_decls,
  ...
  (CmmData section (CmmStaticsRaw lbl [CmmString str])) ->
    ...
    hcat [
      pprLocalness lbl, pprConstness (isSecConstant section), text "char ", pdoc platform lbl,
    ...
    CmmUnsafeForeignCall target@(ForeignTarget fn conv) results args ->
        ...
        fnCall =
            case fn of
              CmmLit (CmmLabel lbl)
                | StdCallConv <- cconv ->
                    pprCall platform (pdoc platform lbl) cconv hresults hargs
                ...
                | not (isMathFun lbl) ->
                    pprForeignCall platform (pdoc platform lbl) cconv hresults hargs
              _ ->
                    pprCall platform cast_fn cconv hresults hargs <> semi
...
pprLit platform lit = case lit of
    ...
    where
        pprCLabelAddr lbl = char '&' <> pdoc platform lbl
...
pprExternDecl platform lbl
  ...
  | otherwise =
        hcat [ visibility, label_type lbl , lparen, pdoc platform lbl, text ");"
 ...
 where
  ...
  stdcall_decl sz =
        text "extern __attribute__((stdcall)) void " <> pdoc platform lbl
