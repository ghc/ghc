...
getUnboxedSumTyThing n
  ...
  | tag == 0x0
  = KnownUniqueWiredIn $ mkAConLike (mkRealDataCon (sumDataCon (alt + 1) arity))
  | tag == 0x1
...
getCTupleDataConTyThing n =
    case n `divMod` 3 of
      (arity,  0) -> KnownUniqueWiredIn $ mkAConLike (mkRealDataCon (cTupleDataCon arity))
...
getTupleDataConTyThing boxity n =
    case n `divMod` 3 of
      (arity, 0) -> KnownUniqueWiredIn $ mkAConLike (mkRealDataCon (tupleDataCon boxity arity))
