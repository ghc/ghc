...
module GHC.Builtin.Types (
        ...
        pairTyCon, pair, isPromotedPairType,
        unboxedUnitTy,
...
isPromotedPairType tc
  | Just (tc1, [_,_,x,y]) <- splitTyConApp_maybe tc, tc1 == (promotedTupleDataCon Boxed 2) = Just (x, y)
  | otherwise = Nothing
...
isMaybeType tc
  | Just (tc1,[_,t]) <- splitTyConApp_maybe tc, tc1 == promotedJustDataCon = return $ Just t
  | Just (tc1,[_]) <- splitTyConApp_maybe tc, tc1 == promotedNothingDataCon = return $ Nothing
  | otherwise = Nothing
