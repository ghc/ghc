...
noinlineId = pcRepPolyId noinlineIdName ty concs info
  where
    info = noCafIdInfo
           `setArityInfo`  1
    ...
    -- See (NOI5) in Note [noinlineId magic]
    strict_sig = mkClosedDmdSig [evalDmd] topDiv

    -- noinline :: forall r (a :: TYPE r). a -> a
    -- It is runtime-rep-polymorphic; see (NOI2) in Note [noinlineId magic]
    ty   = mkInfForAllTy runtimeRep1TyVar   $
           mkSpecForAllTys [rrPolyTyVar1] $
           mkVisFunTyMany rrPolyTy1 rrPolyTy1

    -- The argument should have a concrete runtime-rep
    concs = mkRepPolyIdConcreteTyVars
            [ ((rrPolyTy1, mkArgPos 1 Top), runtimeRep1TyVar)]

noinlineConstraintId :: Id -- See Note [noinlineId magic]
noinlineConstraintId = pcRepPolyId noinlineConstraintIdName ty concs info
  where
    info = noCafIdInfo
    ty   = mkInfForAllTy runtimeRep1TyVar   $
    ...
    concs = mkRepPolyIdConcreteTyVars
            [ ((rrPolyConstraintTy1, mkArgPos 1 Top), runtimeRep1TyVar)]
...
-- | Coerce is representation polymorphic, hence it needs a compulsory
-- unfolding, hence it needs to be wired-in.
coerceId :: Id
...
* In the Simplifier, see the `isNoInlineId` case of `simplOutId`.
  ...
  * We switch off inlining before simplifying `f` (#18995)
    See `GHC.Core.Opt.Simplify.Unils.updModeForNoInline`.
...
(NOI2) `noinline` needs to apply to unboxed types, so that
          noinline (f x y)
   works even if `f` happens to return Int#.  So `noinline` get the
   runtime-rep-polymorphic type:
...
(NOI5) Given
          f a b = noinline const a b
   we want `f` to get the strictness signature `<1L><A>`, like `const`,
   despite the `noinline` (#23911).  Because of (NOI1) the demand analyser
   will see (noinline (const a b)), so all we need to do is to give `noinline`
   an `evalDmd` on its argument, in the defn of `noinlineId`.  Simple.

   We could do more, and give it a special demand transformer (#18396, #16588).
   But it would only make a difference for un-saturated calls or something
   complicated involving data structures, so for now I have stuck with
   the simple story.
