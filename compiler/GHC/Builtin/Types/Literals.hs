...
chars (even though this module is named GHC.Builtin.Types.Literals, it covers all of them).
  ...
  CoAxiomRule
    ...
    , coaxrProves    = \cs ->
        do [Pair s1 s2] <- return cs
           s2' <- isStrLitTy s2
           case nullFS s2' of
             True  -> return (mkTyConApp typeUnconsSymbolTyCon [s1] ===
                       (maybeType charSymbolPairKind Nothing))
             False -> return (mkTyConApp typeUnconsSymbolTyCon [s1] ===
...
interactTopConsSymbol [s,t] r
  ...
  , y `isSuffixOf` z
  , [x] `isPrefixOf` z =
    [ t === (mkCharLitTy $ head z), t === mkStrLitTy (mkFastString $ tail z) ]
...
interactInertConsSymbol [x1] z1 [x2] z2
  | sameZ         = [ x1 === x2 ]
  where sameZ = tcEqType z1 z2
