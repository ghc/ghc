...
axUnconsSymbolDef =
  mkUnAxiom "UnconsSymbolDef" typeUnconsSymbolTyCon isStrLitTy $
    \str ->
      case unconsFS str of
        Nothing         -> Just $ mkPromotedMaybeTy charSymbolPairKind Nothing
        Just (char, fs) -> Just $ charSymbolPair (mkCharLitTy char) (mkStrLitTy fs)
...
matchFamUnconsSymbol [s]
  | Just x <- mbX =
    Just (axUnconsSymbolDef, [s], mkPromotedMaybeTy charSymbolPairKind (fmap reifySymbolPairTy (unconsFS x)))
  where
  mbX = isStrLitTy s
  reifySymbolPairTy (c, s) = charSymbolPair (mkCharLitTy c) (mkStrLitTy s)
