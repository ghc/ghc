...
codeGen dflags this_mod data_tycons
  ...
  = do  {     -- cg: run the code generator, and yield the resulting CmmGroup
        ...
        ; let extract_lf_info info = (name, lf)
                where
                  !id = cg_id info
                  !name = idName id
                  !lf = cg_lf info

          -- LambdaFormInfos to be used in importing modules to generate more
          -- efficient caling code. See Note [Conveying CAF-info and LFInfo
          -- between modules] in GHC.StgToCmm.Types.
        ; let !generatedInfo = mkNameEnv (Prelude.map extract_lf_info (eltsUFM cg_id_infos))
