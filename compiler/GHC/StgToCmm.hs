...
codeGen dflags this_mod ip_map@(InfoTableProvMap _) data_tycons
  ...
  = liftIO initC >>= \c -> flip evalStateT (CodeGenState mempty c) $ do  {     -- cg: run the code generator, and yield the resulting CmmGroup
        ...
        ; let cg :: FCode () -> StateT CodeGenState (Stream IO CmmGroup) ()
              cg fcode = do
                cmm <- withTimingSilent dflags (text "STG -> Cmm") (`seq` ()) $ do
                         ...
                         let !used_info
                                | gopt Opt_InfoTableMap dflags = toOL (mapMaybe topInfoTable a) `mappend` ts
                                | otherwise = mempty
        ...
        ; cg (initInfoTableProv used_info ip_map this_mod)
        ...
        ; return (used_info, generatedInfo)
        }
