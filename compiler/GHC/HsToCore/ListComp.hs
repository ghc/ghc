...
deBindComp pat core_list1 quals core_list2 = do
    ...
    core_match <- matchSimply (Var u2) (StmtCtxt (HsDoStmt ListComp)) (mkVisMatchPat pat) rest_expr core_fail
...
dfBindComp c_id n_id (pat, core_list1) quals = do
    ...
    core_expr <- matchSimply (Var x) (StmtCtxt (HsDoStmt ListComp))
                (mkVisMatchPat pat) core_rest (Var b)
...
dsMcBindStmt pat rhs' bind_op fail_op res1_ty stmts
  = do  { body     <- dsMcStmts stmts
        ...
        ; match <- matchSinglePatVar var Nothing (StmtCtxt (HsDoStmt (DoExpr Nothing))) (mkVisMatchPat pat)
