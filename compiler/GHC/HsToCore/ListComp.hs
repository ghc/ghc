...
dsMcStmt (ApplicativeStmt body_ty args mb_join) stmts = do
    ...
    let match_args pat (vs, body) = do
            ...
            match <- matchSinglePatVar var Nothing (StmtCtxt MonadComp) pat body_ty (cantFailMatchResult body)
            match_code <- dsHandleMonadicFailure MonadComp pat match Nothing
