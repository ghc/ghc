...
deBindComp pat core_list1 quals core_list2 = do
    ...
    core_match <- matchSimply (Var u2) (StmtCtxt $ PsStmt ListComp) pat rest_expr core_fail
...
dfBindComp c_id n_id (pat, core_list1) quals = do
    ...
    core_expr <- matchSimply (Var x) (StmtCtxt $ PsStmt ListComp)
                pat core_rest (Var b)
...
dsMcBindStmt pat rhs' bind_op fail_op res1_ty stmts
  = do  { body     <- dsMcStmts stmts
        ...
        ; match <- matchSinglePatVar var Nothing (StmtCtxt (PsStmt $ DoExpr Nothing)) pat
                                  res1_ty (cantFailMatchResult body)
        ; match_code <- dsHandleMonadicFailure (PsStmt MonadComp :: HsStmtContext GhcRn) pat match fail_op
