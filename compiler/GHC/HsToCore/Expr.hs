...
dsConLike con tvbs tys
  = do { ds_con <- dsHsConLike con
       ; ids    <- newSysLocalsDs tys  -- /Can/ be lev-poly
       ; return (mkLams tvs $
                 mkLams ids $
  ...
  where
    ...
    get_stupid (RealDataCon con) = dataConStupidTheta con
    get_stupid (PatSynCon {})    = []

    drop_stupid = dropList (get_stupid con)
...
Note [Checking for levity-polymorphic functions]o
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
GHC.Tc.Gen.Head.  (NB: in fact, only newtype constructors can be levity
polymorphic.)
...
dsHsWrapped orig_hs_expr
  ...
  where
    ...
    go wrap (XExpr (ConLikeTc con tvs tys))
      = do { let (w_outer, w_inner) = splitWrapper wrap
           ...
           ; checkLevPolyArgs orig_hs_expr inst_ty
           ; return (w_outer' inst_e) }

    go wrap (HsVar _ (L _ var))
      | var `hasKey` withDictKey
      = do { wrap' <- dsHsWrapper wrap
           ; ds_withDict (exprType (wrap' (Var var))) }

      | otherwise
      = do { let (w_outer, w_inner) = splitWrapper wrap
           ; w_outer' <- dsHsWrapper w_outer
           ; w_inner' <- dsHsWrapper w_inner
           ; let inst_e  = w_inner' (Var var)
                 inst_ty = exprType inst_e
           ; checkLevPolyFunction orig_hs_expr var inst_ty
           ; dflags <- getDynFlags
...
-- and outer_wrap is everything else.
