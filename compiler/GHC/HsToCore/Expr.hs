...
But that compulsory unfoldign is a levity-polymrophic lambda, which
is no good.  So we want to reject this.  On the other hand
  const True (unsafeCorce# LiftedRep UnliftedRep)
is abolutely fine.
...
Note that if `f :: forall r (a :: Type r). blah`, then
   const True f
is absolutely fine.  We can `f` is a function, represented by a
  ...
  where
    ...
    go_head wrap var
      = do { let wrapped_e  = wrap (Var var)
           ...
           ; checkLevPolyFunction orig_hs_expr var wrapped_ty
             -- See Note [Checking for levity-polymorphic functions]
             -- Pass hs_e, so that the user can see entire expression
             -- with -fprint-typechecker-elaboration
...
checkLevPolyFunction :: HsExpr GhcTc -> Id -> Type -> DsM ()
checkLevPolyFunction hs_expr var ty
  | let bad_tys = badUseOfLevPolyPrimop var ty
  , not (null bad_tys)
  = levPolyPrimopErr (ppr hs_expr) ty bad_tys
