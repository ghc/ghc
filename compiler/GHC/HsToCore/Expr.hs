...
dsExpr (HsOverLit _ lit)
  = do { warnAboutOverflowedOverLit lit
       ; dsOverLit lit }
dsExpr (XExpr (Right (HsExpanded _ b))) = dsLExpr b
dsExpr hswrap@(XExpr (Left (HsWrap co_fn e)))
  = do { e' <- case e of
                 HsVar _ (L _ var) -> return $ varToCoreExpr var
                 HsConLikeOut _ (RealDataCon dc) -> return $ varToCoreExpr (dataConWrapId dc)
                 XExpr (Left (HsWrap _ _)) -> pprPanic "dsExpr: HsWrap inside HsWrap" (ppr hswrap)
