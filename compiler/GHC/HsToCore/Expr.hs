...
ds_val_bind (is_rec, binds) body
  = do  { MASSERT( isRec is_rec || isSingletonBag binds )
        ...
        -- See notes with GHC.Tc.Simplify.inferLoop [NO TYVARS]
...
dsLExpr (L loc e)
  = putSrcSpanDs loc $
   ...
   -- uncomment this check to test the hsExprType function in GHC.Tc.Syntax
                 ...
                 user_tvs =
                   case con of
                     ...
                     PatSynCon _          -> univ_tvs ++ ex_tvs
                       -- The order here is because of the order in `GHC.Tc.PatSyn`.
