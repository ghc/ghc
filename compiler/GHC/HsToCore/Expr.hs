...
We need to get this:
   ...
   upd = /\s t. \(z::T s t) (y::s) ->
         case z of
            MkT (co :: t ~# Int) _ -> $WMkT @s y |> T (Refl s) (Sym co2)

Note the final cast
   T (Refl s) (Sym co2) :: T s Int ~ T s t
which uses co2, bound by the GADT match.  This is the wrap_co coercion
...
* Form univ_cos, a coercion for each of tc's args: (Refl s) (Sym co2)
  We replaced
     a  by  (Refl s)    since 's' instantiates 'a'
     b  by  (Sym co2)   since 'b' is in the data-con's EqSpec

* Then form the coercion T (Refl s) (Sym co2)
                  ...
                  case con of
                    RealDataCon data_con
                      ...
                      where
                        ...
                        wrap_co  = mkTcFamilyTyConAppCo (dataConTyCon data_con) univ_cos
