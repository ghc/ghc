...
Desugaring foreign declarations (see also GHC.HsToCore.ForeignCall).
-}
...
module GHC.HsToCore.ForeignCallDecl ( dsForeigns ) where
...
import GHC.HsToCore.ForeignCall
...
\end{verbatim}
so we reuse the desugaring code in @GHC.HsToCore.ForeignCall@ to deal with these.
-}
...
typeTyCon ty
  ...
  = tc
  | otherwise
  = pprPanic "GHC.HsToCore.ForeignCallDecl.typeTyCon" (ppr ty)
...
getPrimTyOf ty
  ...
  case splitDataProductType_maybe rep_ty of
     Just (_, _, data_con, [prim_ty]) ->
        ...
        prim_ty
     _other -> pprPanic "GHC.HsToCore.ForeignCallDecl.getPrimTyOf" (ppr ty)
  where
