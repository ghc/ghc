...
checkGrd grd = CA $ \inc -> case grd of
  ...
  PmCon x (PmAltConLike con) _ _ _
    ...
    -> do
       tracePm "consi" (ppr inc $$ ppr initNablas)
...
There's another caveat: Putting potentially failing guards /after/ the
...
That *is* fixable, although we pay with a much more impacted implementation.
-}
