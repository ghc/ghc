    ...
    go (L _ AbsBinds{ abs_tvs = [], abs_ev_vars = []
      ...
      let go_export :: ABExport GhcTc -> PmGrd
          go_export ABE{abe_poly = x, abe_mono = y}
            = ASSERT(idType x `eqType` idType y)
              PmLet x (Var y)
...
@y :: forall a. Eq a => ...@), in which case the exports are a simple renaming
substitution that we can capture with 'PmLet'. Ultimately we'll hit those
renamed 'FunBind's, though, which is the whole point.
