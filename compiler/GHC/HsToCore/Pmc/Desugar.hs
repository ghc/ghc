...
import GHC.Utils.Outputable
    ( ($$), interpp'SP, vbar, (<+>), sep, SDoc, Outputable(ppr) )
...
desugarPat x pat = case pat of
  ...
  XPat ext -> case ext of
    ...
    ExpansionPat expansion -> do
      dflags <- getDynFlags
      case expansion of
        -- Non-RebindableSyntax overloaded list patterns at a list type
        -- shouldn't actually use the overloading mechanism.
        -- See Note [Desugaring overloaded list patterns]
        HsPatExpanded (ListPat {}) (ViewPat arg_ty _lexpr pat)
          | not (xopt LangExt.RebindableSyntax dflags)
          , Just _ <- splitListTyConApp_maybe arg_ty
          -> desugarLPat x pat

        HsPatExpanded _ p -> desugarPat x p
