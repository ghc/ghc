...
after type-checking. See Note [AbsBinds] in "GHC.Hs.Binds".
We make sure that there is no additional polymorphism in the way by only
picking up exports that are a simple renaming substitutions. That is the case
when their 'abe_wrap'per 'isIdHsWrapper'. We capture the renamings with
auxiliary 'PmLet's. Ultimately we'll hit those renamed 'FunBind's, though,
