...
DataCons and PatSynCons:
  ...
  * For pattern synonyms, we directly match `ty` against `res_ty` to get the
    ...
    We do have to consider the required constraints `σ(R)`, though, by solving
    them as Wanted constraints given the inert set of the current type state
    (which is part of a Nabla's TySt). Since spinning up a constraint solver
    session is costly, we only do so in the rare cases that a pattern synonym
    actually carries any required constraints.
...
returning the unrefined the input Nabla. After all, the match might have failed
due to yet incomplete type information in Nabla.
...
> K @σ(us) :: σ(R) => forall es. σ(P) => σ(t1) -> ... -> σ(tn) -> match_ty
...
And finally the extended 'Nabla' is returned if all the constraints where
compatible.
-}
