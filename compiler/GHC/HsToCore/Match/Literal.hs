...
-- See Note [Literals and the OverloadedList extension]
...
{-
Note [Literals and the OverloadedList extension]
~~~~
Consider the Literal `[256] :: [Data.Word.Word8]`

When the `OverloadedList` extension is not active, then the `ol_ext` field
...
When the `OverloadedList` extension is active, then the `ol_ext` field contains
