...
numericConversionKeys :: [KnownKey]
numericConversionKeys
  = [ toIntegerClassOpKey, toRationalClassOpKey
    , fromIntegralIdKey, realToFracIdKey ]
...
warnAboutEmptyEnumerations fam_envs dflags fromExpr mThnExpr toExpr
  ...
  = do
      ...
      if | tc_uniq == intTyConKey     -> case platformWordSize platform of
                                               PW4 -> check @Int32
                                               PW8 -> check @Int64
         | tc_uniq == wordTyConKey    -> case platformWordSize platform of
                                               PW4 -> check @Word32
                                               PW8 -> check @Word64
         | tc_uniq == int8TyConKey    -> check @Int8
         | tc_uniq == int16TyConKey   -> check @Int16
         | tc_uniq == int32TyConKey   -> check @Int32
         | tc_uniq == int64TyConKey   -> check @Int64
         | tc_uniq == word8TyConKey   -> check @Word8
         | tc_uniq == word16TyConKey  -> check @Word16
         | tc_uniq == word32TyConKey  -> check @Word32
         | tc_uniq == word64TyConKey  -> check @Word64
         | tc_uniq == integerTyConKey -> check @Integer
         | tc_uniq == naturalTyConKey -> check @Integer
