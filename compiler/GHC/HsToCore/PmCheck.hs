...
instance Outputable CheckResult where
  ppr (CheckResult c unc pc)
    = text "CheckResult" <+> braces (fsep
        [ field "clauses" c <> comma
        , field "approx" pc <> comma
        , field "uncov" unc])
    where
...
checkSingle dflags ctxt@(DsMatchContext kind locn) var p = do
  ...
  -- Only ever need to run this in a context where we need exhaustivity
  -- warnings (so not in pattern guards or comprehensions, for example).
  -- Omitting this flag emits redundancy warnings twice in obscure cases #17646.
