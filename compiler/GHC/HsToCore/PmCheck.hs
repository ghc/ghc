...
checkSingle' locn var p = do
  ...
  return $ case (cs,ds) of
    (Covered,  _    )         -> PmResult [] us' [] pc -- useful
    (NotCovered, NotDiverged) -> PmResult m  us' [] pc -- redundant
    (NotCovered, Diverged )   -> PmResult [] us' m  pc -- inaccessible rhs
...
checkMatches dflags ctxt@(DsMatchContext hs_ctx _) vars matches = do
  tracePm "checkMatches" (hang (vcat [ppr ctxt
                               , ppr vars
                               , text "Matches:"])
                               2
                               (vcat (map ppr matches)))
  res <- checkMatches' hs_ctx vars matches
  dsPmWarn dflags ctxt vars res

-- | Check a matchgroup (case, functions, etc.).
checkMatches' :: HsMatchContext id -> [Id] -> [LMatch GhcTc (LHsExpr GhcTc)] -> DsM PmResult
checkMatches' ctxt vars matches = do
  missing <- case ctxt of
    CaseAlt -> do
      delta <- getPmDelta
      -- case is strict in its match var x. So we add x /~ ⊥ as the first
      -- step by calling pmCheck on a fake clause @!x | False@.
      grds <- mkSeqClause vars
      presultUncovered <$> pmCheck grds [] 1 delta
    _       -> singleton <$> getPmDelta
...
-- | Returns a clause that is strict in the given variables and always fails.
-- @mkSeqClause [x,y,z] ==> [!x, !y, !z, let b = False, True <- b]@.
mkSeqClause :: [Id] -> DsM GrdVec
mkSeqClause vars = do
  b <- mkPmId boolTy
  let fail_clause = [PmLet b (Var falseDataConId), vanillaConGrd b trueDataCon []]
  pure $ map PmBang vars ++ fail_clause
