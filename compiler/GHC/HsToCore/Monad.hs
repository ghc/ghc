...
initDs :: HscEnv -> TcGblEnv -> DsM a -> IO (Messages DsError, Maybe a)
...
mkDsEnvsFromTcGbl :: MonadIO m
                  => HscEnv -> IORef (Messages DsError) -> TcGblEnv
                  -> m (DsGblEnv, DsLclEnv)
...
runDs :: HscEnv -> (DsGblEnv, DsLclEnv) -> DsM a -> IO (Messages DsError, Maybe a)
...
initTcDsForSolver :: TcM a -> DsM (Messages DsError, Maybe a)
...
mkDsEnvs :: DynFlags -> Module -> GlobalRdrEnv -> TypeEnv -> FamInstEnv
         -> IORef (Messages DsError) -> IORef CostCentreState -> [CompleteMatch]
         -> (DsGblEnv, DsLclEnv)
...
errDs err
  = do  { env <- getGblEnv
        ...
        ; updMutVar (ds_msgs env) (\ (w,e) -> (w, e `snocBag` TcRnError msg)) }
