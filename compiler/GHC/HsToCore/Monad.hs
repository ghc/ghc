...
import GHC.Tc.Errors.Types  as TcRn (DsError, DsWarning, Error(..), Warning(..))
...
import GHC.Types.Error ( snocWarning
                       , snocError
                       )
...
initDs :: HscEnv -> TcGblEnv -> DsM a -> IO (Messages DsWarning DsError, Maybe a)
...
mkDsEnvsFromTcGbl :: MonadIO m
                  => HscEnv -> IORef (Messages DsWarning DsError) -> TcGblEnv
                  -> m (DsGblEnv, DsLclEnv)
...
runDs :: HscEnv -> (DsGblEnv, DsLclEnv) -> DsM a -> IO (Messages DsWarning DsError, Maybe a)
...
initDsWithModGuts :: HscEnv -> ModGuts -> DsM a -> IO (Messages DsWarning DsError, Maybe a)
...
initTcDsForSolver :: TcM a -> DsM (Messages DsWarning DsError, Maybe a)
...
mkDsEnvs :: UnitState -> HomeUnit -> Module -> GlobalRdrEnv -> TypeEnv -> FamInstEnv
         -> IORef (Messages DsWarning DsError) -> IORef CostCentreState -> CompleteMatches
         -> (DsGblEnv, DsLclEnv)
...
warnDs reason warn
  = do { env <- getGblEnv
       ; loc <- getSrcSpanDs
       ; let msg = fmap WarnTcRnRaw $ makeIntoWarning reason $
                   mkWarnMsg loc (ds_unqual env) warn
       ; updMutVar (ds_msgs env) (\ (w,e) -> (w `snocWarning` msg, e)) }
...
errDs err
  = do  { env <- getGblEnv
        ; loc <- getSrcSpanDs
        ; let msg = fmap ErrTcRnRaw $ mkErrMsg loc (ds_unqual env) err
        ; updMutVar (ds_msgs env) (\ (w, e) -> (w, e `snocError` msg)) }
...
askNoErrsDs thing_inside
 = do { errs_var <- newMutVar emptyMessages
      ...
      ; updMutVar (ds_msgs env) (\ (w,e) -> (w `mappend` warns, e `mappend` errs))
