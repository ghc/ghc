...
to a function.
The paper explains it more fully, but briefly: expressions in these contexts
need to be stored in registers, and it's hard (read, impossible) to
store something that's representation-polymorphic.
...
-- of representationy polymorphism checks and then avoiding making a core App.
