...
initDs :: HscEnv -> TcGblEnv -> DsM a -> IO (Messages ErrDoc DsError, Maybe a)
...
mkDsEnvsFromTcGbl :: MonadIO m
                  => HscEnv -> IORef (Messages ErrDoc DsError) -> TcGblEnv
                  -> m (DsGblEnv, DsLclEnv)
...
runDs :: HscEnv -> (DsGblEnv, DsLclEnv) -> DsM a -> IO (Messages ErrDoc DsError, Maybe a)
...
initDsWithModGuts :: HscEnv -> ModGuts -> DsM a -> IO (Messages ErrDoc DsError, Maybe a)
...
initTcDsForSolver :: TcM a -> DsM (Messages ErrDoc DsError, Maybe a)
...
mkDsEnvs :: UnitState -> HomeUnit -> Module -> GlobalRdrEnv -> TypeEnv -> FamInstEnv
         -> IORef (Messages ErrDoc DsError) -> IORef CostCentreState -> CompleteMatches
         -> (DsGblEnv, DsLclEnv)
