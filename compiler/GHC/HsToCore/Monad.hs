...
initDs :: HscEnv -> TcGblEnv -> DsM a -> IO (Messages DecoratedMessage, Maybe a)
...
mkDsEnvsFromTcGbl :: MonadIO m
                  => HscEnv -> IORef (Messages DecoratedMessage) -> TcGblEnv
                  -> m (DsGblEnv, DsLclEnv)
...
runDs :: HscEnv -> (DsGblEnv, DsLclEnv) -> DsM a -> IO (Messages DecoratedMessage, Maybe a)
...
initDsWithModGuts :: HscEnv -> ModGuts -> DsM a -> IO (Messages DecoratedMessage, Maybe a)
...
initTcDsForSolver :: TcM a -> DsM (Messages DecoratedMessage, Maybe a)
...
mkDsEnvs :: UnitEnv -> Module -> GlobalRdrEnv -> TypeEnv -> FamInstEnv
         -> IORef (Messages DecoratedMessage) -> IORef CostCentreState -> CompleteMatches
         -> (DsGblEnv, DsLclEnv)
...
warnIfSetDs flag warn
  = whenWOptM flag $
    diagnosticDs (WarnReasonWithFlag flag) warn

errDs :: SDoc -> DsM ()
errDs err
  = do  { env <- getGblEnv
        ; loc <- getSrcSpanDs
        ; let msg = mkShortMsgEnvelope ErrReason loc (ds_unqual env) err
        ; updMutVar (ds_msgs env) (\ msgs -> msg `addMessage` msgs) }
