...
import qualified Language.Haskell.TH as LangExt
...
mkDependencies :: DynFlags -> UnitId -> [Module] -> TcGblEnv -> IO Dependencies
mkDependencies dflags iuid pluginModules
 ...
 = do
      ...
      let th_enabled    = [thUnitId | xopt LangExt.TemplateHaskellQuotes dflags]
          (home_plugins, package_plugins) = partition ((== iuid) . toUnitId . moduleUnit) pluginModules
          ...
          direct_pkgs_0 = foldr Set.insert (imp_dep_direct_pkgs imports) (th_enabled ++ plugin_dep_pkgs)
