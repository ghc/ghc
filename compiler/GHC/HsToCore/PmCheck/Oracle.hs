...
lookupRefuts MkDelta{ delta_tm_st = ts@(TmSt (SDIE env) _) } k =
  case lookupUDFM env k of
    Nothing -> []
    Just (Indirect y) -> pmAltConSetToList (vi_neg (lookupVarInfo ts y))
    Just (Entry vi)   -> pmAltConSetToList (vi_neg vi)
...
addNotConCt delta@MkDelta{ delta_tm_st = TmSt env reps } x nalt = do
  ...
  let neg'
        ...
        | otherwise              = addPmAltConSet neg nalt
  ...
  = ASSERT( not (sameRepresentativeSDIE env x y) )
    case (lookupSDIE env x, lookupSDIE env y) of
      ...
      (Just vi_x, Just vi_y) -> do
        ...
        delta_neg <- foldlM add_refut delta_pos (pmAltConSetToList (vi_neg vi_x))
    ...
    go (x:xs) n delta = do
      ...
      case pos of
        ...
        []
          ...
          | notNull [ l | PmAltLit l <- pmAltConSetToList neg ]
          -> go xs n delta
