...
dsSpec mb_poly_rhs (L loc (SpecPrag poly_id spec_co spec_inl))
  ...
  = putSrcSpanDs loc $
    do { diagnosticDs WarnReason (text "Ignoring useless SPECIALISE pragma for class method selector"
                                  <+> quotes (ppr poly_id))
  ...
  = putSrcSpanDs loc $
    do { diagnosticDs WarnReason (text "Ignoring useless SPECIALISE pragma for NOINLINE function:"
                                  <+> quotes (ppr poly_id))
...
dsMkUserRule this_mod is_local name act fn bndrs args rhs = do
    ...
    when (isOrphan (ru_orphan rule) && wopt Opt_WarnOrphans dflags) $
        diagnosticDs (WarnReasonWithFlag Opt_WarnOrphans) (ruleOrphWarn rule)
    return rule
