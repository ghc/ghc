...
matchEnv env_ids body = do
    tup_id <- newSysLocalDs (LambdaBound ManyTy) (mkBigCoreVarTupTy env_ids)
...
dsCmdDo ids local_vars res_ty [L _ (LastStmt _ body _ _)] env_ids = do
    ...
    let env_ty = mkBigCoreVarTupTy env_ids
    env_var <- newSysLocalDs (LambdaBound ManyTy) env_ty
...
dsCmdStmt ids local_vars out_ids (BindStmt _ pat cmd) env_ids = do
    ...
    pair_id   <- newSysLocalDs (LambdaBound ManyTy) after_c_ty
    let
