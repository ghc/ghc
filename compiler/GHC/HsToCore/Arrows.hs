...
dsProcExpr pat (L _ (HsCmdTop (CmdTopTc _unitTy cmd_ty ids) cmd)) = do
    ...
    let locals = mkVarSet (collectArrowPatBinders pat)
...
dsCmdLam ids local_vars stack_ty res_ty pats body env_ids = do
    let pat_vars = mkVarSet (collectArrowPatsBinders pats)
...
dsCmdDo ids local_vars res_ty (stmt:stmts) env_ids = do
    let bound_vars  = mkVarSet (collectArrowLStmtBinders stmt)
...
dsCmdStmt ids local_vars out_ids (BindStmt _ pat cmd) env_ids = do
    ...
    let pat_vars = mkVarSet (collectArrowPatBinders pat)
    let
...
leavesMatch (L _ (Match { m_pats = pats
  ...
  = let
        defined_vars = mkVarSet (collectArrowPatsBinders pats)
                        `unionVarSet`
                       mkVarSet (collectLocalBinders binds)
    in
    [(body,
      mkVarSet (collectArrowLStmtsBinders stmts)
