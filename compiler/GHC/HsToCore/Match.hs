...
matchWrapper ctxt' scrs (MG { mg_alts = L _ matches
  ...
  where
    ...
    mk_eqn_info ::  HsMatchContextRn  -> LMatch GhcTc (LHsExpr GhcTc) -> (Nablas, NonEmpty Nablas) -> DsM EquationInfo
    ...
    match_ctxt_mb :: Origin -> LMatch GhcTc (LHsExpr GhcTc) -> Maybe HsMatchContextRn
    match_ctxt_mb origin (L _ match) | e@(StmtCtxt{}) <- m_ctxt match,  isGenerated origin = Just e
    match_ctxt_mb _ _ = Nothing

    mkActualMatchCtxt :: HsMatchContextRn -> Origin -> [LMatch GhcTc (LHsExpr GhcTc)] -> HsMatchContextRn
