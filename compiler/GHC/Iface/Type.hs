...
suppressIfaceInvisibles :: PrintExplicitKindsOption -> [IfaceTyConBinder] -> [a] -> [a]
suppressIfaceInvisibles PrintExplicitKinds     _tys xs = xs
suppressIfaceInvisibles DontPrintExplicitKinds  tys xs
  = suppress tys xs
    where
      suppress _       []      = []
      suppress []      a       = a
...
stripIfaceInvisVars :: PrintExplicitKindsOption -> [IfaceTyConBinder] -> [IfaceTyConBinder]
stripIfaceInvisVars PrintExplicitKinds     tyvars = tyvars
stripIfaceInvisVars DontPrintExplicitKinds tyvars
...
stripInvisArgs :: PrintExplicitKindsOption -> IfaceAppArgs -> IfaceAppArgs
stripInvisArgs PrintExplicitKinds     tys = tys
stripInvisArgs DontPrintExplicitKinds tys = suppress_invis tys
    where
      suppress_invis c
        = case c of
            IA_Nil -> IA_Nil
...
pprIfaceForAllBndr bndr =
  case bndr of
    Bndr (IfaceTvBndr tv) Inferred ->
      ppIfOption sdocPrintExplicitForalls
...
pprTyTcApp' :: PprPrec -> IfaceTyCon -> IfaceAppArgs
            -> PrintExplicitKindsOption -> PprStyle -> SDoc
pprTyTcApp' ctxt_prec tc tys printExplicitKinds style
  ...
  | tc `ifaceTyConHasKey` consDataConKey
  , DontPrintExplicitKinds <- printExplicitKinds
  , IA_Arg _ argf (IA_Arg ty1 Required (IA_Arg ty2 Required IA_Nil)) <- tys
  , isInvisibleArgFlag argf
  = pprIfaceTyList ctxt_prec ty1 ty2
...
ppr_kind_type ctxt_prec =
  ppIfOption sdocStarIsType
    (maybeParen ctxt_prec starPrec $
