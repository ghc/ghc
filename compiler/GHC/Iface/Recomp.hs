...
checkVersions hsc_env mod_summary iface self_recomp
  = do { liftIO $ trace_hi_diffs logger
       ...
       ; hsc_env <- getTopEnv
       ; if mi_src_hash self_recomp /= ms_hs_hash mod_summary
...
checkPlugins plugins self_recomp = liftIO $ do
  ...
  let old_fingerprint = mi_plugin_hash self_recomp
...
checkFlagHash hsc_env iface_mod self_recomp = do
    let logger   = hsc_logger hsc_env
    let old_hash = mi_flag_hash self_recomp
...
checkHpcHash hsc_env self_recomp = do
    let logger   = hsc_logger hsc_env
    let old_hash = mi_hpc_hash self_recomp
...
checkMergedSignatures hsc_env mod_summary self_recomp = do
    ...
    let old_merged = sort [ mod | UsageMergedRequirement{ usg_mod = mod } <- mi_usages self_recomp ]
...
addFingerprints hsc_env iface0
 = do
   ...
   self_recomp <-
    case mi_self_recomp_info iface0 of
      Just self_recomp0 -> do
        sr_info <- mkSelfRecomp
        return $ Just $ set_self_recomp_backend_info self_recomp0 sr_info
      Nothing -> return Nothing

   iface_hash <- computeFingerprint putNameLiterally
                        (mod_hash,
                         mi_src_hash <$> (mi_self_recomp_info iface0),
                         ann_fn (mkVarOccFS (fsLit "module")),  -- See mkIfaceAnnCache
                         mi_usages <$> (mi_self_recomp_info iface0),
                         sorted_deps,
                         mi_hpc iface0)
   ...
   let
    ...
    final_iface = completePartialModIface iface0
                    sorted_decls sorted_extra_decls self_recomp final_iface_exts
   --
   return final_iface
