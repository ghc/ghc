...
check_old_iface hsc_env mod_summary src_modified maybe_iface
  = let dflags = extractDynFlags hsc_env
    ...
    in do
        when src_changed $
            liftIO $ trace_hi_diffs logger (nest 4 $ text "Source file changed or recompilation check turned off")
...
checkOptimHash hsc_env iface = do
    ...
    new_hash <- fingerprintOptFlags (extractDynFlags hsc_env)
                                               putNameLiterally
    if | old_hash == new_hash
         -> up_to_date logger (text "Optimisation flags unchanged")
       | gopt Opt_IgnoreOptimChanges (extractDynFlags hsc_env)
...
checkHpcHash hsc_env iface = do
    ...
    new_hash <- fingerprintHpcFlags (extractDynFlags hsc_env)
                                               putNameLiterally
    if | old_hash == new_hash
         -> up_to_date logger (text "HPC flags unchanged")
       | gopt Opt_IgnoreHpcChanges (extractDynFlags hsc_env)
