...
data RecompileRequiredWithEvidence
...
checkOldIface
  ...
  -> IO RecompileRequiredWithEvidence
...
check_old_iface
  ...
  -> SourceModified
  -> Maybe ModIface
  -> IfG RecompileRequiredWithEvidence
        ...
        case src_changed of
            ...
            False -> do
                ...
                case maybe_iface' of
                    ...
                    Nothing    -> return $ NeedsRecompileWithOld MustCompile Nothing
...
checkVersions :: HscEnv
              ...
              -> IfG RecompileRequiredWithEvidence
checkVersions hsc_env mod_summary iface = replace_either_with_meaningful_type $ do
  lift $ traceHiDiffs $
  ...
  throw_recomp Nothing $ checkMergedSignatures mod_summary iface
  throw_recomp Nothing $ checkHsig mod_summary iface
  throw_recomp Nothing $ checkHie mod_summary
  throw_recomp (Just iface) $ checkDependencies hsc_env mod_summary iface
  throw_recomp Nothing $ checkPlugins hsc_env iface
  ...
  -- quite redundant
  lift $ updateEps_ $ \eps  -> eps { eps_is_boot = mod_deps }
  throw_recomp (Just iface) $
    checkList [checkModUsage this_pkg u | u <- mi_usages iface]
  pure $ iface
  where
    replace_either_with_meaningful_type
      :: ExceptT (CompileReason, Maybe ModIface) IfG ModIface
      -> IfG RecompileRequiredWithEvidence
    replace_either_with_meaningful_type = fmap go . runExceptT
      where
        go = \case
          Right x -> UpToDateWithEvidence x
          Left (y, z) -> NeedsRecompileWithOld y z
    throw_recomp :: a -> IfG RecompileRequired -> ExceptT (CompileReason, a) IfG ()
    throw_recomp x action = lift action >>= \case
      UpToDate -> pure ()
      NeedsRecompile reason -> throwE (reason, x)
