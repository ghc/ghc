...
--
-- Thunks are represented as LFUnknown: top-level thunks are never single-entry,
-- and for updatable thunks LFThunk and LFUnknown calling convention is the
-- same, see GHC.StgToCmm.Closure.getCallMethod.
data IfaceLFInfo
  = IfLFReEntrant !RepArity
  | IfLFCon !Name
  | IfLFUnknown
  | IfLFUnlifted
    -- Generally, top-level things can't be unlifted, but we have an exception
    -- for literal strings -- see Note [Core top-level string literals] in
    -- GHC.Core
