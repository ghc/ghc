...
rnDepModules sel mods = do
    ...
    fmap (nubSort . concat) . T.forM mods $ \mod -> do
        ...
        let mod' = renameHoleModule unit_index (hsc_units hsc_env) hmap mod
        if isHoleModule mod
...
initRnIface hsc_env iface insts nsubst do_this = do
    ...
    let hsubst = listToUFM insts
        rn_mod = renameHoleModule unit_index (hsc_units hsc_env) hsubst
        env = ShIfEnv {
...
rnModule mod = do
    ...
    return (renameHoleModule unit_index unit_state hmap mod)
...
rnIfaceGlobal n = do
    ...
    hmap <- getHoleSubst
    let m = nameModule n
        m' = renameHoleModule unit_index unit_state hmap m
    case () of
...
rnIfaceNeverExported name = do
    ...
    iface_semantic_mod <- fmap sh_if_semantic_module getGblEnv
    let m = renameHoleModule unit_index unit_state hmap $ nameModule name
