...
* GHC.Iface.Tidy.addExternal.  Expose compulsory unfoldings, even with -O0.

* GHC.Iface.Tidy.trimId.  Don't trim a compulsory unfolding, even with
  -O0 (-fomit-interface-pragmas)

* GHC.IfaceToCore.tcIdInfo.  When reading in from interface file, even
  with -O0 (fignore-interface-pragmas.)  we must load a compulsory
  unfolding
...
trimId omit_prags id
  ...
  = id `setIdInfo`      vanillaIdInfo
       `setIdUnfolding` trimmed_unfolding

  | otherwise   -- No trimming
  = id
  where
    unf = idUnfolding id
    trimmed_unfolding  -- See Note [Always expose compulsory unfoldings]
