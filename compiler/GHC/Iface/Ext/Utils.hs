...
selectPoint hf (sl,sc) = getFirst $
  flip foldMap (M.toList (getAsts $ hie_asts hf)) $ \(FilePathFS fs,ast) -> First $
...
resolveTyVarScopes :: M.Map FilePathFS (HieAST a) -> M.Map FilePathFS (HieAST a)
resolveTyVarScopes asts = M.map go asts
  where
    go ast = resolveTyVarScopeLocal ast asts

resolveTyVarScopeLocal :: HieAST a -> M.Map FilePathFS (HieAST a) -> HieAST a
...
getNameBinding :: Name -> M.Map FilePathFS (HieAST a) -> Maybe Span
getNameBinding n asts = do
  (_,msp) <- getNameScopeAndBinding n asts
  msp

getNameScope :: Name -> M.Map FilePathFS (HieAST a) -> Maybe [Scope]
...
getNameBindingInClass
  :: Name
  -> Span
  -> M.Map FilePathFS (HieAST a)
  -> Maybe Span
getNameBindingInClass n sp asts = do
  ast <- M.lookup (FilePathFS (srcSpanFile sp)) asts
...
definedInAsts :: M.Map FilePathFS (HieAST a) -> Name -> Bool
definedInAsts asts n = case nameSrcSpan n of
  RealSrcSpan sp _ -> M.member (FilePathFS (srcSpanFile sp)) asts
  _ -> False
