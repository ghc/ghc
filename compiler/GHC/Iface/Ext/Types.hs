...
instance Binary (HieType TypeIndex) where
  putNoStack_ bh (HTyVarTy n) = do
    putByte bh 0
    put_ bh n
  putNoStack_ bh (HAppTy a b) = do
    putByte bh 1
    put_ bh a
    put_ bh b
  putNoStack_ bh (HTyConApp n xs) = do
    putByte bh 2
    put_ bh n
    put_ bh xs
  putNoStack_ bh (HForAllTy bndr a) = do
    putByte bh 3
    put_ bh bndr
    put_ bh a
  putNoStack_ bh (HFunTy w a b) = do
    putByte bh 4
    put_ bh w
    put_ bh a
    put_ bh b
  putNoStack_ bh (HQualTy a b) = do
    putByte bh 5
    put_ bh a
    put_ bh b
  putNoStack_ bh (HLitTy l) = do
    putByte bh 6
    put_ bh l
  putNoStack_ bh (HCastTy a) = do
    putByte bh 7
    put_ bh a
  putNoStack_ bh (HCoercionTy) = putByte bh 8
...
instance Binary (HieArgs TypeIndex) where
  putNoStack_ bh (HieArgs xs) = put_ bh xs
...
instance Binary (HieASTs TypeIndex) where
  putNoStack_ bh asts = put_ bh $ M.toAscList $ getAsts asts
...
instance Binary (SourcedNodeInfo TypeIndex) where
  putNoStack_ bh asts = put_ bh $ M.toAscList $ getSourcedNodeInfo asts
...
instance Binary NodeAnnotation where
  putNoStack_ bh (NodeAnnotation c t) = do
    put_ bh c
    put_ bh t
...
instance Binary ContextInfo where
  putNoStack_ bh Use = putByte bh 0
  putNoStack_ bh (IEThing t) = do
    putByte bh 1
    put_ bh t
  putNoStack_ bh TyDecl = putByte bh 2
  putNoStack_ bh (ValBind bt sc msp) = do
    putByte bh 3
    put_ bh bt
    put_ bh sc
    put_ bh $ BinSpan <$> msp
  putNoStack_ bh (PatternBind a b c) = do
    putByte bh 4
    put_ bh a
    put_ bh b
    put_ bh $ BinSpan <$> c
  putNoStack_ bh (ClassTyDecl sp) = do
    putByte bh 5
    put_ bh $ BinSpan <$> sp
  putNoStack_ bh (Decl a b) = do
    putByte bh 6
    put_ bh a
    put_ bh $ BinSpan <$> b
  putNoStack_ bh (TyVarBind a b) = do
    putByte bh 7
    put_ bh a
    put_ bh b
  putNoStack_ bh (RecField a b) = do
    putByte bh 8
    put_ bh a
    put_ bh $ BinSpan <$> b
  putNoStack_ bh MatchBind = putByte bh 9
  putNoStack_ bh (EvidenceVarBind a b c) = do
    putByte bh 10
    put_ bh a
    put_ bh b
    put_ bh $ BinSpan <$> c
  putNoStack_ bh EvidenceVarUse = putByte bh 11
...
instance Binary EvVarSource where
  putNoStack_ bh EvPatternBind = putByte bh 0
  putNoStack_ bh EvSigBind = putByte bh 1
  putNoStack_ bh EvWrapperBind = putByte bh 2
  putNoStack_ bh EvImplicitBind = putByte bh 3
  putNoStack_ bh (EvInstBind b cls) = do
    putByte bh 4
    put_ bh b
    put_ bh cls
  putNoStack_ bh (EvLetBind deps) = do
    putByte bh 5
    put_ bh deps
...
instance Binary EvBindDeps where
  putNoStack_ bh (EvBindDeps xs) = put_ bh xs
...
instance Binary Scope where
  putNoStack_ bh NoScope = putByte bh 0
  putNoStack_ bh (LocalScope span) = do
    putByte bh 1
    put_ bh $ BinSpan span
  putNoStack_ bh ModuleScope = putByte bh 2
...
instance Binary TyVarScope where
  putNoStack_ bh (ResolvedScopes xs) = do
    putByte bh 0
    put_ bh xs
  putNoStack_ bh (UnresolvedScope ns span) = do
    putByte bh 1
    put_ bh ns
    put_ bh (BinSpan <$> span)
