...
loadSrcInterface_maybe doc mod want_boot maybe_pkg
  ...
  = do { hsc_env <- getTopEnv
       ...
       ; traceIf $ text "loadSrcInterface_maybe IC deps:"   <+> ppr (explicitUnits $ unitState $ ic_dflags $ hsc_IC hsc_env)
       ; traceIf $ text "loadSrcInterface_maybe IC dflags:" <+> ppr (homeUnitId_ $ ic_dflags $ hsc_IC hsc_env)
       ; traceIf $ text "loadSrcInterface_maybe IC import paths:"
            $$ nest 2 (vcat $ map text $ importPaths $ ic_dflags $ hsc_IC hsc_env)
