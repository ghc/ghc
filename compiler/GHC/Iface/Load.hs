...
pprModIface unit_state iface
 = vcat $ [ text "interface"
        ...
        , withSelfRecomp iface empty $ \(ModIfaceSelfRecomp src usages backend_info) -> vcat
                [ nest 2 (text "src_hash:" <+> ppr src)
                , nest 2 (text "flag hash:" <+> ppr (mi_sr_flag_hash backend_info))
                , nest 2 (text "opt_hash:" <+> ppr (mi_sr_opt_hash backend_info))
                , nest 2 (text "hpc_hash:" <+> ppr (mi_sr_hpc_hash backend_info))
                , nest 2 (text "plugin_hash:" <+> ppr (mi_sr_plugin_hash backend_info))
