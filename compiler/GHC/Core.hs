...
--       in GHC.Core.ConstantFold for rationale.
--
...
-- The instance adheres to the order described in [GHC.Core case invariants]
...
GHC.Core.ConstantFold for the rationale for this restriction.
...
like as many functions as possible to become join points (see GHC.Core.OccurAnal) and
...
The CoreRule type and its friends are dealt with mainly in GHC.Core.Rules,
but GHC.Core.{FVs,Subst,Pretty,Tidy} also inspect the representation.
-}
