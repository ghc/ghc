  ...
  = do
    ...
    where
        ...
        uses_pic_base_implicitly = do
            ...
            when (configPIC config && target32Bit platform) $ do
...
genSwitch dflags expr targets
  ...
  | (configPIC config) || (not $ target32Bit platform)
  = do
        (reg,e_code) <- getSomeReg (cmmOffset platform expr offset)
        let fmt = archWordFormat $ target32Bit platform
            sha = if target32Bit platform then 2 else 3
...
generateJumpTableForInstr :: Config -> Instr
                          -> Maybe (NatCmmDecl RawCmmStatics Instr)
generateJumpTableForInstr config (BCTR ids (Just lbl) _) =
    let jumpTable
            | (configPIC config) || (not $ target32Bit $ configPlatform config)
            = map jumpTableEntryRel ids
            | otherwise = map (jumpTableEntry config) ids
                where jumpTableEntryRel Nothing
                        = CmmStaticLit (CmmInt 0 (configWordWidth config))
                      jumpTableEntryRel (Just blockid)
                        = CmmStaticLit (CmmLabelDiffOff blockLabel lbl 0
                                         (configWordWidth config))
                            where blockLabel = blockLbl blockid
