...
pprProcAlignment :: Config -> SDoc
pprProcAlignment config = maybe empty (pprAlign platform . mkAlignment) (configProcAlignment config)
   where
      platform = configPlatform config

pprNatCmmDecl :: Config -> NatCmmDecl (Alignment, RawCmmStatics) Instr -> SDoc
...
pprBasicBlock :: Config -> LabelMap RawCmmStatics -> NatBasicBlock Instr -> SDoc
...
pprDatas :: Config -> (Alignment, RawCmmStatics) -> SDoc
...
-- POPA  -> text "\tpopal"
   ...
   CVTSS2SD from to
      -> pprRegReg platform (sLit "cvtss2sd") from to

   CVTSD2SS from to
      -> pprRegReg platform (sLit "cvtsd2ss") from to

   CVTTSS2SIQ fmt from to
      -> pprFormatFormatOpReg platform (sLit "cvttss2si") FF32 fmt from to

   CVTTSD2SIQ fmt from to
      -> pprFormatFormatOpReg platform (sLit "cvttsd2si") FF64 fmt from to
  ...
  where
   ...
   pprRegReg :: Platform -> PtrString -> Reg -> Reg -> SDoc
   ...
   pprFormatFormatOpReg :: Platform -> PtrString -> Format -> Format -> Operand -> Reg -> SDoc
   pprFormatFormatOpReg platform name format1 format2 op1 reg2
     = hcat [
