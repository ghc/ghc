...
jumpTableEntry :: Config -> Maybe BlockId -> CmmStatic
jumpTableEntry config Nothing = CmmStaticLit (CmmInt 0 (configWordWidth config))
        ...
        _ ->
            do
               let
                 ...
                 format  = fmt
               --
               platform <- configPlatform <$> getConfig
               return (Fixed format
...
getSimpleAmode is32Bit addr
    | is32Bit = do
        ...
        config <- getConfig
        addr_r <- getNewRegNat (intFormat (configWordWidth config))
...
generateJumpTableForInstr :: Config -> Instr -> Maybe (NatCmmDecl (Alignment, RawCmmStatics) Instr)
...
createJumpTable :: Config -> [Maybe BlockId] -> Section -> CLabel
                -> GenCmmDecl (Alignment, RawCmmStatics) h g
createJumpTable config ids section lbl
    = let jumpTable
            | configPIC config =
                  let ww = configWordWidth config
