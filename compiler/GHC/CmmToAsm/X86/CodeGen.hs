...
getVecRegisterReg :: Platform -> Format -> CmmReg -> Reg
getVecRegisterReg _ format (CmmLocal (LocalReg u ty))
  | isVecType ty = RegVirtual (mkVirtualReg u format)
  | otherwise    = pprPanic "getVecRegisterReg: not a vector register"
                     (ppr ty)
getVecRegisterReg platform _format c = getRegisterReg platform c
...
getRegister' platform is32Bit (CmmReg reg)
  = case reg of
        ...
        _ ->
            do let cmmregtype = cmmRegType reg
               return $
                 if isVecType cmmregtype
                 then vectorRegister cmmregtype
                 else standardRegister cmmregtype
  where
    vectorRegister :: CmmType -> Register
    vectorRegister reg_ty
      = let vecfmt   = cmmTypeFormat reg_ty
        in Fixed vecfmt (getVecRegisterReg platform vecfmt reg) nilOL

    standardRegister crt =
      Fixed (cmmTypeFormat crt) (getRegisterReg platform reg) nilOL
...
getRegister' platform is32Bit (CmmMachOp mop [x]) = do -- unary MachOps
    ...
    case mop of
      ...
      MO_VF_Broadcast l W64 -> vector_float_broadcast_sse l W64 x
      MO_VF_Broadcast {} -> incorrectOperands

      MO_V_Broadcast l W64 -> vector_int_broadcast l W64 x
    ...
    vector_double_insert avx len@2 vecExpr valExpr (CmmLit offset)
      = do
        ...
        let movu = if avx then VMOVU else MOVU
         ...
         in return $ Any fmt code
    vector_double_insert _ len _ _ offset
      = pprPanic "Unsupported vector insert operation" $
          vcat
            [ text "DoubleX" <> ppr len <> text "#"
            , text "offset:" <+> pdoc platform offset ]
...
getRegister' platform _ (CmmLit lit) =
  case fmt of
    VecFormat l sFmt
      ...
      -- TODO: mishandles negative zero (because -0.0 == 0.0 returns True)
      -> do config <- getConfig
...
assignReg_IntCode ty reg (CmmLoad src _ _) = do
...
assignReg_VecCode format reg src = do
  platform <- ncgPlatform <$> getConfig
  src_code <- getAnyReg src
  return (src_code (getVecRegisterReg platform format reg))
