...
getRegister' config plat expr
  = case expr of
    ...
    CmmMachOp op [e] -> do
      ...
      case op of
        ...
        MO_S_Neg w -> truncateSubwordRegister w <$> do
          ...
          pure $ Any (intFormat w) $ \dst -> reg_code `snocOL` (NEG (OpReg op_w dst) (OpReg op_w src))

        MO_F_Neg w -> return $ Any fmt (\dst -> code `snocOL` NEG (OpReg w dst) (OpReg w reg))
    ...
    CmmMachOp (MO_And w) [(CmmReg reg), CmmLit (CmmInt n _)] | Just op_bitmask <- getBitmaskImm n w ->
      return $ Any fmt (\d -> unitOL $ annExpr expr (AND (OpReg w d) (OpReg w' r') op_bitmask))
    ...
    CmmMachOp (MO_Or w) [(CmmReg reg), CmmLit (CmmInt n _)] | Just op_bitmask <- getBitmaskImm n w ->
      return $ Any fmt (\d -> unitOL $ annExpr expr (ORR (OpReg w d) (OpReg w' r') op_bitmask))
      where fmt = intFormat w
            w' = formatToWidth (cmmTypeFormat (cmmRegType reg))
            r' = getRegisterReg plat reg
    ...
    CmmMachOp op [x, y] -> do
      ...
      case op of
        ...
        MO_Add w -> intOpImm False UnknownHighBits w (\d x y -> unitOL $ annExpr expr (ADD d x y)) getArithImm
        -- TODO: Handle sub-word case
        MO_Sub w -> intOpImm False UnknownHighBits w (\d x y -> unitOL $ annExpr expr (SUB d x y)) getArithImm
        ...
        MO_Mul w          -> intOp True UnknownHighBits w (\d x y -> unitOL $ MUL d x y)
        MO_S_MulMayOflo w -> do_mul_may_oflo w x y
        MO_S_Quot w       -> intOp True UnknownHighBits w (\d x y -> unitOL $ SDIV d x y)
        ...
        MO_S_Rem w -> withTempIntReg w $ \t ->
                      intOp True UnknownHighBits w (\d x y -> toOL [ SDIV t x y, MSUB d t y x ])
