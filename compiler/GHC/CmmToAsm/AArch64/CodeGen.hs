...
narrowS' W8  x = fromIntegral (fromIntegral (fromIntegral x :: Int8) :: Word8)
narrowS' W16 x = fromIntegral (fromIntegral (fromIntegral x :: Int16) :: Word16)
    ...
    CmmMachOp op [e] -> do
      ...
      where
        ...
        negate code w reg = do
            let w' =
                  case w of
                    ...
                    W16 -> W32
                    W8  -> W32
                    _   -> panic "negate: unsupported width"
        ...
        ss_conv from to reg code =
            let w' = case max from to of
                       W64 -> W64
                       W32 -> W32
                       W16 -> W32
                       W8  -> W32
                       _   -> panic "ss_conv: unsupported width"
    ...
    CmmMachOp op [x, y] -> do
      ...
      let withTempIntReg w op = OpReg w <$> getNewRegNat (intFormat w) >>= op
          ...
          floatOp w op = do
            ...
            massertPpr (isIntFormat format_x && isIntFormat format_y) $ text "floatOp: non-float"
