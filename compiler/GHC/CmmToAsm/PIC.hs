...
cmmMakePicReference :: Config -> CLabel -> CmmExpr
cmmMakePicReference config lbl
  ...
  | (configPIC config || configExternalDynamicRefs config)
      && absoluteLabel lbl
...
needImportedSymbols :: Config -> Bool
needImportedSymbols config
        ...
        | osElfTarget os
        , arch /= ArchPPC_64 ELF_V1 && arch /= ArchPPC_64 ELF_V2
        = configExternalDynamicRefs config &&
          not (configPIC config)
...
pprImportedSymbol :: DynFlags -> Config -> CLabel -> SDoc
pprImportedSymbol dflags config importedLbl = case (arch,os) of
   ...
   _ | osElfTarget os
     -> case dynamicLinkerLabelInfo importedLbl of
            Just (SymbolPtr, lbl)
              -> let symbolSize = case configWordWidth config of
                         W32 -> sLit "\t.long"
                         W64 -> sLit "\t.quad"
