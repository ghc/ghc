...
instance Foldable SccBits where
        foldr f z (AcyclicBit x)  = f x z
        foldr f z (CyclicBit _ x) = f x z


instance Traversable SccBits where
        traverse f (AcyclicBit x)  = AcyclicBit <$> f x
        traverse f (CyclicBit i x) = CyclicBit i <$> f x
