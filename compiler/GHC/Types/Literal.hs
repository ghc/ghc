...
data Literal
  ...
  | LitRubbish [PrimRep]        -- ^ A nonsense value of the given
                                ...
                                --
                                -- The @[PrimRep]@ can be obtained from
                                -- 'typeMonoPrimRep_maybe'. It becomes empty or
                                -- singleton post-unarisation,
...
litIsLifted (LitNumber nt _) = case nt of
  ...
  LitNumWord64  -> False
litIsLifted _                        = False
  -- Even __RUBBISH[LiftedRep] is unlifted, as rubbish values are always evaluated.
...
Note [Rubbish values]
~~~~~~~~~~~~~~~~~~~~~
Sometimes, we need to cough up rubbish values of a certain type that are used
in place of dead code we aim to eliminate. Such dead code might be evaluated,
but its value has no effect on the dynamic semantics of the program.
...
It's completely irrevant what the *value* of a rubbish value is, e.g., we pick
@0#@ for @Int#@, but we could also pick @42#@; it doesn't matter.
We embed these rubbish values in the 'LitRubbish' case of the 'Literal' data
type. Here are the moving parts:
  ...
  2. Core: 'LitRubbish' carries a @[PrimRep]@ which represents the monomorphic
     ...
     Why not encode a 'RuntimeRep' via a @Type@? Because
     ...
     We have it that @__RUBBISH[IntRep]@ has type @forall (a :: TYPE IntRep). a@,
     and the type application @__RUBBISH[IntRep] \@Int# :: Int#@ represents
     a rubbish value of type @Int#@. Rubbish lits are completely opaque in Core.
     In general, @__RUBBISH[preps] :: forall (a :: TYPE rep). a@, where @rep@
     is the 'RuntimeRep' corresponding to @preps :: [PrimRep]@
     (via 'primRepsToRuntimeRep'). See 'literalType'.

  3. STG: The @\@Int@ type app is erased and we get the (untyped) 'StgLit'
     @__RUBBISH[IntRep] :: Int#@ in STG. It's treated mostly opaque, with the
     exception of the Unariser, where we take apart a case scrutinisation on, or
     arg occurrence of, e.g., @__RUBBISH[IntRep,DoubleRep]@ (which may stand in
     for @(# Int#, Double# #)@) into its sub-parts @__RUBBISH[IntRep]@ and
     @__RUBBISH[DoubleRep]@, similar to unboxed tuples. @__RUBBISH[VoidRep]@ is
     erased.
     See also Note [Post-unarisation invariants].

  4. Cmm: We translate 'LitRubbish' to their actual rubbish value in 'cgLit'.
     The particulars are boring, except for the pick of @()@ as the rubbish val
     for any type of 'LiftedRep' or 'UnliftedRep'.
     Scrutinising @()@ where the code expects e.g. @Either Int Bool@ results in
     undefined behavior: Most probably we'd take the code path of the @Left@
     case and would follow a nonsense pointer if we'd access the @Int@,
     probably leading to a very indirect segfault. Hence it is really important
     that the value really is dead. If it wasn't (like it wasn't in #11126),
     it would be a compiler bug.
     This will also work for unlifted boxed values: The important thing
     is that the GC has a pointer to follow and that the heap object can
     be entered without crashing.

  5. Bytecode: In GHC.ByteCode.Asm we just lower it as a 0 literal, because it's
     all boxed to the host GC anyway.

Why lower in Cmm instead of STG? Because it enables us to use RubbishLit when
lowering unboxed sums in the future, and it allows rubbish values of VecRep,
