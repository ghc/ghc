...
2. Core: 'LitRubbish' carries a `Type` of kind RuntimeRep,
   ...
   INVARIANT: 'rr' has no free variables. Main reason: we don't need to run
...
6. IfaceSyn. Literal is part of IfaceSyn, but Type really isn't.  So in the
   passage from Core to Iface I put LitRubbish into its owns IfaceExpr data
...
b) Literal is in Ord, because (and only because) we use Ord on AltCon when
...
c) Why not lower LitRubbish in CoreToStg? Because it enables us to use
