...
Note [Lazy OccName to save FastStrings]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The Name `n_occ :: OccName` field MUST be kept lazy because it being strict
ends up forcing the creation and z-encoding of a lot more FastStrings than before.

There was a comprehensive investigation of regressions attributed to
mkFastString in !15899, which ended up being caused by attempting to make this
field strict there.

A few examples:

- Making this field strict resulted in forcing the FastString CAFs
for all primop strings in the `primOpInfo`. This showed up in all
compilation profiles (including 'main = pure ()') and regressed various
allocation tests (e.g. in T18140 it caused +1300KB (+2%) allocations).
(In the profiles it showed as `primOpDocsNNNN` CAFs because the top-level
FastStrings are shared in the primop docs and primop Ids, but what forces them
is the `mkPrimOpId <-- primOpOcc <-- primOpInfo` path)

- It made us allocate +30MB in test InstanceMatching because it forced the
z-encoding of a lot more FastStrings. Keeping it lazy avoids forcing the
z-encoding and more FastStrings, saving said 30MB.

Unfortunately, we ran out of steam to investigate WHY exactly we allocate many
more fast strings and z-encodings in this second example, even if the
measurements show it clearly happening.
-}
...
data Name = Name
  ...
  , n_occ  :: OccName
    -- ^ Its occurrence name.
    --
    -- NOTE: MUST be kept lazy for two reasons:
    --
    --  (1) allow known names to be known constructor applications and to
    --  inline better. See Note [Fast comparison for built-in Names]
    --
    --  (2) A strict n_occ forces a lot more FastStrings to be allocated and/or
    --  z-encoded. See Note [Lazy OccName to save FastStrings]
