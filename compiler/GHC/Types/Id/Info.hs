...
data IdInfo
  = IdInfo {
        ...
        -- ^ LambdaFormInfo of the Id. Not necessary, but if we know this we can
        -- generate more efficient calling code.
        --
        -- Not available for the Ids in the current module, and when an imported
        -- Id doesn't have a LambdaFormInfo in the interface (in which case we
        -- generate a conservative LFI based on the Id's type in
        -- GHC.StgToCmm.mkLFImported).
        --
        -- See also getCallMethod in GHC.StgToCmm.Closure for how LFIs are used
        -- when generating calling code, and Note [Conveying CAF-info and LFInfo
        -- between modules] in GHC.StgToCmm.Types for how we implement
        -- cross-module LFI passing.
    }
