...
unpackable_type_datacons ty
  ...
  where
    ...
    -- True if we can  unpack argument of type (T t1 .. tn),
    -- where T is a algebraic data type with these constructors
    -- See Note [Unpacking GADTs and existentials]
    unpackable_cons []   -- Don't upack nullary sums; no need.
...
Note [Unpacking GADTs and existentials]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Can we unpack a value of algebraic data type T? For example
...
(UC1) Single-constructor types (products).  We can just represent it by
   ...
   We can unpack like this.  The worker for MkD takes three unpacked fields:
   ...
   But we /can/ do it for (some) GADT, such as:
...
(UC2) Multi-constructor types, e.g.
  ...
  unboxed sum whose component was a unboxed tuple, whose component(a)
  have kind (CONSTRAINT r); and that's not well-kinded.  Hence the
    all isVanillaDataCon
  in  `unpackable_cons`. See #25672.
p
