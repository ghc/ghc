...
data SubDemand
  ...
  -- Furthermore, absent Polys ('C_00' and 'C_10') are always 'Unboxed'.
  -- Use the smart constructor 'mkPoly' whenever possible to assert this!
...
polyDmd Unboxed C_00 = AbsDmd
polyDmd Unboxed C_10 = BotDmd
polyDmd b       n    = n :* mkPoly b n

topSubDmd, topUbxSubDmd, botSubDmd, seqSubDmd :: SubDemand
topSubDmd    = mkPoly   Boxed C_0N
topUbxSubDmd = mkPoly Unboxed C_0N
botSubDmd    = mkPoly Unboxed C_10
seqSubDmd    = mkPoly Unboxed C_00

-- | A smart constructor for 'Prod' that asserts the invariants
--
--  1. The 'CardNonOnce' indeed satisfies 'isCardNonOnce'
--  2. If the cardinality is absent ('C_00', 'C_10'), then the boxity is 'Unboxed'.
--
mkPoly :: HasDebugCallStack => Boxity -> CardNonOnce -> SubDemand
mkPoly Boxed C_00 = pprPanic "boxed Absent" empty
mkPoly Boxed C_10 = pprPanic "boxed Bot" empty
mkPoly b     n    = Poly b n & assertPpr (isCardNonOnce n) (ppr b <+> ppr n)
...
mkProd b ds
  | all (== AbsDmd) ds, Unboxed <- b = seqSubDmd
  | all (== BotDmd) ds, Unboxed <- b = botSubDmd
...
viewCall (Poly b n)  = Just (n :: Card, Poly b n)
viewCall _           = Nothing

topDmd, absDmd, botDmd, seqDmd :: Demand
topDmd = polyDmd Boxed C_0N
absDmd = AbsDmd
botDmd = BotDmd
...
unboxSubDemand (Poly _ n)  = mkPoly Unboxed n
...
lubSubDmd (Poly b1 n1)  (Poly b2 n2) = mkPoly (lubBoxity b1 b2) (lubCard n1 n2)
...
-- Handle Poly. Exploit reflexivity (so we'll match the Prod or Call cases again).
plusSubDmd (Poly b1 n1) (Poly b2 n2) = mkPoly (plusBoxity b1 b2) (plusCard n1 n2)
...
multSubDmd n    (Poly b m)   = mkPoly b (multCard n m)
...
trimBoxity (n :* sd) = n :* go sd
  where
    go sd@(Poly _ n)
      | isAbs n   = sd -- not trimming needed; B and A can stay unboxed
      | otherwise = mkPoly Boxed n
