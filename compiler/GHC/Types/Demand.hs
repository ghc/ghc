...
'nopDmdType' to model the exceptional control flow) when @foo x s@
  * returns @State# RealWorld@ directly or as an unboxed tuple component.
    (Why not simply unboxed pairs as above? This is motivated by T13380{d,e}.)
  * AND cannot be proven *not* to throw a precise exception.
which is detected by 'DmdAnal.exprMayThrowPreciseException'.

In !3014 we tried something more sophisticated by introducing ConOrDiv (nic) to
the Divergence lattice, but in practice it turned out to be hard to untaint
...
Nevertheless, it's crucial to recognise PrimOps in exprMayThrow*, as they never
