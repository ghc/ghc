...
module GHC.Types.Demand (
        ...
        botDmdType, topDmdType, addDemand,
...
-- But that regresses in too many places to be worth it and is only
-- relevant in higher-order scenarios (e.g. Divergence of @f (throwIO blah)@),
-- for which we currently don't give any guarantees anyway.
...
To detect this scenario, we employ a very simple syntactic analysis
('DmdAnal.exprMayThrowPreciseException'). In !3014 we tried something more
sophisticated by introducing ConOrDiv (nic) to the Divergence lattice, but in
practice it turned out to be hard to untaint from 'topDiv' to 'conDiv', leading
to bugs, performance regressions and complexity that didn't justify the single
fixed testcase T13380c.

We act on this scenario with 'Demand.deferAfterPreciseException', which will
lub with the strictness analysis results of the virtual branch when analysing
the @Case@.

(Historic note: This used to be called the "IO hack".)

Nevertheless, it's crucial to recognise PrimOps in the analysis, as they never
throw precise exceptions (except for RaiseIOOp). Otherwise performance suffers
from the loss in strictness in e.g. GHC.IO.Handle.Internals.wantReadableHandle.
...
-- | Not strictly the Top element of the lattice because of usage demands on
-- free variables, but close enough.
topDmdType :: DmdType
topDmdType = DmdType emptyDmdEnv [] topDiv
...
-- decrease. Currently, it's just topDmdType.
decreaseArityDmdType :: DmdType -> DmdType
decreaseArityDmdType _ = topDmdType
...
deferAfterPreciseException d = lubDmdType d topDmdType
