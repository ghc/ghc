...
module GHC.Types.Termination (
    ...
    applyTerm, lamTerm, lamsTerm, forceTerm, markDivergingTerm, pruneDeepTerm,
    splitConTerm, splitLamTerm, expandConFieldsTerm, trimTermToArity,
    -- * Termination signatures
    Sig (..), TermSig, topTermSig, lamsTermSig, moreTermThanArity, seqTermSig,
...
conTerm dc terms  = Term Terminates (ConSh (dataConTag dc) terms')
  where
    ...
    add t str | isMarkedStrict str = getForcedTerm $ forceTermM seqSubDmd t
              | otherwise          = t
...
-- >>> applyTerm (Term MightDiverge (LamSh (Term Terminates BotSh)))
-- Term MightDiverge BotSh
applyTerm :: Term -> Term
applyTerm (splitLamTerm -> (tf, term)) = term `bothTerm` tf
...
lamsTerm arty res_cpr = iterate lamTerm res_cpr !! arty
...
--
-- Why is this necessary? See for example
-- Note [Understanding DmdType and StrictSig] in "GHC.Types.Demand".
...
-- strict arguments, hence this is not just 'Term'.
...
lamsTermSig :: Arity -> Term -> TermSig
lamsTermSig = coerce lamsTerm
