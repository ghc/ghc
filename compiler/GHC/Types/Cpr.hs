...
-- the way or there is an excess 'LamSh' according to the given arity.
trimCpr :: Arity -> Term -> Cpr -> Cpr
-- See Note [Arity trimming for CPR signatures]
trimCpr 0    !_   (Cpr LamSh{}) = topCpr
-- See Note [Trimming CPR signatures according to Term]
trimCpr arty term cpr
  ...
  = Cpr (ConSh t (zipWith (trimCpr arty) terms cprs))
...
mkBindCprSig arty fun_demand rhs_term rhs_cpr =
  -- pprTrace "mkBindCprSig" (vcat [ppr arty, ppr fun_demand, ppr call_sd, ppr rhs_term, ppr rhs_cpr, ppr demanded_term, ppr final_cpr]) $
  Sig final_cpr
  where
    ...
    -- See Note [Arity trimming for CPR signatures]
    -- See Note [Trimming CPR signatures according to Term]
    final_cpr = trimCpr arty demanded_term rhs_cpr

-- | Compared to 'termTransformSig', this one is not higher-order!
-- ... Except for runRW#.
cprTransformSig :: Unique -> CprSig -> [Cpr] -> Cpr
cprTransformSig id_uniq  _sig      arg_cprs
  | id_uniq == runRWKey -- `runRW  :: (State# RealWorld -> o) -> o`
  , [arg] <- arg_cprs   -- `arg    :: State# RealWorld -> o`        has `L1(,1)`
  = applyCpr arg        -- `result :: o`                            has  `1(,1)`
cprTransformSig _id_uniq (Sig cpr) _arg_cprs
  = cpr
