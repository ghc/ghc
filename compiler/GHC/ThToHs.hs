...
GHC.HsToCore.Splice. This core is then run in the TH engine, and the result
...
specially in `GHC.HsToCore.Splice`, `GHC.ThToHs`, and
`typecheck/TcSplice.hs`:

   (a) When desugaring a pattern synonym from HsSyn to TH.Dec in
       `GHC.HsToCore.Splice`, we represent its *full* type signature in TH, i.e.:
