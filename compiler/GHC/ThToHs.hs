  ...
  where
    ...
    cvt (LitE l)
      ...
      where
        ...
        go cvt_lit mk_expr is_compound_lit = do
          l' <- cvt_lit l
          let e' = mk_expr l'
          return $ if is_compound_lit l' then HsPar noAnn noHsTok (noLocA e') noHsTok else e'
    ...
    cvt (InfixE (Just x) s (Just y)) = ensureValidOpExp s $
      do { x' <- cvtl x
         ...
         ; wrapParLA (\x -> HsPar noAnn noHsTok x noHsTok)
           $ OpApp noAnn px s' py }
    ...
    cvt (InfixE Nothing  s Nothing ) = ensureValidOpExp s $
                                       do { s' <- cvtl s
                                          ; return $ HsPar noAnn noHsTok s' noHsTok }
    ...
    cvt (ParensE e)      = do { e' <- cvtl e; return $ HsPar noAnn noHsTok e' noHsTok }
...
cvtMatch ctxt (TH.Match p body decs)
  = do  { p' <- cvtPat p
        ; let lp = case p' of
                     (L loc SigPat{}) -> L loc (ParPat noAnn noHsTok p' noHsTok) -- #14875
                     _                -> p'
        ; g' <- cvtGuard body
...
cvtp (InfixP p1 s p2)  = do { s' <- cNameN s; p1' <- cvtPat p1; p2' <- cvtPat p2
                            ; wrapParLA (\p -> ParPat noAnn noHsTok p noHsTok) $
                              ConPat
                                { pat_con_ext = noAnn
                                , pat_con = s'
                                , pat_args = InfixCon
...
cvtp (ParensP p)       = do { p' <- cvtPat p;
                            ; case unLoc p' of  -- may be wrapped ConPatIn
                                ...
                                _         -> return $ ParPat noAnn noHsTok p' noHsTok }
