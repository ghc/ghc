...
cvtDec (TH.ValD pat body ds)
  ...
  = do  { pat' <- cvtPat pat
        ...
        ; returnJustLA $ Hs.ValD noExtField $
          PatBind { pat_lhs = pat'
                  , pat_rhs = GRHSs noExtField body' (noLocA ds')
                  , pat_ext = noAnn
                  , pat_ticks = ([],[]) } }
...
cvtClause ctxt (Clause ps body wheres)
  = do  { ps' <- cvtPats ps
        ...
        ; returnL $ Hs.Match noAnn ctxt pps (GRHSs noExtField g' (noLocA ds')) }
...
cvtMatch ctxt (TH.Match p body decs)
  = do  { p' <- cvtPat p
        ...
        ; returnL $ Hs.Match noAnn ctxt [lp] (GRHSs noExtField g' (noLocA decs')) }
