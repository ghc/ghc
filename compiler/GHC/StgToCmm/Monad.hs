...
newtype FCode a = FCode { doFCode :: CgInfoDownwards -> CgState -> IO (a, CgState) }
    deriving (Functor)
...
instance MonadIO FCode where
  liftIO m = FCode $ \_ s -> do
    a <- m
    pure (a, s)

instance MonadUnique FCode where
  getUniqueSupplyM = newUniqSupply
  getUniqueM = newUnique

initC :: CgState
initC = initCgState

runC :: DynFlags -> Module -> CgState -> FCode a -> IO (a,CgState)
...
forkClosureBody body_code
  = do  { platform <- getPlatform
        ...
        ; let body_info_down = info { cgd_sequel    = initSequel
                                    ...
                                    , cgd_self_loop = Nothing }
              fork_state_in = initCgState { cgs_binds = cgs_binds state }
        ; ((),fork_state_out) <- liftIO $ doFCode body_code body_info_down fork_state_in
...
forkLneBody body_code
  = do  { info_down <- getInfoDown
        ; state     <- getState
        ; let fork_state_in = initCgState { cgs_binds = cgs_binds state }
        ; (result, fork_state_out) <- liftIO $ doFCode body_code info_down fork_state_in
        ; setState $ state `addCodeBlocksFrom` fork_state_out
        ; return result }
...
codeOnly body_code
  = do  { info_down <- getInfoDown
        ...
        ; ((), fork_state_out) <- liftIO $ doFCode body_code info_down fork_state_in
        ; setState $ state `addCodeBlocksFrom` fork_state_out }
...
forkAlts branch_fcodes
  = do  { info_down <- getInfoDown
        ; state <- getState
        ; let compile branch = doFCode branch info_down branch_state
                where
                  branch_state = initCgState { cgs_binds  = cgs_binds state
                                             , cgs_hp_usg = cgs_hp_usg state }
        ; results <- liftIO $ traverse compile branch_fcodes
        ; let (branch_results, branch_out_states) = unzip results
