...
getHeapUsage fcode
  = do  { info_down <- getInfoDown
        ...
        -- Because of the loop the fields of CgState are all lazy so now it's
        -- safe, force all the state to remove thunk build-ups
        ; setState $! seqCgState (fstate_out { cgs_hp_usg = cgs_hp_usg state })
        ; return r }
