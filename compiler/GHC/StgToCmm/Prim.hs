...
doIndexOffAddrOpAs maybe_post_read_cast rep idx_rep [res] [addr,idx]
   = mkBasicIndexedRead Unaligned 0 maybe_post_read_cast rep res addr idx_rep idx
...
doIndexByteArrayOpAs maybe_post_read_cast rep idx_rep [res] [addr,idx]
   = do profile <- getProfile
        ...
        let alignment
              | typeWidth rep >= typeWidth idx_rep = NaturallyAligned
              | otherwise                          = Unaligned
...
mkBasicIndexedRead :: AlignmentSpec
                   ...
                   -> CmmExpr      -- Index
                   -> FCode ()
mkBasicIndexedRead alignment off mb_cast ty res base idx_ty idx
   = do platform <- getPlatform
        let x = cmmLoadIndexOffExpr platform alignment off ty base idx_ty idx
        let x' = case mb_cast of
                        Just cast -> CmmMachOp cast [x]
                        Nothing   -> x
...
cmmLoadIndexOffExpr platform alignment off ty base idx_ty idx
   = case alignment of
       NaturallyAligned -> CmmLoad ptr ty
       Unaligned        -> CmmMachOp (MO_UnalignedLoad ty) [ptr]
   where ptr = cmmIndexOffExpr platform off (typeWidth idx_ty) base idx
