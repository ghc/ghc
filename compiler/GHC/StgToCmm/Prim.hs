...
-- First we handle various awkward cases specially.
  ...
  ParOp -> \[arg] -> inlinePrimop $ \[res] ->
    ...
    emitCCall
        [(res,NoHint)]
        (CmmLit (CmmLabel (mkForeignLabel (fsLit "newSpark")
...
fmaCCall width res arg_x arg_y arg_z =
  ...
  where
    fma_lbl = mkForeignLabel fma_op ForeignLabelInExternalPackage ForeignLabelIsFunction
...
emitCopyUpdRemSetPush platform hdr_size dst dst_off n =
  ...
  where
    lbl = mkLblExpr $ mkPrimCallLabel
          $ PrimCall (fastStringToShortText (fsLit "stg_copyArray_barrier")) rtsUnitId
    args =
