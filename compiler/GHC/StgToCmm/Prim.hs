...
emitPrimOp0
  ...
  -> OpDest
emitPrimOp0 dflags = \case
  NewByteArrayOp_Char -> \[(CmmLit (CmmInt n w))] ->
    if asUnsigned w n <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone  $ \ [res] -> doNewByteArrayOp res (fromInteger n)
    else OpDest_External

  NewArrayOp -> \[(CmmLit (CmmInt n w)), init] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \[res] -> doNewArrayOp res (arrPtrsRep dflags (fromInteger n)) mkMAP_DIRTY_infoLabel
  ...
  CopyArrayOp -> \[src, src_off, dst, dst_off, (CmmLit (CmmInt n _))] ->
    OpDest_AllDone $ \ [] -> doCopyArrayOp src src_off dst dst_off (fromInteger n)

  CopyMutableArrayOp -> \[src, src_off, dst, dst_off, (CmmLit (CmmInt n _))] ->
    OpDest_AllDone $ \ [] -> doCopyMutableArrayOp src src_off dst dst_off (fromInteger n)

  CopyArrayArrayOp -> \[src, src_off, dst, dst_off, (CmmLit (CmmInt n _))] ->
    OpDest_AllDone $ \ [] -> doCopyArrayOp src src_off dst dst_off (fromInteger n)

  CopyMutableArrayArrayOp -> \[src, src_off, dst, dst_off, (CmmLit (CmmInt n _))] ->
    OpDest_AllDone $ \ [] -> doCopyMutableArrayOp src src_off dst dst_off (fromInteger n)

  CloneArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneArray mkMAP_FROZEN_CLEAN_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  CloneMutableArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneArray mkMAP_DIRTY_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  FreezeArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneArray mkMAP_FROZEN_CLEAN_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  ThawArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneArray mkMAP_DIRTY_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  NewSmallArrayOp -> \[(CmmLit (CmmInt n w)), init] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] ->
  ...
  CopySmallArrayOp -> \[src, src_off, dst, dst_off, (CmmLit (CmmInt n _))] ->
    OpDest_AllDone $ \ [] -> doCopySmallArrayOp src src_off dst dst_off (fromInteger n)

  CopySmallMutableArrayOp -> \[src, src_off, dst, dst_off, (CmmLit (CmmInt n _))] ->
    OpDest_AllDone $ \ [] -> doCopySmallMutableArrayOp src src_off dst dst_off (fromInteger n)

  CloneSmallArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneSmallArray mkSMAP_FROZEN_CLEAN_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  CloneSmallMutableArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneSmallArray mkSMAP_DIRTY_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  FreezeSmallArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneSmallArray mkSMAP_FROZEN_CLEAN_infoLabel res src src_off (fromInteger n)
    else OpDest_External

  ThawSmallArrayOp -> \[src, src_off, (CmmLit (CmmInt n w))] ->
    if wordsToBytes dflags (asUnsigned w n) <= fromIntegral (maxInlineAllocSize dflags)
    then OpDest_AllDone $ \ [res] -> emitCloneSmallArray mkSMAP_DIRTY_infoLabel res src src_off (fromInteger n)
    else OpDest_External
...
--      r = ((StgArrBytes *)(a))->bytes
  SizeofMutableByteArrayOp -> emitPrimOp0 dflags SizeofByteArrayOp
...
--  #define touchzh(o)                  /* nothing */
  TouchOp -> \[] -> OpDest_AllDone $ \[] -> do
    emitPrimCall [] MO_Touch []
...
-- Getting the size of pointer arrays
  ...
  SizeofArrayOp -> \[arg] -> OpDest_AllDone $ \[res] -> do
    emit $ mkAssign (CmmLocal res) (cmmLoadIndexW dflags arg
      (fixedHdrSizeW dflags + bytesToWordsRoundUp dflags (oFFSET_StgMutArrPtrs_ptrs dflags))
        (bWord dflags))
  SizeofMutableArrayOp -> emitPrimOp0 dflags SizeofArrayOp
  SizeofArrayArrayOp -> emitPrimOp0 dflags SizeofArrayOp
  SizeofMutableArrayArrayOp -> emitPrimOp0 dflags SizeofArrayOp
  ...
  SizeofSmallMutableArrayOp -> emitPrimOp0 dflags SizeofSmallArrayOp
...
-- TODO compiler need not know about these
  ...
  TagToEnumOp -> panic "handled above"
...
emitPrimOp dflags op args = case emitPrimOp0 dflags op args of
  OpDest_Nop -> Just $ \[res] -> case args of
    [arg] -> emitAssign (CmmLocal res) arg

  OpDest_Narrow (mop, rep) -> Just $ \[res] -> case args of
    [arg] -> emitAssign (CmmLocal res) $
