...
emitPrimOp0 dflags reses op args = case op of
  ...
  ReadMutVarOp -> OpDest_AllDone $ do
    [res] <- pure reses
    [mutv] <- pure args
...
--        r = a;
--      }
  UnsafeFreezeArrayOp -> let [res] = reses; [arg] = args in OpDest_AllDone $ emit $ catAGraphs
   [ setInfo arg (CmmLit (CmmLabel mkMAP_FROZEN_DIRTY_infoLabel)),
     mkAssign (CmmLocal res) arg ]
  UnsafeFreezeArrayArrayOp -> let [res] = reses; [arg] = args in OpDest_AllDone $ emit $ catAGraphs
   [ setInfo arg (CmmLit (CmmLabel mkMAP_FROZEN_DIRTY_infoLabel)),
     mkAssign (CmmLocal res) arg ]
  UnsafeFreezeSmallArrayOp -> let [res] = reses; [arg] = args in OpDest_AllDone $ emit $ catAGraphs
   [ setInfo arg (CmmLit (CmmLabel mkSMAP_FROZEN_DIRTY_infoLabel)),
     mkAssign (CmmLocal res) arg ]

--  #define unsafeFreezzeByteArrayzh(r,a)       r=(a)
  UnsafeFreezeByteArrayOp -> let [res] = reses; [arg] = args in OpDest_AllDone $ emitAssign (CmmLocal res) arg
