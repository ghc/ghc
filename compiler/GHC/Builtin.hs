...
(KN2) The flag `-fdefines-known-key-names` is module-wide.  But what if that module
   ...
   Rather, the `toList` from GHC.Internal.IsList is the known-key entity.
...
`base` (e.g. GHC 10.0 against `base` 4.33).

Any compiler-internal machinery (such as wired-in entities) is confined to the
`ghc-internal` package, which is fixed for any GHC version. To support multiple
versions of `base`, `base` includes compatibility code that allows it to compile
against a range of versions of `ghc-internal`.

To avoid all known entities residing in `ghc-internal`, we allow `base` itself
to define known entities; GHC finds them by looking them up in `GHC.Essentials`
as described in Note [Overview of known entities].

Let's compare two examples: `coerce` and `enumFromTo`.

  coerce (wired-in)

    `coerce` is defined in `ghc-internal:GHC.Internal.Prim` and wired-in with a
    fixed unique (see `GHC.Builtin.WiredIn.Ids.coerce{Id,Name}`).
    It is re-exported by `base:Data.Coerce`.

    Suppose GHC-x compiles a module containing an occurrence of `Data.Coerce.coerce`.
    As `base` imports `coerce` from `ghc-internal-x` and re-exports it, GHC
    resolves `coerce` to the wired-in `Id` with its known unique.

  enumFromTo (known-occ)

    `enumFromTo` is a method of the `Enum` typeclass, exposed to users via `Prelude`.
    GHC records only its occurrence name (see `GHC.Builtin.KnownOccs.enumFromTo_RDR`)

    Suppose GHC-x needs to compile `[a..b]`. It needs to desugar this syntax to
    `enumFromTo a b`; to do this it loads the interface file of `GHC.Essentials`
    defined in the version of `base` we are compiling against, and looks up
...
-- also Note [Overview of known entities] in GHC.Builtin)
wiredInNames :: [Name]
wiredInNames
  | debugIsOn
  ...
  where
    ...
    -- These can't be just known-key because they are defined in the ghc
    -- package (thus can't be exported from GHC.Essentials).
    wired_in_names = [pluginTyConName, frontendPluginTyConName]
