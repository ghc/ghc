...
--
-- Fails if no interpreter is setup
withInterp :: HscEnv -> (Interp -> a) -> a
withInterp hsc_env action = case hsc_interp hsc_env of
   Nothing -> throw (InstallationError "No interpreter")
   Just i  -> action i
...
withIServ (IServ mIServState) action =
  gmask $ \restore -> do
...
purgeLookupSymbolCache hsc_env = case hsc_interp hsc_env of
  ...
  Just (ExternalInterp (IServ mstate)) -> do
    state <- takeMVar mstate
    putMVar mstate $ case state of
      IServPending {}    -> state
...
stopInterp hsc_env = case hsc_interp hsc_env of
  ...
  Just (ExternalInterp (IServ mstate)) ->
    gmask $ \_restore -> do
      ...
      case state of
        IServPending {} -> do
          -- we never spawned the interpreter
        ...
        IServRunning i  -> do
          ...
          putMVar mstate (IServPending (iservConfig i))
...
mkFinalizedHValue hsc_env rref = do
   ...
   !free <- case hsc_interp hsc_env of
     ...
     Just (ExternalInterp i) -> return $ withIServ i $ \iserv -> do
       let iserv' = iserv{iservPendingFrees = hvref:iservPendingFrees iserv}
       return (iserv', ())
