          ...
          Resume { resumeStmt = expr, resumeContext = fhv
               ...
               withVirtualCWD $ do
                when (isJust mb_brkpt && isJust mbCnt) $ do
                  setIgnoreCount hsc_env (fromJust mb_brkpt) (fromJust mbCnt)
...
setIgnoreCount :: GhcMonad m => HscEnv -> BreakInfo -> Int -> m ()   -- #19157
setIgnoreCount hsc_env brkInfo cnt = do
  let modl :: Module = breakInfo_module brkInfo
      breaks hsc_env modl = getModBreaks $ expectJust "setIgnoreCount" $
         lookupHpt (hsc_HPT hsc_env) (moduleName modl)
      ix = breakInfo_number brkInfo
      modBreaks  = breaks hsc_env modl
      breakarray = modBreaks_flags modBreaks
  _ <- liftIO $ GHCi.storeIgnoreCount hsc_env breakarray ix cnt
  pure ()
