...
import GHC.Driver.Errors.Types (hoistTcRnDsMessage)
...
getInstancesForType ty = withSession $ \hsc_env ->
  liftIO $ runInteractiveHsc hsc_env $
    ioMsgMaybe $ hoistTcRnDsMessage $ runTcInteractive hsc_env $ do
...
parseInstanceHead str = withSession $ \hsc_env0 -> do
  (ty, _) <- liftIO $ runInteractiveHsc hsc_env0 $ do
    hsc_env <- getHscEnv
    ty <- hscParseType str
    ioMsgMaybe $ hoistTcRnDsMessage $ tcRnType hsc_env SkolemiseFlexi True ty
