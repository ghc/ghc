...
NB 1: DuplicateRecordFields makes no difference to the first row of this
table, except that if `size` is a field of more than one data type, then a
naked use of the record selector `size` may well be ambiguous.  You have
to use a qualified name. And there is no way to do this if both data types
