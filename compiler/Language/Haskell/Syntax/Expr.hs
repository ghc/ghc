...
GhcPs (LHsExpr GhcPs)@ values as the type is defined today. We avoid
...
data Fbind b = Fbind (LHsRecField GhcPs (Located b))
             | Pbind ( LHsRecProj GhcPs (Located b))
@
(these names being chosen with @Fbind@ meaning "field binding" and
@Pbind@ meaning "projection binding" in mind).
