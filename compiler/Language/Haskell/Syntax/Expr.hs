...
data Match p body
  = Match {
        ...
        m_ctxt :: HsMatchContext (XRec p (CtxIdP p)),
...
data ApplicativeArg idL
  ...
  | ApplicativeArgMany     -- do { stmts; return vars }
    ...
    , stmt_context      :: HsStmtContext (XRec idL (CtxIdP idL))
...
data HsMatchContext n
  = FunRhs { mc_fun        :: n -- ^ function binder of @f@
  ...
  | StmtCtxt (HsStmtContext n)  -- ^Pattern of a do-stmt, list comprehension,
                                -- pattern guard, etc
...
isPatSynCtxt :: HsMatchContext n -> Bool
isPatSynCtxt ctxt =
  case ctxt of
    PatSyn -> True
    _      -> False
...
data HsStmtContext n
  = ArrowExpr                        -- ^do-notation in an arrow-command context

  | PatGuard (HsMatchContext n)      -- ^Pattern guard for specified thing
  | ParStmtCtxt (HsStmtContext n)    -- ^A branch of a parallel stmt
  | TransStmtCtxt (HsStmtContext n)  -- ^A branch of a transform stmt
...
isComprehensionContext (HsDoStmt ListComp)  = True
isComprehensionContext (HsDoStmt MonadComp) = True
...
isMonadStmtContext (HsDoStmt MonadComp)            = True
isMonadStmtContext (HsDoStmt DoExpr{})             = True
isMonadStmtContext (HsDoStmt MDoExpr{})            = True
isMonadStmtContext (HsDoStmt GhciStmtCtxt)         = True
...
isMonadCompContext (HsDoStmt MonadComp) = True
...
pprMatchContext :: Outputable n
                => HsMatchContext n -> SDoc
...
pprMatchContextNoun :: Outputable n
                    => HsMatchContext n -> SDoc
...
-----------------
pprAStmtContext, pprStmtContext :: Outputable n
                                => HsStmtContext n -> SDoc
...
pprStmtContext (HsDoStmt p)      = pprHsDoFlavour p
