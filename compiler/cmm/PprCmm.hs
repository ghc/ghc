...
instance Outputable CmmTopInfo where
    type OutputableNeedsOfConfig CmmTopInfo = PairConstraint
...
instance Outputable ForeignConvention where
    type OutputableNeedsOfConfig ForeignConvention = HasPprConfig
...
type OutputableNeedsOfConfigAllCOs n = PairConstraint
  (PairConstraint
    (OutputableNeedsOfConfig (n C C))
    (OutputableNeedsOfConfig (n C O)))
  (PairConstraint
    (OutputableNeedsOfConfig (n O C))
    (OutputableNeedsOfConfig (n O O)))

instance OutputableAllCOs n => Outputable (Block n C C) where
    type OutputableNeedsOfConfig (Block n C C) = OutputableNeedsOfConfigAllCOs n
    ppr = pprBlock
instance OutputableAllCOs n => Outputable (Block n C O) where
    type OutputableNeedsOfConfig (Block n C O) = OutputableNeedsOfConfigAllCOs n
    ppr = pprBlock
instance OutputableAllCOs n => Outputable (Block n O C) where
    type OutputableNeedsOfConfig (Block n O C) = OutputableNeedsOfConfigAllCOs n
    ppr = pprBlock
instance OutputableAllCOs n => Outputable (Block n O O) where
    type OutputableNeedsOfConfig (Block n O O) = OutputableNeedsOfConfigAllCOs n
    ppr = pprBlock

instance Outputable (Graph CmmNode e x) where
    type OutputableNeedsOfConfig (Graph CmmNode e x) = PairConstraint
      (PairConstraint
        (PairConstraint (PairConstraint HasPlatform HasPlatformConstants) HasPlatformMisc)
        (PairConstraint HasPprConfig (PairConstraint HasNameSuppress HasPackageState)))
      HasCoreDebugSuppress
...
instance Outputable CmmGraph where
    type OutputableNeedsOfConfig CmmGraph = PairConstraint
      (PairConstraint
        (PairConstraint (PairConstraint HasPlatform HasPlatformConstants) HasPlatformMisc)
        (PairConstraint HasPprConfig (PairConstraint HasNameSuppress HasPackageState)))
      HasCoreDebugSuppress
...
pprBlock
  :: forall n e x r
  .  ( IndexedCO e (SDoc' r) (SDoc' r) ~ SDoc' r
     , IndexedCO x (SDoc' r) (SDoc' r) ~ SDoc' r
     , OutputableAllCOs n
     , OutputableNeedsOfConfigAllCOs n r
     )
  => Block n e x -> SDoc' r
pprBlock block
    = foldBlockNodesB3 @n @(SDoc' r) @(SDoc' r) @(SDoc' r)
                       ( ($$) . ppr
                       , ($$) . (nest 4) . ppr
                       , ($$) . (nest 4) . ppr
                       )
...
pprGraph (GMany entry body exit)
  ...
  where pprMaybeO
          :: ( Outputable (Block CmmNode e x)
             , OutputableNeedsOfConfig (Block CmmNode e x) r
             )
