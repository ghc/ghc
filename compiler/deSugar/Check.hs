...
checkMatches' vars matches
  ...
  where
    ...
    hsLMatchToLPats (dL->(l , Match { m_pats = pats })) = cL l pats
...
translatePat fam_insts pat = case pat of
  ...
  NPlusKPat ty (dL->(_ , _n)) _k1 _k2 _ge _minus -> mkCanFailPmPat ty
...
translateMatch :: FamInstEnvs -> LMatch GhcTc (LHsExpr GhcTc)
               -> DsM (PatVec,[PatVec])
translateMatch fam_insts (dL->(_ , Match { m_pats = lpats
                                         , m_grhss = grhss })) = do
  pats'   <- concat <$> translatePatVec fam_insts pats
  guards' <- mapM (translateGuards fam_insts) guards
  return (pats', guards')
  where
    extractGuards :: GRHS GhcTc (LHsExpr GhcTc) -> [GuardStmt GhcTc]
    extractGuards (GRHS _ gs _) = map unLoc gs
    extractGuards (XGRHS _)     = panic "translateMatch"

    pats   = map unLoc lpats
    guards = map (extractGuards . unLoc) (grhssGRHSs grhss)
...
dsPmWarn dflags ctx@(DsMatchContext kind loc) pm_result
  = when (flag_i || flag_u) $ do
      ...
      when exists_r $ forM_ redundant $ \(dL->(l , q)) -> do
        putSrcSpanDs l (warnDs (Reason Opt_WarnOverlappingPatterns)
                               (pprEqn q "is redundant"))
      when exists_i $ forM_ inaccessible $ \(dL->(l , q)) -> do
        putSrcSpanDs l (warnDs (Reason Opt_WarnOverlappingPatterns)
