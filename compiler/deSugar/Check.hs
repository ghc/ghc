...
checkMatches' vars matches
  ...
  where
    ...
    go (m:ms) missing = do
      ...
      scrs               <- runMany (pmcheckI clause guards vars (length missing)) missing
      let r@(PartialResult cs missing' ds pc1) = approximateResults dflags scrs
...
This mechanism is implemented by capturing the incoming Delta and outgoing
PartialResult of a pattern match checking action in a SingleClauseResult.
Then the caller of 'runMany' can choose to 'acceptAllResults' or to
'approximateResults' according to 'maxPmCheckModels'.
Note that we can alawys use the refined Uncovered set if the "branching factor"
was below 2. Hence the arity 2 match in the example above: Just @f True@ would
have a branching factor of 1, thus not be affected by the limit.
...
We simply apply the same mechanism as in Note [Countering exponential blowup]. But we don't
want to forget about actually useful info from pattern match clauses just
because we had one clause with many guards. So we set the limit for guards
much lower and make it dependent on the branching factor k.
See 'tryMultiplyDeltas'.
...
-- | Represents a 'Delta' and the 'PartialResult' from checking the 'Delta'
-- against a pattern match.
-- See Note [Countering exponential blowup]
data SingleClauseResult
  = SingleClauseResult !Delta !PartialResult

-- | Returns the unmodified 'PartialResult' from the pattern match.
acceptSingleClauseResult :: SingleClauseResult -> PartialResult
acceptSingleClauseResult (SingleClauseResult _ pr) = pr

-- | Returns a 'PartialResult' with an Uncovered set of size at most 1,
-- potentially approximating it by a singleton set of the incoming 'Delta',
-- which represents a superset of the actual Uncovered set.
-- This is like forgetting that we matched on the clause and just carry on with
-- the original 'Delta'.
-- See Note [Countering exponential blowup]
approximateSingleClauseResult :: SingleClauseResult -> PartialResult
approximateSingleClauseResult (SingleClauseResult in_delta out_pr)
  | presultUncovered out_pr `lengthAtMost` 1
  -- The outgoing Uncovered didn't increase in size, so we can just return it
  = out_pr
  | otherwise
  -- Approximate
  = out_pr { presultUncovered = [in_delta], presultApprox = Approximate }

-- | Accepts all 'PartialResult's from the 'SingleClauseResult's and combines
-- them.
acceptAllResults :: [SingleClauseResult] -> PartialResult
acceptAllResults = foldMap acceptSingleClauseResult

-- | If the size of the 'presultUncovered' in the result of 'acceptAllResults'
-- exceeds 'maxPmCheckModels', 'approximateSingleClauseResult' every *every*
-- 'SingleClauseResult' before 'combinePartialResults'.
-- See Note [Countering exponential blowup]
...
-- 'Uncovered' set, returning a list of 'SingleClauseResult's with the original
-- 'Delta' and the 'PartialResult' from applying it to the action.
runMany :: (Delta -> DsM PartialResult) -> Uncovered -> DsM [SingleClauseResult]
runMany f = traverse go
  where
    go delta = do
      res <- f delta
      pure (SingleClauseResult delta res)
...
pmcheckGuards (gv:gvs) n delta = do
  ...
  let (n', unc')
        ...
        | otherwise                                          = (n, [delta])
  -- I'd like to use approximateResults or a similar mechanism here, but it
  -- doesn't really fit because we need to consider @n@ here, the number of
  -- similar deltas to handle. We'd need a similar structure as in checkMatches.
  -- So we are stuck with tryMultiplyDeltas and use acceptAllResults here.
  (PartialResult css uncs dss pcs) <- acceptAllResults <$> runMany (pmcheckGuards gvs n') unc'
