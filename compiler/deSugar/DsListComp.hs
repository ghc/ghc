...
dsMcStmt (ApplicativeStmt body_ty args mb_join) stmts = do
    let (pats, rhss) = unzip (do_arg . snd <$> args)
        do_arg = \ case
            ApplicativeArgOne _ pat expr _ fail_op -> ((pat, fail_op), dsLExpr expr)
            ApplicativeArgMany _ stmts ret pat -> ((pat, noSyntaxExpr), dsMonadComp (stmts ++ [noLoc $ LastStmt noExtField (noLoc ret) False (mkSyntaxExpr ret)]))
            XApplicativeArg nec -> noExtCon nec
    rhss' <- sequenceA rhss
    body' <- dsLExpr $ noLoc $ HsDo body_ty MonadComp (noLoc stmts)
    let match_args (pat, fail_op) (vs, body) = do
