...
-- incorrect. These already have a fixed but unknown definition. It's better to
-- regard them as rigid meta variables (which we can't at this point).
-- Remember, this is the expression rather than the pattern match syntax.
-- For an example for why we don't want this, consider `dictVarsAreTypeIndexed`
-- in `testsuite/tests/pmcheck/should_compile/PmExprVars.hs`.
-- If it weren't for dictionary variables, we'd still face the same issues as
-- the HsConLikeOut case below. Better translate it as PmExprOther.
