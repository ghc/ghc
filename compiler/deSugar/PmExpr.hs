...
module PmExpr (
        ...
        mkPmExprLit, decEqPmAltCon, pmExprAsList
    ) where
...
--
-- * @pmExprAsList (1:2:[]) == Just ([1,2], Nothing)@, a regular,
--   @[]@-terminated list. Should be pretty-printed as @[a,b]@.
-- * @pmExprAsList (1:2:x) == Just ([1,2], Just x)@, a list prefix ending in
--   a wildcard variable x (of list type). Should be pretty-printed as (1:2:_).
-- * @pmExprAsList [] == Just ([], Nothing)@, the only case where we return an
--   empty list is for when the 'PmExpr' is the nil constructor.
...
instance Outputable PmExpr where
  ppr = go (0 :: Int)
    where
      ...
      go _    (pmExprAsList -> Just (list, suff)) = case as_string list of
        Just lits -> case suff of
          Nothing -> str
          Just x  -> str <> text "++" <> ppr x
          where str = doubleQuotes $ hcat $ map ppr lits
        Nothing   -> case suff of
