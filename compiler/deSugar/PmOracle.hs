...
data VarInfo
  ...
  -- in the sense that 'eqPmAltCon' returns @Nothing@ for any pairing between
  -- 'vi_pos' and 'vi_neg'.
...
* @x ~ Just y@, @x /~ [Just]@. 'eqPmAltCon' returns Just True, so refute.
* @x ~ Nothing@, @x /~ [Just]@. 'eqPmAltCon' returns Just False, so negative
  info is redundant and should be discarded.
* @x ~ I# y@, @x /~ [4,2]@. 'eqPmAltCon' returns Nothing, so orthogal. We
...
* Adding negative information. Example: Add the fact @x /~ Nothing@ (see 'tryAddRefutableAltCon')
  ...
  - (Redundant) If we have @x ~ K2@ and @eqPmAltCon K K2 == Just False@
...
tryAddRefutableAltCon delta@MkDelta{ delta_tm_cs = TS env } x nalt = runMaybeT $ do
  ...
  let contradicts nalt (cl, _args) = eqPmAltCon cl nalt == Just True
  guard (not (any (contradicts nalt) pos))
  -- 2. Only record the new fact when it's not already implied by one of the
  -- solutions
  let implies nalt (cl, _args) = eqPmAltCon cl nalt == Just False
  let neg'
...
-- impossible matches.
ensureAllPossibleMatchesInhabited :: Delta -> PmM (Maybe Delta)
...
trySolve delta@MkDelta{ delta_tm_cs = TS env } x alt args = do
  ...
  case find ((== Just True) . eqPmAltCon alt . fst) pos of
    ...
    Nothing -> do
      ...
      -- the new solution)
      let neg' = filter ((== Nothing) . eqPmAltCon alt) neg
