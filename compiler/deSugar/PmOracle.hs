...
-- @arg_tys@ with fresh variables. It adds a new term equality equating @x@ is
-- to the resulting 'PmAltCon' app and new type equalities arising from GADT
-- matches. If successful, returns the new @delta@ and the fresh term variables,
-- or @Nothing@ otherwise.
refineToAltCon :: Delta -> Id -> PmAltCon -> [Type] -> DsM (Maybe (Delta, [Id]))
refineToAltCon delta x l@PmAltLit{}           _arg_tys = runMaybeT $ do
  delta' <- addVarConCt delta x l []
  pure (delta', [])
refineToAltCon delta x alt@(PmAltConLike con) arg_tys  = do
  ...
  (arg_vars, new_ty_cs, strict_arg_tys) <- mkOneConFull arg_tys con
  ...
  where
    ...
    split_at_con rec_ts delta n x xs cl = do
      ...
      case guessConLikeUnivTyArgsFromResTy env res_ty cl of
        ...
        Just arg_tys -> do
          ev_pos <- refineToAltCon delta x (PmAltConLike cl) arg_tys >>= \case
            Nothing                -> pure []
            Just (delta', arg_vas) ->
              go rec_ts (arg_vas ++ xs) n delta'
