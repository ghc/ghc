...
-- Also returns instantiated evidence variables from the match and strictness
-- constraints on strict field binders.
mkOneConFull :: [Type] -> ConLike -> DsM ([Id], Bag TyCt, Bag TmCt, [TyVar])
--  * 'con' K is a ConLike
...
tmOracle :: Delta -> Bag TmCt -> DsM (Maybe Delta)
  ...
  where
    ...
    inh_test con = do
      ...
      case guessConLikeUnivTyArgsFromResTy env (vi_ty vi) con of
        ...
        Just arg_tys -> do
          (_vars, ty_cs, strict_tm_cs, _ex_tyvars) <- mkOneConFull arg_tys con
...
refineToAltCon delta x alt@(PmAltConLike con) arg_tys  ex_tvs1  = do
  ...
  (arg_vars, theta_ty_cs, strict_tm_cs, ex_tvs2) <- mkOneConFull arg_tys con
  ...
  let new_tm_cs = unitBag (TmVarCon x alt arg_vars) `unionBags` strict_tm_cs

  -- Now check satifiability
  mb_delta <- pmIsSatisfiable delta new_tm_cs new_ty_cs
