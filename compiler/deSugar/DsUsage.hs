  ...
  where
    ...
    mkUsage mod
      ...
      where
        ...
        finsts_mod   = mi_finsts (mi_exts iface)
        hash_env     = mi_hash_fn (mi_exts iface)
        mod_hash     = mi_mod_hash (mi_exts iface)
        export_hash | depend_on_exports = Just (mi_exp_hash (mi_exts iface))
                    | otherwise         = Nothing
