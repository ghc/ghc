...
patchMagicDefn orig_pair@(orig_id, _)
  | Just mk_magic_pair <- lookupNameEnv magicDefnsEnv (getName orig_id)
  = do { magic_pair@(magic_id, _) <- mk_magic_pair
...
magicDefns :: [(Name, DsM (Id, CoreExpr))]
magicDefns = [ (unsafeCoercePrimName, mkUnsafeCoercePrimPair) ]

magicDefnsEnv :: NameEnv (DsM (Id, CoreExpr))
...
mkUnsafeCoercePrimPair :: DsM (Id, CoreExpr)
mkUnsafeCoercePrimPair
  = do { unsafe_equality_proof_id <- dsLookupGlobalId unsafeEqualityProofName
       ...
       ; let [unsafe_refl_data_con] = tyConDataCons unsafe_equality_tc
             ...
             expr = Var id  -- this should never get used
       ; return (id, expr) }
