...
pprIfaceDecl ss (IfaceData { ifName = tycon, ifCType = ctype,
  ...
  where
    ...
    print_tlks =
      -- If we print a top-level kind signature for a data instance, we leak
    ...
    pp_tlks = ppWhen print_tlks $
              pprTLKS name_doc (mkIfaceTyConKind binders kind)

    -- See Note [Suppressing binder signatures] in IfaceType
    suppress_bndr_sig = SuppressBndrSig print_tlks
...
pprTLKS :: SDoc -> IfaceType -> SDoc
pprTLKS tyCon ty = text "type" <+> tyCon <+> text "::" <+> ppr ty
