...
suppressIfaceInvisibles cfg tys xs
  | typeSuppress_printExplicitKinds $ getTypeSuppress cfg = xs
...
stripIfaceInvisVars cfg tyvars
  | typeSuppress_printExplicitKinds $ getTypeSuppress cfg = tyvars
...
stripInvisArgs cfg tys
  | typeSuppress_printExplicitKinds $ getTypeSuppress cfg = tys
...
if_print_coercions yes no
  = sdocWithDynFlags $ \cfg ->
    getPprStyle $ \style ->
    if typeSuppress_printExplicitCoercions (getTypeSuppress cfg)
...
eliminateRuntimeRep f ty
  = sdocWithDynFlags $ \cfg ->
    getPprStyle      $ \sty    ->
    if userStyle sty && not (typeSuppress_printExplicitRuntimeReps $ getTypeSuppress cfg)
...
ppr_app_arg ctx_prec (t, argf) =
  sdocWithDynFlags $ \cfg ->
  let print_kinds = typeSuppress_printExplicitKinds $ getTypeSuppress cfg
  in case argf of
...
pprIfaceForAllBndr (Bndr (IfaceTvBndr tv) Inferred)
  = sdocWithDynFlags $ \cfg ->
                          if typeSuppress_printExplicitForalls $ getTypeSuppress cfg
...
pprUserIfaceForAll tvs
   = sdocWithDynFlags $ \cfg ->
     ...
     ppWhen (any tv_has_kind_var tvs
             ...
             || typeSuppress_printExplicitForalls (getTypeSuppress cfg)) $
     pprIfaceForAll tvs
   where
...
pprTyTcApp' ctxt_prec tc tys cfg style
  ...
  | tc `ifaceTyConHasKey` consDataConKey
  , not $ typeSuppress_printExplicitKinds $ getTypeSuppress cfg
  , IA_Arg _ argf (IA_Arg ty1 Required (IA_Arg ty2 Required IA_Nil)) <- tys
  , isInvisibleArgFlag argf
  = pprIfaceTyList ctxt_prec ty1 ty2
  ...
  where
    ...
    print_equality' (ki1, ki2, ty1, ty2) style cfg
      ...
      where
        ...
        print_kinds = typeSuppress_printExplicitKinds $ getTypeSuppress cfg
        print_eqs   = typeSuppress_printEqualityRelations (getTypeSuppress cfg) ||
                      dumpStyle style || debugStyle style
