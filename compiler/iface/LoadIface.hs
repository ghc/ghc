                ...
                whenGeneratingDynamicToo dflags $ withDoDynamicToo $ do
                  ...
                  case r of
                      Succeeded (dynIface, _)
                       | mi_mod_hash (mi_exts iface) == mi_mod_hash (mi_exts dynIface) ->
                          return ()
                       | otherwise ->
...
ghcPrimIface
  = empty_iface {
        ...
        mi_exts = (mi_exts empty_iface){ mi_fix_fn = mkIfaceFixCache fixities }
        }
  where
...
-- Show a ModIface
pprModIface iface@ModIface{ mi_exts = exts }
 = vcat [ text "interface"
