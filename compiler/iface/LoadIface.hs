...
loadInterface doc_str mod from
  ...
  | otherwise
  = do  { -- Check if there are modules, where the optimization info (rewrite
          -- rules and inline pragmas, short pragmas) have not yet
          -- been loaded. See Note [Changing Optimization Level]
        ; condLoadAllIgnoredPragmas doc_str
          -- Read the state
        ...
        ; let add_mod = ignore_prags && not (mi_boot iface)
              ...
              msg True  True  = "added to module list with ignored pragmas."
              msg _     False = "no optimizaton pragmas available."
              msg False True  = "optimization pragmas processed."
...
{-
Note [Changing Optimization Level] (#13002)
-------------------------------------------

When the optimization level is -O0 then the flag
Opt_IgnoreInterfacePragmas is True, and GHC ignores
optimization pragmas like rewrite rules or inline pragmas from the
interface files.
...
interface file and store the optimization pragmas from the interface files.

However the optimization pragmas from interface modules processed
while the Opt_IgnoreInterfacePragmas was True are missing!

Therefore, when we don't process the optimization pragmas then we add the
module to the list eps_ignored_mods. When the flag Opt_IgnoreInterfacePragmas
changes, we reprocess the interface files for the modules in eps_ignored_mods
and store the optimization pragmas.
...
pragmas and just to use the wrapper functions. This was rejected in patch
...
-- | Check, whether ignored optimization pragmas from the interface files
--   should be loaded.
condLoadAllIgnoredPragmas :: SDoc -> IfM lcl ()
condLoadAllIgnoredPragmas doc_str = do
    ...
    unless ignore_pragmas $ do
        ...
        unless (null ignored_mods)
            (loadAllIgnoredPragmas doc_str ignored_mods)

-- | Load and store all the ignored optmization data from the interface files.
loadAllIgnoredPragmas :: SDoc -> [Module] -> IfM lcl ()
loadAllIgnoredPragmas doc_str mods = do
    declrules <- mapM (loadIgnoredModulePragmas doc_str) $ reverse mods
...
loadIgnoredModulePragmas  ::
    SDoc -> Module -> IfM lcl ([(Name,TyThing)], [CoreRule])
loadIgnoredModulePragmas doc_str mod = do
