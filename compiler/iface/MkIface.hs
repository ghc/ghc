      ...
      lookup mod = do
        ...
        iface <- case lookupIfaceByModule dflags hpt pit mod of
                  ...
                  Nothing -> do
                      ...
                      return iface
        return $ snd (mi_hash_fn (mi_exts iface) occ `orElse`
...
addFingerprints hsc_env iface0 new_decls
 = do
   ...
   let
    ...
    final_iface = iface0 { mi_decls = sorted_decls, mi_exts = final_iface_exts }
   --
   return final_iface
...
checkPlugins hsc iface = liftIO $ do
  ...
  let old_fingerprint = mi_plugin_hash (mi_exts iface)
...
checkFlagHash hsc_env iface = do
    let old_hash = mi_flag_hash (mi_exts iface)
...
checkOptimHash hsc_env iface = do
    let old_hash = mi_opt_hash (mi_exts iface)
...
checkHpcHash hsc_env iface = do
    let old_hash = mi_hpc_hash (mi_exts iface)
...
checkModUsage _this_pkg UsagePackageModule{
  ...
  = needInterface mod $ \iface -> do
    ...
    checkModuleFingerprint reason old_mod_hash (mi_mod_hash (mi_exts iface))
...
checkModUsage this_pkg UsageHomeModule{
  ...
  = do
    ...
    let
        new_mod_hash    = mi_mod_hash (mi_exts iface)
        new_decl_hash   = mi_hash_fn  (mi_exts iface)
        new_export_hash = mi_exp_hash (mi_exts iface)
