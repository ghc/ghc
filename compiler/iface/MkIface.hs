...
sortDependencies d
 = Deps { dep_mods   = sortBy (compare `on` (moduleNameFS . moduleNameWithIsBoot_moduleName)) (dep_mods d),
 ...
 where
   ...
   dep_missing (mb_pkg, L _ mod) = do
     ...
     case find_res of
        Found _ mod
          | pkg == this_pkg
           -> if moduleName mod `notElem` map moduleNameWithIsBoot_moduleName prev_dep_mods ++ prev_dep_plgn
   ...
   projectNonBootNames = map moduleNameWithIsBoot_moduleName
     . filter (not . moduleNameWithIsBoot_isBoot)
     ...
     if not (isOldHomeDeps mname)
       then return (UpToDate, [])
         ...
         mb_result <- getFromModIface "need mi_deps for" mod $ \imported_iface -> do
           let mnames = mname :
                 projectNonBootNames (dep_mods $ mi_deps imported_iface)
