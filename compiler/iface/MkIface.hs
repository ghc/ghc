...
checkDependencies hsc_env summary iface
 = anyRecomp [
   ...
   , do
     let
       old_deps = Set.fromList $ map fst prev_dep_mods
     ...
     anyRecomp $
       (flip map (ms_home_imps summary) $ \(L _ mname) -> do
         let
           ...
           reason = moduleNameString mname ++ " changed"
         needInterface mod $ \imported_iface -> do
           let mnames = map fst $ dep_mods $ mi_deps imported_iface
       ...
       ++ [(do
        ...
        if not (null cur_unseen_old_deps)
          then do
            ...
            traceHiDiffs $
              text "missing old dependency " <> quotes (ppr missing_dep)
