...
As noted in the definition of InScopeSet (in VarEnv), in-scope sets can act as
substitutions. Specifically, they act as a substitution from variable to
...
in-scope set. When simplifying occurrences of `x`, they are replaced by their
“updated” version from the in-scope set, hence inherit the unfolding.
