...
* We do the substitution before gathering occurrence info. So in
  ...
  of b, and we that's what we gather in the UsageDetails.  It's as
...
* We need care when shadowing.  Suppose [x :-> b] is in occ_bs_env,
  and we encounter:
     ...
     - \b. blah
       ...
       variables, we discard all of occ_bs_env.  Save, but a bit
       brutal.
...
* The occurrence analyser itself does /not/ do cloning. It could, in
  ...
  great benefit. The simplifer usee cloning to get a no-shadowing
...
* The domain of occ_bs_env can include GlobaIds.  Eg
  ...
  But it is a major reason why we perform the substitition in the
  occurrence analyser, rather than adding a let-binding. (A let
  binding would have a GlobalId on the LHS which isn't allowed.
  We used to have it that way but it was a major pain: #16296,)
