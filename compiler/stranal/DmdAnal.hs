...
mkRhsDmd env rhs_arity rhs = fromMaybe cleanEvalDmd $ do
  let ts = findTypeShape (ae_fam_envs env) (exprType rhs)
  TsProd tss <- peelTsFuns rhs_arity ts
  pure (mkCallDmds rhs_arity (cleanEvalProdDmd (length tss)))
