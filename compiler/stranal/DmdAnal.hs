...
Demand signatures are single-point approximations of the abstract
...
transformer) to compute the strictness signature for? We argue that idArity is
...
is too weak to unleash the pre-computed strictness signature. In this case, just
unleashing nopSig (the top of the domain) is a safe bet that won't cost us
much precision. Most importantly, it's also a conservative approximation of the
abstract transformer at those points, guaranteeing soundness.

For a call with at least idArity incoming arguments, the resulting call demand
is strong enough to unleash the strictness signature. This will be a
