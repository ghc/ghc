...
#define OPTM_RESULT(x, y) (# x, y #)

#else

data OptMResult a = OptMResult !a ![CLabel]
#define OPTM_RESULT(x, y) (OptMResult (x) (y))
...
instance Applicative CmmOptM where
    pure x = CmmOptM $ \_ _ imports -> OPTM_RESULT(x, imports)
    (<*>) = ap
  ...
  (CmmOptM f) >>= g =
    CmmOptM $ \dflags this_mod imports0 ->
                case f dflags this_mod imports0 of
                  OPTM_RESULT(x, imports1) ->
                    case g x of
                      CmmOptM g' -> g' dflags this_mod imports1

instance CmmMakeDynamicReferenceM CmmOptM where
    addImport = addImportCmmOpt
    getThisModule = CmmOptM $ \_ this_mod imports -> OPTM_RESULT(this_mod, imports)

addImportCmmOpt :: CLabel -> CmmOptM ()
addImportCmmOpt lbl = CmmOptM $ \_ _ imports -> OPTM_RESULT((), lbl:imports)

instance HasDynFlags CmmOptM where
    getDynFlags = CmmOptM $ \dflags _ imports -> OPTM_RESULT(dflags, imports)

runCmmOpt :: DynFlags -> Module -> CmmOptM a -> (a, [CLabel])
runCmmOpt dflags this_mod (CmmOptM f) =
  case f dflags this_mod [] of
    OPTM_RESULT(result, imports) -> (result, imports)
