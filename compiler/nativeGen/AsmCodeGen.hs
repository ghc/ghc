...
cmmNativeGen dflags this_mod modLoc ncgImpl us fileIds dbgMap cmm count
 = do
        ...
        -- also drops dead code. We don't keep the cfg in sync on x86 so don't use it there.
...
maybeDumpCfg dflags (Just cfg) msg proc_name = do
        dumpIfSet_dyn dflags Opt_D_dump_cfg_weights msg $ (proc_name <> char ':' $$ pprEdgeWeights cfg)
