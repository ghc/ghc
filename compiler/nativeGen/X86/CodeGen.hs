...
genCCall dflags is32Bit mop@(PrimTarget (MO_AtomicRMW _ _))
                                           [dst] [addr, n] bid
    = genCCallMultiBlock dflags is32Bit mop [dst] [addr, n] bid
...
genCCallMultiBlock
    ...
    -> BlockId      -- The block we are in
    -> NatM (InstrBlock, Maybe BlockId)
genCCallMultiBlock dflags is32Bit (PrimTarget (MO_AtomicRMW width amop))
                                           [dst] [addr, n] bid = do
  ...
  where
    ...
    format = intFormat width
genCCallMultiBlock _ _ mop _ _ _ =
  pprPanic "Impossible mop case:" $ ppr mop
