...
utf8DecodeByteString (BS.PS fptr offset (I# len#))
...
utf8DecodeShortByteString (SBS ba#)
  = unsafeDupablePerformIO $ utf8DecodeLazy# index# len#
  where
    index# = indexWord8Array# ba#
    len# = sizeofByteArray# ba#

utf8DecodeLazy# :: (Int# -> Word#) -> Int# -> IO [Char]
utf8DecodeLazy# indexWord8# len#
  = unpack 0#
  where
    unpack i#
        | isTrue# (i# >=# len#) = return []
...
countUTF8Chars ptr len = go ptr 0
  where
        ...
        go p !n
           | p >= end = return n
           | otherwise  = do
                case utf8DecodeChar# (indexWord8OffAddr# (unPtr p)) of
...
utf8EncodeChar write# c =
  ...
  case () of
    _ | x > 0 && x <= 0x007f -> do
          write 0 (fromIntegral x)
          return 1
        -- NB. '\0' is encoded as '\xC0\x80', not '\0'.  This is so that we
        -- can have 0-terminated UTF-8 strings (see GHC.Base.unpackCStringUtf8).
      | x <= 0x07ff -> do
          write 0 (fromIntegral (0xC0 .|. ((x `shiftR` 6) .&. 0x1F)))
          write 1 (fromIntegral (0x80 .|. (x .&. 0x3F)))
          return 2
      | x <= 0xffff -> do
          write 0 (fromIntegral (0xE0 .|. (x `shiftR` 12) .&. 0x0F))
          write 1 (fromIntegral (0x80 .|. (x `shiftR` 6) .&. 0x3F))
          write 2 (fromIntegral (0x80 .|. (x .&. 0x3F)))
          return 3
      | otherwise -> do
          write 0 (fromIntegral (0xF0 .|. (x `shiftR` 18)))
          write 1 (fromIntegral (0x80 .|. ((x `shiftR` 12) .&. 0x3F)))
          write 2 (fromIntegral (0x80 .|. ((x `shiftR` 6) .&. 0x3F)))
          write 3 (fromIntegral (0x80 .|. (x .&. 0x3F)))
          return 4
  where
    write (I# off#) (W# c#) = ST $ \s ->
      case write# off# c# s of
        s -> (# s, () #)
...
utf8EncodeString ptr str = go ptr str
  where go !_   []     = return ()
        go ptr (c:cs) = do
          off <- stToIO $ utf8EncodeChar (writeWord8OffAddr# (unPtr ptr)) c
          go (ptr `plusPtr` off) cs
