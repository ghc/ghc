...
module Outputable (
        -- * Type classes
        Outputable(..), OutputableBndr(..), OutputableBndrNeedsOfConfig',
...
sdocWithPlatform f = sdocWithDynFlags (f . view platform)
...
instance Outputable Float where
    type OutputableNeedsOfConfig Float = NoConstraint
    ppr f = float f

instance Outputable Double where
    type OutputableNeedsOfConfig Double = NoConstraint
    ppr f = double f
...
--
-- TODO Have 'forall r. OutputableBndrNeedsOfConfig a r =>
-- OutputableNeedsOfConfig a r' quantified superclass constraint.
class Outputable a => OutputableBndr a where
   -- | This wouldn't be needed except for an orphan instance for 'Var'.
   type OutputableBndrNeedsOfConfig a :: * -> Constraint
   -- type OutputableBndrNeedsOfConfig a = NoConstraint
   pprBndr
     :: ( OutputableNeedsOfConfig a r
        , OutputableBndrNeedsOfConfig a r
        )
     => BindingSite -> a -> SDoc' r
   pprBndr _b x = ppr x
...
-- | Both constraint functions together.
type OutputableBndrNeedsOfConfig' a = PairConstraint
  (OutputableNeedsOfConfig a)
  (OutputableBndrNeedsOfConfig a)
