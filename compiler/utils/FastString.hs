...
mkNewFastString fp ptr len uid n_zencs = do
  let bs = BS.fromForeignPtr fp 0 len
      zstr = mkZFastString (zEncodeString (utf8DecodeByteString bs))
  zstr' <- unsafeInterleaveIO $ atomicModifyIORef' n_zencs $ \n -> (n+1, zstr)
...
mkNewFastStringByteString bs ptr len uid n_zencs = do
  let zstr = mkZFastString (zEncodeString (utf8DecodeByteString bs))
      zstr' = unsafePerformIO $ atomicModifyIORef' n_zencs $ \n -> (n+1, zstr)
  n_chars <- countUTF8Chars ptr len
  return (FastString uid n_chars bs zstr')
