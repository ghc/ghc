...
mkFastStringBytes !ptr !len =
    ...
    unsafeDupablePerformIO $ do
        sbs <- newSBSFromPtr ptr 0 len
        mkFastStringWith (mkNewFastStringShortByteString sbs) sbs

newSBSFromPtr :: Ptr a -> Int -> Int -> IO ShortByteString
newSBSFromPtr (Ptr src#) (I# dst_off#) (I# len#) =
  stToIO $ ST $ \s ->
    case newByteArray# len# s of { (# s, dst# #) ->
    case copyAddrToByteArray# src# dst# dst_off# len# s of { s ->
...
lengthFS (FastString _uid sbs _zstr) = inlinePerformIO $
  let bs = SBS.fromShort sbs in
