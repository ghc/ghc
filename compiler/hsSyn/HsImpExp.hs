...
instance (p ~ GhcPass pass, OutputableBndrId p)
    ...
    type OutputableNeedsOfConfig (ImportDecl p) = OutputableBndrNeedsOfConfig' (IdP p)
...
instance (p ~ GhcPass pass, OutputableBndrId p) => Outputable (IE p) where
    type OutputableNeedsOfConfig (IE p) = PairConstraint
...
instance (OutputableBndr name) => OutputableBndr (IEWrappedName name) where
  type OutputableBndrNeedsOfConfig (IEWrappedName name) = NoConstraint
...
instance (OutputableBndr name) => Outputable (IEWrappedName name) where
  type OutputableNeedsOfConfig (IEWrappedName name) = OutputableBndrNeedsOfConfig' name
