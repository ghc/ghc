...
instance (p ~ GhcPass pass, OutputableBndrId p)
    ...
    type OutputableNeedsOfConfig (ImportDecl p) = PairConstraint
...
instance (OutputableBndr name) => Outputable (IEWrappedName name) where
  type OutputableNeedsOfConfig (IEWrappedName name) = PairConstraint
    (OutputableNeedsOfConfig name)
    (OutputableBndrNeedsOfConfig name)
