...
mkHsOpTy ty1 op ty2 = HsOpTy PlaceHolder ty1 op ty2

mkHsAppTy :: LHsType (GhcPass p) -> LHsType (GhcPass p) -> LHsType (GhcPass p)
mkHsAppTy t1 t2 = addCLoc t1 t2 (HsAppTy PlaceHolder t1 t2)
...
getAppsTyHead_maybe tys = case splitHsAppsTy tys of
  ...
  ([app1l:appsl, app1r:appsr], [L loc op]) ->  -- one operator
    Just ( L loc (HsTyVar PlaceHolder NotPromoted (L loc op))
