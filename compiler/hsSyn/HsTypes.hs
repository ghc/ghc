...
module HsTypes
  ( HsType
      ...
      , pattern HsRecTy
      , pattern HsCoreTy
      , pattern HsExplicitListTy
      , pattern HsExplicitTupleTy
      , pattern HsTyLit
      , pattern HsWildCardTy
  , LHsType
  , HsTyVarBndr
      , pattern UserTyVar
      , pattern KindedTyVar
  , LHsTyVarBndr
  , HsTupleSort
      , pattern HsUnboxedTuple
      , pattern HsBoxedTuple
      , pattern HsConstraintTuple
      , pattern HsBoxedOrConstraintTuple
  , Promoted
      , pattern Promoted
      , pattern NotPromoted
  , FieldOcc
...
type
   HsType pass = AST.Type (GHC pass)
   -- ^ Haskell Type
pattern
  HsForAllTy ::            -- See Note [HsType binders]
    [LHsTyVarBndr pass] -> -- Explicit, user-supplied 'forall a b c'
...
pattern
  HsQualTy ::          -- See Note [HsType binders]
    LHsContext pass -> -- Context C => blah
    LHsType pass ->
    HsType pass

pattern
  HsTyVar ::
    Promoted ->    -- whether explicitly promoted, for the pretty printer
    (LIdP pass) -> -- Type variable, type constructor, or
                   -- data constructor
...
pattern
  HsDocTy ::
    (LHsType pass) ->
    LHsDocString ->   -- A documented type
    HsType pass
  -- ^ - 'ApiAnnotation.AnnKeywordId' : None
...
pattern
  HsBangTy ::
    HsSrcBang ->      -- Bang-style type annotations
    (LHsType pass) ->
    HsType pass
...
pattern
  HsCoreTy ::
    Type ->  -- An escape hatch for tunnelling a *closed*
...
pattern
  HsTyLit ::
    HsTyLit -> -- A promoted numeric literal.
    HsType pass
  -- ^ - 'ApiAnnotation.AnnKeywordId' : None
...
pattern
  HsWildCardTy ::
    (HsWildCardInfo pass) -> -- A type wildcard
    HsType pass
...
pattern
  HsForAllTy { hst_bndrs, hst_body }
    = AST.ForAllTy NoFieldExt hst_bndrs hst_body
pattern
  HsQualTy   { hst_ctxt, hst_bodyy }
    = AST.QualTy NoFieldExt hst_ctxt hst_bodyy
pattern
  HsTyVar a b
    = AST.TyVar NoFieldExt a b
pattern
  HsAppsTy a
    = AST.AppsTy NoFieldExt a
pattern
  HsAppTy a b
    = AST.AppTy NoFieldExt a b
pattern
  HsFunTy a b
    = AST.FunTy NoFieldExt a b
pattern
  HsListTy a
    = AST.ListTy NoFieldExt a
pattern
  HsPArrTy a
    = AST.PArrTy NoFieldExt a
pattern
  HsTupleTy a b
    = AST.TupleTy NoFieldExt a b
pattern
  HsSumTy a
    = AST.SumTy NoFieldExt a
pattern
  HsOpTy a b c
    = AST.OpTy NoFieldExt a b c
pattern
  HsParTy a
    = AST.ParTy NoFieldExt a
pattern
  HsIParamTy a b
    = AST.IParamTy NoFieldExt a b
pattern
  HsEqTy a b
    = AST.EqTy NoFieldExt a b
pattern
  HsKindSig a b
    = AST.KindSig NoFieldExt a b
pattern
  HsSpliceTy a b
    = AST.SpliceTy b a
pattern
  HsDocTy a b
    = AST.DocTy NoFieldExt a b
pattern
  HsBangTy a b
    = AST.BangTy NoFieldExt a b
pattern
  HsRecTy a
    = AST.RecTy NoFieldExt a
pattern
  HsCoreTy a
    = AST.NewType (NHsCoreTy a)
pattern
  HsExplicitListTy  a b c
    = AST.ExplicitListTy b a c
pattern
  HsExplicitTupleTy a b
    = AST.ExplicitTupleTy a b
pattern
  HsTyLit a
    = AST.TyLit NoFieldExt a
pattern
  HsWildCardTy a
    = AST.WildCardTy a
...
type instance
   AST.XForAllTy        (GHC pass) = NoFieldExt
type instance
   AST.XQualTy          (GHC pass) = NoFieldExt
type instance
   AST.XTyVar           (GHC pass) = NoFieldExt
type instance
   AST.XAppsTy          (GHC pass) = NoFieldExt
type instance
   AST.XAppTy           (GHC pass) = NoFieldExt
type instance
   AST.XFunTy           (GHC pass) = NoFieldExt
type instance
   AST.XListTy          (GHC pass) = NoFieldExt
type instance
   AST.XPArrTy          (GHC pass) = NoFieldExt
type instance
   AST.XTupleTy         (GHC pass) = NoFieldExt
type instance
   AST.XSumTy           (GHC pass) = NoFieldExt
type instance
   AST.XOpTy            (GHC pass) = NoFieldExt
type instance
   AST.XParTy           (GHC pass) = NoFieldExt
type instance
   AST.XIParamTy        (GHC pass) = NoFieldExt
type instance
   AST.XEqTy            (GHC pass) = NoFieldExt
type instance
   AST.XKindSig         (GHC pass) = NoFieldExt
type instance
   AST.XSpliceTy        (GHC pass) = PostTc pass Kind
type instance
   AST.XDocTy           (GHC pass) = NoFieldExt
type instance
   AST.XBangTy          (GHC pass) = NoFieldExt
type instance
   AST.XRecTy           (GHC pass) = NoFieldExt
type instance
   AST.XExplicitListTy  (GHC pass) = PostTc pass Kind
type instance
   AST.XExplicitTupleTy (GHC pass) = [PostTc pass Kind]
type instance
   AST.XTyLit           (GHC pass) = NoFieldExt
type instance
   AST.XWildCardTy      (GHC pass) = HsWildCardInfo pass
type instance
   AST.XNewType         (GHC pass) = NewHsType pass
...
type LHsType pass = AST.LType (GHC pass)
...
type
  LHsQTyVars pass = AST.LQTyVars (GHC pass) -- See Note [HsType binders]
  -- ^ Located Haskell Quantified Type Variables
pattern
  HsQTvs ::
    (PostRn pass [Name]) ->   -- implicit (dependent) variables
    ([LHsTyVarBndr pass]) ->  -- explicit variables
                              -- See Note [HsForAllTy tyvar binders]
    (PostRn pass NameSet) ->  -- which explicit vars are dependent
                              -- See Note [Dependent LHsQTyVars] in TcHsType
    LHsQTyVars pass

pattern
  HsQTvs { hsq_implicit, hsq_explicit, hsq_dependent }
    = AST.QTvs (hsq_implicit, hsq_dependent) hsq_explicit
...
type instance
  AST.XHsQTvs        (GHC pass) = (PostRn pass [Name], PostRn pass NameSet)
type instance
  AST.XNewLHsQTyVars (GHC pass) = NoConExt

-- ------------------------------------

type
  HsImplicitBndrs pass thing = AST.ImplicitBndrs (GHC pass) thing
--            HsImplicitBndrs
...
pattern
  HsIB ::                   -- See Note [HsType binders]
    (PostRn pass [Name]) -> -- Implicitly-bound kind & type vars
    thing ->                -- Main payload (type or list of types)
    (PostRn pass Bool) ->   -- Taking the hsib_vars into account,
                            -- is the payload closed? Used in
                            -- TcHsType.decideKindGeneralisationPlan
    HsImplicitBndrs pass thing

pattern
  HsIB { hsib_vars, hsib_body, hsib_closed }
    = AST.IB (hsib_vars, hsib_closed) hsib_body
...
type instance
  AST.XIB               (GHC pass) thing = ( PostRn pass [Name]
                                           , PostRn pass Bool
                                           )
type instance
  AST.XNewImplicitBndrs (GHC pass) thing = NoConExt

-- ------------------------------------

type
  HsWildCardBndrs pass thing = AST.WildCardBndrs (GHC pass) thing
...
pattern
  HsWC ::
    (PostRn pass [Name]) -> -- Wild cards, both named and anonymous
                            -- after the renamer
    thing ->                -- Main payload (type or list of types)
                            -- If there is an extra-constraints wildcard,
                            -- it's still there in the hsc_body.
    HsWildCardBndrs pass thing

pattern
  HsWC { hswc_wcs, hswc_body }
    = AST.WC hswc_wcs hswc_body
...
type instance
  AST.XWC               (GHC pass) thing = PostRn pass [Name]
type instance
  AST.XNewWildCardBndrs (GHC pass) thing = NoConExt
...
-- See Note [Located RdrNames] in HsExpr
type
  AmbiguousFieldOcc pass = AST.AmbiguousFieldOcc (GHC pass)
pattern
  Unambiguous ::
    (Located RdrName) ->
    (PostRn pass (IdP pass)) ->
    AmbiguousFieldOcc pass

pattern
  Ambiguous ::
    (Located RdrName) ->
    (PostTc pass (IdP pass)) ->
    AmbiguousFieldOcc pass
...
type instance
  AST.XUnambiguous          (GHC pass) = PostRn pass (IdP pass)
type instance
  AST.XAmbiguous            (GHC pass) = PostTc pass (IdP pass)
type instance
  AST.XNewAmbiguousFieldOcc (GHC pass) = NoConExt

-- ------------------------------------

type
  FieldOcc pass = AST.FieldOcc (GHC pass)
  -- ^ Field Occurrence
  --
...
pattern
  FieldOcc {rdrNameFieldOcc,  selectorFieldOcc}
    = AST.FieldOcc selectorFieldOcc rdrNameFieldOcc
...
type instance
  AST.XFieldOcc    (GHC pass) = PostRn pass (IdP pass)
type instance
  AST.XNewFieldOcc (GHC pass) = NoConExt

-- | Located Field Occurrence
type LFieldOcc pass = AST.LFieldOcc (GHC pass)

-- ------------------------------------

type
  HsConDetails arg rec = AST.ConDetails arg rec
...
type
  ConDeclField pass = AST.ConDeclField (GHC pass)
-- ^ Constructor Declaration Field

-- Record fields have Haddoc docs on them

pattern
  ConDeclField ::
    [LFieldOcc pass] -> -- ^ See Note [ConDeclField passs]
    LBangType pass ->
    Maybe LHsDocString ->
    ConDeclField pass
  -- ^ - 'ApiAnnotation.AnnKeywordId' : 'ApiAnnotation.AnnDcolon'
...
pattern
  ConDeclField { cd_fld_names, cd_fld_type, cd_fld_doc }
    = AST.ConDeclField NoFieldExt cd_fld_names cd_fld_type cd_fld_doc
...
type instance
   AST.XConDeclField    (GHC pass) = NoFieldExt
type instance
   AST.XNewConDeclField (GHC pass) = NoConExt

-- | Located Constructor Declaration Field
type LConDeclField pass = AST.LConDeclField (GHC pass)
      -- ^ May have 'ApiAnnotation.AnnKeywordId' : 'ApiAnnotation.AnnComma' when
      --   in a list
...
-- | Haskell Tuple Sort
type HsTupleSort = AST.TupleSort
...
type
  HsAppType pass = AST.AppType (GHC pass)
  -- ^ Haskell Application Type
pattern
  HsAppInfix ::
    LIdP pass ->
    HsAppType pass
  -- either a symbol or an id in backticks
pattern
  HsAppPrefix ::
    LHsType pass ->
    HsAppType pass
...
pattern
  HsAppInfix a
    = AST.AppInfix NoFieldExt a
pattern
  HsAppPrefix a
    = AST.AppPrefix NoFieldExt a
...
type instance
  AST.XAppInfix   (GHC pass) = NoFieldExt
type instance
  AST.XAppPrefix  (GHC pass) = NoFieldExt
type instance
  AST.XNewAppType (GHC pass) = NoConExt

-- | Located Haskell Application Type
type LHsAppType pass = AST.LAppType (GHC pass)
     -- ^ 'ApiAnnotation.AnnKeywordId' : 'ApiAnnotation.AnnSimpleQuote'

-- ------------------------------------

type
  HsTyVarBndr pass = AST.TyVarBndr (GHC pass)
...
pattern
  UserTyVar a
    = AST.UserTyVar' NoFieldExt a
pattern
  KindedTyVar a b
    = AST.KindedTyVar' NoFieldExt a b
...
type instance
  AST.XUserTyVar    (GHC pass) = NoFieldExt
type instance
  AST.XKindedTyVar  (GHC pass) = NoFieldExt
type instance
  AST.XNewTyVarBndr (GHC pass) = NoConExt

-- | Located Haskell Type Variable Binder
type LHsTyVarBndr pass = AST.LTyVarBndr (GHC pass)
                         -- See Note [HsType binders]
...
type LHsIPName = AST.LIPName

-- ------------------------------------

-- | Bang Type
type BangType  pass = AST.BangType (GHC pass)
     -- Bangs are in the HsType data type

-- | Located Bang Type
type LBangType pass = AST.LBangType (GHC pass)

-- ------------------------------------

-- | Haskell Context
type HsContext  pass = AST.Context (GHC pass)

-- | Located Haskell Context
type LHsContext pass = AST.LContext (GHC pass)
...
-- | Haskell Kind
type HsKind  pass = AST.Kind  (GHC pass)

-- | Located Haskell Kind
type LHsKind pass = AST.LKind (GHC pass)
      -- ^ 'ApiAnnotation.AnnKeywordId' : 'ApiAnnotation.AnnDcolon'
...
type LHsSigType   pass = AST.LSigType   (GHC pass) -- Implicit only

-- | Located Haskell Wildcard Type
type LHsWcType    pass = AST.LWcType    (GHC pass) -- Wildcard only

-- | Located Haskell Signature Wildcard Type
type LHsSigWcType pass = AST.LSigWcType (GHC pass) -- Both
...
deriving instance
  (Data (PostTc pass Kind),
   ForallX Data pass, DataId pass) => Data (NewHsType pass)

deriving instance
  (DataId pass) => Data (LHsQTyVars pass)

deriving instance
  (DataId pass, Data thing) => Data (HsImplicitBndrs pass thing)

deriving instance
  (DataId pass, Data thing) => Data (HsWildCardBndrs pass thing)

deriving instance
  Data HsIPName

deriving instance
  (Data pass, Data (PostTc pass (IdP pass)), Data (PostRn pass (IdP pass))) =>
    Data (AmbiguousFieldOcc pass)

deriving instance
  (DataId pass) => Data (FieldOcc pass)

deriving instance
  (Data arg, Data rec) => Data (HsConDetails arg rec)

deriving instance
  (DataId pass) => Data (ConDeclField pass)
...
deriving instance
  (DataId pass) => Data (HsAppType pass)

deriving instance
  (DataId pass) => Data (HsTyVarBndr pass)

deriving instance
  (DataId pass) => Data (HsType pass)

deriving instance
  Data HsTyLit

deriving instance
  (DataId pass) => Data (HsWildCardInfo pass)
...
deriving instance Eq Promoted

deriving instance Show Promoted
...
splitLHsQualTy (L _ (HsQualTy { hst_ctxt = ctxt, hst_bodyy = body })) = (ctxt,     body)
...
ppr_mono_ty (HsQualTy { hst_ctxt = L _ ctxt, hst_bodyy = ty })
