...
Note [ExplicitTuple]
~~~~~~~~~~~~~~~~~~~~
An ExplicitTuple is never just a data constructor like (,,,). That is parsed
into an HsVar, and it stays that way. This is important for two reasons:
...
Note [Empty lists]
~~~~~~~~~~~~~~~~~~
Empty lists are both a data constructor and an ExplicitList. This Note describes
how they flow through the various phases and why.

Parsing
-------
An empty list is parsed by the sysdcon nonterminal. It thus comes to life via
nilDataCon (defined in TysWiredIn). A freshly-parsed (HsExpr GhcPs) empty list
