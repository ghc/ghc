...
import {-# SOURCE #-} HsExpr ( pprExpr, LHsExpr,
                               ...
                               GRHSs, pprPatBind, pprFunBind' )
import {-# SOURCE #-} HsPat  ( LPat, pprPat )
...
pprLHsBinds
  :: ( OutputableBndrId (GhcPass idL), OutputableBndrIdNeedsOfConfig (GhcPass idL) r
     , OutputableBndrId (GhcPass idR), OutputableBndrIdNeedsOfConfig (GhcPass idR) r
     )
...
pprLHsBindsForUser
  :: ( OutputableBndrId (GhcPass idL), OutputableBndrIdNeedsOfConfig (GhcPass idL) r
     , OutputableBndrId (GhcPass idR), OutputableBndrIdNeedsOfConfig (GhcPass idR) r
     , OutputableBndrId (GhcPass id2), OutputableBndrIdNeedsOfConfig (GhcPass id2) r
     )
...
instance ( idR ~ GhcPass pr, OutputableBndrId idL, OutputableBndrId idR
  ...
  ppr (PSB{ psb_id = (L _ psyn), psb_args = details, psb_def = pat,
    ...
    where
      ...
      ppr_simple syntax = syntax <+> pprPat pat
...
ppr_sig
  :: ( OutputableBndrId (GhcPass p), OutputableBndrIdNeedsOfConfig (GhcPass p) r
     , HasPprConfig r
     , HasNameSuppress r
     , HasPackageState r
     , HasTypeSuppress r
...
instance (p ~ GhcPass pass, OutputableBndrId p)
  ...
  type OutputableNeedsOfConfig (FixitySig p) = OutputableBndrIdNeedsOfConfig p
...
pprVarSig
  :: ( OutputableBndr id, OutputableNeedsOfConfig id r
     , HasPprConfig r
     )
  => [id] -> SDoc' r -> SDoc' r
...
instance Outputable TcSpecPrag where
  type OutputableNeedsOfConfig TcSpecPrag = PairConstraint
