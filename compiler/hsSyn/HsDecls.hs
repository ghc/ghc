...
ppr_fam_inst_eqn (HsIB { hsib_body = FamEqn { feqn_tycon  = L _ tycon
                                            ...
                                            , feqn_fixity = fixity
                                            , feqn_rhs    = rhs }})
    = pprFamInstLHS tycon bndrs pats fixity noLHsContext <+> equals <+> ppr rhs
...
pprDataFamInstDecl top_lvl (DataFamInstDecl { dfid_eqn = HsIB { hsib_body =
  ...
  where
    pp_hdr ctxt = ppr_instance_keyword top_lvl
              <+> pprFamInstLHS tycon bndrs pats fixity ctxt
...
pprFamInstLHS :: (OutputableBndrId (GhcPass p))
   ...
   -> LHsContext (GhcPass p)
   -> SDoc
pprFamInstLHS thing bndrs typats fixity mb_ctxt
