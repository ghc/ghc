...
matchN (in_scope, id_unf) rule_name tmpl_vars tmpl_es target_es rhs
  ...
  where
    ...
    go menv subst (t:ts) (e:es) = pprTrace "match call" (ppr t $$ ppr e $$ ppr tmpl_vars) $
...
match_co renv subst co1 co2
  ...
  = pprTrace "match_co:matchVar" (ppr co1 $$ ppr co2) $
    match_var renv subst cv (Coercion co2)
  | Just (ty1, r1) <- pprTrace "match_co:not-matchVar" (ppr co1 $$ ppr co2) $
                      isReflCo_maybe co1
  = do { (ty2, r2) <- isReflCo_maybe co2
       ; guard (r1 == r2)
       ; pprTrace "match_co:refl" (ppr ty1 $$ ppr ty2) $
         match_ty renv subst ty1 ty2 }
match_co renv subst co1 co2
