...
scExpr' env (ConApp dc cargs) = -- safe use of compressed args
    -- TODO #12618: Can a datacon be in scSubstId?
    do  { (arg_usgs, cargs') <- unzip <$> mapM (scExpr env) cargs
        ; return (combineUsages arg_usgs, ConApp dc cargs')
        }
...
isValue _env e@(ConApp dc _)
  = Just (ConVal (DataAlt dc) args)
...
samePat (vs1, as1) (vs2, as2)
  ...
  where
    ...
    same (ConApp dc1 cargs1) (ConApp dc2 cargs2)
        = dc1 == dc2 && all2 same cargs1 cargs2
