...
specProgram guts@(ModGuts { mg_module = this_mod
  ...
  = do { dflags <- getDynFlags
       ...
       ; (binds', uds) <- runSpecM this_mod (go binds)
...
--      switch off specialisation for inline functions
  ...
  where
    ...
    already_covered :: RuleFunEnv -> [CoreRule] -> [CoreExpr] -> Bool
    already_covered ruleFunEnv new_rules args      -- Note [Specialisations already covered]
       = isJust (lookupRule ruleFunEnv (in_scope, realIdUnfolding)
                            (const True) fn args
    ...
    spec_call spec_acc@(rules_acc, pairs_acc, uds_acc)
      ...
      = ASSERT(call_arity <= fn_arity)
        ...
        do { (rhs_env2, unused_bndrs, rule_bndrs, rule_args, unspec_bndrs, dx_binds, spec_args)
           ...
           ; ruleFunEnv <- SpecM $ (liftM spec_rule_fun_env get)
           ; dflags <- getDynFlags
           ; if already_covered ruleFunEnv rules_acc rule_args
...
instance HasDynFlags SpecM where
    getDynFlags = SpecM $ liftM (dynFlags . spec_rule_fun_env) get

instance HasModule SpecM where
    getModule = SpecM $ liftM spec_module get

runSpecM :: Module -> SpecM a -> CoreM a
runSpecM this_mod (SpecM spec)
