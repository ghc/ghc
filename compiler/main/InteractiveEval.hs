...
{-
  ...
  2. For every class, get a list of all instances that unify with the query type.
  3. For every unifying instance, ask GHC for the context the instance dictionary needs.
...
getDictionaryBindings :: Var -> TcM WantedConstraints
...
checkForExistence res mb_inst_tys = do
  ...
  return $ toMaybe (all (isTyVarClassPred . ctPred) wantedRes) subst
...
findMatchingInstances ty = do
  ...
  return . concat $ map (\cls ->
    -- for any given type we should have at most one exact match, but we might also
    -- have overlapping instances that unified but didn't fully match our type
    -- for our purposes we want to report both instances.
