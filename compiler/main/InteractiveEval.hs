...
checkForExistence res mb_inst_tys = do
  ...
  if all (isSatisfiablePred . ctPred) all_residual_constraints && (null $ wc_impl residuals)
  then return . Just $ substInstArgs subst res
  else return Nothing
...
-}
substInstArgs ::  TCvSubst -> ClsInst -> ClsInst
substInstArgs subst inst = let
    tau   = mkClassPred cls (substTheta subst args)
    substitutedConstraints = substTheta subst dfun_theta
    -- Eliminate fully satisfied constraints like `Functor Maybe`
