...
{-
  ...
  But if there are overlapping instances, we want to return all the the matching,
  overlapping instances for example if we had instances:

  instance C a => C [a] where
  instance C [Int] where

  We want the query `[Int]` to return both instances.
...
{-
  Given an instance who's head matches against a list of types, we check if the instance
  really exists. To do this first, we create a binding for the dictionary and ask
  GHC to tell us all the implied constraints it can't discharge.

  We then check to make sure all of those residual all mention type-holes,
  if there's a residual constraint mentioning a concrete type, then it means that
  the instance can't be satisfied.

  For example, if we were given the instance `Monoid a => Applicative (,) a` and the
  list [(,) Int]. We ask GHC for a dictionary of type `Applicative (,) Int`, and GHC
  will helpfully inform us that we need to provide one of type `Monoid Int`, which doesn't
  exist.

  The returned instance will be one where we've included all the remaining residual constraints
  and substituted our query into the head of the instance, for example given:
