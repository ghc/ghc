...
data Target
  = Target {
      ...
      --
      -- If a corresponding source file does not exist on disk will result
      -- in a 'SourceError' exception if @targetId = TargetModule _@ is
      -- used. However together with @targetId = TargetFile _@ GHC will not
      -- complain about the file missing.
    }
