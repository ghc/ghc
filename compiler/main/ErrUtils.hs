...
-- | Dump options
--
-- Dumps are printed on stdout by default except when the `dumpMandatoryFile`
-- field is set to True.
--
-- When `dumpMandatoryFile` is True or when `-ddump-to-file` is set, dumps are
-- written into a file whose suffix is given in the `dumpSuffix` field.
--
data DumpOptions = DumpOptions
   { dumpMandatoryFile :: Bool   -- ^ Must be dumped into a file, even if
...
dumpOptionsFromFlag Opt_D_th_dec_file =
   DumpOptions                         -- -dth-dec-file dumps expansions of TH
      { dumpMandatoryFile = True       -- splices into MODULE.th.hs even when
      , dumpSuffix        = "th.hs"    -- -ddump-to-file isn't set
      }
dumpOptionsFromFlag flag =
   DumpOptions
      { dumpMandatoryFile = False
