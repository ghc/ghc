...
findExactModule hsc_env mod = case
    M.lookup iuid (hsc_internalUnitEnv hsc_env)
  of
    Just unitEnv -> findInstalledHomeModule
      (hsc_FC hsc_env) iuid
      (internalUnitEnv_dflags unitEnv) (installedModuleName iuid)
...
handleAnyHomeModule :: HscEnv -> ModuleName -> FindResult -> InstalledUnitId -> IO FindResult
handleAnyHomeModule hsc_env mod_name fr iUnitId = do
  result <- findInstalledHomeModule (hsc_FC hsc_env) iUnitId dflags mod_name
...
findHomeModule :: HscEnv -> InstalledUnitId -> ModuleName -> IO FindResult
findHomeModule hsc_env iuid mod_name = do
  let
    internalUnitEnv = case M.lookup iuid (hsc_internalUnitEnv hsc_env) of
      Just unitEnv -> unitEnv
      Nothing -> pprPanic "findInstalledHomeModule: packageNotFound" $ ppr iuid
    dflags = internalUnitEnv_dflags internalUnitEnv
