...
compileOne' m_tc_result mHscMessage
 ...
 = do
   ...
   case (frontend_status, hsc_lang) of
     (HscFrontendStatus_UpToDate iface hmi_details, _) ->
       -- TODO recomp014 triggers this assert. What's going on?!
       -- ASSERT( isJust mb_old_linkable || isNoLink (ghcLink dflags) )
       return $! HomeModInfo iface hmi_details mb_old_linkable
     (HscFrontendStatus_UpdateBoot iface hmi_details, HscInterpreted) -> do
       return $! HomeModInfo iface hmi_details Nothing
     (HscFrontendStatus_UpdateBoot iface hmi_details, _) -> do
       touchObjectFile dflags object_filename
       return $! HomeModInfo iface hmi_details Nothing
     (HscFrontendStatus_UpdateSig iface hmi_details, HscInterpreted) -> do
       let !linkable = LM (ms_hs_date summary) this_mod []
       return $! HomeModInfo iface hmi_details (Just linkable)
     (HscFrontendStatus_UpdateSig iface hmi_details, _) -> do
       ...
       _ <- runPipeline StopLn hsc_env'
         ...
         , Just $ HscOut src_flavour mod_name $
             HscPhase_FrontendOut $ HscFrontendStatus_UpdateSig iface hmi_details
         )
         (Just basename)
         Persistent
         (Just location)
       ...
       return $! HomeModInfo iface hmi_details (Just linkable)
     (HscFrontendStatus_Recomp _ _ summary tc_result mb_old_hash, HscInterpreted) -> do
       middle_status <- runHsc hsc_env $ hscIncrementalCompileMiddle summary tc_result mb_old_hash
       case (middle_status, hsc_lang) of
         (HscMiddleStatus_NotGeneratingCode, _) -> panic "compileOne HscNotGeneratingCode"
     ...
     (HscFrontendStatus_Recomp iface hmi_details summary tc_result mb_old_hash, HscNothing) -> do
       -- We only do this at all because we still want to catch the errors that
       -- come from desugaring.
       middle_status <- runHsc hsc_env $ hscIncrementalCompileMiddle summary tc_result mb_old_hash
       case middle_status of
         ...
         HscMiddleStatus_Recomp {} -> panic "compileOne 'HscNothing' means we should always get 'HscMiddleStatus_NotGeneratingCode'"
     (HscFrontendStatus_Recomp {}, _) -> do
       output_fn <- getOutputFilename next_phase
...
runPhase (HscOut src_flavour mod_name (HscPhase_FrontendOut result)) _ dflags = do
        ...
        case result of
            HscFrontendStatus_UpToDate _ _ ->
                do liftIO $ touchObjectFile dflags o_file
                   -- The .o file must have a later modification date
                   -- than the source file (else we wouldn't get Nothing)
                   -- but we touch it anyway, to keep 'make' happy (we think).
                   return (RealPhase StopLn, o_file)
            HscFrontendStatus_UpdateBoot _ _ ->
                do -- In the case of hs-boot files, generate a dummy .o-boot
                   -- stamp file for the benefit of Make
                   liftIO $ touchObjectFile dflags o_file
                   return (RealPhase StopLn, o_file)
            HscFrontendStatus_UpdateSig _ _ ->
                do -- We need to create a REAL but empty .o file
                   ...
                   return (RealPhase StopLn, o_file)
            HscFrontendStatus_Recomp _ _ summary tc_result mb_old_hash -> do
              PipeState {hsc_env = hsc_env'} <- getPipeState
              middle_status <- liftIO $ runHsc hsc_env' $
                hscIncrementalCompileMiddle summary tc_result mb_old_hash
              return ( HscOut src_flavour mod_name $ HscPhase_MiddleOut middle_status
                     , o_file
                     )
