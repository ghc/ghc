{-# LANGUAGE CPP, BangPatterns, Strict #-}
...
updateRuleCafInfos type_env rule@Rule{ ru_rhs = rhs } = rule{ ru_rhs = updateGlobalIds type_env rhs }
...
updateIdUnfolding type_env id =
    case idUnfolding id of
      unf@CoreUnfolding{ uf_tmpl = tmpl } ->
        setIdUnfolding id unf{ uf_tmpl = updateGlobalIds type_env tmpl }
      unf@DFunUnfolding{ df_args = args } ->
        setIdUnfolding id unf{ df_args = map (updateGlobalIds type_env) args }
      _ -> id
