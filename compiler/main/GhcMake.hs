...
-- | 'Bool' indicating if a module is a boot module or not.
            ...
            handleSourceError (\err -> do logger err; return Nothing) $ do
                ...
                lcl_hsc_env'' <- case finish_loop of
                    ...
                    Just loop -> typecheckLoop lcl_dflags lcl_hsc_env' $
                                 filter (/= moduleName (moduleWithIsBoot_module this_build_mod)) $
                                 map (moduleName . moduleWithIsBoot_module) loop
 ...
 where
  ...
  keep_going this_mods old_hpt done mods mod_index nmods uids_to_check done_holes = do
    ...
    when (not $ null dropped_ms) $ do
        dflags <- getSessionDynFlags
        liftIO $ fatalErrorMsg dflags (keepGoingPruneErr $ moduleNameWithIsBoot_moduleName <$> dropped_ms)
