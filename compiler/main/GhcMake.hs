...
enableCodeGenForTH :: HscTarget
  ...
  -> IO (NodeMap [Either ErrMsg ModSummary])
enableCodeGenForTH =
  enableCodeGenWhen isTemplateHaskellOrQQNonBoot should_modify
  where
...
enableCodeGenForUnboxedTuples =
  enableCodeGenWhen condition should_modify
  where
    condition ms =
...
enableCodeGenWhen
  ...
  -> NodeMap [Either ErrMsg ModSummary]
  -> IO (NodeMap [Either ErrMsg ModSummary])
enableCodeGenWhen condition should_modify target nodemap =
