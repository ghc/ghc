...
deriveClause rep_tc mb_lderiv_strat deriv_preds err_ctxt
  = addErrCtxt err_ctxt $ do
      traceTc "deriveClause" $ vcat
        ...
        , text "deriv_preds"     <+> ppr deriv_preds ]
      (mb_deriv_strat', via_tvs, deriv_pred_parts)
        <- tcExtendTyVarEnv tvs $
           -- Deriving predicates may mention the type variables bound by the
           -- data type declaration, so we use tcExtendTyVarEnv to bring these
           -- type variables into scope.
           tcDerivStrategy mb_lderiv_strat $
           -- Moreover, when using DerivingVia one can bind type variables in
           -- the `via` type as well, so these type variables must also be
           -- brought into scope. This is accomplished in `tcDerivStrategy`.
           traverse tcHsDeriv deriv_preds
      ...
      let -- This returns a Maybe because the user might try to derive
          -- Typeable, which is a no-op nowadays.
          derive_one_pred ::
               LHsSigType GhcRn
               -- The class to be derived, as written in the source syntax
      ...
      mapMaybeM (\(loc, (cls_tvs, cls, cls_tys, cls_arg_kinds)) ->
                  recoverM (pure Nothing) $
                    derive_one_pred loc cls_tvs cls cls_tys cls_arg_kinds)
                (zipEqual "deriveClause" deriv_preds deriv_pred_parts)
  where
...
recoverM, as in the following pseudocode:
  ...
  recoverM (pure Nothing) $ mapM derive_one_pred [E, F, G]

We instead use recoverM in each iteration of the loop:

  mapM (recoverM (pure Nothing) . derive_one_pred) [E, F, G]
