...
checkHsigDeclM sig_iface sig_thing real_thing = do
    ...
    let sig_fixity = case mi_fix_fn (mi_exts sig_iface) (occName name) of
  ...
  orig_tcg_env lcl_iface0 = setSrcSpan loc $ do
    ...
    tcg_env <- (\x -> foldM x tcg_env infos)
        ...
        let merge_inst (insts, inst_env) inst
            ...
            iface' = iface { mi_exts = (mi_exts iface){ mi_orphan = False, mi_finsts = False } }
        ...
        return tcg_env {
            ...
            tcg_merged   =
                if outer_mod == mi_module iface
                    ...
                    else (mi_module iface, mi_mod_hash (mi_exts iface)) : tcg_merged tcg_env
            }
