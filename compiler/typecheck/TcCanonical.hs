...
canonicalize (CNonCanonical { cc_ev = ev })
  = {-# SCC "canNC" #-}
    case classifyPredType pred of
      ...
      EqPred eq_rel _ _ ty1 ty2 -> do traceTcS "canEvNC:eq" (ppr ty1 $$ ppr ty2)
...
canonicalize (CIrredCan { cc_ev = ev })
  | EqPred eq_rel _ _ ty1 ty2 <- classifyPredType (ctEvPred ev)
  ...
  = do { let pred = ctEvPred ev
    ...
    do { -- Re-classify, in case flattening has improved its shape
       ; case classifyPredType (ctEvPred new_ev) of
           ...
           EqPred eq_rel _ _ ty1 ty2 -> canEqNC new_ev eq_rel ty1 ty2
           _                         -> continueWith $
                                        mkIrredCt new_ev } }
