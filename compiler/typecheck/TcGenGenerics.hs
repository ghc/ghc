...
mkBindsRep gk tycon = (binds, sigs)
      where
        ...
        -- Both methods are marked as INLINE[0] so that GHC is able to optimize
        -- away intermediate Generic representation for more types (#11068).
        sigs = [ inline0 from01_RDR
               , inline0 to01_RDR
               ]
         where
           inline0 f = L loc . InlineSig noExtField (L loc f)
                     $ alwaysInlinePragma { inl_act = activeDuringFinal }
