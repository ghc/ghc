...
solve_simple_wanteds (WC { wc_simple = simples1, wc_insol = insols1, wc_impl = implics1 })
  = nestTcS $
    do { solveSimples simples1
       ...
       ; return ( WC { wc_simple = others `andCts` unflattened_eqs
...
kickOutAfterUnification new_tv
  = do { ics <- getInertCans
       ; let (kicked_out1, ics1) = kick_out_model new_tv ics
             (kicked_out2, ics2) = kick_out (Given,NomEq) new_tv ics1
...
kick_out_model :: TcTyVar -> InertCans -> (WorkList, InertCans)
kick_out_model new_tv ics@(IC { inert_model = model, inert_eqs = eqs })
