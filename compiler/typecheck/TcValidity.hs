...
checkUserTypeError = check
  where
  check ty
    | Just msg      <- userTypeError_maybe ty   = add_user_msg failWithTcM msg
    | Just (msg, _) <- userTypeWarning_maybe ty = add_user_msg (addWarnTcM NoReason) msg
  ...
  add_user_msg tcm_f  msg = do { env0 <- tcInitTidyEnv
                     ; let (env1, tidy_msg) = tidyOpenType env0 msg
                     ; tcm_f (env1, pprUserTypeErrorTy tidy_msg) }
