...
tcPatToExpr name args pat = go pat
  where
    ...
    mkPrefixConExpr lcon@(dL->L loc _) tyargs pats = do
      exprs <- mapM go pats
      let con = foldl' (\x t -> HsAppType noExtField (cL loc x) t) (HsVar noExtField lcon) tyargs
      return (foldl' (\x y -> HsApp noExtField (cL loc x) y) con exprs)
