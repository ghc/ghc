...
reifyType ty@(ForAllTy (Bndr _ argf) _)
                            = reify_for_all (argToForallVisFlag argf) ty
...
reifyType ty@(FunTy t1 t2)
  | isPredTy t1 = reify_for_all ForallInvis ty  -- Types like ((?x::Int) => Char -> Char)
...
reify_for_all :: TyCoRep.ForallVisFlag -> TyCoRep.Type -> TcM TH.Type
-- Arg of reify_for_all is always ForAllTy or a predicate FunTy
reify_for_all fvf ty = do
  tvs' <- reifyTyVars tvs
  case fvf of
  ...
  where
    (tvs, phi) = tcSplitForAllTysSameVis fvf ty
