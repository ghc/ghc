...
tcTypedBracket rn_expr brack@(TExpBr _ expr) res_ty
  = addErrCtxt (quotationCtxtDoc brack) $
    do { cur_stage <- getStage
       ...
       -- Bundle them together so they can be used in GHC.HsToCore.Splice for desugaring
       -- brackets.
...
In both cases, desugaring happens like this:
  * HsTcBracketOut is desugared by GHC.HsToCore.Splice.dsBracket.  It
  ...
  * When GHC.HsToCore.Splice (used to desugar the body of the bracket) comes across
    a splice, it looks up the splice's Name, n, in the ds_meta envt,
    to find an (HsExpr Id) that should be substituted for the splice;
    it just desugars it to get a CoreExpr (GHC.HsToCore.Splice.repSplice).
...
2. Desugaring quotations (see GHC.HsToCore.Splice)
