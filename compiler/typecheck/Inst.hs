...
newMethodFromName
  ...
  -> TcRhoType             -- ^ type with which to instantiate the class
  -> Bool                  -- ^ does the class also need a runtime rep arg?
  -> TcM (HsExpr GhcTcId)
...
newMethodFromName origin name inst_ty needs_rep_app
  = do { id <- tcLookupId name
       ...
       ; let ty_args = [ getRuntimeRep inst_ty | needs_rep_app ] ++ [ inst_ty ]
             ty = piResultTys (idType id) ty_args
...
tcSyntaxName orig ty (std_nm, HsVar _ (L _ user_nm))
  | std_nm == user_nm
  = do rhs <- newMethodFromName orig std_nm ty False
       return (std_nm, rhs)
