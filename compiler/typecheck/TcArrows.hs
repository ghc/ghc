...
tcProc pat cmd exp_ty
  = newArrowScope $
    do  { exp_ty <- expTypeToType exp_ty  -- no higher-rank stuff with arrows
        ...
        ; (pat', cmd') <- tcPat ProcExpr pat (mkCheckExpType arg_ty $ text "TcArrows.tcProc") $
...
tc_cmd env in_cmd@(HsCmdCase x scrut matches) (stk, res_ty)
  = addErrCtxt (cmdCtxt in_cmd) $ do
      ...
      matches' <- tcMatchesCase match_ctxt scrut_ty matches (mkCheckExpType res_ty $ text "tc_cmd: case statement")
...
tc_cmd env (HsCmdIf x (Just fun) pred b1 b2) res_ty -- Rebindable syntax for if
  = do  { pred_ty <- newOpenFlexiTyVarTy
        ...
        ; (pred', fun')
            <- tcSyntaxOp IfOrigin fun (map (flip synKnownType $ text "tc_cmd: rebindable syntax for if")
                                            [pred_ty, r_ty, r_ty])
                                       (mkCheckExpType r_ty $ text "tc_cmd: rebindable syntax for if") $ \ _ ->
               tcMonoExpr pred (mkCheckExpType pred_ty $ text "tc_cmd: rebindable syntax for if")
...
tc_cmd env cmd@(HsCmdArrApp _ fun arg ho_app lr) (_, res_ty)
  = addErrCtxt (cmdCtxt cmd)    $
    do  { arg_ty <- newOpenFlexiTyVarTy
        ...
        ; fun' <- select_arrow_scope (tcMonoExpr fun (mkCheckExpType fun_ty $ text "tc_cmd.HsCmdArrApp: arrow application for fun"))

        ; arg' <- tcMonoExpr arg (mkCheckExpType arg_ty $ text "tc_cmd.HsCmdArrApp: arrow application for arg")
...
tc_cmd env cmd@(HsCmdApp x fun arg) (cmd_stk, res_ty)
  = addErrCtxt (cmdCtxt cmd)    $
    do  { arg_ty <- newOpenFlexiTyVarTy
        ...
        ; arg'   <- tcMonoExpr arg (mkCheckExpType arg_ty $ text "tc_cmd.HsCmdApp")
  ...
  = addErrCtxt (pprMatchInCtxt match)        $
    do  { (co, arg_tys, cmd_stk') <- matchExpectedCmdArgs n_pats cmd_stk
        ...
        ; (pats', grhss') <- setSrcSpan mtch_loc                              $
                             tcPats LambdaExpr pats (map (flip mkCheckExpType $ text "tc_cmd.HsCmdLam.pattern") arg_tys) $
                             tc_grhss grhss cmd_stk' (mkCheckExpType res_ty   $ text "tc_cmd.HsCmdLam.GRHSs")
...
tcArrDoStmt env ctxt (BindStmt _ pat rhs _ _) res_ty thing_inside
  = do  { (rhs', pat_ty) <- tc_arr_rhs env rhs
        ; (pat', thing)  <- tcPat (StmtCtxt ctxt) pat (mkCheckExpType pat_ty $ text "tcArrDoStmt.BindStmt") $
                            thing_inside res_ty
  ...
  = do  { let tup_names = rec_names ++ filterOut (`elem` rec_names) later_names
        ...
        { (stmts', tup_rets)
                <- tcStmtsAndThen ctxt (tcArrDoStmt env) stmts res_ty   $ \ _res_ty' ->
                   ...
                   zipWithM tcCheckId tup_names (map (flip mkCheckExpType $ text "tcArrDoStmt.RecStmt") tup_elt_tys)
