...
tc_hs_sig_type skol_info hs_sig_type ctxt_kind
  ...
  = do { (tc_lvl, (wanted, (spec_tkvs, ty)))
       ...
       --     in TcSimplify
       ; if insolubleWC resid_wc
         then do { _ <- simplifyTop resid_wc -- don't forget to report!
                        -- NB: always report before failing
                        -- Using simplifyTop (instead of, say, reportUnsolved)
                        -- does defaulting, which improves error messages.
                        -- e.g. typecheck/should_fail/T14232, which mentioned
                        -- TYPE t0 instead of * in its error message.
                 ; failM }
         else emitConstraints resid_wc
