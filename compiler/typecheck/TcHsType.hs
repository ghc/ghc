...
tc_infer_hs_type mode (HsOpTy _ lhs (L loc_op op) rhs)
  ...
  = do { (op', op_kind) <- tcTyVar mode op
       ...
       ; tcTyApps mode (noLoc $ HsTyVar PlaceHolder NotPromoted (L loc_op op))
                  op' op_kind' [lhs, rhs] }
...
tc_fun_type mode ty1 ty2 exp_kind = case mode_level mode of
  TypeLevel ->
    do { arg_k <- newOpenTypeKind
       ...
       ; checkExpectedKind (HsFunTy PlaceHolder ty1 ty2) (mkFunTy ty1' ty2')
    ...
    kc_hs_tv (UserTyVar lname@(L _ name))
      = do { tv_pair@(tv, scoped) <- tcHsTyVarName Nothing name
           ...
           ; when (open_fam && not scoped) $ -- (don't default class tyvars)
             discardResult $ unifyKind
                                 (Just (HsTyVar PlaceHolder NotPromoted lname))
       ...
       ; case mb_tv of
           Just (ATyVar _ tv)
             -> do { whenIsJust m_kind $ \ kind ->
                     discardResult $
                     unifyKind
                       (Just (HsTyVar PlaceHolder NotPromoted (noLoc name)))
                       kind (tyVarKind tv)
                   ; return (tv, True) }
