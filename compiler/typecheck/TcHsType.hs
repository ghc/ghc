...
tcHsTypeApp wc_ty kind
  ...
  = do { ty <- solveLocalEqualities "tcHsTypeApp" $
       ...
       -- failure if we don't promote. kindGeneralizeNone does the job:
...
-- The supplied predicate says which variables. But in all cases,
...
kindGeneralizeSome should_gen kind_or_type
  = do { traceTc "kindGeneralizeSome {" (ppr kind_or_type)
       ...
       ; let mono_tvs = unionVarSets [gbl_tvs, promote_kvs, promote_tvs]
             -- NB: Use the original kvs/tvs in mono_tvs, as quantifyTyVars
             -- doesn't look through variables when deciding what to quantify

       ; qkvs <- quantifyTyVars mono_tvs dvs
