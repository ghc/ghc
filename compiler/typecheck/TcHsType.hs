  ...
  = addSigCtxt ctxt hs_ty $
    do { (implicit_tvs, (explicit_tvs, (wcs, wcx, theta, tau)))
            <- solveLocalEqualities "tcHsPartialSigType"    $
                 -- This solveLocalEqualities fails fast if there
                 -- are insoluble equalities. See TcSimplify
                 -- Note [Fail fast if there are insoluble kind equalities]
                 -- Example   f :: _ -> Maybe
                 --           f = e
                 -- We don't want to try to check 'e :: Maybe'; that
                 -- would cause a cascade of irrelevant errors
