...
module TcRnTypes(
        ...
        -- Usage Env
        UsageEnv(..), unitUE, mkUE, zeroUE, emptyUE, addUE, addUEs, scaleUE, supUE,
        deleteUEAsserting, deleteUE, lookupUE
...
import Control.Monad (ap, liftM, msum, guard)
...
pprCtOrigin (DerivOriginDC dc n _)
  = hang (ctoHerald <+> text "the" <+> speakNth n
       ...
       2 (parens (text "type" <+> quotes (ppr (coreWeightedThing ty))))
  where
