...
2. Nested functions aren't too bad, on the other hand. However, consider this
   ...
   Because F is injective, we want to see if we should try unifying ty1 and
   ty2. To do this, we need to know if fsk1 and fsk2 are equal. In this case,
   they are, but only because G Int and G Bool reduce to the same value. If we
   know that CTyEqCan RHS's don't have these functions, then we're sure to be
   able to witness the equality between fsk1 and fsk2.
