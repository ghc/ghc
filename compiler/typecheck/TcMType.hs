      ...
      = do { tv_kind <- zonkTcType (tyVarKind tv)
           ...
           ; if |  tcTyVarLevel tv <= cur_lvl
                ...
                -> do { traceTc "Naughty quantifier" $
                      ...
                      ; orig_ty1 <- zonkTcType orig_ty  -- in case it's not zonked

                      ; escapees <- mapM zonkTcTyVarToTyVar $
                                         nonDetEltsUniqSet $
                                         intersectVarSet bound tv_kind_vars
                      ...
                      ; let fvs  = tyCoVarsOfTypeWellScoped orig_ty1
                            ...
                            doc = pprWithExplicitKindsWhen True $
                                  vcat [ text "Cannot generalise type because a skolem would escape."
                                       , text "When generalising this type:"
                                       , nest 2 (pprTidiedType orig_ty')
                                       , text "these skolems would escape:"
                                       , nest 2 (ppr_tidied escapees)
