      ...
      = do { tv_kind <- zonkTcType (tyVarKind tv)
           ...
           ; if |  tcTyVarLevel tv <= cur_lvl
                ...
                -> do { traceTc "Naughty quantifier" $
                      ...
                      ; (env, orig_ty') <- zonkTidyTcType emptyTidyEnv orig_ty
                      ; let escapees = intersectVarSet bound tv_kind_vars
                            ...
                            doc = pprWithExplicitKindsWhen True $
                                  vcat [ text "Skolem escape leads to quantification failure."
                                       , text "When quantifying this type:"
                                       , nest 2 (ppr orig_ty')
                                       , text "these skolems would escape:"
                                       , nest 2 (pprVarSet escapees ppr_tidied)
                                       , text "if we quantified this variable:"
                                       , nest 2 (ppr_tidied [tv])
                                       , text "(Indeed, I struggle even printing this correctly,"
