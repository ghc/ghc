...
collect_cand_qtvs is_dep bound dvs ty
  ...
  where
    ...
    -----------------
    go_tv tc_lvl dv@(DV { dv_kvs = kvs, dv_tvs = tvs }) tv
      | tv `elemDVarSet` kvs
      ...
      | not (tcTyVarLevel tv `strictlyDeeperThan` tc_lvl)
      = return dv  -- this tyvar is from an outer context: see Wrinkle
