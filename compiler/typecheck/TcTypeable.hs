...
In today's GHC, we normalize all types before computing their TypeRep. That
is, if we have `type instance F Int = Bool` and we wish to have a rep for
`F Int -> Type`, then we actually get the rep for `Bool -> Type`. Another
way to phrase this is: if we have a type definition with casts, then the
