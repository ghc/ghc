...
check_main dflags tcg_env explicit_mod_hdr export_ies
 ...
 | otherwise
 = do   { mains <- lookupInfoOccRn main_fn
        ...
        ; case mains of {
             ...
             _ -> do {
               -- There are multiple main functions in scope                       #16453
               ; traceTc "check_main multiple main functions in scope: " $ ppr mains
               ; let exp_mains = filterModules ( exportedMainFuncs export_ies ) mains
               ; case exp_mains of
                  [m] -> use m
                  _   -> do
  ...
  where
    ...
    complain_no_main = unless (interactive && not explicit_mod_hdr)
        ...
        -- Without an explicit module header...
          ...
          -- in other modes, add error message and go on with checking.
    ...
    -- Get the main functions from the export list.
    -- Only the module name is needed, the function name is fix.
    exportedMainFuncs :: Maybe (Located [LIE GhcPs]) -> [ModuleName] -- #16453
    exportedMainFuncs Nothing = [moduleName main_mod]
        -- no main specified, but there is a header!!!!
    exportedMainFuncs (Just exps) = fmap fst $
        filter (\(_,n) -> n == occ_main_fn ) texp
      where
        ies = fmap unLoc $ unLoc exps
        texp = catMaybes $ fmap transformIE ies

    -- Filter all main functions inscope that match the export specs
    filterModules :: [ModuleName] -> [Name] -> [Name]                -- #16453
    filterModules exp_mod mains_inscope =
      [mod | mod <- mains_inscope,
            (moduleName . nameModule) mod `elem` exp_mod]

    -- Transform an export_ie to a (ModuleName OccName) pair.
    transformIE :: IE GhcPs -> Maybe (ModuleName, OccName)           -- #16453
    transformIE (IEVar _  var) = isQual_maybe $ ieWrappedName $ unLoc var
    transformIE (IEModuleContents _ mod) = Just (unLoc mod, occ_main_fn)
    transformIE _ = Nothing
