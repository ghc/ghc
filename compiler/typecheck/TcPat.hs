...
tc_pat penv (AppTypePat x pat ap_ty) pat_ty thing_inside
  = do  { (pat', res) <- tc_lpat pat pat_ty penv thing_inside
        ; ty <- readExpType pat_ty
        ; liftIO (putStrLn (showSDocUnsafe (ppr pat_ty)))
        ; liftIO (putStrLn (showSDocUnsafe (ppr ty)))
...
------------------------
-- Data constructors
tc_pat penv (ConPatIn con arg_types arg_pats) pat_ty thing_inside
  = tcConPat penv con arg_types pat_ty arg_pats thing_inside
...
tcConPat :: PatEnv -> Located Name
         ...
         -> TcM (Pat GhcTcId, a)
tcConPat penv con_lname@(dL->L _ con_name) arg_types pat_ty arg_pats thing_inside
