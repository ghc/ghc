...
disambiguateRecordBinds :: LHsExpr Name -> TcType   -- Record expression and its type
...
disambiguateRecordBinds record_expr record_tau rbnds res_ty
  = case mapM isUnambiguous rbnds of
     ...
     Nothing     -> do
      ...
      ; p <- case possibleParents rbnds_with_parents of
               ...
               _ | obviousSig record_expr
                 , Just p <- tyConOf fam_inst_envs record_tau -> return p
  ...
  where
    ...
    obviousSig :: LHsExpr Name -> Bool
    obviousSig (L _ (ExprWithTySig {})) = True
    obviousSig (L _ (HsPar p))          = obviousSig p
    obviousSig _                        = False
