...
gen_Enum_binds loc tycon = do
  ...
  where
    ...
    succ_enum dflags
      = mk_easy_FunBind loc succ_RDR (pure @NonEmpty a_Pat) $
    ...
    pred_enum dflags
      = mk_easy_FunBind loc pred_RDR (pure @NonEmpty a_Pat) $
        untag_Expr dflags tycon [(a_RDR, ah_RDR)] $
    ...
    to_enum dflags
      = mk_easy_FunBind loc toEnum_RDR (pure @NonEmpty a_Pat) $
    ...
    enum_from dflags
      = mk_easy_FunBind loc enumFrom_RDR (pure @NonEmpty a_Pat) $
          untag_Expr dflags tycon [(a_RDR, ah_RDR)] $
          nlHsApps map_RDR
    ...
    from_enum dflags
      = mk_easy_FunBind loc fromEnum_RDR (pure @NonEmpty a_Pat) $
          untag_Expr dflags tycon [(a_RDR, ah_RDR)] $
          (nlHsVarApps intDataCon_RDR [ah_RDR])
...
gen_Ix_binds loc tycon = do
  ...
  where
    ...
    enum_range dflags
      = mk_easy_FunBind loc range_RDR (pure @NonEmpty $ nlTuplePat [a_Pat, b_Pat] Boxed) $
          untag_Expr dflags tycon [(a_RDR, ah_RDR)] $
          untag_Expr dflags tycon [(b_RDR, bh_RDR)] $
          nlHsApp (nlHsVarApps map_RDR [tag2con_RDR dflags tycon]) $
              nlHsPar (enum_from_to_Expr
...
gen_data dflags data_type_name constr_names loc rep_tc
  ...
  where
    ...
    mk_gcast dataCast_RDR gcast_RDR
      = unitBag (mk_easy_FunBind loc dataCast_RDR (pure @NonEmpty $ nlVarPat f_RDR)
...
mkRdrFunBindSE arity
  ...
  -- which can happen with -XEmptyDataDecls
  -- See #4302
  ([], _) -> L loc $ mkFunBind fun $ pure @[] $ mkMatch
    (mkPrefixFunRhs fun)
    (nlWildPat :| replicate (arity - 1) nlWildPat)
    expr (noLoc emptyLocalBinds)
  (_ : _,  _) -> L loc $ mkFunBind fun matches
  where
    expr = error_Expr str
