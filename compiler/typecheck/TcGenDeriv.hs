  ...
  where
    ...
    enum_index dflags
      = mk_easy_FunBind loc unsafeIndex_RDR
                [noLoc (AsPat mempty (noLoc c_RDR)
      ...
      | otherwise   =
         ([a_Pat, con_pat],
          showParen_Expr (genOpApp a_Expr ge_RDR (nlHsLit
                         (HsInt PlaceHolder (mkIntegralLit con_prec_plus_one))))
...
mk_showsPrec_app p x
  = nlHsApps showsPrec_RDR [nlHsLit (HsInt PlaceHolder (mkIntegralLit p)), x]
...
genAuxBindSpec dflags loc (DerivCon2Tag tycon)
  ...
  where
    ...
    sig_ty = mkLHsSigWcType $ L loc $ NewHsType $ NHsCoreTy $
...
genAuxBindSpec dflags loc (DerivTag2Con tycon)
  ...
  where
    sig_ty = mkLHsSigWcType $ L loc $
             NewHsType $ NHsCoreTy $ mkSpecForAllTys (tyConTyVars tycon) $
...
genAuxBindSpec dflags loc (DerivMaxTag tycon)
  ...
  where
    ...
    sig_ty = mkLHsSigWcType (L loc (NewHsType (NHsCoreTy intTy)))
