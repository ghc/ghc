        ...
        do { is_filled <- isFilledMetaTyVar tv
           ; elim <- case is_filled of
               False -> do { traceTcS "unflatten_eq 2" (ppr ct)
                           ; tryFill      ev eq_rel       tv rhs (mkNomReflCo (tyVarKind tv)) }
    ...
    try_fill_rhs ev eq_rel tclvl lhs_tv rhs
      ...
      = do { is_filled <- isFilledMetaTyVar rhs_tv
           ; if is_filled then return False
                          ...
                          (mkTyVarTy lhs_tv `mkCastTy` mkSymCo co) co }
...
tryFill :: CtEvidence         -- :: (tv |> co) ~ (rhs |> co)   or   (rhs |> co) ~ (tv |> co)
        -> EqRel
        -> TcTyVar -> TcType  -- tv, rhs
        -> TcCoercionN        -- co
        -> TcS Bool
...
tryFill ev eq_rel tv rhs co
  = ASSERT2( not (isGiven ev), ppr ev )
    do { rhs' <- zonkTcType rhs
       ; case tcGetTyVar_maybe rhs' of {
            Just tv' | tv == tv' -> do { setReflEvidence ev eq_rel (rhs `mkCastTy` co)
                                       ; return True } ;
            _other ->
    do { case occCheckExpand tv rhs' of
           Just rhs''    -- Normal case: fill the tyvar
             -> do { setReflEvidence ev eq_rel (rhs'' `mkCastTy` co)
                   ; unifyTyVar tv rhs''
                   ; return True }
