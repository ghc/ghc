...
-- Note [No derived kind equalities]
  = do { flattened_args <- fl [] binders roles tys
  ...
  where
    ...
    fl :: [(Xi, Coercion)]
       ...
       -> [Type]
       -> FlatM [(Xi, Coercion)]
    fl acc _ _ [] = return $ reverse acc

    fl acc (binder:binders) (role:roles) (arg:args)
      = do { arg_out <- if isNamedTyCoBinder binder
                        then noBogusCoercions $ fl1 role arg
                        else                    fl1 role arg
           ; fl (arg_out : acc) binders roles args }

    fl acc [] roles args
      = do { args_out <- noBogusCoercions $ zipWithM fl1 roles args
           ; return (reverse acc ++ args_out) }

    fl _ _ [] _ = panic "flatten_args_slow ran out of roles"
