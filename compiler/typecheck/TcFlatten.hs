...
flatten_args_slow binders inner_ki fvs roles tys
  = simplifyArgsWorker flatten_nominal flatten_representational flatten_phantom
                       binders inner_ki fvs roles tys
  where
    {-# INLINE flatten_nominal #-}
    flatten_nominal binder ty
      = setEqRel NomEq $
        if isNamedTyCoBinder binder
        then noBogusCoercions $ flatten_one ty
        else                    flatten_one ty

    {-# INLINE flatten_representational #-}
    flatten_representational binder ty
      = ASSERT( isAnonTyCoBinder binder )
        setEqRel ReprEq $ flatten_one ty

    {-# INLINE flatten_phantom #-}
    flatten_phantom binder ty
