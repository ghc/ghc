...
gen_Functor_binds loc tycon
  ...
  where
    ...
    ft_fmap = FT { ft_triv = \x -> pure x
                 ...
                 , ft_ty_app = \_ tvp g x ->
                     -- Use TyVarPos to generate more efficient code.
                     -- See Note [Avoid unnecessary eta expansion in derived fmap implementations]
                     case tvp of
                       Nested -> do
                         gg <- mkSimpleLam g
                       ...
                       Immediate ->
                         pure $ nlHsApps fmap_RDR [f_Expr,x]
                   -- fmap f x = fmap g x
                 , ft_forall = \_ g x -> g x
                 ...
                 , ft_ty_app = \_ tvp g x ->
                       ...
                       case tvp of
                         Nested -> do
                           ...
                           pure $ nlHsApps fmap_RDR [gg,x]
                         Immediate ->
                           pure $ nlHsApps replace_RDR [z_Expr,x]
                   -- p <$ x = fmap (p <$) x
                 , ft_forall = \_ g x -> g x
...
The ft_ty_app field of FFoldType (see Note [FFoldType and functorLikeTraverse])
distinguishes between these two $fmap cases by providing an argument of type
TyVarPos. If the last type argument occurs directly (e.g., Maybe a), then the
Immediate constructor of TyVarPos is used. If the last type argument occurs
nested under some other type (e.g., Maybe [a]), then the Nested constructor of
TyVarPos is used. To see how GHC makes use of this information, see the
ft_ty_app case in ft_fmap.

Note that the ft_ty_app case of ft_replace (which derives implementations of
(<$)) also uses TyVarPos to generate more efficient code.
See Note [Deriving <$].
...
We use the TyVarPos argument in ft_ty_app
(see Note [FFoldType and functorLikeTraverse]) to distinguish between these
two cases. This is very similar to a trick that derived fmap implementations
...
-- | Used in the 'ft_ty_app' case of 'FFoldType' to represent whether the last
-- type argument appears directly or not. This information is used by
-- @DeriveFunctor@ to generate more efficient code for derived implementations
-- of 'fmap' (see @Note [Avoid unnecessary eta expansion in derived fmap implementations]@)
-- and ('<$') (see @Note [Deriving <$]@).
data TyVarPos
  = Immediate
    -- ^ The last type argument occurs directly
    --   (e.g., @Maybe a@).
  | Nested
    -- ^ The last type argument occurs within some other type
    --   (e.g., @Maybe [a]@).

instance Outputable TyVarPos where
  ppr tvp = text $ case tvp of
                     Immediate -> "Immediate"
                     Nested    -> "Nested"
...
functorLikeTraverse var (FT { ft_triv = caseTrivial,     ft_var = caseVar
  ...
  where
    ...
    go co (AppTy    x y) | xc = (caseWrongArg,   True)
                         | yc = (caseTyApp x (ty_var_pos y) yr, True)
        where (_, xc) = go co x
              (yr,yc) = go co y
    go co ty@(TyConApp con args)
       ...
       | Just (fun_ty, arg_ty) <- splitAppTy_maybe ty    -- T (..no var..) ty
                          = (caseTyApp fun_ty (ty_var_pos arg_ty) (last xrs), True)
    ...
    -- Determines whether the last argument in an application of some type is
    -- `var` directly (i.e., Immediate) or nested under some other type
    -- (i.e., Nested).
    -- Invariant: `ty` contains `var`.
    ty_var_pos :: Type -> TyVarPos
    ty_var_pos ty = case tcGetTyVar_maybe ty of
                      Just v  -> ASSERT( v == var )
                                 Immediate
                      Nothing -> Nested
...
* ft_ty_app:  A type is being applied to the last type parameter, where the
              ...
              The TyVarPos argument to ft_ty_app encodes whether the last type
              parameter occurs directly as the last argument (e.g., Maybe a) or
              if it is nested under another type (e.g., Maybe [a]).
              If it is the former, the Immediate constructor of TyVarPos is used.
              If it is the latter, the Nested constructor of TyVarPos is used.
              Currently, this information is only used in DeriveFunctor to
              generate more efficient implementations of
              fmap (see Note [Avoid unnecessary eta expansion in derived fmap implementations]) and
              (<$) (see Note [Deriving <$]).

              Note that functions, tuples, and foralls are distinct cases
              and take precedence over ft_ty_app. (For example, (Int -> a) would
              fall under (ft_fun Int a), not (ft_ty_app ((->) Int) Immediate a).

              Examples: Maybe a, Either b a               (Immediate)
                        Maybe [a], Either b (Either c a)  (Nested)
