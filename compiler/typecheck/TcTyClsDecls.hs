...
kcAddDeclCtxt decl thing_inside
  ...
  where
    ...
    has_vdq tycon = ASSERT( isTcTyCon tycon )
                    ASSERT( not (tcTyConIsPoly tycon) )
                    ASSERT( not (isForAllTy tc_kind) )
                    any is_vdq_tcb (tyConBinders tycon)
      where tc_kind        = tyConKind tycon
