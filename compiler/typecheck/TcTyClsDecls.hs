  ...
  = -- See Note [Type-checking default assoc decls]
    ...
    do { traceTc "tcDefaultAssocDecl 1" (ppr tc_name)
       ...
       ; (qtvs, pats, rhs_ty) <- tcTyFamInstEqnGuts fam_tc NotAssociated
         ...
         -- See Note [Type-checking default assoc decls]
       ; let -- Checks that a pattern on the LHS of a default is a type
             -- variable. If so, return the underlying type variable, and if
             -- not, throw an error.
             extract_tv :: Type -> TcM TyVar
             extract_tv pat =
               case getTyVar_maybe pat of
                 ...
                 Nothing ->
                   ...
                   hang (text "Illegal argument" <+> quotes (ppr pat)
                         <+> text "in:")
                      2 (vcat [ quotes (text "type"
       ...
       ; pat_tvs <- traverse extract_tv pats
