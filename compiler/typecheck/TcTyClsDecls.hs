...
kcConDecls new_or_data res_kind cons
  ...
  where
    ...
    -- or  newtype instance N a :: Type -> Type  where ..
    -- in which case: res_kind = Type->Type
    -- We want to look past those arrows to the Type
    -- in the corner, to pass to kcConDecl c.f. #16828
...
kcConDecl :: NewOrData
          -> Kind  -- Result kind of the type constructor
                   -- Usually Type but can be TYPE UnliftedRpe
