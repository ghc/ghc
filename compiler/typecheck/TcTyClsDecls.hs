...
tcTyClGroup (TyClGroup { group_tyclds = tyclds
  ...
  = do { let role_annots = mkRoleAnnotEnv roles
       ...
       ; tlks_env <-
           ...
           mkNameEnv <$> traverse tcTLKS tlkss
  ...
  = do { let flav = newOrDataToFlavour new_or_data
       ; tc <- kcDeclHeader name flav ki ktvs $
               case m_sig of
                  ...
                  Nothing   -> Just $ dataDeclDefaultResultKind new_or_data
       ; return [tc] }
...
checkInitialKind ki
  ...
  = do  { tycon <- kcDeclHeader name TypeSynonymFlavour ki ktvs $
                   case kind_annotation rhs of
                     Just ksig -> Just (tcLHsKindSig (TySynKindCtxt name) ksig)
                     Nothing -> Nothing
        ; return [tycon] }
  where
...
checkFamDeclInitialKind ki mb_parent_tycon
    (FamilyDecl { fdLName     = unLoc -> name
                ...
                , fdInfo      = info })
  = do { tc <- kcDeclHeader name flav ki ktvs $
...
checkFamDeclInitialKind _ _ (XFamilyDecl nec) = noExtCon nec
...
checkAssocFamDeclInitialKinds cls decls =
    mapM (addLocM checkAssocFamDeclInitialKind)
         (map (mapLoc (filterOutClassHeaderKiVars cls)) decls)
  where
    ...
    checkAssocFamDeclInitialKind (XFamilyDecl nec) = noExtCon nec

filterOutClassHeaderKiVars
  :: TyCon  -- parent class
  -> FamilyDecl GhcRn
  -> FamilyDecl GhcRn
filterOutClassHeaderKiVars cls famDecl@FamilyDecl{fdTyVars=ktvs} =
    famDecl{ fdTyVars = filterOutLHsQTyVars ktvs }
  where
    isHeaderKiVar n = n `elem` map fst (tcTyConHeaderKiVars cls)
    filterOutLHsQTyVars HsQTvs{ hsq_ext = i, hsq_explicit = e } =
      HsQTvs{ hsq_ext = filterOut isHeaderKiVar i, hsq_explicit = e }
    filterOutLHsQTyVars (XLHsQTyVars nec) = noExtCon nec
filterOutClassHeaderKiVars _ (XFamilyDecl nec) = noExtCon nec
