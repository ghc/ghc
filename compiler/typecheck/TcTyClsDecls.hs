...
This looks like it should work. However, it is polymorphic recursive,
...
The context is added in kcAddDeclCtxt, which looks for a visible TyConBinder
...
TcHsType defeats that approach.) kcAddDeclCtxt is used in kcTyClDecl,
...
kcAddDeclCtxt :: TyClDecl GhcRn -> (TcTyCon -> TcM a) -> TcM a
-- See Note [Inferring visible dependent quantification]
kcAddDeclCtxt decl thing_inside
  = do { tycon <- kcLookupTcTyCon name
       ; pprTraceM "RAE1" (ppr tycon $$ ppr (tyConKind tycon))
       ; if has_vdq tycon
         then addLandmarkErrCtxt vdq_warning $
              do_thing tycon
         else do_thing tycon }
  where
    name = tcdName decl

    do_thing tycon = addErrCtxt (tcMkDeclCtxt decl) $
                     thing_inside tycon

    has_vdq tycon = any is_vdq_tcb (tyConBinders tycon)
      where kind_fvs = tyCoVarsOfType (tyConKind tycon)
            is_vdq_tcb tcb = (binderVar tcb `elemVarSet` kind_fvs) &&
                             isVisibleTyConBinder tcb

    vdq_warning = vcat
      [ text "NB: Type" <+> quotes (ppr name) <+>
        text "was inferred to use visible dependent quantification."
      , text "Most types with visible dependent quantification are"
      , text "polymorphic recursive and need a standalone kind"
