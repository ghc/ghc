...
zonkExpr env (XExpr (Left (HsWrap co_fn expr)))
  = do (env1, new_co_fn) <- zonkCoFn env co_fn
       new_expr <- zonkExpr env1 expr
       return (XExpr (Left (HsWrap new_co_fn new_expr)))

zonkExpr env (XExpr (Right (HsExpanded a b)))
  = XExpr . Right . HsExpanded a <$> zonkLExpr env b
