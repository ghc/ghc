...
{-# LANGUAGE CPP, GADTs, TypeApplications, DerivingStrategies, DerivingVia,
             ScopedTypeVariables #-}
...
*ordered*, and we define a Semigroup instance that prefers the maximal element,
making a nice use of DerivingVia. We then define VarUsage, which is just a pair
...
-- Wrapper used in a `via` clause
newtype Max a = Max a
instance Ord a => Semigroup (Max a) where
  Max x <> Max y = Max (max x y)
  -- should really be (<>) = coerce (max @a)
  -- but that causes a warning due to #10347
...
data Visibility
  ...
  deriving       (Semigroup) via Max Visibility
...
data NeedsUndecInstFlag
  ...
  deriving stock (Eq, Ord)
  deriving       (Semigroup) via Max NeedsUndecInstFlag

-- See Note [Coverage condition for injective type families]
data VarUsage
  = VarUsage Visibility NeedsUndecInstFlag
instance Semigroup VarUsage where
  VarUsage v1 f1 <> VarUsage v2 f2 = VarUsage (v1 S.<> v2) (f1 S.<> f2)
