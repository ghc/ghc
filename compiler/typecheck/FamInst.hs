...
{-# LANGUAGE CPP, GADTs, TypeApplications, GeneralizedNewtypeDeriving #-}
...
module FamInst (
        ...
        -- * Injectivity
        makeInjectivityErrors, injTyVarsOfTypes
    ) where
  ...
  = ASSERT2( any id inj, text "No injective type variables" )
    let lhs             = coAxBranchLHS axiom
        ...
        (unused_inj_tvs, unused_vis, undec_inst_flag)
                        = unusedInjTvsInRHS dflags fam_tc inj lhs rhs
...
terminate. This is precisely the same problme that we have with functional
...
-- Is a variable usage visible or not?
data Visibility
  = Visible
  | Invisible
...
argFlagToVisibility :: ArgFlag -> Visibility
argFlagToVisibility f | isVisibleArgFlag f = Visible
                      | otherwise          = Invisible

-- Does a variable usage not exist? Does it need -XUndecidableInstances in order
-- to prove injectivity?
-- See Note [Coverage condition for injective type families]
data NeedsUndecInstFlag
  = NotPresent
  | NeedsUndecInst
  | Doesn'tNeedUndecInst
  deriving (Eq, Ord, Show)

instance Outputable NeedsUndecInstFlag where
  ppr = text . show

-- See Note [Coverage condition for injective type families]
data VarUsage
  = VarUsage Visibility NeedsUndecInstFlag
  deriving Show

instance Semigroup VarUsage where
  VarUsage v1 f1 <> VarUsage v2 f2 = VarUsage (max v1 v2) (max f1 f2)

instance Outputable VarUsage where
  ppr = text . show

-- See Note [Coverage condition for injective type families]
newtype VarUsages = VU (VarEnv VarUsage)
  deriving Outputable  -- GeneralisedNewtypeDeriving

instance Semigroup VarUsages where
  (<>) = coerce (plusVarEnv_C @VarUsage (S.<>))
instance Monoid VarUsages where
  mempty = coerce (emptyVarEnv @VarUsage)

-- Build a VarUsages for one variable
singletonUsage :: TyVar -> VarUsage -> VarUsages
singletonUsage tv usage = coerce (unitVarEnv tv usage)

-- | Return a list of type variables that the function is declared to be
-- injective in but that do not appear on injective positions in the RHS
-- of a family instance declaration. Also returned are whether any such
-- variable was used invisibly and whether any such variable would be
-- considered injective if -XUndecidableInstances were on.
unusedInjTvsInRHS :: DynFlags
                  -> TyCon  -- type family
                  -> [Bool] -- which arguments are injective, in 1-to-1 with ...
                  -> [Type] -- ... these arguments, the LHS
                  -> Type   -- the RHS
                  -> (TyVarSet, Visibility, NeedsUndecInstFlag)
...
unusedInjTvsInRHS dflags tycon injList lhs rhs =
  -- Note [Coverage condition for injective type families], step 5
  (mkVarSet bad_tvs, maximum bad_viss, maximum bad_fs)
    where
      ...
      all_invis_vars = invis_vars `unionVarSet` invis_vars'

      set_to_usages :: Visibility -> VarSet -> VarUsages
      set_to_usages vis set = coerce $ mapVarEnv (const (VarUsage vis NotPresent))
                                                 (getUniqSet set)

      -- Note [Coverage condition for injective type families], step 1
      lhs_usages = set_to_usages Invisible all_invis_vars S.<>
                   set_to_usages Visible   vis_vars

      -- Note [Coverage condition for injective type families], step 2
      rhs_usages = injTyVarUsages rhs

      -- Note [Coverage condition for injective type families], step 3
      VU usages = lhs_usages S.<> rhs_usages

      all_lhs_vars = all_invis_vars `unionVarSet` vis_vars

      -- Note [Coverage condition for injective type families], step 4
      (bad_tvs, bad_viss, bad_fs)
        = unzip3 [ (tv, vis, f)
                 | (tv, VarUsage vis f) <- nonDetEltsUFM $
                     intersectUFM_C (,) (getUniqSet all_lhs_vars) usages
                 , not (is_allowed f) ]
          -- non-determinism is OK here, because any order dependence
          -- is erased before this function returns (by mkVarSet and maximum)

      is_allowed NotPresent           = False
      is_allowed NeedsUndecInst       = xopt LangExt.UndecidableInstances dflags
      is_allowed Doesn'tNeedUndecInst = True

injTyVarUsages :: TcTauType -> VarUsages
-- Collect 'VarUsages' for all the free variables in the given
-- type that are used in an injective position.
-- Determining the overall type determines these variables,
-- possibly requiring -XUndecidableInstances. See
-- Note [Coverage condition for injective type families]
--
-- Postcondition: no usage is NotPresent
injTyVarUsages orig_ty = go Doesn'tNeedUndecInst Visible orig_ty
  where
    go f v ty | Just ty' <- tcView ty = go f v ty' -- #12430
    go f v (TyVarTy tv)      = singletonUsage tv (VarUsage v f) S.<>
                               go f Invisible (tyVarKind tv)
    go f v (AppTy ty1 ty2)   = go_app f v ty1 [ty2]
    go f v (TyConApp tc tys)
      | isTypeFamilyTyCon tc
      = case tyConInjectivityInfo tc of
          NotInjective  -> mempty
          Injective inj -> uncurry (go_args NeedsUndecInst v)
                                   (unzip $ filterByList inj (arg_flags `zip` tys))
      | otherwise
      = go_args f v arg_flags tys
      where
        arg_flags = tyConArgFlags tc tys

    go _ _ (ForAllTy {}) = pprPanic "injTyVarUsages" (ppr orig_ty)
      -- No forall types in the RHS of a type family

    go f v (FunTy _ arg res) = go f v arg S.<> go f v res
    go _ _ (LitTy {})        = mempty
    go f v (CastTy ty _)     = go f v ty
    go _ _ (CoercionTy {})   = mempty

    go_app f v (AppTy fun arg) args = go_app f v fun (arg:args)
    go_app f v hd args = go f v hd S.<> go_args f v (appTyArgFlags hd args) args

    go_args f v arg_flags tys = mconcat $ zipWith (go f) visibilities tys
      where
        visibilities | Invisible <- v = repeat Invisible
                     | otherwise      = map argFlagToVisibility arg_flags

-- Assuming -XUndecidableInstances, what variables are in injective positions
-- in a type?
injTyVarsOfTypes :: [TcType] -> TcTyVarSet
injTyVarsOfTypes tys = restrictUniqSetToUFM all_tvs usages
  where
    all_tvs   = tyCoVarsOfTypes tys
    VU usages = foldMap injTyVarUsages tys
...
unusedInjectiveVarsErr :: TyVarSet -> Visibility -> NeedsUndecInstFlag
                       -> InjErrorBuilder -> CoAxBranch
                       -> (SDoc, SrcSpan)
unusedInjectiveVarsErr tvs vis undec_inst errorBuilder tyfamEqn
  = let (doc, loc) = errorBuilder (injectivityErrorHerald True $$ msg)
                                  [tyfamEqn]
    in (pprWithExplicitKindsWhen has_kinds doc, loc)
    where
      has_kinds = vis == Invisible
      ...
      extra | NeedsUndecInst <- undec_inst = text "Using UndecidableInstances might help"
            | otherwise                    = empty
