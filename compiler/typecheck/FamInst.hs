...
unusedInjTvsInRHS dflags tycon injList lhs rhs =
  ...
  (mkVarSet bad_tvs, maximum bad_viss, maximum bad_fs)
    where
      ...
      all_lhs_vars = nonDetEltsUniqSet $ all_invis_vars `unionVarSet` vis_vars
        -- non-determinism is OK because we return only the result of
        -- commutative operations on these results (that is, mkVarSet and maximum)

      -- Note [Coverage condition for injective type families], step 4
      (bad_tvs, bad_viss, bad_fs)
        = unzip3 [ (tv, vis, f)
                 | tv <- all_lhs_vars
                 , Just (VarUsage vis f) <- return (lookupVarEnv usages tv)
                 , not (is_allowed f) ]
