...
reportWanteds ctxt tc_lvl (WC { wc_simple = simples, wc_impl = implics })
 ...
 where
    ...
    utterly_wrong _ (EqPred{ep_rel = NomEq, ep_ty1 = ty1, ep_ty2 = ty2})
                      = isRigidTy ty1 && isRigidTy ty2
    utterly_wrong _ _ = False

    -- Things like (a ~N Int)
    very_wrong _ (EqPred{ep_rel = NomEq, ep_ty1 = ty1, ep_ty2 = ty2})
                   = isSkolemTy tc_lvl ty1 && isRigidTy ty2
    very_wrong _ _ = False

    -- Things like (a ~N b) or (a  ~N  F Bool)
    skolem_eq _ (EqPred{ep_rel = NomEq, ep_ty1 = ty1})
                  = isSkolemTy tc_lvl ty1
    skolem_eq _ _ = False

    -- Things like (F a  ~N  Int)
    non_tv_eq _ (EqPred{ep_rel = NomEq, ep_ty1 = ty1})
                  = not (isTyVarTy ty1)
    non_tv_eq _ _ = False
    ...
    is_homo_equality _ (EqPred{ep_ki1 = ki1, ep_ki2 = ki2})
                         = ki1 `tcEqType` ki2
...
eq_lhs_type ct1 ct2
  = case (classifyPredType (ctPred ct1), classifyPredType (ctPred ct2)) of
       (EqPred{ep_rel = eq_rel1, ep_ty1 = ty1}, EqPred{ep_rel = eq_rel2, ep_ty1 = ty2}) ->
