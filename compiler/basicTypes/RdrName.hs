...
pickBothGRE mod gre@(GRE { gre_name = n })
  -- isBuiltInSyntax filters out names for built-in syntax. They just clutter
  -- up the environment (esp tuples), and the parser will generate Exact RdrNames
  -- for them, so the cluttered envt is no use.  Really, it's only useful for
