...
GHC supports 2 kinds of arbitrary precision integers (a.k.a BigNum):
   ...
   * Integer: integer represented a an Int# or as a BitNat (Integer's
   constructors indicate the sign)

BigNum literal instances are removed from Core during the CorePrep phase
(although TidyPgm looks ahead at what the core will be, so that it can see
whether it involves CAFs). They are replaced with expression to build them at
runtime from machine literals (Word#,Int#,etc.) or from a list of Words.
...
wrapLitNumber dflags v@(LitNumber nt i) = case nt of
...
mkLitNumberWrap :: DynFlags -> LitNumType -> Integer -> Literal
mkLitNumberWrap dflags nt i = wrapLitNumber dflags (LitNumber nt i)
...
-- | Create a numeric 'Literal' of the given type
mkLitNumber :: DynFlags -> LitNumType -> Integer -> Literal
mkLitNumber dflags nt i =
...
mkLitIntWrapC dflags i = (n, i /= i')
  where
    n@(LitNumber _ i') = mkLitIntWrap dflags i
...
mkLitWordWrapC dflags i = (n, i /= i')
  where
    n@(LitNumber _ i') = mkLitWordWrap dflags i
...
mapLitValue dflags f (LitNumber nt i) = wrapLitNumber dflags (LitNumber nt (f i))
...
word2IntLit dflags (LitNumber LitNumWord w)
...
int2WordLit dflags (LitNumber LitNumInt i)
...
litIsDupable dflags (LitNumber nt i) = case nt of
  LitNumInteger -> inIntRange dflags i
  LitNumNatural -> inWordRange dflags i
  LitNumInt     -> True
  LitNumInt64   -> True
  LitNumWord    -> True
  LitNumWord64  -> True
