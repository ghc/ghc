...
#define UNIQ_RESULT(x, y) (# x, y #)

#else

data UniqResult result = UniqResult !result {-# UNPACK #-} !UniqSupply
#define UNIQ_RESULT(x, y) (UniqResult (x) (y))
...
instance Functor UniqSM where
    fmap f (USM x) = USM (\us0 -> case x us0 of
                                 UNIQ_RESULT(r, us1) -> UNIQ_RESULT(f r, us1))

instance Applicative UniqSM where
    pure = returnUs
    (USM f) <*> (USM x) = USM $ \us0 -> case f us0 of
                            UNIQ_RESULT(ff, us1) -> case x us1 of
                              UNIQ_RESULT(xx, us2) -> UNIQ_RESULT(ff xx, us2)
    (*>) = thenUs_
...
initUs init_us m = case unUSM m init_us of { UNIQ_RESULT(r, us) -> (r, us) }

-- | Run the 'UniqSM' action, discarding the final 'UniqSupply'
initUs_ :: UniqSupply -> UniqSM a -> a
initUs_ init_us m = case unUSM m init_us of { UNIQ_RESULT(r, _) -> r }
...
liftUSM (USM m) us0 = case m us0 of UNIQ_RESULT(a, us1) -> (a, us1)

instance MonadFix UniqSM where
    mfix m = USM (\us0 -> let (r,us1) = liftUSM (m r) us0 in UNIQ_RESULT(r, us1))

thenUs :: UniqSM a -> (a -> UniqSM b) -> UniqSM b
thenUs (USM expr) cont
  = USM (\us0 -> case (expr us0) of
                   UNIQ_RESULT(result, us1) -> unUSM (cont result) us1)
...
thenUs_ (USM expr) (USM cont)
  = USM (\us0 -> case (expr us0) of { UNIQ_RESULT(_, us1) -> cont us1 })

returnUs :: a -> UniqSM a
returnUs result = USM (\us -> UNIQ_RESULT(result, us))

getUs :: UniqSM UniqSupply
getUs = USM (\us0 -> case splitUniqSupply us0 of (us1,us2) -> UNIQ_RESULT(us1, us2))
...
getUniqueUs = USM (\us0 -> case takeUniqFromSupply us0 of
                           (u,us1) -> UNIQ_RESULT(u, us1))

getUniquesUs :: UniqSM [Unique]
getUniquesUs = USM (\us0 -> case splitUniqSupply us0 of
                            (us1,us2) -> UNIQ_RESULT(uniqsFromSupply us1, us2))
