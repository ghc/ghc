...
-- N.B. Don't forget that the new unique must have the same tag as the old
-- unique.
uniqAway' (InScope set) var
  = case IntMap.lookupLT (getKey maxBound) (ufmToIntMap $ getUniqSet set) of
      Just (uniq,_) -> setVarUnique var (deriveUnique (getUnique uniq) 1)
      Nothing -> panic "uniqAway': this cannot happen"
  where
    (tag, _) = unpkUnique (getUnique var)
    maxBound = mkUnique (succ tag) 0
