...
uniqAway' in_scope var
  = setVarUnique var (unsafeGetFreshUnique in_scope)
...
-- introduce non-unique 'Unique's this way.
unsafeGetFreshUnique :: InScopeSet -> Unique
unsafeGetFreshUnique (InScope set)
  | Just (uniq,_) <- IntMap.lookupLT (getKey maxDerivedUnique) (ufmToIntMap $ getUniqSet set)
  , let uniq' = mkDerivedUnique uniq
  , not $ uniq' `ltUnique` minDerivedUnique
  = incrUnique uniq'

  | otherwise
  = minDerivedUnique
  where
    minDerivedUnique = mkDerivedUnique 0
