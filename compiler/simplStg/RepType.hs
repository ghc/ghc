...
One other small difference: PrimRep contains VoidRep, while RuntimeRep does
not. RuntimeRep instead uses TupleRep '[] to denote a void argument. When
converting a TupleRep '[] into a list of PrimReps, we get an empty list.
After the unariser, all identifiers have exactly one PrimRep, but void arguments
still exist. Thus, PrimRep includes VoidRep to describe these binders.
Perhaps post-unariser representations (which need VoidRep) should be a
different type than pre-unariser representations (which use a list and
do not need VoidRep), but we have what we have.
