...
-- The module interacts with the iserv interpreter, whether it is running
-- on an external process or in the current process.
--
module GHCi
...
handleSeqHValueStatus hsc_env eval_status = do
  case eval_status of
    (EvalBreak is_exception _ ix mod_uniq resume_ctxt _) -> do
      ...
      -- resume the seq (:force) processing in the iseq process
