...
typedoc :: { LHsType GhcPs }
        ...
        | docnext btype '->' ctypedoc    {% ams $2 [mu AnnRarrow $3] -- See note [GADT decl discards annotations]
                                         >> ams (sLL $1 $> $
                                                ...
                                                [mu AnnRarrow $3] }
-- See Note [Constr variatons of non-terminals]
constr_btype :: { forall b. DisambTD b => PV (Located b) }
       : constr_ftype { superInfixTyHead resultOfPV $
                        $1 >>= \ $1 ->
                        mkInfixTyHeadPV $1 }
       | constr_ftype tyop maybe_docprev constr_btype
                      { superInfixTyArg resultOfPV $
                        $1 >>= \ $1 ->
                        $4 >>= \ $4 ->
...
constr :: { LConDecl GhcPs }
        : maybe_docnext forall constr_context '=>' constr_btype
              {% runPV $5 >>= \ $5 ->
                 let ConstrStuff con details doc_prev = unLoc $5 in
        ...
        | maybe_docnext forall constr_btype
              {% runPV $3 >>= \ $3 ->
                 let ConstrStuff con details doc_prev = unLoc $3 in
