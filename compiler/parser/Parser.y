...
rule    :: { LRuleDecl GhcPs }
        : STRING rule_activation rule_foralls infixexp '=' exp
         {% ecExp $4 >>= \e4 ->
            ecExp $6 >>= \e6 ->
            ams (sLL $1 e6 $ HsRule { rd_ext = noExt
...
rhs     :: { Located ([AddAnn],GRHSs GhcPs (LHsExpr GhcPs)) }
        : '=' exp wherebinds    {% ecExp $2 >>= \e2 ->
                                  return $ sL (comb3 $1 e2 $3)
                                    ((mj AnnEqual $1 : (fst $ unLoc $3))
                                    ,GRHSs noExt (unguardedRHS (comb3 $1 e2 $3) e2)
                                   (snd $ unLoc $3)) }
...
exp   :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : infixexp '::' sigtype {% ecExp $1 >>= \e1 ->
                                   fmap ecFromExp $
                                   ams (sLL e1 $> $ ExprWithTySig noExt e1 (mkLHsSigWcType $3))
                                       [mu AnnDcolon $2] }
        | infixexp '-<' exp     {% ecExp $1 >>= \e1 ->
                                   ecExp $3 >>= \e3 ->
                                   fmap ecFromCmd $
                                   ams (sLL e1 e3 $ HsCmdArrApp noExt e1 e3
                                                        HsFirstOrderApp True)
                                       [mu Annlarrowtail $2] }
        | infixexp '>-' exp     {% ecExp $1 >>= \e1 ->
                                   ecExp $3 >>= \e3 ->
                                   fmap ecFromCmd $
                                   ams (sLL e1 e3 $ HsCmdArrApp noExt e3 e1
...
infixexp :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : exp10 { $1 }
        | infixexp qop exp10  {
            let anns = [mj AnnVal $2] in
                 -- AnnVal annotation for NPlusKPat, which discards the operator
            EC { ecExp = do
                   e1 <- ecExp $1
                   e3 <- ecExp $3
                   ams (sLL e1 e3 $ OpApp noExt e1 $2 e3) anns
               , ecCmd = do
                   let cmdArg c = cL (getLoc c) $ HsCmdTop noExt c
                   c1 <- ecCmd $1
                   c3 <- ecCmd $3
                   ams (sLL c1 c3 $ HsCmdArrForm noExt $2 Infix Nothing
                                    [cmdArg c1, cmdArg c3]) anns
               }
          }
...
infixexp_top :: { LHsExpr GhcPs }
            ...
            | infixexp_top qop exp10_top
                                      {% do { e3 <- ecExp $3
                                            ; when (srcSpanEnd (getLoc $2)
                                                == srcSpanStart (getLoc e3)
                                                && checkIfBang $2) $
                                                warnSpaceAfterBang (comb2 $2 e3)
                                            ; ams (sLL $1 e3 (OpApp noExt $1 $2 e3))
                                                     [mj AnnVal $2]
                                            }
                                      }

exp10_top :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : '-' fexp                      {% ecExp $2 >>= \e2 ->
                                           fmap ecFromExp $
                                           ams (sLL $1 e2 $ NegApp noExt e2 noSyntaxExpr)
                                               [mj AnnMinus $1] }


        | hpc_annot exp        {% ecExp $2 >>= \e2 ->
                                  fmap ecFromExp $
                                  ams (sLL $1 e2 $ HsTickPragma noExt (snd $ fst $ fst $ unLoc $1)
                                                                (snd $ fst $ unLoc $1) (snd $ unLoc $1) e2)
                                      (fst $ fst $ fst $ unLoc $1) }

        | '{-# CORE' STRING '#-}' exp  {% ecExp $4 >>= \e4 ->
                                          fmap ecFromExp $
                                          ams (sLL $1 e4 $
...
exp10 :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : exp10_top            { $1 }
        | scc_annot exp        {% ecExp $2 >>= \e2 ->
                                  fmap ecFromExp $
                                  ams (sLL $1 e2 $
...
fexp    :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : fexp aexp {% do
            e2 <- ecExp $2
            checkBlockArguments e2
            return EC
              { ecExp = do
                  e1 <- ecExp $1
                  checkBlockArguments e1
                  return (sLL e1 e2 (HsApp noExt e1 e2))
              , ecCmd = do
                  c1 <- ecCmd $1
                  -- TODO (int-index): checkBlockArguments c1
                  return (sLL c1 e2 (HsCmdApp noExt c1 e2))
              }
          }
        | fexp TYPEAPP atype         {% ecExp $1 >>= \e1 ->
                                        checkBlockArguments e1 >>= \_ ->
                                        fmap ecFromExp $
                                        ams (sLL e1 $> $ HsAppType noExt e1 (mkHsWildCardBndrs $3))
                                            [mj AnnAt $2] }
        | 'static' aexp              {% ecExp $2 >>= \e2 ->
                                        fmap ecFromExp $
                                        ams (sLL $1 e2 $ HsStatic noExt e2)
                                            [mj AnnStatic $1] }
        | aexp                       { $1 }

aexp    :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : qvar '@' aexp         {% ecExp $3 >>= \e3 ->
                                   fmap ecFromExp $
                                   ams (sLL $1 e3 $ EAsPat noExt $1 e3) [mj AnnAt $2] }
        ...
        | 'let' binds 'in' exp {
            ...
            EC
              { ecExp = do
                  e <- ecExp $4
                  ams (sLL $1 e $ HsLet noExt lb e) anns
              , ecCmd = do
                  c <- ecCmd $4
                  ams (sLL $1 c $ HsCmdLet noExt lb c) anns
              }
          }
        | '\\' 'lcase' altslist
            {% ecExp $3 >>= \alts ->
        ...
        | 'if' exp optSemi 'then' exp optSemi 'else' exp {% do
            ...
            return EC
              { ecExp = do
                  ...
                  ams (sLL $1 e8 $ mkHsIf e2 e5 e8) anns
              , ecCmd = do
                  c5 <- ecCmd $5
                  c8 <- ecCmd $8
                  -- TODO (int-index): checkDoAndIfThenElse
                  ams (sLL $1 c8 $ mkHsCmdIf e2 c5 c8) anns
              }
          }
        ...
        | 'proc' aexp '->' exp
                       {% (checkPattern empty <=< ecExp) $2 >>= \ p ->
                           ...
                           fmap ecFromExp $
                           ams (sLL $1 cmd $
                                  HsProc noExt p (sLL $1 cmd $ HsCmdTop noExt cmd))
                                            -- TODO: is LL right here?
                               [mj AnnProc $1,mu AnnRarrow $3] }

        | aexp1                 { $1 }

aexp1   :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : aexp1 '{' fbinds '}' {% do { e1 <- ecExp $1
                                     ; r <- mkRecConstrOrUpdate e1 (comb2 $2 $4)
                                                                   (snd $3)
                                     ; _ <- amsL (comb2 e1 $>) (moc $2:mcc $4:(fst $3))
                                     ; fmap ecFromExp $
                                       checkRecordSyntax (sLL e1 $> r) }}
        | aexp2                { $1 }

aexp2   :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
...
--                                       (getSTRING $1) noExt) }
        ...
        | '(' texp ')' {
            let anns = [mop $1,mcp $3] in
            EC { ecExp = do
                   e <- ecExp $2
                   ams (sLL $1 $> (HsPar noExt e)) anns
               , ecCmd = do
                   c <- ecCmd $2
                   ams (sLL $1 $> (HsCmdPar noExt c)) anns
               }
          }
        ...
        | '(#' texp '#)'                {% ecExp $2 >>= \e2 ->
                                           fmap ecFromExp $
                                           ams (sLL $1 $> (ExplicitTuple noExt [cL (gl e2)
...
texp :: { EC (LHsExpr GhcPs) (LHsCmd GhcPs) }
        : exp                           { $1 }
        ...
        -- inside parens.
        | infixexp qop        {% ecExp $1 >>= \e1 ->
                                 return $ ecFromExp $ sLL e1 $> $ SectionL noExt e1 $2 }
        | qopm infixexp       {% ecExp $2 >>= \e2 ->
                                 return $ ecFromExp $ sLL $1 e2 $ SectionR noExt $1 e2 }

       -- View patterns get parenthesized above
        | exp '->' texp   {% ecExp $1 >>= \e1 ->
                             ecExp $3 >>= \e3 ->
                             fmap ecFromExp $
                             ams (sLL e1 e3 $ EViewPat noExt e1 e3) [mu AnnRarrow $2] }

-- Always at least one comma or bar.
tup_exprs :: { ([AddAnn],SumOrTuple) }
           : texp commas_tup_tail
                          {% ecExp $1 >>= \e1 ->
                             do { addAnnotation (gl e1) AnnComma (fst $2)
                                ; return ([],Tuple ((sL1 e1 (Present noExt e1)) : snd $2)) } }

           | texp bars    {% ecExp $1 >>= \e1 ->
...
tup_tail :: { [LHsTupArg GhcPs] }
          : texp commas_tup_tail {% ecExp $1 >>= \e1 ->
                                    addAnnotation (gl e1) AnnComma (fst $2) >>
                                    return ((cL (gl e1) (Present noExt e1)) : snd $2) }
          | texp                 {% ecExp $1 >>= \e1 ->
                                    return [cL (gl e1) (Present noExt e1)] }
...
lexps :: { Located [LHsExpr GhcPs] }
        : lexps ',' texp          {% ecExp $3 >>= \e3 ->
                                     addAnnotation (gl $ head $ unLoc $1)
                                                            AnnComma (gl $2) >>
                                      return (sLL $1 e3 (((:) $! e3) $! unLoc $1)) }
        | texp ',' texp            {% ecExp $1 >>= \e1 ->
                                      ecExp $3 >>= \e3 ->
                                      addAnnotation (gl e1) AnnComma (gl $2) >>
                                      return (sLL e1 e3 [e3,e1]) }
...
transformqual :: { Located ([AddAnn],[LStmt GhcPs (LHsExpr GhcPs)] -> Stmt GhcPs (LHsExpr GhcPs)) }
    ...
    : 'then' exp              {% ecExp $2 >>= \e2 ->
                                 return $ sLL $1 e2 ([mj AnnThen $1],
                                                     \ss -> (mkTransformStmt ss e2)) }
    | 'then' exp 'by' exp     {% ecExp $2 >>= \e2 ->
                                 ecExp $4 >>= \e4 ->
                                 return $ sLL $1 e4 ([mj AnnThen $1,mj AnnBy  $3],
                                                     \ss -> (mkTransformByStmt ss e2 e4)) }
    | 'then' 'group' 'using' exp
            {% ecExp $4 >>= \e4 ->
               return $ sLL $1 e4 ([mj AnnThen $1,mj AnnGroup $2,mj AnnUsing $3],
                                   \ss -> (mkGroupUsingStmt ss e4)) }

    | 'then' 'group' 'by' exp 'using' exp
            {% ecExp $4 >>= \e4 ->
               ecExp $6 >>= \e6 ->
               return $ sLL $1 e6 ([mj AnnThen $1,mj AnnGroup $2,mj AnnBy $3,mj AnnUsing $5],
...
pat     :: { LPat GhcPs }
pat     :  exp          {% (checkPattern empty <=< ecExp) $1 }
        | '!' aexp      {% ecExp $2 >>= \e2 ->
                           amms (checkPattern empty (sLL $1 e2 (SectionR noExt
...
bindpat :  exp            {% ecExp $1 >>=
        ...
        | '!' aexp        {% ecExp $2 >>= \e2 ->
                             amms (checkPattern
                                     (text "Possibly caused by a missing 'do'?")
                                     (sLL $1 e2 (SectionR noExt (sL1 $1 (HsVar noExt (sL1 $1 bang_RDR))) e2)))
                                  [mj AnnBang $1] }

apat   :: { LPat GhcPs }
apat    : aexp                  {% (checkPattern empty <=< ecExp) $1 }
        | '!' aexp              {% ecExp $2 >>= \e2 ->
                                   amms (checkPattern empty
                                            (sLL $1 e2 (SectionR noExt
                                                (sL1 $1 (HsVar noExt (sL1 $1 bang_RDR))) e2)))
                                        [mj AnnBang $1] }
...
ec_qual  :: { EC (LStmt GhcPs (LHsExpr GhcPs)) (LStmt GhcPs (LHsCmd GhcPs)) }
    : bindpat '<-' exp {
        let { mkStmt :: Located (body GhcPs) -> LStmt GhcPs (Located (body GhcPs))
            ; mkStmt a = sLL $1 a (mkBindStmt $1 a)
            ; anns = [mu AnnLarrow $2] }
        in EC { ecExp = ecExp $3 >>= \e -> ams (mkStmt e) anns
              , ecCmd = ecCmd $3 >>= \c -> ams (mkStmt c) anns } }
    | exp { EC { ecExp = fmap mkLBodyStmt (ecExp $1)
               , ecCmd = fmap mkLBodyStmt (ecCmd $1) } }
    | 'let' binds {%
      do { let { stmt :: LStmt GhcPs (Located a)
               ; stmt = sLL $1 $> $ LetStmt noExt (snd $ unLoc $2) }
         ; _ <- ams stmt (mj AnnLet $1:(fst $ unLoc $2))
         ; return EC { ecExp = return stmt
                     , ecCmd = return stmt } } }
...
fbind   :: { LHsRecField GhcPs (LHsExpr GhcPs) }
        : qvar '=' texp {% ecExp $3 >>= \e3 ->
                           ams  (sLL $1 e3 $ HsRecField (sL1 $1 $ mkFieldOcc $1) e3 False)
                                [mj AnnEqual $2] }
...
dbind   :: { LIPBind GhcPs }
dbind   : ipvar '=' exp                {% ecExp $3 >>= \e3 ->
                                          ams (sLL $1 e3 (IPBind noExt (Left $1) e3))
                                              [mj AnnEqual $2] }
