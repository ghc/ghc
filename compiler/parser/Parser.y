...
rule    :: { LRuleDecl GhcPs }
        : STRING rule_activation rule_foralls infixexp '=' exp
         {%runPV (runExpCmdP $4) >>= \ $4 ->
           runPV (runExpCmdP $6) >>= \ $6 ->
...
annotation :: { LHsDecl GhcPs }
    : '{-# ANN' name_var aexp '#-}'      {% runPV (runExpCmdP $3) >>= \ $3 ->
                                            ams (sLL $1 $> (AnnD noExt $ HsAnnotation noExt
                                            (getANN_PRAGs $1)
                                            (ValueAnnProvenance $2) $3))
                                            [mo $1,mc $4] }

    | '{-# ANN' 'type' tycon aexp '#-}'  {% runPV (runExpCmdP $4) >>= \ $4 ->
                                            ams (sLL $1 $> (AnnD noExt $ HsAnnotation noExt
                                            (getANN_PRAGs $1)
                                            (TypeAnnProvenance $3) $4))
                                            [mo $1,mj AnnType $2,mc $5] }

    | '{-# ANN' 'module' aexp '#-}'      {% runPV (runExpCmdP $3) >>= \ $3 ->
                                            ams (sLL $1 $> (AnnD noExt $ HsAnnotation noExt
                                                (getANN_PRAGs $1)
...
rhs     :: { Located ([AddAnn],GRHSs GhcPs (LHsExpr GhcPs)) }
        : '=' exp wherebinds    {% runPV (runExpCmdP $2) >>= \ $2 -> return $
                                  sL (comb3 $1 $2 $3)
...
gdrh :: { LGRHS GhcPs (LHsExpr GhcPs) }
        : '|' guardquals '=' exp  {% runPV (runExpCmdP $4) >>= \ $4 ->
...
exp   :: { ExpCmdP }
        : infixexp '::' sigtype {% runPV (runExpCmdP $1) >>= \ $1 ->
                                   fmap ecFromExp $
                                   ams (sLL $1 $> $ ExprWithTySig noExt $1 (mkLHsSigWcType $3))
                                       [mu AnnDcolon $2] }
        | infixexp '-<' exp     {% runPV (runExpCmdP $1) >>= \ $1 ->
                                   runPV (runExpCmdP $3) >>= \ $3 ->
                                   fmap ecFromCmd $
...
exp10 :: { ExpCmdP }
        : exp10_top            { $1 }
        | scc_annot exp        {% runPV (runExpCmdP $2) >>= \ $2 ->
                                  fmap ecFromExp $
...
aexp    :: { ExpCmdP }
        : qvar '@' aexp         {% runPV (runExpCmdP $3) >>= \ $3 ->
                                   fmap ecFromExp $
        ...
        | 'case' exp 'of' altslist    {% runPV (runExpCmdP $2) >>= \ $2 ->
                                         return $ ExpCmdP $
                                           $4 >>= \ $4 ->
                                           ams (cL (comb3 $1 $3 $4) $
...
aexp1   :: { ExpCmdP }
        : aexp1 '{' fbinds '}' {% runPV (runExpCmdP $1) >>= \ $1 ->
...
--                                       (getSTRING $1) noExt) }
        ...
        | '(|' aexp2 cmdargs '|)'  {% runPV (runExpCmdP $2) >>= \ $2 ->
                                      fmap ecFromCmd $
...
texp :: { ExpCmdP }
        ...
        -- inside parens.
        | infixexp qop       {% runPV (runExpCmdP $1) >>= \ $1 ->
                                return $ ecFromExp $
                                sLL $1 $> $ SectionL noExt $1 $2 }
        | qopm infixexp      {% runPV (runExpCmdP $2) >>= \ $2 ->
                                return $ ecFromExp $
                                sLL $1 $> $ SectionR noExt $1 $2 }

       -- View patterns get parenthesized above
        | exp '->' texp   {% runPV (runExpCmdP $1) >>= \ $1 ->
                             runPV (runExpCmdP $3) >>= \ $3 ->
                             fmap ecFromExp $
...
tup_exprs :: { ([AddAnn],SumOrTuple) }
           : texp commas_tup_tail
                          {% runPV (runExpCmdP $1) >>= \ $1 ->
                             do { addAnnotation (gl $1) AnnComma (fst $2)
                                ; return ([],Tuple ((sL1 $1 (Present noExt $1)) : snd $2)) } }

           | texp bars   {% runPV (runExpCmdP $1) >>= \ $1 -> return $
...
list :: { ([AddAnn],HsExpr GhcPs) }
        ...
        | texp '..' exp         {% runPV (runExpCmdP $1) >>= \ $1 ->
                                  ...
                                  return ([mj AnnDotdot $2],
                                   ArithSeq noExt Nothing
                                                               (FromTo $1 $3)) }
        | texp ',' exp '..' exp {% runPV (runExpCmdP $1) >>= \ $1 ->
                                   runPV (runExpCmdP $3) >>= \ $3 ->
                                   runPV (runExpCmdP $5) >>= \ $5 ->
                                  return ([mj AnnComma $2,mj AnnDotdot $4],
                                    ArithSeq noExt Nothing
                                                (FromThenTo $1 $3 $5)) }
...
lexps :: { Located [LHsExpr GhcPs] }
        : lexps ',' texp          {% runPV (runExpCmdP $3) >>= \ $3 ->
                                     addAnnotation (gl $ head $ unLoc $1)
                                                            AnnComma (gl $2) >>
                                      return (sLL $1 $> (((:) $! $3) $! unLoc $1)) }
        | texp ',' texp            {% runPV (runExpCmdP $1) >>= \ $1 ->
                                      runPV (runExpCmdP $3) >>= \ $3 ->
                                      addAnnotation (gl $1) AnnComma (gl $2) >>
                                      return (sLL $1 $> [$3,$1]) }
...
transformqual :: { Located ([AddAnn],[LStmt GhcPs (LHsExpr GhcPs)] -> Stmt GhcPs (LHsExpr GhcPs)) }
    ...
    : 'then' exp              {% runPV (runExpCmdP $2) >>= \ $2 -> return $
                                 sLL $1 $> ([mj AnnThen $1], \ss -> (mkTransformStmt ss $2)) }
    | 'then' exp 'by' exp     {% runPV (runExpCmdP $2) >>= \ $2 ->
                                 runPV (runExpCmdP $4) >>= \ $4 ->
...
fbind   :: { LHsRecField GhcPs (LHsExpr GhcPs) }
        : qvar '=' texp {% runPV (runExpCmdP $3) >>= \ $3 ->
...
dbind   :: { LIPBind GhcPs }
dbind   : ipvar '=' exp                {% runPV (runExpCmdP $3) >>= \ $3 ->
