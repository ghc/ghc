...
-- @
module Parser (parseModule, parseSignature, parseImport, parseBackpack,
...
altslist :: { forall b. ExpCmdI b => P (Located ([AddAnn],[LMatch GhcPs (Located (b GhcPs))])) }
        : '{'            alts '}'  { $2 >>= \ $2 -> return $
...
alts    :: { forall b. ExpCmdI b => P (Located ([AddAnn],[LMatch GhcPs (Located (b GhcPs))])) }
        : alts1                    { $1 >>= \ $1 -> return $
...
alts1   :: { forall b. ExpCmdI b => P (Located ([AddAnn],[LMatch GhcPs (Located (b GhcPs))])) }
        : alts1 ';' alt         { $1 >>= \ $1 ->
                                  $3 >>= \ $3 ->
                                     if null (snd $ unLoc $1)
                                     then return (sLL $1 $> (mj AnnSemi $2:(fst $ unLoc $1)
...
alt     :: { forall b. ExpCmdI b => P (LMatch GhcPs (Located (b GhcPs))) }
           : pat alt_rhs  { $2 >>= \ $2 ->
...
alt_rhs :: { forall b. ExpCmdI b => P (Located ([AddAnn],GRHSs GhcPs (Located (b GhcPs)))) }
        : ralt wherebinds           { $1 >>= \alt ->
                                      return $ sLL alt $> (fst $ unLoc $2, GRHSs noExt (unLoc alt) (snd $ unLoc $2)) }

ralt :: { forall b. ExpCmdI b => P (Located [LGRHS GhcPs (Located (b GhcPs))]) }
        : '->' exp            { runExpCmdP $2 >>= \ $2 ->
...
gdpats :: { forall b. ExpCmdI b => P (Located [LGRHS GhcPs (Located (b GhcPs))]) }
...
gdpat   :: { forall b. ExpCmdI b => P (LGRHS GhcPs (Located (b GhcPs))) }
        : '|' guardquals '->' exp
                                   { runExpCmdP $4 >>= \ $4 ->
                                     ams (sL (comb2 $1 $>) $ GRHS noExt (unLoc $2) $4)
...
stmtlist :: { forall b. ExpCmdI b => P (Located ([AddAnn],[LStmt GhcPs (Located (b GhcPs))])) }
        : '{'           stmts '}'       { $2 >>= \ $2 -> return $
                                          sLL $1 $> ((moc $1:mcc $3:(fst $ unLoc $2))
...
stmts :: { forall b. ExpCmdI b => P (Located ([AddAnn],[LStmt GhcPs (Located (b GhcPs))])) }
        : stmts ';' stmt  { $1 >>= \ $1 ->
                            $3 >>= \ $3 ->
                            if null (snd $ unLoc $1)
                              then return (sLL $1 $> (mj AnnSemi $2:(fst $ unLoc $1)
...
stmt  :: { forall b. ExpCmdI b => P (LStmt GhcPs (Located (b GhcPs))) }
        : qual                          { $1 }
        | 'rec' stmtlist                {  $2 >>= \ $2 ->
                                           ams (sLL $1 $> $ mkRecStmt (snd $ unLoc $2))
                                               (mj AnnRec $1:(fst $ unLoc $2)) }

qual  :: { forall b. ExpCmdI b => P (LStmt GhcPs (Located (b GhcPs))) }
