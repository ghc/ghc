        ...
        | context '=>' ctype          {% addAnnotation (gl $1) (toUnicodeAnn AnnDarrow $2) (gl $2)
                                         >> return (sLL $1 $> $
                                            HsQualTy { hst_ctxt = $1
                                                     ...
                                                     , hst_body = $3 }) }
        | ipvar '::' type             {% ams (sLL $1 $> (HsIParamTy PlaceHolder $1 $3))
                                             [mu AnnDcolon $2] }
        | type                        { $1 }
...
type :: { LHsType GhcPs }
        ...
        | btype '->' ctype             {% ams $1 [mu AnnRarrow $2] -- See note [GADT decl discards annotations]
                                       >> ams (sLL $1 $> $ HsFunTy PlaceHolder $1 $3)
                                              [mu AnnRarrow $2] }


typedoc :: { LHsType GhcPs }
        : btype                          { $1 }
        | btype docprev                  { sLL $1 $> $ HsDocTy PlaceHolder $1 $2 }
        | btype '->'     ctypedoc        {% ams (sLL $1 $> $ HsFunTy PlaceHolder $1 $3)
                                                [mu AnnRarrow $2] }
        | btype docprev '->' ctypedoc    {% ams (sLL $1 $> $
                                                 HsFunTy PlaceHolder (L (comb2 $1 $2)
                                                            (HsDocTy PlaceHolder $1 $2))
                                                         $4)
                                                [mu AnnRarrow $3] }
...
btype :: { LHsType GhcPs }
        : tyapps                      {%  splitTildeApps (reverse (unLoc $1)) >>=
                                          \ts -> return $ sL1 $1 $ HsAppsTy PlaceHolder ts }
...
btype_no_ops :: { LHsType GhcPs }
        : btype_no_ops atype            { sLL $1 $> $ HsAppTy PlaceHolder $1 $2 }
        | atype                         { $1 }
...
atype :: { LHsType GhcPs }
        : ntgtycon                       { sL1 $1 (HsTyVar PlaceHolder NotPromoted $1) }      -- Not including unit tuples
        | tyvar                          { sL1 $1 (HsTyVar PlaceHolder NotPromoted $1) }      -- (See Note [Unit tuples])
        | strict_mark atype              {% ams (sLL $1 $> (HsBangTy PlaceHolder (snd $ unLoc $1) $2))
                                                (fst $ unLoc $1) }  -- Constructor sigs only
        | '{' fielddecls '}'             {% amms (checkRecordSyntax
                                                    (sLL $1 $> $ HsRecTy PlaceHolder $2))
                                                        -- Constructor sigs only
                                                 [moc $1,mcc $3] }
        | '(' ')'                        {% ams (sLL $1 $> $ HsTupleTy PlaceHolder
        ...
        | '(' ctype ',' comma_types1 ')' {% addAnnotation (gl $2) AnnComma
                                            ...
                                            ams (sLL $1 $> $ HsTupleTy PlaceHolder
                                             HsBoxedOrConstraintTuple ($2 : $4))
                                                [mop $1,mcp $5] }
        | '(#' '#)'                   {% ams (sLL $1 $> $ HsTupleTy PlaceHolder HsUnboxedTuple [])
                                             [mo $1,mc $2] }
        | '(#' comma_types1 '#)'      {% ams (sLL $1 $> $ HsTupleTy PlaceHolder HsUnboxedTuple $2)
                                             [mo $1,mc $3] }
        | '(#' bar_types2 '#)'        {% ams (sLL $1 $> $ HsSumTy PlaceHolder $2)
                                             [mo $1,mc $3] }
        | '[' ctype ']'               {% ams (sLL $1 $> $ HsListTy PlaceHolder  $2) [mos $1,mcs $3] }
        | '[:' ctype ':]'             {% ams (sLL $1 $> $ HsPArrTy PlaceHolder  $2) [mo $1,mc $3] }
        ...
        | STRING               { sLL $1 $> $ HsTyLit PlaceHolder $ HsStrTy (getSTRINGs $1)
                                                                           (getSTRING  $1) }
...
qtycondoc :: { LHsType GhcPs } -- Qualified or unqualified
        : qtycon            { sL1 $1                                 (HsTyVar PlaceHolder NotPromoted $1)      }
        | qtycon docprev    { sLL $1 $> (HsDocTy PlaceHolder (sL1 $1 (HsTyVar PlaceHolder NotPromoted $1)) $2) }
...
literal :: { Located (HsLit GhcPs) }
        ...
        | PRIMSTRING        { sL1 $1 $ HsStringPrim (sst $ getPRIMSTRINGs $1)
                                                         $ getPRIMSTRING $1 }
        | PRIMFLOAT         { sL1 $1 $ HsFloatPrim  PlaceHolder  $ getPRIMFLOAT $1 }
        | PRIMDOUBLE        { sL1 $1 $ HsDoublePrim PlaceHolder  $ getPRIMDOUBLE $1 }
