...
newtype HdkM a = MkHdkM (ReaderT LocRange (State [PsLocated HdkComment]) a)
...
withLayoutInfoHdkA (VirtualBraces n) (MkHdkA l m) =
  MkHdkA l (m `inLocRange` LocRange mempty mempty (ColumnFrom (n+1)))
...
addHaddockExports (L l_exports exports) =
  delimitHdkA l_exports $ do
    exports' <- addHaddockInterleaveItems NoLayoutInfo getDocIE (mkHdkA pure) exports
...
data LowerLocBound = StartOfFile | StartLoc BufLoc
...
data UpperLocBound = EndOfFile | EndLoc BufLoc
...
locRangeFrom (RealSrcLoc _ (Just l)) = LocRange (StartLoc l) EndOfFile mempty
locRangeFrom (RealSrcLoc _ Nothing) = mempty
locRangeFrom (UnhelpfulLoc _) = mempty

locRangeTo :: SrcLoc -> LocRange
locRangeTo (RealSrcLoc _ (Just l)) = LocRange StartOfFile (EndLoc l) mempty
locRangeTo (RealSrcLoc _ Nothing) = mempty
...
takeHdkComments f =
  ...
  where
    ...
    zoom_column (ColumnFrom n) m =
      \comments ->
        let
          is_indented (L l_comment _) = bufLocCol (bufSpanStart (psBufSpan l_comment)) >= n
