...
In the past, Haddock comments were incorporated into the grammar.
See Note [Old solution: Haddock in the grammar].

Here's the general overview of the current design:

  1. The lexer collects Haddock comments in a list (hdk_comments in PState).

  2. After parsing, do another pass over the AST (addHaddockToModule)
     to associate Haddock comments with AST nodes using location info (BufPos).
     See Note [How addHaddock works].

  3. Report leftover comments to the user as a warning (-Wignored-haddock).
...
--
-- Reports badly positioned comments when -Wignored-haddock is enabled.
