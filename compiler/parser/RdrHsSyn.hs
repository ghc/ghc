...
checkPatBind :: Located (PatBuilder GhcPs)
             -> Located (a,GRHSs GhcPs (LHsExpr GhcPs))
             -> P ([AddAnn],HsBind GhcPs)
...
splitBang (dL->L _ (PatBuilderOpApp l_arg (dL->L bang op) r_arg))
...
class b ~ (Body b) GhcPs => DisambECP b where
  ...
  mkHsTySigPV :: SrcSpan -> Located b -> SrcSpan -> LHsType GhcPs -> PV (Located b)
...
instance p ~ GhcPs => DisambECP (HsCmd p) where
  ...
  mkHsVarPV (dL->L l v) = addFatalError l $ text "Parse error in command:" <+> ppr v
  mkHsLitPV (dL->L l a) = addFatalError l $ text "Parse error in command:" <+> ppr a
  mkHsOverLitPV (dL->L l a) = addFatalError l $ text "Parse error in command:" <+> ppr a
  mkHsWildCardPV l = addFatalError l $ text "Parse error in command:" <+> text "_"
  mkHsTySigPV _ a l sig = addFatalError l $
    text "Parse error in command:" <+> ppr a <+> text "::" <+> ppr sig
  mkHsExplicitListPV l xs = addFatalError l $
    text "Parse error in command:" <+> brackets (fsep (punctuate comma (map ppr xs)))
  mkHsSplicePV (dL->L l sp) = addFatalError l $ text "Parse error in command:" <+> ppr sp
  mkHsRecordPV l _ a (fbinds, ddLoc) = addFatalError l $
    text "Parse error in command:" <+> ppr a <+> ppr (mk_rec_fields fbinds ddLoc)
  mkHsNegAppPV l a = addFatalError l $ text "Parse error in command:" <+> text "-" <> ppr a
  mkHsSectionR_PV l op c = addFatalError l $
    text "Parse error in command:" <+> ppr op <> ppr c
  mkHsViewPatPV l a b = addFatalError l $
    text "Parse error in command:" <+> ppr a <+> text "->" <+> ppr b
  mkHsAsPatPV l v c = addFatalError l $
    text "Parse error in command:" <+> pprPrefixOcc (unLoc v) <> text "@" <> ppr c
  mkHsLazyPatPV l c = addFatalError l $
    text "Parse error in command:" <+> text "~" <> ppr c
  mkSumOrTuplePV l boxity a = addFatalError l $
    text "Parse error in command:" <+> pprSumOrTuple boxity a
...
instance p ~ GhcPs => DisambECP (HsExpr p) where
  ...
  mkHsTySigPV l a _ sig = return $ cL l (ExprWithTySig noExt a (mkLHsSigWcType sig))
...
data PatBuilder p
  ...
  | PatBuilderWithTySig (Located (PatBuilder p)) (LHsSigWcType GhcPs)
...
instance p ~ GhcPs => DisambECP (PatBuilder p) where
  ...
  mkHsLamPV l _ = addFatalError l $ text "Lambda-syntax in pattern"
  ...
  superFunArg m = m
  mkHsAppPV l p1 p2 =
    return $ cL l (PatBuilderApp p1 p2)
  ...
  mkHsTySigPV l a _ sig = return $ cL l (PatBuilderWithTySig a (mkLHsSigWcType sig))
  mkHsExplicitListPV l xs = do
    ps <- traverse checkLPat xs
    return (cL l (PatBuilderPat (ListPat noExt ps)))
  mkHsSplicePV sp = return $ mapLoc (PatBuilderPat . SplicePat noExt) sp
  mkHsRecordPV l _ a (fbinds, ddLoc) = do
    r <- mkPatRec a (mk_rec_fields fbinds ddLoc)
    checkRecordSyntax (cL l r)
  mkHsNegAppPV l a = return $ cL l $ PatBuilderNegApp a
  mkHsSectionR_PV l op p
    | isBangRdr (unLoc op) = return $ cL l $ PatBuilderBang (getLoc op) p
    | otherwise = addFatalError l $ text "Operator section in pattern"
...
How do we know that (Con a b) is a PatBind but (f a b) is a FunBind? We learn
this by inspecting an intermediate representation in 'isFunLhs' and seeing that
'Con' is a data constructor but 'f' is not. We need an intermediate
