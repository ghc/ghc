...
or an extra pass over the entire AST, is to parse into a product of possible
interpretations, for example:

    -- See Note [Parser-Validator]
    data ExpCmdPat = ( PV (LHsExpr GhcPs)
                     , PV (LHsCmd GhcPs)
                     , PV (LHsPat GhcPs) )
...
Fortunately, there's nothing that a dozen of GHC extensions cannot fix. We can
use GADTs and RankNTypes to model products as dependent functions:

    data ExpCmdG b where
      ExpG :: ExpCmdG HsExpr
      CmdG :: ExpCmdG HsCmd

    type ExpCmd = forall b. ExpCmdG b -> PV (LHsExpr GhcPs)

    checkExpOf2 f = f ExpG  -- interpret as an expression
    checkCmdOf2 f = f CmdG  -- interpret as a command

At this point, the 'alts' production becomes
...
Basically, the 'ExpCmdG' tag is a first-class record selector, so we are able
to define both pair components at once by abstracting over it.

Furthermore, note that 'ExpCmdG' is a singleton type, the value is completely
...
And now the 'alts' production is simplified further, as we no longer need to
...
rule, so unlike a naive product-based approach, it scales well to large parser
productions.
...
This is a viable alternative, but it does bloat code size.
