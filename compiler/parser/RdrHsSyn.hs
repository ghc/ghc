...
mkSpliceDecl lexpr@(L loc expr)
  ...
  | otherwise
  = SpliceD noExtField (SpliceDecl noExtField (L loc (mkUntypedSplice NoParens lexpr))
                              ImplicitSplice)
...
checkValDef strictness lhs Nothing g@(L l (_,grhss))
...
splitBang (L _ (PatBuilderOpApp l_arg op r_arg))
  | isBangRdr (unLoc op)
  = Just (l_arg, L l' (PatBuilderBang (getLoc op) arg1) : argns)
  where
    l' = combineLocs op arg1
    (arg1,argns) = split_bang r_arg []
    split_bang (L _ (PatBuilderApp f e)) es = split_bang f (e:es)
...
isFunLhs e = go e [] []
 where
   ...
   go (L _ (PatBuilderBang _ (L _ (PatBuilderVar (L l var))))) [] ann
   ...
   go e@(L loc (PatBuilderOpApp l (L loc' op) r)) es ann
  ...
  where
    ...
    go k acc ops_acc ((L l x) : xs)
      ...
      , let guess [] = True
            ...
            guess ((L _ (TyElKindApp _ _)):_) = False
      ...
      = if not (null acc) && (k > 1 || length acc > 1)
        then do { a <- eitherToP (mergeOpsAcc acc)
                ; failOpStrictnessCompound (L l str) (ops_acc a) }
...
patBuilderBang bang p =
  L (bang `combineSrcSpans` getLoc p) $
  PatBuilderBang bang p
...
instance DisambECP (PatBuilder GhcPs) where
  ...
  mkHsOpAppPV l p1 op p2 = do
    ...
    return $ L l $ PatBuilderOpApp p1 op p2
  ...
  mkHsSectionR_PV l op p
    | isBangRdr (unLoc op) = return $ L l $ PatBuilderBang (getLoc op) p
...
warnSpaceAfterBang (L opLoc op) argLoc = do
...
failOpStrictnessCompound (L _ str) (L loc ty) = addFatalError loc msg
  where
...
failOpStrictnessPosition (L loc _) = addFatalError loc msg
  where
