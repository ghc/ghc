...
mkDataFamInst :: SrcSpan
              ...
              -> HsDeriving GhcPs
              -> P (LInstDecl GhcPs)
mkDataFamInst loc new_or_data cType (dL->( _ , (mcxt, bndrs, tycl_hdr))) ksig
...
checkTyClHdr is_cls ty
  ...
  where
    ...
    go _ (HsParTy _ (dL->(l , HsStarTy _ isUni))) acc ann fix
      = do { warnStarBndr l
...
checkFunBind :: SDoc
             ...
             -> P ([AddAnn],HsBind GhcPs)
checkFunBind msg strictness ann lhs_loc fun is_infix pats (dL->(rhs_span,grhss))
...
splitBang (dL->(_ ,OpApp _ l_arg bang@(dL->(_ , HsVar _ (dL->(_ , op)))) r_arg))
...
pStrictMark ((dL->(l1 , x1)) : (dL->(l2 , x2)) : xs)
...
pBangTy
  ...
  -> ( Bool           {- has a strict mark been consumed? -}
     ...
     , P ()           {- add annotations -}
     , [Located TyEl] {- remaining TyEl -})
pBangTy lt@(dL->(l1 , _)) xs =
  case pStrictMark xs of
    Nothing -> (False, lt, pure (), xs)
    Just (dL->(l2 , strictMark), anns, xs') ->
...
mergeOps all_xs = go (0 :: Int) [] id all_xs
  where
    ...
    go k acc ops_acc ((dL->(l , TyElUnpackedness (anns, unpkSrc, unpk))):xs) =
    ...
    go _ _ _ ((dL->(l , TyElDocPrev _)):_) =
      failOpDocPrev l

    -- to improve error messages, we do a bit of guesswork to determine if the
    -- user intended a '!' or a '~' as a strictness annotation
    go k acc ops_acc ((dL->(l , x)) : xs)
      | Just (_, str) <- tyElStrictness x
      , let guess []                         = True
            guess (TyElOpd _          : _)   = False
            guess (TyElOpr _          : _)   = True
            guess (TyElTilde          : _)   = True
            guess (TyElBang           : _)   = True
            guess (TyElUnpackedness _ : _)   = True
            guess (TyElDocPrev _      : xs') = guess xs'
        in guess (map unLoc xs)
    ...
    -- to build its lhs.
    go k acc ops_acc ((dL->(l , TyElOpr op)):xs) =
      ...
      where
        isTyElOpd (dL->(_ , TyElOpd _)) = True
        isTyElOpd _ = False

    -- clause [opr.1]: interpret 'TyElTilde' as an operator
    go k acc ops_acc ((dL->(l  , TyElTilde)):xs) =
      let op = eqTyCon_RDR
      in go k acc ops_acc (cL l (TyElOpr op):xs)

    -- clause [opr.2]: interpret 'TyElBang' as an operator
    go k acc ops_acc ((dL->(l , TyElBang)):xs) =
      let op = mkUnqual tcClsName (fsLit "!")
      in go k acc ops_acc (cL l (TyElOpr op):xs)

    -- clause [opd]:
    -- whenever an operand is encountered, it is added to the accumulator
    go k acc ops_acc ((dL->(l , TyElOpd a)):xs) = go k (cL l a:acc) ops_acc xs

    go _ _ _ (_:_) = panic "mergeOps.go: impossible match"
...
pInfixSide ((dL->(l , TyElOpd t)):xs)
  | (True, t', addAnns, xs') <- pBangTy (cL l t) xs
  = Just (t', addAnns, xs')
pInfixSide ((dL->(l1 , TyElOpd t1)):xs1) = go [cL l1 t1] xs1
  where
    go acc ((dL->(l , TyElOpd t)):xs) = go (cL l t:acc) xs
...
pDocPrev = go Nothing
  where
    go mTrailingDoc ((dL->(l , TyElDocPrev doc)):xs) =
...
mergeDataCon all_xs =
  ...
  where
    ...
    singleDoc = isJust mTrailingDoc &&
                null [ () | (dL->(_ , TyElDocPrev _)) <- all_xs' ]
...
failOpStrictnessPosition (dL->(loc , _)) = parseErrorSDoc loc msg
  where
