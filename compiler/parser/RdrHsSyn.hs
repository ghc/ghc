...
data PseudoArrow a b = PseudoArrow

resultOfPV :: PseudoArrow (PV (Located b)) b
resultOfPV = PseudoArrow
...
class b ~ (Body b) GhcPs => DisambECP b where
  ...
  superInfixOp :: PseudoArrow r b -> (DisambInfixOp (InfixOp b) => r) -> r
  ...
  -- See Note [UndecidableSuperClasses for associated types]
  superFunArg :: PseudoArrow r b -> (DisambECP (FunArg b) => r) -> r
...
class DisambTD b where
  ...
  -- See Note [UndecidableSuperClasses for associated types]
  superInfixTyHead
    :: PseudoArrow r b  -- for type inference
  ...
  superInfixTyArg
    :: PseudoArrow r b  -- for type inference
    -> ((DisambTD (InfixTyArg b), DisambPrefixTD (InfixTyArg b)) => r) -> r
  mkInfixTyAppPV
...
data PrefixDataConBuilder =
  PrefixDataConBuilder [(LHsType GhcPs, Maybe LHsDocString)] (Located RdrName) (Maybe LHsDocString)
...
instance DisambPrefixTD (HsType GhcPs) where
  mkAppTyHeadPV t mDoc = do
    whenIsJust mDoc $ \(L l _) -> failOpDocPrev l
    return t
...
instance DisambPrefixTD a => DisambPrefixTD (a, Maybe LHsDocString) where
  mkAppTyHeadPV t mDoc = do
    L l' t' <- mkAppTyHeadPV t Nothing
    return (L l' (t', mDoc))
  mkAppTyAppPV (L l1 (t1, mLhsDoc)) mUnpk t2 mDoc = do
    whenIsJust mLhsDoc $ \(L l _) -> failOpDocPrev l
    L l t <- mkAppTyAppPV (L l1 t1) mUnpk t2 Nothing
    return (L l (t, mDoc))
  mkAppTyKindAppPV (L l_t (t, mLhsDoc)) l_at ki = do
    whenIsJust mLhsDoc $ \(L l _) -> failOpDocPrev l
    L l t' <- mkAppTyKindAppPV (L l_t t) l_at ki
    return (L l (t', Nothing))

stripDocTag :: PseudoArrow r (a, Maybe LHsDocString) -> PseudoArrow r a
stripDocTag _ = PseudoArrow

instance DisambTD a => DisambTD (a, Maybe LHsDocString) where
  type InfixTyHead (a, Maybe LHsDocString) = (InfixTyHead a, Maybe LHsDocString)
  superInfixTyHead p = superInfixTyHead (stripDocTag p)
  mkInfixTyHeadPV (L l (t, mDoc)) = do
    L l' t' <- mkInfixTyHeadPV (L l t)
    return (L l' (t', mDoc))
  type InfixTyArg (a, Maybe LHsDocString) = (InfixTyArg a, Maybe LHsDocString)
  superInfixTyArg p = superInfixTyArg (stripDocTag p)
  mkInfixTyAppPV (L l_lhs (lhs, mLhsDoc)) op mOpDoc (L l_rhs (rhs, mRhsDoc)) = do
    whenIsJust mLhsDoc $ \(L l _) -> failOpDocPrev l
    L l t <- mkInfixTyAppPV (L l_lhs lhs) op mOpDoc (L l_rhs rhs)
    return (L l (t, mRhsDoc))
  mkInfixTyUnpackednessPV Nothing a = return a
  mkInfixTyUnpackednessPV (Just unpk) (L l (t, mDoc)) = do
...
instance DisambTD ConstrStuff where
  type InfixTyHead ConstrStuff = PrefixDataConBuilder
  superInfixTyHead _ a = a
  ...
  mkInfixTyHeadPV (L l (PrefixDataConBuilder [(L l_t (HsRecTy _ fields), mDoc)] data_con mConDoc)) = do
    ...
    return $ L l (ConstrStuff data_con (RecCon (L l_t fields)) mConDoc)
  ...
  mkInfixTyHeadPV (L l (PrefixDataConBuilder flds data_con mConDoc)) = do
    ...
    return $ L l (ConstrStuff data_con (PrefixCon flds') mConDoc)

  type InfixTyArg ConstrStuff = (HsType GhcPs, Maybe LHsDocString)
  superInfixTyArg _ a = a
  ...
  mkInfixTyAppPV (L l_lhs (lhs, mLhsDoc)) (L l_tc tc) Nothing (L l_rhs (rhs, Just rhsDoc)) =
    mkInfixTyAppPV (L l_lhs (lhs, mLhsDoc)) (L l_tc tc) (Just rhsDoc) (L l_rhs (rhs, Nothing))

  mkInfixTyAppPV (L l_lhs (lhs, mLhsDoc)) (L l_tc tc) mConDoc (L l_rhs (rhs, mRhsDoc)) = do
      ...
      return $ L l (ConstrStuff data_con (InfixCon lhs' rhs') mConDoc)
    where
  ...
  mkInfixTyUnpackednessPV (Just unpk) constr_stuff
    | L _ (ConstrStuff data_con (InfixCon lhs rhs) con_doc) <- constr_stuff
    = -- When the user writes  data T = {-# UNPACK #-} Int :+ Bool
      ...
      do lhs' <- addUnpackednessP unpk lhs
         ...
         return $ L l (ConstrStuff data_con (InfixCon lhs' rhs) con_doc)
    | otherwise =
...
failOpDocPrev :: MonadP m => SrcSpan -> m ()
failOpDocPrev loc = addError loc msg
  where
