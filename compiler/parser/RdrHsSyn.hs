...
mkPatSynMatchGroup :: Located RdrName
                   ...
                   -> P (MatchGroup0 GhcPs (LHsExpr GhcPs))
...
checkFunBind :: [AddAnn]
             ...
             -> P ([AddAnn],HsBind GhcPs)
checkFunBind ann lhs_loc fun is_infix pats (L rhs_span grhss)
...
makeFunBind :: Located RdrName -> [LMatch GhcPs (LHsExpr GhcPs)]
            -> HsBind GhcPs
makeFunBind fn ms = mkFunBind FromSource fn ms

-- See Note [FunBind vs PatBind]
checkPatBind :: LPat GhcPs
             -> Located (a,GRHSs GhcPs (LHsExpr GhcPs))
             -> P ([AddAnn],HsBind GhcPs)
checkPatBind lhs (L _ (_,grhss)) -- TODO use loc?
    | BangPat _ p <- unLoc lhs
    , VarPat _ v <- unLoc p
    = return ([], mkHsVarBind0' v grhss)
...
isFunLhs e = go e [] []
 where
   go (L loc (PatBuilderVar (L _ f))) es ann
       | e : es <- es
       , not (isRdrDataCon f)
       = return (Just (L loc f, Prefix, e :| es, ann))
       | otherwise
       = return Nothing
