...
opt_trans_rule _ co1 co2        -- Identity rule
  | let ty1 = coercionLeftKind co1
        r   = coercionRole co1
        ty2 = coercionRightKind co2
  , ty1 `eqType` ty2
...
opt_trans_rule_app is orig_co1 orig_co2 co1a co1bs co2a co2bs
  ...
  = ASSERT( co1bs `equalLength` co2bs )
    ...
    let rt1a = coercionRightKind co1a

        lt2a = coercionLeftKind co2a
        rt2a = coercionRole  co2a

        rt1bs = map coercionRightKind co1bs
        lt2bs = map coercionLeftKind co2bs
...
checkAxInstCo (AxiomInstCo ax ind cos)
  = let branch       = coAxiomNthBranch ax ind
        ...
        (tys, cotys) = splitAtList tvs (map coercionLeftKind cos)
...
compatible_co co1 co2
  = x1 `eqType` x2
  where
    x1 = coercionRightKind co1
    x2 = coercionLeftKind  co2
