...
emptyFlattenEnv :: FlattenEnv
emptyFlattenEnv
  = FlattenEnv { fe_type_map = emptyTypeMap
               , fe_in_scope = emptyInScopeSet }

-- See Note [Flattening]
flattenTys :: InScopeSet -> [Type] -> [Type]
flattenTys in_scope tys
  = snd $ coreFlattenTys (mkEmptyTCvSubst in_scope) emptyFlattenEnv tys
...
coreFlattenTy subst = go
  where
    ...
    go env (TyVarTy tv)
      | Just ty <- lookupTyVar subst tv = (env, ty)
...
coreFlattenCo subst env co
  ...
  where
    ...
    env2          = env1 { fe_in_scope =
                           extendInScopeSet (fe_in_scope env1) covar }
