...
normalise_var_bndr tcvar
  ...
  = do { lc1 <- getLC
       ; liftCoSubstVarBndrUsing callback lc1 tcvar }
  where
    callback :: LiftingContext -> Type -> NormM (CoercionN, Kind)
    callback lc ki = withLC lc $ normalise_type ki
...
initNormM rec_nts env role vars (NormM thing_inside)
  = fst <$> thing_inside rec_nts env lc role
  where
    in_scope = mkInScopeSet vars
    lc       = emptyLiftingContext in_scope

getRole :: NormM Role
getRole = NormM (\ rec_nts _ _ r -> Just (r, rec_nts))

getLC :: NormM LiftingContext
getLC = NormM (\ rec_nts _ lc _ -> Just (lc, rec_nts))

getEnv :: NormM FamInstEnvs
getEnv = NormM (\ rec_nts env _ _ -> Just (env, rec_nts))
...
checkRecursion :: TyCon -> NormM ()
checkRecursion tc
  = NormM $ \ rec_nts _ _ _ -> case checkRecTc rec_nts tc of
                                 Just new_rec_nts -> Just ((), new_rec_nts)
                                 Nothing          -> Nothing

instance Monad NormM where
  ma >>= fmb = NormM $ \ rec_nts1 env lc r ->
               case runNormM ma rec_nts1 env lc r of
                 Just (a, rec_nts2) -> runNormM (fmb a) rec_nts2 env lc r
                 Nothing            -> Nothing

instance Applicative NormM where
  pure x = NormM $ \ rec_nts _ _ _ -> Just (x, rec_nts)
  (<*>)  = ap
