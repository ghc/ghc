...
2. There are two different reasons we might add a variable
   ...
   normally do. We thus must track *two* in-scope sets: one for flattening
   variables, and one for other variables. The former (part of FlattenEnv) is
   threaded through (like a state monad), while the other (part of a TCvSubst)
   is just passed down (like a reader monad). We union these together when
   creating fresh variables at usage sites.
...
flattenTys in_scope tys = snd $ coreFlattenTys subst env tys
  where
...
coreFlattenTys :: TCvSubst -> FlattenEnv
               -> [Type] -> (FlattenEnv, [Type])
coreFlattenTys subst = go []
  where
    go rtys env []         = (env, reverse rtys)
    go rtys env (ty : tys)
      = let (env', ty') = coreFlattenTy subst env ty in
        go (ty' : rtys) env' tys

coreFlattenTy :: TCvSubst -> FlattenEnv
              -> Type -> (FlattenEnv, Type)
coreFlattenTy subst = go
  where
    go env ty | Just ty' <- coreView ty = go env ty'

    go env (TyVarTy tv)    = (env, substTyVar subst tv)
    ...
    go env (ForAllTy (Bndr tv vis) ty)
      = let (env1, tv') = coreFlattenVarBndr subst env tv
            -- Extend the substitution with [tv |-> tv'], and add tv' to the
            -- FlattenEnv's in-scope set.
            -- See Note [Flattening], wrinkle 2B.
            subst'      = extendTvSubst subst tv (mkTyVarTy tv')
            env1'       = env1 { fe_in_scope =
                                 extendInScopeSet (fe_in_scope env1) tv' }
...
coreFlattenCo :: TCvSubst -> FlattenEnv
...
coreFlattenCo subst env co
  ...
  where
    ...
    env1          = env { fe_in_scope =
                          extendInScopeSet (fe_in_scope env) covar }
    (env2, kind') = coreFlattenTy subst env1 (coercionType co)

coreFlattenVarBndr :: TCvSubst -> FlattenEnv
                   -> TyCoVar -> (FlattenEnv, TyCoVar)
coreFlattenVarBndr subst env tv
  = let new_tv = uniqAway in_scope' (setVarType tv kind') in
    (env', new_tv)
  where
    -- Union the two in-scope sets. See Note [Flattening], wrinkle 2.
    in_scope'     = getTCvInScope subst `unionInScope` fe_in_scope env
    kind          = varType tv
    (env', kind') = coreFlattenTy subst env kind

coreFlattenTyFamApp :: TCvSubst -> FlattenEnv
                    -> TyCon         -- type family tycon
                    -> [Type]        -- args, already flattened
                    -> (FlattenEnv, Type)
coreFlattenTyFamApp subst env fam_tc fam_args
