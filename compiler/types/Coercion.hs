...
module Coercion (
        ...
        coercionKind, coercionLeftKind, coercionRightKind,
...
mkForAllCo_NoRefl v kind_co co
  | ASSERT( varType v `eqType` coercionLeftKind kind_co ) True
  , ASSERT( isTyVar v || almostDevoidCoVarOfCo v co)   True
...
--    mkAxInstRhs ax index tys = coercionRightKind (mkAxInstCo ax index tys)
...
mkCoCast c g
  ...
  where
    ...
    -- g1 :: s1 ~# s2
    -- g2 :: t1 ~# t2
    (tc, _) = splitTyConApp (coercionLeftKind g)
...
liftEnvSubstLeft = liftEnvSubst coercionLeftKind

liftEnvSubstRight :: TCvSubst -> LiftCoEnv -> TCvSubst
liftEnvSubstRight = liftEnvSubst coercionRightKind

liftEnvSubst :: (Coercion -> Kind) -> TCvSubst -> LiftCoEnv -> TCvSubst
liftEnvSubst co_lr_kind subst lc_env
  = -- co_lr_kind is either coercionLeftKind or coercionRightKind
...
If co :: t1 ~ t2, then
   ...
   coercionKind co      =  Pair t1 t2
   coercionLeftKind co  =  t1
   coercionRightKind co =  t2
   coercionType co      =  t1 ~# t2

We /often/ need only 't1' or 't2, and hence call coercionLeftKind or
coercionRightKind resp. If you only want one, it's much more efficient to
find just the LKind or RKind than to find both at once and pick the
one you want.  So we define coercionKind by calling coercionLeftKind and
coercionRightKind, rather than vice versa.

A notable example in in mkCastTy, which is what provoked this
line of reasoning.
-}
...
coercionKind co = Pair (coercionLeftKind co) (coercionRightKind co)
                  -- See Note [coercionKind]

coercionLeftKind :: Coercion -> Type
coercionLeftKind co
  = go co
  where
    go (Refl ty)                = ty
    ...
    go (SymCo co)               = coercionRightKind co
...
go_nth d ty
  ...
  | otherwise
  = pprPanic "coercionLeftKind:nth" (ppr d <+> ppr ty)

coercionRightKind :: Coercion -> Type
coercionRightKind co
  = go co
  where
    go (Refl ty)                = ty
    ...
    go (SymCo co)               = coercionLeftKind co
  ...
  where
    ...
    go acc_xis acc_cos lc [] inner_ki roles args
      = let co1 = liftCoSubst Nominal lc inner_ki
            ...
            unflattened_tys       = map (coercionRightKind . snd) args
