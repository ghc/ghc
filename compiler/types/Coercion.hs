...
TODO
-- In our running example, these types are (forall j. j -> Type) and
-- (forall (j :: Star). (j |> asStar) -> Star). One might think that we could just
-- arrange to instantiate this coercion with co3, the coercion associated with
-- flattening the next (3rd) argument. That isn't well-kinded, though: co3 is
-- homogeneous (as all flattening coercions are), and yet the two types related
-- by co1 expect different kinds (the one on the left wants a Type, while the one
-- on the right wants a Star).

-- So, we extract the coercion between the arguments of co1, using NthCo. In our
-- case, we get (NthCo 0 co1 :: Type ~ Star). Recall that the right-hand side of
-- co1 (which, recall, is important because it was the result of lifitng k, the
-- variable at the end of a's kind) is the original type, and the left-hand side
-- is the flattened one. Thus, the kind of both sides of co3 must be the kind on
-- the right of (NthCo 0 co1). We thus create

--   co3' = mkCoherenceLeftCo Nominal (Bool |> sym axStar) (sym (NthCo 0 co1)) co3
...
simplifyArgsWorker :: [TyCoBinder] -> Kind
                   ...
                   -> [(Type, Coercion)] -- TODO list of type arguments, arg (in 1-to-1 correspondence with roles)
...
simplifyArgsWorker orig_ki_binders orig_inner_ki orig_fvs
  ...
  where
    ...
    go :: [Type]      -- Xis accumulator, in reverse order
       ...
       -> [(Type, Coercion)]      -- TODO ... unflattened types
       -> ([Type], [Coercion], CoercionN)
    ...
    go _ _ _ _ _ _ _ = pprPanic
        "simplifyArgsWorker wandered into deeper water than usual" (vcat [])
