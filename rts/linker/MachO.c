...
relocateSection(ObjectCode* oc, int curSection)
{
    ...
    for(uint32_t i = 0; i < msect->nreloc; i++)
    {
        ...
        if (type == X86_64_RELOC_GOT || type == X86_64_RELOC_GOT_LOAD)
        {
            ...
            IF_DEBUG(linker,
                     debugBelch("relocateSection: making jump island for %s, extern = %d, X86_64_RELOC_GOT\n",
                                nm, reloc->r_extern));

            CHECKM(0 != reloc->r_extern,
