           ...
           for (size_t j = 0; j < symTab->n_symbols; j++) {
               ...
               if (shndx == SHN_COMMON) {
                   ...
                   */
                   ...
                   if(symbol->addr == 0x0) {
                        debugBelch("ocGetNames_ELF: %s in section %s (no %d, kind: %d), from %s has an empty address", nm, oc->sections[secno].info->name, secno, oc->sections[secno].kind, OC_INFORMATIVE_FILENAME(oc));
                   }
#endif
