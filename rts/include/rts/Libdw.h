...
/* Free a backtrace */
RTS_PUBLIC void backtraceFree(Backtrace *bt);

/* Request a backtrace of the current stack state.
 * May return NULL if a backtrace can't be acquired. */
RTS_PUBLIC Backtrace *libdwGetBacktrace(LibdwSession *session);

/* Lookup Location information for the given address.
 * Returns 0 if successful, 1 if address could not be found. */
RTS_PUBLIC int libdwLookupLocation(LibdwSession *session, Location *loc, StgPtr pc);

/* Pretty-print a backtrace to the given FILE */
RTS_PUBLIC void libdwPrintBacktrace(LibdwSession *session, FILE *file, Backtrace *bt);
