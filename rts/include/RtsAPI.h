...
extern RTS_PUBLIC const RtsConfig defaultRtsConfig;
...
RTS_PUBLIC void getRTSStats (RTSStats *s);
RTS_PUBLIC int getRTSStatsEnabled (void);

// Returns the total number of bytes allocated since the start of the program.
// TODO: can we remove this?
RTS_PUBLIC uint64_t getAllocations (void);
...
extern RTS_PUBLIC void startupHaskell         ( int argc, char *argv[],
                                     void (*init_root)(void) );

/* DEPRECATED, use hs_exit() instead  */
extern RTS_PUBLIC void shutdownHaskell        ( void );

/* Like hs_init(), but allows rtsopts. For more complicated usage,
 * use hs_init_ghc. */
extern RTS_PUBLIC void hs_init_with_rtsopts (int *argc, char **argv[]);
 ...
 */
extern RTS_PUBLIC void hs_init_ghc (int *argc, char **argv[],   // program arguments
                         RtsConfig rts_config);      // RTS configuration

extern RTS_PUBLIC void shutdownHaskellAndExit (int exitCode, int fastExit)
    STG_NORETURN;

#if !defined(mingw32_HOST_OS)
extern RTS_PUBLIC void shutdownHaskellAndSignal (int sig, int fastExit)
    STG_NORETURN;
#endif

extern RTS_PUBLIC void getProgArgv            ( int *argc, char **argv[] );
extern RTS_PUBLIC void setProgArgv            ( int argc, char *argv[] );
extern RTS_PUBLIC void getFullProgArgv        ( int *argc, char **argv[] );
extern RTS_PUBLIC void setFullProgArgv        ( int argc, char *argv[] );
extern RTS_PUBLIC void freeFullProgArgv       ( void ) ;
...
RTS_PUBLIC Capability *rts_lock (void);

// releases the token acquired with rts_lock().
// See Note [Locking and Pausing the RTS] for correct usage.
RTS_PUBLIC void rts_unlock (Capability *token);
...
RTS_PUBLIC Capability *rts_unsafeGetMyCapability (void);
...
RTS_PUBLIC void rts_setInCallCapability (int preferred_capability, int affinity);
...
/* ----------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
RTS_PUBLIC HaskellObj   rts_mkChar       ( Capability *, HsChar   c );
RTS_PUBLIC HaskellObj   rts_mkInt        ( Capability *, HsInt    i );
RTS_PUBLIC HaskellObj   rts_mkInt8       ( Capability *, HsInt8   i );
RTS_PUBLIC HaskellObj   rts_mkInt16      ( Capability *, HsInt16  i );
RTS_PUBLIC HaskellObj   rts_mkInt32      ( Capability *, HsInt32  i );
RTS_PUBLIC HaskellObj   rts_mkInt64      ( Capability *, HsInt64  i );
RTS_PUBLIC HaskellObj   rts_mkWord       ( Capability *, HsWord   w );
RTS_PUBLIC HaskellObj   rts_mkWord8      ( Capability *, HsWord8  w );
RTS_PUBLIC HaskellObj   rts_mkWord16     ( Capability *, HsWord16 w );
RTS_PUBLIC HaskellObj   rts_mkWord32     ( Capability *, HsWord32 w );
RTS_PUBLIC HaskellObj   rts_mkWord64     ( Capability *, HsWord64 w );
RTS_PUBLIC HaskellObj   rts_mkPtr        ( Capability *, HsPtr    a );
RTS_PUBLIC HaskellObj   rts_mkFunPtr     ( Capability *, HsFunPtr a );
RTS_PUBLIC HaskellObj   rts_mkFloat      ( Capability *, HsFloat  f );
RTS_PUBLIC HaskellObj   rts_mkDouble     ( Capability *, HsDouble f );
RTS_PUBLIC HaskellObj   rts_mkStablePtr  ( Capability *, HsStablePtr s );
RTS_PUBLIC HaskellObj   rts_mkBool       ( Capability *, HsBool   b );
RTS_PUBLIC HaskellObj   rts_mkString     ( Capability *, char    *s );

RTS_PUBLIC HaskellObj   rts_apply        ( Capability *, HaskellObj, HaskellObj );

/* ----------------------------------------------------------------------------
   Deconstructing Haskell objects
   ------------------------------------------------------------------------- */
RTS_PUBLIC HsChar       rts_getChar      ( HaskellObj );
RTS_PUBLIC HsInt        rts_getInt       ( HaskellObj );
RTS_PUBLIC HsInt8       rts_getInt8      ( HaskellObj );
RTS_PUBLIC HsInt16      rts_getInt16     ( HaskellObj );
RTS_PUBLIC HsInt32      rts_getInt32     ( HaskellObj );
RTS_PUBLIC HsInt64      rts_getInt64     ( HaskellObj );
RTS_PUBLIC HsWord       rts_getWord      ( HaskellObj );
RTS_PUBLIC HsWord8      rts_getWord8     ( HaskellObj );
RTS_PUBLIC HsWord16     rts_getWord16    ( HaskellObj );
RTS_PUBLIC HsWord32     rts_getWord32    ( HaskellObj );
RTS_PUBLIC HsWord64     rts_getWord64    ( HaskellObj );
RTS_PUBLIC HsPtr        rts_getPtr       ( HaskellObj );
RTS_PUBLIC HsFunPtr     rts_getFunPtr    ( HaskellObj );
RTS_PUBLIC HsFloat      rts_getFloat     ( HaskellObj );
RTS_PUBLIC HsDouble     rts_getDouble    ( HaskellObj );
RTS_PUBLIC HsStablePtr  rts_getStablePtr ( HaskellObj );
RTS_PUBLIC HsBool       rts_getBool      ( HaskellObj );
...
RTS_PUBLIC void rts_eval (/* inout */ Capability **,
               /* in    */ HaskellObj p,
               /* out */   HaskellObj *ret);

RTS_PUBLIC void rts_eval_ (/* inout */ Capability **,
                /* in    */ HaskellObj p,
                /* in    */ unsigned int stack_size,
                /* out   */ HaskellObj *ret);

RTS_PUBLIC void rts_evalIO (/* inout */ Capability **,
                 /* in    */ HaskellObj p,
                 /* out */   HaskellObj *ret);

RTS_PUBLIC void rts_evalStableIOMain (/* inout */ Capability **,
                           /* in    */ HsStablePtr s,
                           /* out */   HsStablePtr *ret);

RTS_PUBLIC void rts_evalStableIO (/* inout */ Capability **,
                       /* in    */ HsStablePtr s,
                       /* out */   HsStablePtr *ret);

RTS_PUBLIC void rts_evalLazyIO (/* inout */ Capability **,
                     /* in    */ HaskellObj p,
                     /* out */   HaskellObj *ret);
...
RTS_PUBLIC void rts_inCall (/* inout */ Capability **,
                 /* in    */ HaskellObj p,
                 /* out */   HaskellObj *ret);

RTS_PUBLIC void rts_checkSchedStatus (char* site, Capability *);
...
RTS_PUBLIC void rts_clearMemory(void);
