...
{
#if defined(HAVE_PTHREAD_GET_CPUCLOCKID) && defined(HAVE_CLOCK_GETTIME)
    clockid_t clockid;
    if (pthread_getcpuclockid(t, &clockid) != 0) {
        sysErrorBelch("pthread_getcpuclockid");
        stg_exit(EXIT_FAILURE);
    }
    return getClockTime(clockid);
#else
    (void) t;
    abort(); // TODO
#endif
