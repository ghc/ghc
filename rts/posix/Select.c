...
static bool wakeUpSleepingThreads (Capability *cap, LowResTime now)
{
    ...
    while (cap->iomgr.sleeping_queue != END_TSO_QUEUE) {
        tso = cap->iomgr.sleeping_queue;
        if (((long)now - (long)tso->block_info.target) < 0) {
            break;
        }
        cap->iomgr.sleeping_queue = tso->_link;
...
awaitEvent(Capability *cap, bool wait)
{
    ...
    do {
      ...
      for(tso = cap->iomgr.blocked_queue_hd;
          tso != END_TSO_QUEUE;
          tso = next) {
        next = tso->_link;
      ...
      if (!wait) {
          ...
          tv.tv_usec = 0;
          ptv = &tv;
      } else if (cap->iomgr.sleeping_queue != END_TSO_QUEUE) {
          ...
          Time min = LowResTimeToTime(
                       cap->iomgr.sleeping_queue->block_info.target - now
                     );
      ...
      prev = NULL;
      {
          ...
          if (prev == NULL)
              cap->iomgr.blocked_queue_hd =
                cap->iomgr.blocked_queue_tl = END_TSO_QUEUE;
          else {
              prev->_link = END_TSO_QUEUE;
              cap->iomgr.blocked_queue_tl = prev;
          }
      }
