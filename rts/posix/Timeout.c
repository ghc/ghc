...
int syncDelayTimeout(Capability *cap, StgTSO *tso, HsInt us_delay)
{
    ...
    if (RTS_UNLIKELY(timeout == NULL)) { return sizeofW(StgTimeout); }
    union NotifyCompletion notify = { .tso = tso };
    initElemTimeoutQueue(timeout, notify, NotifyTSO);
...
void processTimeoutCompletions(Capability *cap, Time now)
{
    ...
    while (!isEmptyTimeoutQueue(iomgr->timeout_queue)) {
        ...
        StgTimeout *timeout = findMinTimeoutQueue(iomgr->timeout_queue);
        notifyTimeoutCompletion(cap, timeout);
        deleteMinTimeoutQueue(&iomgr->timeout_queue);
    }
}
