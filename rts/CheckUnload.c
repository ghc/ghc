...
//
// Note that, crucially, we don't load an object code even if it's not reachable
// from the heap, unless it's explicitly asked for unloading (via `unloadObj`).
// This is a feasture and not a but! (though I don't know the use case myself)
//
...
// - Maintain a "snapshot":
//
//   - Copy `loaded_objects` as the root set
//
