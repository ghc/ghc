...
static void pruneCCList(ObjectCode *oc)
{
...
void checkUnload (StgClosure *static_objects)
{
  ...
  // marked as unreferenced can be physically unloaded.
  prev = NULL;
...
#if defined(PROFILING)
          // CCS tree doesn't include any CCS in this object, and object is
          // unloaded, so none will be created. We can prune the global CC_LIST
          // of any CCs from this object.
          pruneCCList(oc);
#endif /* PROFILING */
