...
typedef struct traverseState_ {
     ...
     * objects.
     *
     * When the visited bit is not equal to the value of 'flip' the closure data
     * is valid otherwise not (see isTravDataValid). We then invert the value of
     * 'flip' after each each profiling run (see
     * traverseInvalidateAllProfHeaders).
     *
     ...
     * @param acc   The current value of the accumulator for 'c' on the stack. It's about to
     *              be removed, hence the 'const' qualifier. This is the same accumulator
     *              'visit_cb' got passed when 'c' was visited.
     *
     * @param c_parent    The parent closure of 'c'
     * @param acc_parent  The accumulator associated with 'c_parent', currently on the stack.
     */
