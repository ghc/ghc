 ...
 *
 * Precondition: the new capacity must be at least as big as the old.
 ...
 *
 * Returns 0 on success, n > 0 if it could not allocate enough memory, with n
 * being the number of words needed. Depending on the calling context, you can
 * arrange for a GC -- to free up at least n words -- and then try again.
 */
int enlargeClosureTable(Capability *cap, ClosureTable *t, int capacity);
 ...
 *
 * Preserving compactness has the consequence that the index of another
 * existing entry will change. Outputs are provided to report the remapping of
 * the index of an existing element that got moved as a result of deleting the
 * requested element. The output variables ix_from and ix_to give the index an
 * element was moved from and to. This output allows, for example, the caller
 * to apply the remapping to an auxiliary table.
 */
...
bool isFullClosureTable(ClosureTable *t)
{
    ...
    ASSERT(t->free_hd == -1 ? t->elems == t->capacity
                            : t->elems <  t->capacity);
    return (t->free_hd == -1);
}
