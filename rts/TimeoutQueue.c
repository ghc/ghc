...
#if defined(THREADED_RTS) && MAX_N_CAPABILITIES != 256
#error MAX_N_CAPABILITIES changed: check StgTimeoutQueue capno bitfield size
/* It is declared as StgHalfWord capno: 8; to match current max capno */
#endif
...
{
    /* TODO: should use the CCS from the calling context */
    SET_HDR(t, &stg_TIMEOUT_QUEUE_info, CCS_SYSTEM);
    t->parent      = EMPTY;
    t->a           = EMPTY;
    t->b           = EMPTY;
    t->notify      = notify;
...
/* Deletes the root node, containing the minimum key value.
 *
