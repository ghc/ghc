...
void postTickyCounterDefs()
{
    ACQUIRE_LOCK(&eventBufMutex);
    for (StgEntCounter *p = ticky_entry_ctrs; p != NULL; p = p->link) {
        postTickyCounterDef(p);
    }
    RELEASE_LOCK(&eventBufMutex);
}
...
void postTickyCounterSamples()
{
    ACQUIRE_LOCK(&eventBufMutex);
    for (StgEntCounter *p = ticky_entry_ctrs; p != NULL; p = p->link) {
        postTickyCounterSample(p);
    }
    RELEASE_LOCK(&eventBufMutex);
}
#endif /* TICKY_TICKY */
