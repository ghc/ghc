...
- The current IpeBufferListNode's buffer is full: Allocate a new one and link it to
  the previous one, making this one the new current.
...
function generated by the compiler (CodeOutput.hs) in a *.c file for each module.
*/
void registerInfoProvList(InfoProvEnt **ent_list)
{
    ...
    if (ipeBufferList == NULL) {
        ...
        ipeBufferList = stgMallocBytes(sizeof(IpeBufferListNode), "registerInfoProvList-firstNode");
        ...
        if (ipeBufferList->count < IPE_LIST_NODE_BUFFER_SIZE) {
            ...
            ASSERT(ipeBufferList->next == NULL || ipeBufferList->next->count == IPE_LIST_NODE_BUFFER_SIZE);
        } else {
            IpeBufferListNode* newNode = stgMallocBytes(sizeof(IpeBufferListNode), "registerInfoProvList-nextNode");
            ipeBufferList->buffer[0] = ent_list;
            ipeBufferList->count = 1;
...
void updateIpeMap() {
    ...
    while (ipeBufferList != NULL) {
        ASSERT(ipeBufferList->next == NULL || ipeBufferList->next->count == IPE_LIST_NODE_BUFFER_SIZE);
        ASSERT(ipeBufferList->count > 0 && ipeBufferList->count <= IPE_LIST_NODE_BUFFER_SIZE);

        IpeBufferListNode* currentNode = ipeBufferList;

        for (int i = 0; i < currentNode->count; i++) {
            for(InfoProvEnt** ipeList = currentNode->buffer[i]; *ipeList != NULL; ipeList++) {
