...
allocatePinned (Capability *cap, W_ n /*words*/, W_ alignment /*bytes*/, W_ align_off /*bytes*/)
{
    ...
    if (bd == NULL || (bd->free + off_w + n) > (bd->start + BLOCK_SIZE_W)) {
        ...
        if (bd == NULL) {
            ...
            if (cap->r.rCurrentNursery){
              cap->r.rCurrentNursery = cap->r.rCurrentNursery->link;
            }
        }
...
block, which was provided from an arbitrary megablock which had some free space.
