...
allocatePinned (Capability *cap, W_ n /*words*/, W_ alignment /*bytes*/, W_ align_off /*bytes*/)
{
    ...
    if (bd == NULL || (bd->free + off_w + n) > (bd->start + BLOCK_SIZE_W)) {
        ...
        if (bd == NULL) {
            ...
            // here).
            ACQUIRE_SM_LOCK;
            bd = allocNursery(cap->node, NULL, PINNED_NURSERY_SIZE);
            RELEASE_SM_LOCK;
        }
...
data between blocks. Over time, the effect is that most megablocks end up quite full because
data will be copied out of fragmented megablocks, likely into a less fragmented block.
...
some programs (see #19481), the fragmentation is so bad that for a program with
live data of only 4mb, can retain over 500 megablocks because each of these
...
the nursery which shrunk the size of the nursery by 1. Then next time the size
of the nursery was checked, the nursery requested an extra
block, which was provided from a random megablock which had some free space.
The effect over time as more allocation happened was the nursery became made up
of blocks from many different megablocks.
...
threshold but it doesn't "use" more memory.
