...
/* ----------------------------------------------------------------------------
   ...
   This way we can initialize chain with untagged roots (e.g. mutable lists,
   spark queues) but still tag the pointers from mutators when we add them to
   the same chain: when adding a new field, if the chain has tags 1 or 2 we walk
   down the chain, tag the info ptr, and tag the current field with 3.
