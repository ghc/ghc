...
waitForGcThreads (Capability *cap, bool idle_cap[])
{
    ...
    ASSERT(n_threads < (int)n_capabilities); // must be less becasue we don't count ourself
 ...
 *   (b) in notify_todo_block, if there are any threads not running, then we
 ...
 *      gc_running_threads.
 *    (d) notify_todo_block does not hold gc_running_mutex while inspecting
 *      gc_running_mutex.
 ...
 * final sequential collections, for the benefit of notify_todo_block.
 * */
