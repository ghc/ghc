...
scheduleDetectDeadlock (Capability **pcap, Task *task)
{
    if (isIdleGcPending()) {
        ...
        scheduleDoGC (pcap, task,
                      ...
                      false /* nonconcurrent */);
        /* When force_major == true, then when scheduleDoGC completes, it calls
         * notifyIdleGcDone which set recent activity to ACTIVITY_DONE_GC and
         * pauses the timer.
         */
    }
}
...
/* -----------------------------------------------------------------------------
   ...
   the I/O manager will (eventually) collect and ignore the notification in
