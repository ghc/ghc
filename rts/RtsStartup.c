...
static void initBuiltinGcRoots(void)
{
    ...
    /* Note [runIO is referred to by foreign export stubs]
     * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     * We create stable pointers to GHC.TopHandler.{runIO,runNonIO} to ensure
     * that they are considered live since they may be reachable from the
     * foreign export stubs generated by
     * GHC.HsToCore.Foreign.Decl.mkFExportCBits.
     */
    getStablePtr((StgPtr)runIO_closure);
    getStablePtr((StgPtr)runNonIO_closure);

    /* The symbols below are all reachable from the RTS itself */
