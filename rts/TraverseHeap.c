        ...
        for (n = 0; n < n_capabilities; n++) {
          for (bd = capabilities[n]->mut_lists[g]; bd != NULL; bd = bd->link) {
            for (ml = bd->start; ml < bd->free; ml++) {
                invalidateTravData(ts, (StgClosure *)*ml);
            }
          }
        }
    }
...
inner_loop:
    ...
    if(first_child == NULL && ts->return_cb) { // no children
        ...
        // Pushing a return frame for first_child is pretty inefficent. I
        // believe chains of closures with just one pointer are rather common,
        // since '[a]' and 'String' gives rise to these linked lists for
        // example. I will clean this up in a future commit but for now this
        // preserves the invariant that return_cb will be called for all
        // traversed closures exactly once when enabled.
        sep = traversePushReturn(ts, c, accum, sep);
    } else {                          // many children
        se.sep = sep;
        se.data = child_data;
