...
function h$rts_eval(action, unbox) {
  return new Promise((accept, reject) =>
    h$run(MK_AP3( h$ghczmbaseZCGHCziJSziPrimziresolveIO
                , x => { accept(unbox(x))}
...
function h$rts_eval_sync(closure, unbox) {
  var res, status = 0;
  try {
  h$runSync(MK_AP3( h$ghczmbaseZCGHCziJSziPrimziresolveIO
...
function h$rts_toIO(x) {
  return MK_AP1(h$ghczmbaseZCGHCziJSziPrimzitoIO, x);
}
