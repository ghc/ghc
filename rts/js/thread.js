...
function h$Thread() {
    ...
    this.stack = [h$done, 0, h$ghczmbaseZCGHCziConcziSynczireportError, h$catch_e];
...
function h$handleBlockedSyncThread(c) {
  ...
  if(h$currentThread.isSynchronous && h$currentThread.status === THREAD_BLOCKED) {
    if(h$currentThread.continueAsync) {
      ...
      h$sp += 2;
      h$currentThread.sp = h$sp;
      h$stack[h$sp-1] = h$ghczmbaseZCGHCziJSziPrimziInternalziwouldBlock;
      h$stack[h$sp]   = h$raiseAsync_frame;
      h$forceWakeupThread(h$currentThread);
...
function h$runSyncReturn(a, cont) {
  ...
  var aa = MK_AP1(h$ghczmbaseZCGHCziJSziPrimziInternalzisetCurrentThreadResultValue, a);
...
function h$runSyncAction(t, a, cont) {
  h$runInitStatic();
  var c = h$return;
  t.stack[2] = h$ghczmbaseZCGHCziJSziPrimziInternalzisetCurrentThreadResultException;
  t.stack[4] = h$ap_1_0;
  t.stack[5] = a;
  t.stack[6] = h$return;
  t.sp = 6;
...
function h$blockOnBlackhole(c) {
  ...
  if(BLACKHOLE_TID(c) === h$currentThread) {
    ...
    return h$throw(h$ghczmbaseZCControlziExceptionziBasezinonTermination, true);
  }
