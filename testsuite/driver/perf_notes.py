...
# Get the baseline (expected value) of a test at a given commit.
# This is the expected value *before* the commit is applied (i.e. on the parent
# commit). This searches git notes from older commits for recorded metrics
# (locally and from ci). More recent commits are favoured, then local results
# over ci results are favoured.
#
...
def baseline_metric(commit, name, test_env, metric, way):
    ...
    def search(useCiNamespace, depth):
        ...
        # metrics on ancestor commits will not be a valid basline.
