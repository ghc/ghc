...
def tabulate_metrics(metrics: List[PerfMetric]) -> None:
    ...
    has_baseline = any([x.baseline is not None for x in metrics])
    hide_baseline_env = not has_baseline or all(
        [x.baseline is None or
         x.stat.test_env == x.baseline.perfStat.test_env
         for x in metrics])
    def row(cells: Tuple[str, str, str, str, str, str, str]) -> List[str]:
        return list(cells[:3]+cells[4:]) if hide_baseline_env else list(cells)
    ...
    if has_baseline and hide_baseline_env:
