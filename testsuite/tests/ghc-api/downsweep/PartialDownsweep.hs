  ...
  runGhc (Just libdir) $ do
    ...
    go "Parse error in export list"
        ...
        , [ "module B !parse_error where"
          --  ^ this used to cause parseHeaderImports to throw an exception instead
    ...
    go "Parse error in import list"
        ...
        , [ "module B where"
          , "!parse_error"
          --  ^ this is silently ignored, parseHeaderImports assumes the import
...
go label mods cnd =
  defaultErrorHandler defaultFatalMessager defaultFlushOut $ do
    ...
    hsc_env <- getSession
    (_, nodes) <- liftIO $ downsweep hsc_env emptyDownsweepCache [] False
