...
void checkGcRoots(void)
{
    ...
    {
        StgTSO *tso = (StgTSO *)UNTAG_CLOSURE((StgClosure *)tsos[i]);
        if (get_itbl((StgClosure *)tso)->type != TSO)
        {
    ...
    for (int i = 0; i < miscRootsCount; i++)
    {
        ...
        {
            fprintf(stderr, "rts_listThreads unexpectedly returned an TSO type at index %i (TSO=%u)\n", i, TSO);
            exit(1);
        }
    }
