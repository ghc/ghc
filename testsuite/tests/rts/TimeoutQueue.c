...
{
    for (int i = 0; i < N; i++) {
        printf("&elem[%i] = %p, elem[%i] = { .waketime=%lli, .rank=%i, a=%10p, b=%10p, p=%10p }\n",
...
int main_test (void)
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        initElemTimeoutQueue(&elems[i], notify, NotifyMVar);
    }
...
#if defined(DEBUG)
        int sz = size(root);
        printf("i = %3i, x = %3lli, size(root) = %3i, loop1: %i, loop2: %i\n",
...
#if defined(DEBUG)
        int sz = size(root);
        printf("i = %3i, x = %3lli, size(root) = %3i, loop1: %i, loop2: %i, loop3: %i\n",
...
int main_bench (bool showtiming)
{
    ...
    if (showtiming) {
      ...
      printf("completed in %lli nsec, %.1f ns per op\n", ns, (double)ns/N);
    }
#if defined(DEBUG)
