...
pathchar* mkPath(char* path)
{
    ...
    {
        errorBelch("mkPath failed converting char* to wchar_t*: %s", path);
        exit(1);
    }
    ret[required] = '\0';
    return ret;
...
void check_object_freed(char *obj_path) {
    OStatus st;
    st = getObjectLoadStatus(mkPath(obj_path));
...
void test_no_dangling_references_to_unloaded_objects()
{
    ...
    unloadObj(mkPath("A.o"));
    unloadObj(mkPath("B.o"));
    unloadObj(mkPath("C.o"));
    unloadObj(mkPath("D.o"));
    performMajorGC();
