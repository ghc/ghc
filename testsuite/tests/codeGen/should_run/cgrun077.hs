...
slowLzcnt :: Int -> Word -> Word
slowLzcnt size x = fromIntegral $ min size $ length $ takeWhile (== False) $ reverse $ map (testBit x) [0 .. size - 1]

test_lzCnt = test "lzcnt" lzcnt (slowLzcnt (8 * SIZEOF_HSWORD))
