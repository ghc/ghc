...
-- (YIELD_CHECK, which yields in place and is resumed by the scheduler). A
-- rise in the GC count while this loop runs is therefore evidence that the
-- safepoint fired; a loop whose safepoint never fired could add at most
-- about two GCs (at the first BCO entry after the loop), while this one adds
-- about 15 at -A1m. See Note [Join points as loops] in StgToByteCode.hs.
