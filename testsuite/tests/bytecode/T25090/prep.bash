...
ghc "-c ${dep@Q}/Dep.hs ${dep@Q}/DepApi.hs"
ghc "-dynamic -c -osuf dyn_o -hisuf dyn_hi ${dep@Q}/Dep.hs ${dep@Q}/DepApi.hs"

if [[ "$library" == 'shared' ]]
then
  ghc "-dynamic -shared -o ${dep@Q}/libHSdep-1.0-ghc${version}${so_ext} ${dep@Q}/Dep.dyn_o ${dep@Q}/DepApi.dyn_o"
  echo 'hs-libraries: HSdep-1.0' >> "$conf_dep"
elif [[ "$library" == 'shared-empty' ]]
then
  echo 'module Dummy where' > Dummy.hs
  ghc "-dynamic-too -c Dummy.hs"
  ghc "-dynamic -shared -o ${dep@Q}/libHSdep-1.0-ghc${version}${so_ext} Dummy.dyn_o"
  echo 'hs-libraries: HSdep-1.0' >> "$conf_dep"
