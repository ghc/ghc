...
-- If we did, we'd get a warning that the first two cases were redundant,
-- implying the latter two are not. Arguably this might be better than not
-- warning at all, but it's very surprising having to supply the third case but
-- not the first two cases. And it's probably buggy somwhere else. Delete this
-- when we detect that all but the last case is redundant.
consAreRigid :: Int
...
-- 'CoreExpr', we'd see the type application and all would be well. The
