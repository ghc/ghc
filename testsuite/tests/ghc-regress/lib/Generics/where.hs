...
intention to illustrate a few dimension for designing traversals.

1. We can decide on whether prefer "rewrite steps" (i.e., monomorphic
functions of data) that succeed either in all cases or only if they
actually encountered a term to be replaced. In the first case, the
catch-all equation describes identity (see stepid below). In the
second case, the catch-call equation describes failure (see stepfail
below). As an intermediate assessment, the failure approach is more
general, but often the identity approach is more convenient and
sufficient.
...
category while somewhere belongs to the second category as it uses the
operation mplus of a MonadPlus. So while everywhere makes very 
well sense without a monad, somewhere is necessarily monadic.

3. We can now also decide on whether we want rewrite steps to succeed
for all possible subterms, at least one, exactly one, and others.
The various traversal schemes make assumptions in this respect.
...
b) everywhereM
   ...
   Attempts to reach all nodes where all the subtraversals are performed
   ...
   except for the purpose of propagating an error. For example, "mkM
   stepfail" should not be passed to everywhereM but see "recovered".

c) somewhere

   Descends into term in a top-down manner, and stops in a given branch
   when the argument succeeds for the subterm at hand. To this end,
   it takes potentially failing argument, and recovers from failure
   when mapping over the immediate subterms while insisting success for
   at least one subterm (say, branch). This scheme is appropriate if
   you want to make sure that a given rewrite step was actually used in
   a traversal. So failure of the traversal would mean that the argument
   failed for all subterms.
...
-- Two mutually recursive datatypes datatypes
