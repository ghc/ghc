...
- GHC will no longer implicitly link against ``base`` when invoked as linker.
  In some uncommon workflows this can result in ``undefined reference`` linker errors
  if ``base`` is not explicitly given as package to link against using ``-package base``.
