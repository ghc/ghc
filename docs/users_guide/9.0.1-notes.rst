...
- Since GHC 8.10, modules using the extensions :extension:`UnboxedTuples`
  or :extension:`UnboxedSums` are automatically compiled to object code.
  GHC 8.10 allowed disabling this feature by using :ghc-flag:`-fbyte-code` in
  an ``OPTIONS_GHC`` pragma, but this approach was too coarse-grained, as
  :ghc-flag:`-fbyte-code` also performs other side effects. In this release, a
  new flag :ghc-flag:`-fobject-code-if-unboxed` now controls whether
  :extension:`UnboxedTuples` or :extension:`UnboxedSums` imply the use of
  object code, not :ghc-flag:`-fbyte-code`.
