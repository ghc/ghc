...
* :extension:`UnliftedDatatypes`: The `Unlifted Datatypes Proposal`_ has been
  implemented. That means GHC Haskell has algebraic data types with strict
  semantics like in OCaml or Idris! The distinction to ordinary lifted data
  types is made in the kind system: Unlifted data types live in kind
  ``TYPE (BoxedRep Unlifted)``. :extension:`UnliftedDatatypes` allows to give
  data declaration such return kinds, in the following example with the help of
  ...
  are not levity-polymorphic (yet, for some of them,) and hence won't
  work with unlifted data types.
