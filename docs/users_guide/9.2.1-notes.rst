...
- There is a new binding ``GHC.Exts.considerAccessible`` that is equivalent to ``True``
  and allows the programmer to turn off redundancy warnings for particular
  clauses, like the third one here ::
