...
- The parallel garbage collector is now significantly more performant. Heavily
  ...
  garbage collector with :rts-flag:`:-qg`.
  ...
  nurseries (:rts-flag:`:-A`), disabling load balancing (:rts-flag:`:-qb`), or
  limiting parallel GC to older generations (:rts-flag:`:-qg`); then you may
...
- The default nursery size, :rts-flag:`-A`, has been increased from 1mb to 4mb.
  GHC itself uses the default nursery size, and so now runs with a larger
  nursery by default.
