...
* The `Record Dot Syntax Proposal`_ has been implemented:
  ...
  - A new extension :extension:`OverloadedRecordUpdate` provides record '.' syntax in record updates e.g. ``x{foo.bar = 1}``.  *This design of this extension may well change in the future.*
