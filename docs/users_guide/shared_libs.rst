...
.. code-block:: none
    ...
    ghc --make -dynamic -shared -fPIC -fshared-link-rts Foo.hs -o libfoo.so
...
:ghc-flag:`-fshared-link-rts` additionally links against the shared library
version of the ``rts`` package (linking against the ``rts`` package is not
enabled by default when building shared libraries).
...
.. code-block:: none
    ...
    ghc -dynamic -shared -fshared-link-rts Foo.o -o libfoo.so
