...
- A new extension, :extension:`ImplicitForAll`, has been added that makes explicit
  ``forall`` on declarations with type variables mandatory when disabled.
...
- A new warning, :ghc-flag:`-Wpattern-signature-binds`, has been added that
  alerts the user when a pattern signature binds a type variable that isn't
  in scope.
