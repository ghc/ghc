...
roots ``a`` and ``b`` share some data ``c`` but since neither root is
reachable from the other, not their own constructors. Here we can think of
the root profiler producing three groups, though in reality it just counts
the closure sizes ::

    a   -> {A _ _, 1}, size: 3 + 1 = 4 words
    b   -> {B _ _, 2}, size: 3 + 1 = 4 words
    a,b -> {3},        size: 1 word

So much for theory, to run a root profile compile the program with
profiling enabled like usual and then pass the :rts-flag:`-ho`, like so:
...
so a short program like above will not produce any output. To force
