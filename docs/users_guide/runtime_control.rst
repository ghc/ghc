...
Another confusing aspect of the RTS is the sometimes big difference between
...
.. math::
  ...
  \texttt{F}' = \frac{\texttt{F}}{2 ^ \frac{\texttt{t}}{\texttt{Fd}}}
...
Therefore after 30 seconds of inactivity there will have been approximately 10 idle
collections which means the scaled ``F`` factor is computed by

.. math::

  \texttt{F}' = \frac{2}{2^{(\frac{10}{4})}} \approx 0.35
...
it is to return memory to a theoretical baseline. In theory the compacting
