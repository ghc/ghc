...
Breakpoints can be set on all functions in every loaded and interpreted
...
reference by ⟨ModQual⟩.

⟨nestedIdent⟩ is optional and the name of a function nested in a let or
where clause inside the function ⟨topLevelIdent⟩.

If ⟨ModQual⟩ is a module name, then ⟨toplevelIdent⟩ may be any top level
