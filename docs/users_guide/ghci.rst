...
Providing :ghc-flag:`-pgmi /path/to/iserv-proxy <-pgmi ⟨cmd⟩>` and :ghc-flag:`-opti
write-handle -opti read-handle -opti slave-ip -opti slave-port [-opti -v] <-opti
⟨option⟩>` in addition to :ghc-flag:`-fexternal-interpreter` will then make ghc
go through the proxy instead.
