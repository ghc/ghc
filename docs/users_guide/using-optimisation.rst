...
.. ghc-flag:: -fptr-eq
    :shortdesc: Enable a pointer-equality fast path in derived ``Eq``/``Ord`` instances.
    :type: dynamic
    :reverse: -fno-ptr-eq
    :category:
    ...
    When deriving ``Eq`` or ``Ord`` for datatypes with fields, GHC can first
    test whether the two arguments are the very same heap object using
    ``reallyUnsafePtrEquality#``. If they are, the comparison succeeds
    immediately without inspecting any fields. Enumeration types (all-nullary
    constructors) skip this fast path because their tag-based comparisons are
    already cheap. This option is off by default.
