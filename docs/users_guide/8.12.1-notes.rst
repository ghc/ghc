...
- The ``-XTemplateHaskellQuotes`` extension now allows nested splices as nested
 ...
 - There are now two new functions ``putDoc`` and ``getDoc``, which allow
   Haddock documentation to be attached and read from module headers,
   declarations, function arguments and instances.
