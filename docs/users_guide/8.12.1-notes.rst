...
- There are two new functions ``putDoc`` and ``getDoc``, which allow Haddock
  ...
  These functions are quite low level, so the ``withDoc`` function provides a
  more ergonomic interface for this. Similarly ``funD_doc`` and ``dataD_doc``
  provide an easy way to document functions and constructors alongside their
  arguments simultaneously. ::

    $(withDoc "This does good thigngs" $ head <$> [d| foo x = 42 |])
