...
* GADT constructor types now properly adhere to :ref:`forall-or-nothing`. As
  ...
  ``forall`` triggers implicit quantification, making the explicit foralls
  nested. Furthermore, GADT constructors do not permit the use of nested
  foralls, as explained in :ref:`formal-gadt-syntax`.

  In addition to rejecting nested foralls, GHC is now more stringent about
