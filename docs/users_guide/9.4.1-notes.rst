...
Runtime system
~~~~~~~~~~~~~~
- DTrace probe points are now provided on Linux, adding to the existing support for Darwin and FreeBSD. To compile GHC with DTrace support pass the ``--enable-dtrace`` flag to `configure` and ensure that `libsystemtap` is available.
