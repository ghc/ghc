...
- Added a new primitive ``GHC.Exts.reallyUnsafeHetPtrEquality#`` for comparing the
  underlying pointers of two values, which is more general than
  ``GHC.Exts.reallyUnsafePtrEquality#`` by virtue of being heterogeneous
  and levity-polymorphic: ::
   ...
   This means that ``GHC.Exts.reallyUnsafeHetPtrEquality#`` can be used
