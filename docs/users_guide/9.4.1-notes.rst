...
- Added a new primitive ``GHC.Exts.reallyUnsafeHetPtrEquality#`` which is heterogeneous
  and levity-polymorphic: ::
   ...
   This means that ``GHC.Exts.reallyUnsafeHetPtrEquality#`` can be used on primitive arrays,
   such as ``GHC.Exts.Array#`` and ``GHC.Exts.ByteArray#``.

- Deprecated ``GHC.Exts.reallyUnsafePtrEquality#`` in favour of
  ``GHC.Exts.reallyUnsafeHetPtrEquality#``.
