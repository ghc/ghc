...
* With :extension:`LinearTypes` record selector functions are now
  multiplicity-polymorphic when possible. Take the following example:

  ::

      data Record = Rec { x %'Many :: Int, y :: Char }

  The selector function ``y`` will now have type ``y :: Record %m -> Char``,
  because the ``x`` field is allowed to be discarded.
  ...
  but for technical reasons this is not supported, see :ghc-ticket:`23380`)
