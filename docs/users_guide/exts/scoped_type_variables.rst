...
.. tip::
    ...
    Explicit ``forall`` is not always required -- see :ref:`pattern signature equivalent <pattern-equiv-form>` for the example in this section.
...
An equivalent form for that example, avoiding explicit ``forall`` uses :extension:`PatternSignatures` in combination with :extension:`ScopedTypeVariables``: ::
...
-  A pattern type signature (:extension:`PatternSignatures`), but only when :extension:`ScopedTypeVariables` is enabled
...
.. extension:: PatternSignatures
    ...
    Allow type signatures in patterns.
...
However, in case you have both :extension:`PatternSignatures` and :extension:`ScopedTypeVariables`
enabled, in all patterns *other* than pattern bindings, a pattern type
