...
A more explicit declaration that fills in the invisible kind arguments would be ::
...
to ``(Type->Type)`` in the instance, thus ::

   data instance T @(Type->Type) p q where
      MkT :: forall r. r Int -> T r Int

This latter form is accepted, but the former is not: in data/newtype
instance declararations (unlike ordinary data/newtype declarations)
we do *not* look at the constructor declarations.  The principle is
that *the instantiation of the data instance should be apparent from the header alone*.
This principle makes the program easier to understand; but it also
avoids a swamp of complexity about which type specialisation comes from the
data instance, and which from the individual GADT data constructors.
