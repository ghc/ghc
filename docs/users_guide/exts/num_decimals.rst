...
scientific notation syntax for instances of ``Integral``, and have values
like ``(1.2e6 :: Num a => a)``. This applies only to literals that really
turn out to have integral values. For example ``1.23e1 :: Fractional a => a``
since ``1.23e1 == 12.3``, however ``1.23e2 :: Num a => a`` as ``1.23e2 == 123``.

Integral literals written using scientific notation will be desugared using
``fromInteger``, whereas any literals which aren't integeral will be desugared
