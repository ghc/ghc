...
.. ghc-flag:: -Wredundant-bang-patterns
    ...
    on a redundant clause" should fix that. That is because in that case, it is
    recommended to delete the clause wholly, including its leading pattern match.
    ...
    are not considered dead, and ``-Wredundant-bang-patterns`` will not warn about
    them.
