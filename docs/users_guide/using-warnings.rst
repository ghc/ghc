...
.. ghc-flag:: -Wmissing-deriving-strategies
   ...
   strategy. If the warning is enabled, but :extension:`DerivingStrategies`
   is not enabled, the compiler will suggest turning on the :extension:`DerivingStrategies`
   extension. This option is not on by default, having
   to be turned on manually or with :ghc-flag:`-Weverything`.
