...
But there is one exception: variables mentioned in the outermost kind
signature on the right hand side are quantified implicitly. Thus, in the
following example the variables ``a``, ``b``, and ``k`` are all in scope::
...
Here, ``a`` is brought into scope by ``:: Maybe (Maybe k)``. This rule works
for kind and type variables alike: ::

  type E = 'Left a :: Either Type a
