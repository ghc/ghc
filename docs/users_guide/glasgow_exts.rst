...
to the fact that ``Int`` is not of kind ``Bool``). The key observation is that
...
kinds, as allowing them in the types of terms would require full dependent
types. Therefore, visible dependent quantifiers are rejected in any context
