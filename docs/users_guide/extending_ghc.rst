...
* return ``TcPluginOk`` with lists of solved and new constraints.

* return ``TcPluginContradiction`` with a list of impossible
  constraints, so they can be turned into errors; or
...
Where ``Reduction`` is defined as follows: ::
...
The evidence required for a rewriting is a ``Coercion`` between the provided
rewriting (on the LHS of the coercion) and the type family application
(on the RHS).
