...
hsConFieldSpecToFunTy :: HsConFieldSpec DocNameI -> LHsType DocNameI -> LHsType DocNameI
hsConFieldSpecToFunTy cfs tgt =
  noLocA (HsFunTy noAnn (cfs_multiplicity cfs) (hsConFieldSpecToHsTypeNoMult cfs) tgt)

hsConFieldSpecToHsTypeNoMult :: HsConFieldSpec DocNameI -> LHsType DocNameI
  ...
  where
    ...
    keep d
      ...
      where
        ...
        field_types flds = [t | L _ (ConDeclField _ _ t) <- flds]
    keep _ = Nothing
