...
modulePackageInfo _unit_index _unit_state _flags Nothing = (Nothing, Nothing)
modulePackageInfo unit_index unit_state flags (Just modu) =
  ( optPackageName flags <|> fmap unitPackageName pkgDb
  , optPackageVersion flags <|> fmap unitPackageVersion pkgDb
  )
  where
    pkgDb = lookupUnit unit_index unit_state (moduleUnit modu)
