...
ppr_mono_ty (HsExplicitTupleTy _ IsPromoted tys _) u = Pretty.quote $ parenList $ map (pprTupArg u) tys  -- FIXME (int-index): ppr unboxed
ppr_mono_ty (HsExplicitTupleTy _ NotPromoted tys _) u = parenList $ map (pprTupArg u) tys                -- FIXME (int-index): ppr unboxed
...
pprTupArg u (Present _ t) = ppLType u t
