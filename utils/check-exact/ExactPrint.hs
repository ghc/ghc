...
-- based on pprConDecl
  ...
  exact (ConDeclGADT { con_g_ext = an
    ...
    case body of
        (PrefixConGADT body') ->
          exact_prefix_con_gadt_body body'
        (RecConGADT fields res_ty) -> do
          markAnnotated fields
  ...
  --   markTrailingSemi
      where
        exact_prefix_con_gadt_body (PCGResult res_ty) =
          markAnnotated res_ty
        exact_prefix_con_gadt_body (PCGAnonArg arg_ty body') = do
