...
enterAnn (Entry anchor' cs flush canUpdateAnchor) a = do
  ...
  case anchor' of
    ...
    EpaSpan _ -> do
      ...
      -- case cs of
      --   EpaComments _ -> addCommentsA (priorComments cs)
      --   EpaCommentsBalanced pc fc -> do
...
markExternalSourceText l (SourceText txt) _ = printStringAtRs (realSrcSpan "aa3" l) (unpackFS txt) >> return ()
      ...
      = do
          ...
          ds <- withSortKey sortKey
                               [(ClsAtdTag, prepareListAnnotationA ats),
                                ...
                                (ValDTag, prepareListAnnotationA (bagToList binds)),
                                (SigDTag, prepareListAnnotationA sigs)
                               ]
...
withSortKey :: (Monad m, Monoid w)
  => AnnSortKey -> [(DeclTag, [(RealSrcSpan, EP w m Dynamic)])] -> EP w m [Dynamic]
...
instance ExactPrint (TyClDecl GhcPs) where
  ...
  exact (ClassDecl {tcdCExt = (an, sortKey),
      ...
      = do
          ...
          ds <- withSortKey sortKey
                               [(SigDTag, prepareListAnnotationA sigs),
                                (ValDTag, prepareListAnnotationA (bagToList methods)),
