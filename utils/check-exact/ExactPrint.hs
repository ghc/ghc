...
instance ExactPrint (HsExpr GhcPs) where
  ...
  exact (HsDo an do_or_list_comp stmts) = do
    debugM $ "HsDo"
    markAnnList an $ exactDo an (PsStmt do_or_list_comp) stmts
...
exactDo an (PsStmt (DoExpr m))    stmts = exactMdo an m AnnDo             >> markAnnotatedWithLayout stmts
exactDo an (PsStmt GhciStmtCtxt)  stmts = markLocatedAAL an al_rest AnnDo >> markAnnotatedWithLayout stmts
exactDo an ArrowExpr     stmts = markLocatedAAL an al_rest AnnDo >> markAnnotatedWithLayout stmts
exactDo an (PsStmt (MDoExpr m))   stmts = exactMdo an m AnnMdo            >> markAnnotatedWithLayout stmts
exactDo _  (PsStmt ListComp)      stmts = markAnnotatedWithLayout stmts
exactDo _  (PsStmt MonadComp)     stmts = markAnnotatedWithLayout stmts
...
instance ExactPrint (LocatedN (CtxIdGhcP 'Parsed)) where
  getAnnotationEntry (L l (CtxIdRdrName n)) = getAnnotationEntry (L l n)
  getAnnotationEntry (L l (CtxIdName n)) = getAnnotationEntry (L l (nameRdrName n))
  exact (L l (CtxIdRdrName n)) = exact (L l n)
  exact (L l (CtxIdName n)) = exact (L l (nameRdrName n))
