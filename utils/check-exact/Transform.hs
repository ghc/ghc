...
setEntryDPDecl decl@(L _  (ValD x (FunBind a b (L d (MG c ms ))))) dp
                   = L l' (ValD x (FunBind a b (L d (MG c ms'))))
    where
...
pushDeclDP (ValD x (FunBind a b (L d  (MG c ms )))) dp
...
balanceCommentsFB (L lf (FunBind x n (L lm (MG o matches)))) second
