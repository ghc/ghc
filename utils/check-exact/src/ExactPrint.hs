...
instance ExactPrint (LHsDecl GhcPs) where
  ...
  exact (L _ (TyClD _ d)) = exact d
  exact (L _ (InstD _ d)) = exact d
  exact (L _ (ValD  _ d)) = exact d
  exact (L _ (SigD  _ d)) = exact d
...
instance ExactPrint (InstDecl GhcPs) where
  getApiAnnotation (ClsInstD     an _) = NoEntryVal
  getApiAnnotation (DataFamInstD {}) = NoEntryVal
  getApiAnnotation (TyFamInstD   {}) = NoEntryVal
...
instance ExactPrint (Match GhcPs (LHsExpr GhcPs)) where
  ...
  exact (Match anns@(ApiAnn _ ann cs) mctxt pats (GRHSs _ grhs lb)) = do
...
instance ExactPrint (HsLocalBinds GhcPs) where
  ...
  exact (HsValBinds _ bs) = exact bs
...
instance ExactPrint (HsExpr GhcPs) where
  ...
  exact (HsLam _ (MG _ (L _ [match]) _)) = markAnnotated match
...
--       else markLocated e1
  ...
  exact (HsCmdArrForm _ e fixity _mf cs) = do
    markAnnotated e
...
instance ExactPrint (Pat GhcPs) where
  getApiAnnotation (VarPat{}) = NoEntryVal
  getApiAnnotation _ = NoEntryVal
