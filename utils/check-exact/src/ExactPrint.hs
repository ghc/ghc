...
data EPState = EPState
             ...
             , epMarkAnchor  :: !Bool -- ^ Do not capture epColDelta on
                                     -- entering ann. Used ot reset
                                     -- the delta for top level items.

             , epColDelta   :: !Int -- ^ Current difference between
...
enterAnn (Entry anchor' cs) a = do
  ...
  -- -----------------------------------------------
  -- Advance by any discrepancy between origPos and topAnchor
  -- But only do it once per anchor value, for a nested AST having the
  -- same start location
  p'' <- getPos
  markAnchor <- gets epMarkAnchor
  op'' <- if markAnchor
             then getOrigPos
             else return p''
  let delta = (snd op'') - (snd p'')
  modify (\s -> s { epMarkAnchor = True } )
  debugM $ "enterAnn:(pos,origpos,delta,markAnchor)=" ++ show (p'',op'',delta,markAnchor)
  lhs <- gets epLHS
  anc <- gets epAnchorLHS
  debugM $ "enterAnn:(curAnchor,epLHS,epAnchorLHS)=" ++ show (rs2range curAnchor,lhs,anc)
  -- -----------------------------------------------
...
instance ExactPrint HsModule where
  ...
  exact (HsModule an _lo mmn mexports imports decls mdeprec mbDoc) = do
    ...
    -- setLayoutAnchor $ markAnnotated decls
...
markTopLevelList ls = mapM_ (\a -> setLayoutAnchor $ markAnnotated a) ls
...
instance ExactPrint (HsExpr GhcPs) where
  ...
  exact x@(HsCase an e alts) = do
    ...
    setLayoutAnchor $ markAnnotated alts
...
-- TODO: this is a bad name, give it something useful, to indicate we
-- disable computing delta, for top-level lists
-- And use 'local', designed for this
setLayoutAnchor :: (Monad m, Monoid w) => EP w m () -> EP w m ()
setLayoutAnchor k = do
  debugM $ "setLayoutAnchor entered"
  oldAnchorOffset <- getAnchorOffset
  modify (\a -> a { epMarkAnchor = False
                  , epAnchorLHS = 0} )
  k
  setAnchorOffset oldAnchorOffset
...
getColDelta :: (Monad m, Monoid w) => EP w m Int
getColDelta = gets epColDelta

setColDelta :: (Monad m, Monoid w) => Int -> EP w m ()
setColDelta c = do
  debugM $ "setColDelta:" ++ show c
  modify (\s -> s { epColDelta = c })
