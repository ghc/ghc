...
balanceCommentsMatch (L l (Match am mctxt pats (GRHSs xg grhss binds))) = do
  logTr $ "balanceCommentsMatch: (loc1,move',stay',logInfo)=" ++ showGhc (ss2range (locA l),move',stay',logInfo)
  logTr $ "balanceCommentsMatch: (loc1,anc1,cs1f)=" ++ showGhc (ss2range (locA l),anc1,cs1f)
  ...
  where
    ...
    (l'', grhss', logInfo)
      = case reverse grhss of
          [] -> (l, [], (AnnComments [], SrcSpanAnn ApiAnnNotUsed noSrcSpan))
          (L lg (GRHS ag grs rhs):gs) ->
            let
              ...
              ag' = ApiAnn anc an (lgc' <> (AnnCommentsBalanced [] move))
            in (an1', (reverse $ (L lg (GRHS ag' grs rhs):gs)), (anc1',an1'))
...
-- pushTrailingComments ::  (Data b,Monad m)
--   => LHsBind GhcPs -> TransformT m (LHsBind GhcPs)
-- pushTrailingComments (L lf (FunBind x n (MG mx (L lm matches) o) t)) = do
--   -- logTr $ "pushTrailingComments entered"
--   let (m:ms) = reverse matches
--   m',second') <- balanceComments' m second
--   return (L lf (FunBind x n (MG mx (L lm (reverse (m':ms))) o) t))
-- pushTrailingComments f = return f
...
commentOrigDeltas ls@(L _ (GHC.AnnComment _ p):_) = go p ls
  where
    ...
    go p (L (Anchor la _) (GHC.AnnComment t pp):ls)
      ...
      where
        p' = p
        op = MovedAnchor (ss2delta (ss2pos pp) la)
...
whereAnnotation :: (Monad m) => TransformT m (ApiAnn' AnnList)
whereAnnotation = do
  newSpan <- uniqueSrcSpanT
