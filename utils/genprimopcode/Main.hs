...
gen_hs_source (Info defaults entries) =
    ...
    ++ "{-# OPTIONS_GHC -Wno-deprecations -O0 -fno-do-eta-reduction #-}\n"
            -- Very important OPTIONS_GHC!  See Note [OPTIONS_GHC in GHC.Prim]
    ...
    ++ "{-# OPTIONS_GHC -fno-warn-semigroup #-}\n"
            -- We must disable this warning as it will otherwise cause GHC to
            -- try to load GHC.Types for the Monoid class.
    ++ "{-# OPTIONS_GHC -dno-typeable-binds #-}\n"
            -- We must not generate a $trModule binding since this would
            -- require that we import GHC.Types, which itself depends upon
            -- GHC.Prim.
    ...
    ++ "\n" ++ unlines (concatMap entry entries') ++ "\n\n\n"
  ...
  where
    ...
    entry :: Entry -> [String]
    entry   (Section {})         = []
    entry o@(PrimOpSpec {})      = spec o
    entry o@(PrimVecOpSpec {})   = spec o
    entry o@(PrimTypeSpec {})    = spec o
    entry o@(PrimVecTypeSpec {}) = spec o
    entry o@(PseudoOpSpec {})    = spec o

    spec :: Entry -> [String]
    spec ent = ([ "" ] ++) . concat $
      -- Doc comments
      [ case desc ent ++ extra ent of
          "" -> []
          cmmt -> map ("-- " ++) $ lines $ "|" ++ cmmt

      -- Declarations (see Note [Primop wrappers])
      , case ent of
          ...
          PrimVecTypeSpec { ty = t }         -> prim_data t
            ...
            -- Typeable-deriving logic will attempt to load GHC.Types.Module
          Section { }                        -> error "Section is not an entity"
      ]

    extra :: Entry -> String
    extra ent = case on_llvm_only ent ++ can_fail ent of
      [m1,m2] -> "\n\n__/Warning:/__ this " ++ m1 ++ " and " ++ m2 ++ "."
      [m] -> "\n\n__/Warning:/__ this " ++ m ++ "."
      _ -> ""

    on_llvm_only :: Entry -> [String]
    on_llvm_only ent
      = [ "is only available on LLVM"
        | True <- [isLlvmOnly ent] ]

    can_fail :: Entry -> [String]
    can_fail ent
      = [ "can fail with an unchecked exception"
        | Just (OptionTrue _) <- [lookup_attrib "can_fail" (opts ent)] ]
    ...
    prim_func n t llvm_only
      ...
      | otherwise =
        [ wrapOp n ++ " :: " ++ pprTy t
        , wrapOp n ++ " " ++ unwords args ++ " = " ++ rhs ++ " " ++ unwords args
        , "{-# NOINLINE " ++ wrapOp n ++ " #-}"
        ]
      where
        args = map (\i -> "a" ++ show i) [1 .. arity t]
        bottom = "let x = x in x"
        rhs
          | "tagToEnum#" <- n = bottom
