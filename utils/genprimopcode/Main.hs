   ...
   = "\\primopdefaults{"
     ...
     where mk_entry (PrimOpSpec {cons=constr,name=n,ty=t,cat=c,desc=d,opts=o}) =
           ...
           mk_effect o = case lookup_attrib "effect" o of
             Just "NoEffect"        -> ""
             Just "ThrowsImprecise" -> "May throw an imprecise exception."
             Just "WriteEffect"     -> "May throw an imprecise exception or perform a write effect."
             Just "ThrowsPrecise"   -> "May throw a precise exception (or any other effect)."
