...
data Info
   = Info [Option] [EntryOrMacroDef]   -- defaults, primops
     deriving Show
...
data EntryOrMacroDef
    = Entry Entry
    | MacroDef MacroDef
    deriving (Eq, Ord, Show)

extractAllEntries :: EntryOrMacroDef -> [Entry]
extractAllEntries (Entry e) = pure e
extractAllEntries (MacroDef md) = case md of
  (MacroDef_Guarded _ es) -> es
  _ -> []
...
data MacroDef
    ...
    = MacroDef_Macro
      { macroDef_macro_name :: MacroVar
      , macroDef_macro_body :: MacroBody
      }
    -- | Enties that are only conditionally available
    | MacroDef_Guarded
      { macroDef_guarded_condition :: MacroExpr
      ,  -- | not 'EntryOrMacroDef', don't need or want nested conditional sections
...
data Option
   ...
   | OptionVector [(String, Either MacroVar String, Int)]  -- name = [(,...),...]
