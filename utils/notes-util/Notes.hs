...
findNotes fname t =
  ...
  where
    ...
    go !lineNo (l0 : l1 : lines)
      | hasRule = Right (NoteDef srcLoc name) : go (lineNo+2) lines
      where
    ...
    go lineNo (l0 : lines) =
        ...
        , let startCol = T.length prefix
              srcLoc = SrcLoc fname lineNo startCol
              (name, suffix) = T.breakOn "]" (T.drop 6 rest<>" "<>T.concat (map stripBeginningOfLineComment $ take 1 lines))
        , "]" `T.isPrefixOf` suffix
        ] ++ go (lineNo+1) lines
...
brokenNoteRefs db =
    [ ref
    | (fname, refs) <- M.toList (noteRefs db)
    , ref <- S.toList refs
